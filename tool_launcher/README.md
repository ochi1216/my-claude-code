...
> しておくとよい。
>
> **既知の問題と対策（20260915_04）**：`cmd.exe` 経由でショートカットを作ると、
> 「ショートカット→cmd.exe→cmd.exe→python.exe」という起動チェーンになり、
> Windowsが新しいウィンドウへ自動的にフォーカスを渡さず、起動してもランチャーが
> 前面に出てこないことがあった。起動時に一瞬だけ「最前面固定」にしてから
> 解除する対策を入れている。
...
- 多重起動防止は**Windowsの名前付きミューテックス**で行う。Windows以外では機能しない。
  ...
  - **プロセスが正常に起動中の場合**：警告ダイアログを出すだけでなく、**既存の
    ...
    警告ダイアログが出る。前面化は `SetWindowPos` の最前面固定→解除トグルで行い
    （`SetForegroundWindow` 単独が受けるフォアグラウンドロックの制限を回避する
    ため）、`SetForegroundWindow` は入力フォーカスを移すために併用する
    （20260915_04）。
