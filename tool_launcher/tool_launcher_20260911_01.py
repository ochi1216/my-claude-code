...
# -*- coding: utf-8 -*-
"""
tool_launcher_20260911_01.py  --  my-claude-code ツールランチャー v2
...
def launch_tool(tool, legacy_root):
    ...
    if kind in ("py", "streamlit"):
        ...
        if kind == "streamlit":
            ...
            args = [sys.executable, "-m", "streamlit", "run", latest]
        else:
            args = [sys.executable, latest]
...
# ---------------------------------------------------------------------------
def scan_unregistered_bats(registered_paths):
    ...
    for name in names:
        ...
        for bat in sorted(glob.glob(os.path.join(folder, "run_*.bat"))):
            if os.path.normcase(os.path.abspath(bat)) in registered_paths:
                continue
            found.append({
...
class LauncherApp(object):
    ...
    def _setup_fonts(self):
        ...
        self.f_logo = (jp, 15, "bold")
        self.f_meta = (mono, 8)
        self.f_cat = (jp, 9, "bold")
        self.f_tile = (jp, 10)
        self.f_badge = (mono, 7, "bold")
        self.f_btn = (jp, 10, "bold")
        self.f_log = (mono, 9)
        self.f_status = (mono, 8)
    ...
    def _build_header(self):
        ...
        tk.Button(right, text="⚙ tools.json", command=self.open_tools_json,
                  bg=C_BAR, fg=C_INK2, font=(self.f_btn[0], 9),
    ...
    def _render_categories(self, board):
        ...
        registered = set()
        for tool in tools:
            if tool.get("kind") == "bat":
                path = tool_target_path(tool, self.legacy_root)
                if path:
                    registered.add(os.path.normcase(os.path.abspath(path)))

        auto = scan_unregistered_bats(registered)
        if auto:
    ...
    def _build_status(self):
        ...
        tk.Button(inner, text="終了", command=self.on_close,
                  bg=C_BAR, fg=C_ACCENT, font=(self.f_btn[0], 9),
