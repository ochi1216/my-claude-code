...
# 20260915_02: 「前回のランチャーがプロセスとしては残っているが、ウィンドウが
# 無く応答もしていない」状態（ハング）を検知するための補助ファイル。
# ミューテックス自体はこのファイルの有無に一切依存しない（排他制御の正はあくまで
# ミューテックス）。このファイルは「誰が持っているか」を突き止めるための手がかりに
# すぎないため、内容が壊れていても古くても、安全側（＝終了させない）に倒れるだけで
# 排他制御そのものは壊れない。
PID_FILE = os.path.join(SCRIPT_DIR, ".tool_launcher.pid")
...
PROCESS_QUERY_LIMIT_INFORMATION = 0x1000
STILL_ACTIVE = 259

# 20260915_02: ゾンビ（応答なし）判定の猶予。前回インスタンスが起動処理の途中で
# たまたまウィンドウをまだ出していないだけ、という誤判定を避けるため、即断せず
# 短い間隔で複数回チェックする。合計の最大待ち時間は ZOMBIE_CHECK_ATTEMPTS ×
...
# 20260915_03: 既存のランチャーを最前面表示できたかどうか。main() が「起動中」
# 警告を出すかどうかの判断に使う（前面表示できたなら、その手前に警告を
# 重ねて表示する意味が薄いため出さない）。
_brought_existing_to_front = False
...
def _process_is_alive(pid):
    """指定PIDのプロセスが今も実行中か（終了コード未確定=STILL_ACTIVEか）を返す。"""
    if not pid:
        return False
    kernel32 = ctypes.windll.kernel32
    handle = kernel32.OpenProcess(PROCESS_QUERY_LIMIT_INFORMATION, 0, pid)
    if not handle:
        return False
    try:
...
def _process_image_basename(pid):
    """指定PIDの実行ファイル名（例: python.exe）を小文字で返す。取得できなければNone。"""
    kernel32 = ctypes.windll.kernel32
    handle = kernel32.OpenProcess(PROCESS_QUERY_LIMIT_INFORMATION, 0, pid)
    if not handle:
        return None
...
def _find_visible_windows(pid):
    """指定PIDが所有する、目に見える(可視状態の)トップレベルウィンドウの
    ハンドル一覧を返す。メインウィンドウに限らず、確認ダイアログのような
    子ウィンドウでもヒットする（＝「何かしら画面に出ている」ことの判定）。

    列挙に失敗した場合は None を返す（「無い」という結果と区別するため）。
    """
    ...
    def callback(hwnd, _lparam):
        ...
        if owner_pid.value == pid and user32.IsWindowVisible(hwnd):
            found.append(hwnd)
        return 1  # 「最前面表示」用に全ウィンドウを集めたいので途中で打ち切らない
...
def _pid_has_visible_window(pid):
    """指定PIDに可視ウィンドウが1つでもあるかを返す。

    判定に失敗した場合は True（＝「ウィンドウがある」＝終了させない）を返し、
    誤ってプロセスを強制終了しない安全側に倒す。
    """
    found = _find_visible_windows(pid)
    if found is None:
        return True
    return bool(found)
...
def _bring_existing_to_front(pid):
    ...
    20260915_03: 越智さんの指摘により追加。「起動中」と判定した際、ダイアログで
    知らせるだけでなく、既存のランチャーの方をユーザーの前まで持ってくる。

    ウィンドウが見つからない・操作に失敗した場合は何もしない（例外を外へ
    投げない）。破壊的な操作（強制終了）ではなく単なるウィンドウ操作のため、
    失敗しても実害はなく、これまでどおり「起動中」ダイアログが出るだけになる。
    複数ウィンドウが見つかった場合（例: 確認ダイアログが同時に開いている）は、
    タイトルにランチャーのウィンドウタイトルを含むものを優先し、無ければ
    先頭のものを対象にする。
    ...
    try:
        ...
        if len(windows) > 1:
            ...
            for hwnd in windows:
                ...
                if "TOOL LAUNCHER" in title_buf.value.upper():
                    target = hwnd
                    break

        if user32.IsIconic(target):
            user32.ShowWindow(target, SW_RESTORE)
        # 20260915_04: SetForegroundWindow単独だと、Windowsの「フォアグラウンド
        # ロック」により、無関係な別プロセスからの呼び出しがOSに無視されることが
        # ある（実機で、通常の初回起動でもウィンドウが前面に出ない不具合が確認
        # されたための対策）。SetWindowPosによる「最前面固定→解除」のトグルは
        # この制限を受けないため、まずこちらでウィンドウを前面へ出してから、
        # 入力フォーカスも移すためSetForegroundWindowも合わせて呼ぶ。
        user32.SetWindowPos(target, HWND_TOPMOST, 0, 0, 0, 0,
                            SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW)
        user32.SetWindowPos(target, HWND_NOTOPMOST, 0, 0, 0, 0,
                            SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW)
        user32.SetForegroundWindow(target)
        _brought_existing_to_front = True
        return True
    except Exception:
        return False
...
def force_own_window_to_front(root):
    """自分自身のTkウィンドウを確実に最前面へ出す。

    20260915_04: 実機で、デスクトップショートカット（cmd.exe経由で
    run_tool_launcher.bat を呼ぶ形）から起動した際に、root.deiconify() だけ
    ではウィンドウが前面に出てこない不具合が報告された。ショートカット→
    cmd.exe→cmd.exe→python.exe という起動チェーンを経由すると、Windowsが
    新しいウィンドウへ自動的にフォーカスを渡さないことがあるため。

    「最前面固定→解除」を一瞬だけ行う定番の手当てを追加した。この方法は
    SetForegroundWindow が受けるフォアグラウンドロックの制限を受けないため、
    起動チェーンの長さに関わらず確実にウィンドウを前面へ出せる。
    """
    try:
        root.lift()
...
def _looks_like_zombie(pid):
    """PIDが「プロセスは生きているが、画面上に何も出ていない」状態かを判定する。

    正常に起動処理中のインスタンスをゾンビと誤判定しないよう、短い間隔で
    複数回チェックする（ZOMBIE_CHECK_ATTEMPTS 回、合計最大 ~1.2秒）。
    途中で1回でも可視ウィンドウが見つかれば、直ちに「正常」と判定する。
    """
    if not _process_is_alive(pid):
        # 既に終了済みなら、ミューテックスもまもなくOSが自然に解放するはずで、
        # 強制終了の対象ではない（呼び出し側で再取得を試みる）。
        return False
    for _ in range(ZOMBIE_CHECK_ATTEMPTS):
        try:
            if _pid_has_visible_window(pid):
                return False
        except Exception:
            return False
        time.sleep(ZOMBIE_CHECK_INTERVAL)
    return True
...
def acquire_single_instance():
    ...
    20260915_02: 「前回のランチャーがプロセスとしては残っているのに、ウィンドウが
    無く応答もしていない」ケース（クラッシュではなくハング）だけは、ミューテックスが
    自動解放されないため従来の方式では検知できなかった。この関数はその場合に限り、
    旧プロセスを終了してから自分がミューテックスを取得し直す。
    判定に確信が持てない場合は、常に「起動中」として扱う安全側に倒す
    （＝無関係なプロセスを誤って終了させることは絶対にしない）。

    Returns:
        True  : このプロセスが唯一のランチャー（旧ゾンビを片付けた場合を含む）
        False : すでに別のランチャーが起動している（通常どおり警告を出す）
    """
    ...
    try:
        ...
        # ここから「既に誰かが起動している」場合の分岐。
        if handle:
            kernel32.CloseHandle(ctypes.c_void_p(handle))

        stale_pid = _read_pid_file()
        if not stale_pid:
            # 前回の所有者が分からない（PIDファイルが無い・壊れている）。
            # 判定材料が無いため、これまでどおり「起動中」として扱う。
            return False

        if _process_is_alive(stale_pid) and not _looks_like_zombie(stale_pid):
            # 生きていて、かつ画面が確認できた＝正常に起動中。
            # 20260915_03: 「起動中」ダイアログを出すだけでなく、既存のランチャーの
            # ウィンドウをユーザーの前まで持ってくる。失敗しても実害はない
            # （ウィンドウ操作の失敗は無視され、従来どおりダイアログのみ出る）。
            _bring_existing_to_front(stale_pid)
            return False

        # ここに来るのは (a) 完全なゾンビ（生きているが画面なし）、
        # または (b) チェックの間に相手が終了していた、のいずれか。
        if _process_is_alive(stale_pid):
            basename = _process_image_basename(stale_pid)
            if basename in ("python.exe", "pythonw.exe"):
                ...
                log_message(
                    "⚠️ 前回のランチャーのPID({})が別プロセスに再利用されている疑いがあるため、"
                    "終了せず通常の起動中判定にフォールバックします".format(stale_pid))
                return False

        # 終了直後はOSがミューテックスを解放するまで一瞬のラグがあるため、
        # 短い待機を挟みながら数回だけ再取得を試みる。
        for _ in range(3):
            time.sleep(0.5)
            retry_handle = kernel32.CreateMutexW(None, 1, MUTEX_NAME)
            if kernel32.GetLastError() != ERROR_ALREADY_EXISTS:
                _mutex_handle = retry_handle
                _write_pid_file()
                log_message("✅ 新しいランチャーを起動します")
                return True
            if retry_handle:
                kernel32.CloseHandle(ctypes.c_void_p(retry_handle))

        # それでも取得できなければ、安全側（従来どおりの警告）にフォールバックする。
        return False
    except Exception:
...
class LauncherApp(object):
    ...
    def _build_header(self):
        ...
        self.pull_button = tk.Button(right, text="🔄 Pull", command=self.on_git_pull,
    ...
    def on_git_pull(self):
        ...
        self.pull_button.configure(state="disabled", text="🔄 Pull中...")
    ...
    def _pull_failed(self, error):
        self.pull_button.configure(state="normal", text="🔄 Pull")
...
def main():
    ...
    if not acquire_single_instance():
        # 20260915_03: 既存のランチャーを最前面に表示できた場合は、その手前に
        # 警告ダイアログを重ねて出さない（せっかく前面に出したのに、別ウィンドウの
        # ダイアログがさらにその上に乗って隠れてしまうのを避けるため）。
        if not _brought_existing_to_front:
    ...
    root.deiconify()
    force_own_window_to_front(root)
