...
_SHAPE_TAG_RE = re.compile(r'<(/?)([A-Za-z_][\w.\-:]*)((?:[^>"\']|"[^"]*"|\'[^\']*\')*?)(/?)>')


def _shape_find_drawing_insert_position(sheet_xml):
    """sheet_xml内で<drawing>要素を挿入すべき位置を返す。

    <worksheet>直下(ネストしていない、深さ1)の子要素の中に_SHAPE_TAGS_AFTER_DRAWING
    のいずれかがあれば、その直前を返す。無ければ</worksheet>の直前を返す。

    【重要】以前の実装は re.search(f"<{tag}[ />]", sheet_xml) を単純にファイル
    全体へ適用しており、タグの入れ子(ネスト)を一切考慮していなかった。実際の
    資料ファイルでは、<sheetPr>内のタブ色拡張や、条件付き書式・スパークライン
    などの機能でごく普通に<extLst>が<worksheet>の"直下"ではなくさらに深い場所に
    現れる(例: <sheetPr><extLst>...</extLst></sheetPr>)。この場合、旧実装は
    その"ネストした、本来無視すべき早い位置のextLst"を誤って検出し、<drawing>を
    <sheetPr>の内部などスキーマ上あり得ない位置へ挿入してしまい、不正なXMLを
    生成していた(ユーザー提供の実データで実際に発生・確認済み。合成テストでは
    ネストした拡張要素を一切含まない単純なシートしか使っていなかったため
    検出できなかった)。本実装はタグの深さを追跡し、<worksheet>の直接の子
    (深さ1)だけを対象にする。
    """
    worksheet_open = re.search(r'<worksheet\b[^>]*>', sheet_xml)
    close_idx = sheet_xml.rfind("</worksheet>")
    if not worksheet_open:
        return close_idx

    depth = 0
    for m in _SHAPE_TAG_RE.finditer(sheet_xml, worksheet_open.end(), close_idx):
        is_close, name, _attrs, self_close = m.groups()
        local_name = name.split(":")[-1]
        if is_close:
            depth -= 1
            continue
        if depth == 0 and local_name in _SHAPE_TAGS_AFTER_DRAWING:
            return m.start()
        if not self_close:
            depth += 1
    return close_idx
...
def restore_and_translate_shapes(original_path, output_path, sheet_names, target_language):
    ...
    for sheet_name, data in per_sheet_data.items():
        ...
        if sheet_rels_path in existing_data:
            ...
            # openpyxlが独自に書き出したdrawingへの関連付けを取り除く(理由は後述の
            # <drawing>要素の除去と同じ)。printerSettings・hyperlink等、drawing以外の
            # 関連付けはそのまま残す。
            for rel in list(rels_root):
                if rel.get("Type", "").endswith("/relationships/drawing"):
        ...
        # openpyxlが保存したsheetN.xmlは、他にr:id参照(pageSetup・hyperlink等)を
        # 持たない場合はxmlns:r宣言自体を出力しないことがある。その状態で
        # <drawing r:id=.../>を挿入すると「未宣言のプレフィックス」を使った
        # 不正なXMLになり、LibreOffice/Excelがこの要素だけを黙って無視して
        # 図形が描画されない(実機相当の描画検証で実際に踏んだ不具合)。
        # ルート<worksheet>要素にxmlns:r宣言が無ければ補う。
        #
        # 【重要】判定対象は<worksheet ...>タグ自身であり、その前に付く
        # XML宣言(<?xml version="1.0" ...?>)ではない。旧実装は
        # sheet_xml.split(">", 1)[0] で判定していたが、これは最初の">"、
        # つまりXML宣言の"?>"までしか見ておらず、<worksheet>タグの中身を
        # 一切見ていなかった。このためpageSetup等でxmlns:rが実際には
        # 既に宣言されている(印刷設定を持つ通常のシートでは大半がこれに該当)
        # 場合でも「無い」と誤判定し、xmlns:rを二重に追加してしまい、
        # 「同じ属性の重複」という不正なXMLを生成していた。これは
        # openpyxl・LibreOfficeの検証だけでは表面化せず、実際にExcelで
        # 開いたときに「内容の一部に問題が見つかりました」という修復
        # ダイアログとして初めて顕在化した(ユーザー提供の実データで発生・
        # 確認済み)。<worksheet ...>タグそのものを正しく取り出して判定する。
        worksheet_tag_match = re.search(r'<worksheet\b[^>]*>', sheet_xml)
        worksheet_tag = worksheet_tag_match.group(0) if worksheet_tag_match else ""
        if "xmlns:r=" not in worksheet_tag:
            sheet_xml = sheet_xml.replace(
                "<worksheet ", f'<worksheet xmlns:r="{_SHAPE_NS["r"]}" ', 1)

        # openpyxlが既に書き出した<drawing>要素があれば取り除く。
        #
        # 【重要】openpyxlは「図形は捨てるが、画像(とグラフ)は保持する」ため、
        # 元の図形XMLに画像が含まれていたシートについては、openpyxl自身が
        # 独自の<drawing>要素と、画像だけを含む独自のdrawingパートを書き出す。
        # 一方こちらは元ファイルの図形XML(画像も図形も含む完全な内容)を復元する
        # ため、両方が残ると<drawing>要素がシートに2つ存在することになる。
        # CT_Worksheetは<drawing>を1つしか許さないため、これはスキーマ違反
        # となり、Excelが「内容に問題が見つかりました」と判定して修復する。
        #
        # この不具合は、XML構文としては整形式であるため、XMLの妥当性チェック
        # でもLibreOfficeでの描画確認でも検出できなかった。さらに、openpyxlが
        # 画像を保持するのは Pillow が導入されている環境だけ(未導入だと画像も
        # 捨てられる)であり、検証用コンテナにPillowが無かったため、合成テスト
        # では一度も再現できていなかった。実データ(ユーザー提供のNPI Process
        # Flow、sheet5=Documents in Enovia)で3回連続して発生し、実ファイルを
        # 直接調べて初めて判明した。
        sheet_xml = re.sub(r'<drawing\b[^>]*/>', '', sheet_xml)
        sheet_xml = re.sub(r'<drawing\b[^>]*>.*?</drawing>', '', sheet_xml, flags=re.S)

        insert_tag = f'<drawing r:id="{new_rid}"/>'
        insert_pos = _shape_find_drawing_insert_position(sheet_xml)
