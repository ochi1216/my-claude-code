"""excel_translation_20260911_05.py の図形(Shape)保持・翻訳機能の検証テスト。
...
【_20260911_05での追加 / 真因】_20260911_04を適用しても同じ症状が3回目の再発を
した。ユーザーから破損した出力ファイルと元ファイルの両方を提供いただき、
実物を直接調べて真因が判明した。sheet5.xmlには<drawing>要素が2つあった:

    <drawing xmlns:r="..." r:id="rId1"/>   <- openpyxlが書いたもの
    <drawing r:id="rId2"/>                 <- 本ツールが復元のために挿入したもの

openpyxlは「図形(sp/cxnSp)は捨てるが、画像(pic)とグラフは保持する」ため、元の
図形XMLに画像が含まれていたシートについては、openpyxl自身が画像だけを含む
drawingパートと<drawing>要素・関連付け(rels)を書き出す。本ツールは元ファイルの
図形XML(画像も図形も含む完全な内容)を復元するため、両方が残ると<drawing>が
シートに2つ存在することになる。CT_Worksheetは<drawing>を1つしか許さないため、
これはOOXMLのスキーマ違反となり、Excelが修復対象と判定する。

この不具合が_02〜_04を通して一度もテストで再現できなかった理由は2つある:
(1) XML構文としては完全に整形式であるため、XMLの妥当性チェックでもLibreOffice
での描画確認でも検出できない(検出できるのはスキーマまで検証するExcel自身だけ)。
(2) openpyxlが画像を保持するのは **Pillowが導入されている環境だけ** であり
(未導入の場合は画像も一緒に捨てられる)、検証用コンテナにPillowが無かったため、
合成テストでは<drawing>が1つしか生成されず、そもそも再現しようがなかった。

本ファイルでは、この状態を2通りで再現する回帰テストを追加した:
(A) openpyxlを実際に通す再現(画像を含むxlsxをload_workbook->saveし、openpyxlが
    自前の<drawing>を書くところから確認する)。Pillow必須のため未導入環境では
    自動的にスキップする。
(B) openpyxl保存後の状態を直接組み立てる合成再現(Pillowの有無に依存しない)。
...
実行方法:
    python3 tests/test_excel_translation_20260911_05.py
"""
...
# ============================================================
# 回帰テスト(A): openpyxlが画像を保持して自前の<drawing>を書いた場合に、
# <drawing>要素が2つになる(スキーマ違反)ことを防ぐ
# ============================================================
# _20260911_04を適用しても3回目の再発をした不具合の、実機構そのものの再現テスト。
# openpyxlは図形は捨てるが画像は保持するため、元の図形XMLに画像が含まれていた
# シートでは、openpyxlが「画像だけのdrawingパート + <drawing>要素 + rels」を
# 独自に書き出す。そこへ本ツールが元の図形XMLを復元すると、<drawing>が2つに
# なりCT_Worksheetのスキーマに違反する(Excelが修復対象と判定する)。
#
# openpyxlが画像を保持するのは Pillow が導入されている環境だけなので、
# 未導入の環境ではこのケース自体が発生せず、テストとして成立しない。
# その場合はスキップし、Pillowに依存しない合成版(回帰テストB)で確認する。
DUP_SRC = os.path.join(WORKDIR, "dup_drawing_src.xlsx")
DUP_OUT = os.path.join(WORKDIR, "dup_drawing_out.xlsx")


def _build_xlsx_with_shape_and_image(path):
    """AutoShape(openpyxlが捨てる)と画像(openpyxlが保持する)を1つずつ持つxlsxを作る。"""
    from PIL import Image  # Pillow未導入なら呼び出し側でスキップされる
    buf = io.BytesIO()
...
<Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing" Target="../drawings/drawing1.xml"/>
    ...
    # 1つ目のアンカー: AutoShape(openpyxlは捨てる) / 2つ目: 画像(openpyxlは保持する)
    DRAWING1 = '''<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
...
<xdr:to><xdr:col>4</xdr:col><xdr:colOff>0</xdr:colOff><xdr:row>6</xdr:row><xdr:rowOff>0</xdr:rowOff></xdr:to>
<xdr:sp macro="" textlink=""><xdr:nvSpPr><xdr:cNvPr id="2" name="Rectangle 1"/><xdr:cNvSpPr/></xdr:nvSpPr>
<xdr:spPr><a:xfrm><a:off x="0" y="0"/><a:ext cx="2743200" cy="1524000"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></xdr:spPr>
<xdr:txBody><a:bodyPr/><a:p><a:r><a:rPr lang="en-US" sz="1800"/><a:t>Generate manhours and cost.</a:t></a:r></a:p></xdr:txBody>
</xdr:sp><xdr:clientData/>
</xdr:twoCellAnchor>
...
<xdr:spPr><a:xfrm><a:off x="0" y="0"/><a:ext cx="914400" cy="914400"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></xdr:spPr>
...
<Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/image1.png"/>
    ...
    with zipfile.ZipFile(path, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        z.writestr("xl/worksheets/_rels/sheet1.xml.rels", SHEET1_RELS)
        z.writestr("xl/drawings/drawing1.xml", DRAWING1)
        z.writestr("xl/drawings/_rels/drawing1.xml.rels", DRAWING1_RELS)
        z.writestr("xl/media/image1.png", png)


def _count_drawing_refs(zf, sheet_path="xl/worksheets/sheet1.xml"):
    """シートXMLの<drawing>要素と、シートrelsのdrawing種別の関連付けを数える。"""
    sheet_xml = zf.read(sheet_path).decode("utf-8")
    elems = re.findall(r'<drawing\b[^>]*>', sheet_xml)
    rels_path = sheet_path.replace("xl/worksheets/", "xl/worksheets/_rels/") + ".rels"
    rels = []
    if rels_path in zf.namelist():
        for el in ET.fromstring(zf.read(rels_path)):
            if el.get("Type", "").endswith("/relationships/drawing"):
                rels.append((el.get("Id"), el.get("Target")))
    return sheet_xml, elems, rels
...
if _HAS_PILLOW:
    _build_xlsx_with_shape_and_image(DUP_SRC)
    shutil.copy2(DUP_SRC, DUP_OUT)
    wb = openpyxl.load_workbook(DUP_OUT)
    wb.save(DUP_OUT)
    wb.close()

    # 前提条件: openpyxlが図形を捨てたうえで、画像のために自前の<drawing>を書くこと。
    # これが成立しない環境ではこの回帰テストは意味を持たない。
    with zipfile.ZipFile(DUP_OUT) as z:
        _sx, _elems, _rels = _count_drawing_refs(z)
        check("openpyxlは画像を保持するため自前の<drawing>を書き出す(前提条件の確認)",
              len(_elems) == 1 and len(_rels) == 1, f"elems={_elems} rels={_rels}")
        check("openpyxl保存直後はAutoShapeが失われている(前提条件の確認)",
              "Generate manhours and cost." not in z.read("xl/drawings/drawing1.xml").decode("utf-8"))

    n_dup = mod.restore_and_translate_shapes(DUP_SRC, DUP_OUT, {"Flow Chart"}, "Japanese")

    with zipfile.ZipFile(DUP_OUT) as z:
        sheet_xml, elems, rels = _count_drawing_refs(z)
        check("画像を含むシートでも<drawing>要素が1つだけになる"
              "(回帰テスト: 実データで3回連続再発した不具合の真因)",
              len(elems) == 1, f"{len(elems)}個: {elems}")
        check("画像を含むシートでもdrawing種別の関連付けが1つだけになる",
              len(rels) == 1, f"{len(rels)}件: {rels}")
        if len(elems) == 1 and len(rels) == 1:
            rid = re.search(r'r:id="([^"]+)"', elems[0])
            check("残った<drawing>のr:idが、復元した図形XMLを指している"
                  "(openpyxlの画像だけのdrawingではない)",
                  rid is not None and rid.group(1) == rels[0][0]
                  and rels[0][1].endswith("drawings/drawing1.xml"),
                  f"elem={elems[0]} rel={rels[0]}")
        check("画像パートが失われていない", "xl/media/image1.png" in z.namelist())
        check("AutoShapeが復元され、その中の文字が翻訳されている",
              n_dup == 1 and "YAKU" in z.read("xl/drawings/drawing1.xml").decode("utf-8"),
              f"{n_dup}件")
        try:
            ET.fromstring(sheet_xml)
            valid = True
            ...
            valid = False
            print(f"   ParseError: {e}")
        check("画像を含むシートでも、結果のsheet1.xmlが妥当なXMLである", valid)

    if soffice := shutil.which("soffice"):
        try:
            subprocess.run([soffice, "--headless", "--convert-to", "pdf", "--outdir", WORKDIR, DUP_OUT],
                           capture_output=True, timeout=60, check=True)
            check("画像+図形ケースでもLibreOfficeが修復ダイアログ無しで変換できる",
                  os.path.exists(os.path.join(WORKDIR, "dup_drawing_out.pdf")))
        except Exception as e:
            check("画像+図形ケースでもLibreOfficeが修復ダイアログ無しで変換できる", False, str(e))
else:
    print("SKIP: Pillow未導入のため、openpyxlを実際に通す<drawing>重複の再現は省略"
          "(Pillowが無いとopenpyxlは画像も捨てるため、この状態自体が発生しない)")


# ============================================================
# 回帰テスト(B): 上記と同じ状態を直接組み立てて確認する(Pillow非依存)
# ============================================================
# 回帰テスト(A)はopenpyxlとPillowの実際の挙動に依存するため、それらのバージョンや
# 導入状況によっては前提条件が崩れてスキップされうる。真因(シートに<drawing>が
# 2つ生まれること)そのものは、openpyxl保存後の状態を直接組み立てれば環境に依存
# せず再現できるため、こちらも併せて固定化しておく。
# あわせて、drawing以外の関連付け(printerSettings)が巻き添えで消えないことも確認する。
DUP_SYNTH_OUT = os.path.join(WORKDIR, "dup_drawing_synth.xlsx")


def _build_already_saved_xlsx_with_openpyxl_drawing(path):
    """openpyxlが画像を保持して<drawing>を書いた後の状態を、実際の出力と同じ形で作る。

    実データ(ユーザー提供の破損ファイル)のsheet5.xml・そのrelsと同じ形にしてある:
      - <drawing xmlns:r="..." r:id="rId1" />  (xmlns:rをこの要素自身に持つ)
      - rels の Target は "/xl/drawings/drawing1.xml" (先頭スラッシュの絶対形式)
    """
    CONTENT_TYPES = '''<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types">
<Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/>
...
<Relationship Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/printerSettings" Target="../printerSettings/printerSettings1.bin" Id="rId2"/>
</Relationships>'''
    # openpyxlが書き出す「画像だけのdrawing」(元の図形は含まれない)
    OPENPYXL_DRAWING = ('<?xml version="1.0" encoding="UTF-8"?>'
    ...
    with zipfile.ZipFile(path, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        z.writestr("xl/worksheets/sheet1.xml", SHEET1)
        z.writestr("xl/worksheets/_rels/sheet1.xml.rels", SHEET1_RELS)
        z.writestr("xl/drawings/drawing1.xml", OPENPYXL_DRAWING)
        z.writestr("xl/printerSettings/printerSettings1.bin", b"dummy")


_build_already_saved_xlsx_with_openpyxl_drawing(DUP_SYNTH_OUT)
mod.restore_and_translate_shapes(ORIGINAL, DUP_SYNTH_OUT, {"Flow Chart"}, "Japanese")

with zipfile.ZipFile(DUP_SYNTH_OUT) as z:
    sheet_xml, elems, rels = _count_drawing_refs(z)
    check("openpyxlが書いた<drawing>が残らず、要素は1つだけになる(合成再現・Pillow非依存)",
          len(elems) == 1, f"{len(elems)}個: {elems}")
    check("openpyxlが書いたdrawingの関連付けが残らず、1件だけになる(合成再現)",
          len(rels) == 1, f"{len(rels)}件: {rels}")
    all_rels = [(el.get("Id"), el.get("Type", "").rsplit("/", 1)[-1])
                for el in ET.fromstring(z.read("xl/worksheets/_rels/sheet1.xml.rels"))]
    check("drawing以外の関連付け(printerSettings)は巻き添えで消えない",
          any(t == "printerSettings" for _i, t in all_rels), str(all_rels))
    check("printerSettingsのr:id(rId2)と、新しいdrawingのr:idが衝突していない",
          len({i for i, _t in all_rels}) == len(all_rels)
          and 'r:id="rId2"' in sheet_xml, str(all_rels))
    try:
        ET.fromstring(sheet_xml)
        valid = True
    except ET.ParseError as e:
        valid = False
        print(f"   ParseError: {e}")
    check("合成再現ケースでも、結果のsheet1.xmlが妥当なXMLである", valid)
    check("合成再現ケースでも図形が復元され、翻訳されている",
          "YAKU" in z.read("xl/drawings/drawing1.xml").decode("utf-8"))

if soffice := shutil.which("soffice"):
    try:
        subprocess.run([soffice, "--headless", "--convert-to", "pdf", "--outdir", WORKDIR, DUP_SYNTH_OUT],
                       capture_output=True, timeout=60, check=True)
        check("合成再現ケースでもLibreOfficeが修復ダイアログ無しで変換できる",
              os.path.exists(os.path.join(WORKDIR, "dup_drawing_synth.pdf")))
    except Exception as e:
        check("合成再現ケースでもLibreOfficeが修復ダイアログ無しで変換できる", False, str(e))
