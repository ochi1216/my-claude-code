...
TRANSCRIBE_PROMPT = (
    ...
    "出力形式は1行ごとに次の形式にしてください(前置きや説明文は書かないこと)。\n"
    "[分:秒] 話者A: 発言内容\n"
...
TIMESTAMP_PATTERN = re.compile(r"^\[(\d+):(\d{2})\]\s*([^:：]{1,20})[:：]\s*(.+)$")
...
def parse_transcript_response(text):
    ...
    for raw_line in text.splitlines():
        ...
        if not match:
            ...
            continue
        minutes, secs, speaker, content = match.groups()
        segments.append({
...
def transcribe_with_gemini(audio_path, config, log):
    ...
    if not segments:
        raise RuntimeError("Geminiの文字起こし結果を解析できませんでした。")
    return segments
...
def write_transcript(segments, output_dir, source_file, mode):
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        ...
        for seg in segments:
            ...
            f.write(f"[{format_time(seg['start'])}] {speaker_label}{seg['text']}\n\n")
...
def run_pipeline(input_path, mode, config, log, progress):
    ...
    try:
        ...
        if mode == "cloud":
            segments = transcribe_with_gemini(audio_path, config, log)
        else:
