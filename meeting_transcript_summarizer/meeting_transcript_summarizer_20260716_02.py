...
def transcribe_with_gemini(audio_path, config, log, debug_dir=None):
    ...
    log("Geminiへ音声をアップロードしています...")
    uploaded = client.files.upload(file=audio_path)
    uploaded = _wait_for_gemini_file_active(client, uploaded, log)

    log("Geminiで文字起こしを実行しています(音声の長さにより数分かかる場合があります)...")
    ...
    segments = parse_transcript_response(raw_text)
    if not segments:
        preview = raw_text.strip()[:500] or "(空の応答)"
        location_hint = f"\n応答全文はこちらに保存しました: {debug_path}" if debug_path else ""
        raise RuntimeError(f"Geminiの応答を解析できませんでした。応答内容(先頭500文字):\n{preview}{location_hint}")
    return segments
...
def transcribe_with_whisper(audio_path, config, log):
    try:
...
def run_pipeline(input_path, mode, config, log, progress):
    ...
    try:
        ...
        if mode == "cloud":
            segments = transcribe_with_gemini(audio_path, config, log, debug_dir=output_dir)
        else:
            segments = transcribe_with_whisper(audio_path, config, log)
        progress(55)
        ...
        summary_md_path, _ = write_summary(summary, output_dir)
...
class App(tk.Tk):
    ...
    def _build_widgets(self):
        ...
        self.run_button.grid(row=5, column=0, pady=10, sticky="w")

        self.progress = ttk.Progressbar(frame, orient="horizontal", mode="determinate")
        self.progress.grid(row=6, column=0, columnspan=2, sticky="we")

        ttk.Label(frame, text="ログ:").grid(row=7, column=0, sticky="w", pady=(10, 0))
        ...
        def worker():
            try:
                output_dir = run_pipeline(
                    input_path, self.mode.get(), self.config_data, self._log, self._set_progress
                )
