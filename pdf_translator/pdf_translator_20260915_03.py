...
v20260915_03での変更点（v20260915_02で箇条書きの過剰結合は改善したが、「翻訳じたいは
かなり改善されたが、箇条書きの・や、アンダーバーの配置、フォント等が次のレベルの課題」
という報告を受け、久保内様の実データを再調査して特定した3件の不具合・改善）:

    - **【課題1・最重要】同一行内のスパン並べ替えで、箇条書き記号・波ダッシュ・括弧の
      位置がずれる。** `extract_translatable_blocks` 内で、複数行にまたがる段落の
      「上から下」順序を保証するために行っていた `(round(y0,1), x0)` によるスパンの
      並べ替えが、同一行内のスパンまで並べ替えてしまっていた。箇条書き記号「・」や
      波ダッシュ「～」は本文と異なるフォント・サイズで描画されており、実際の文字
      インクのbboxのy0が本文よりわずか（1〜2pt）にズレることがあるため、この並べ替え
      が本来は行頭にあるはずの記号を行末や文中へ移動させていた（実データで確認:
      `・Solve customer technical problems.` が
      `Solve customer technical problems. ・` になる、
      `Mar. 2019 ～ Present` が `Mar. 2019 Present ～` になる、
      `(AVNET KK, KEC-Japan)` が `AVNET KK , KEC-Japan( )` になる、等）。
      → 該当のソート処理を削除した。この時点で各スパン列は既に正しい読み順
      （PyMuPDFの行・X座標の昇順で構築済み）になっており、並べ替え自体が不要かつ
      有害だった。
    - **【課題2】見出し・氏名の下線が、墨消しの白塗りに隠れて消える。** 原文PDFの
      下線はフォントの装飾ではなく、テキストの直下に別途描画された細い塗り矩形
      だった。この矩形自体は墨消しの対象外だが、翻訳文を書き戻す前に描く墨消し用の
      白塗り矩形がちょうど上に重なるため、見た目上は完全に隠れていた。
      → `_detect_underlines` で墨消し前のページから下線らしき矩形を検出し、
      `_draw_text_fitted`／`_draw_mixed_text` が翻訳文の実際の描画幅に合わせて
      下線を引き直すようにした（元の下線の幅をそのまま使うと、文字幅が変わった
      場合にズレるため）。
    - **【課題4】箇条書き記号の翻訳結果が一貫しない。** 同じ「◇」記号が、あるブロック
      ではそのまま保持される一方、別のブロックではGeminiに「：」等へ意訳されてしまう
      表記ゆれがあった。→ Geminiへのプロンプトに、箇条書き記号（•・◦▪◇‣∙○■➤→･等）
      は翻訳・変換せずそのまま保持するよう明示的な指示を追加した。
    - 課題3（フォントがオリジナルと不統一）・課題5（アンダースコア「_」が原文のまま
      残る）は、ユーザー判断によりそれぞれ保留・対応不要とした（詳細はCHANGELOG参照）。

v20260915_02での変更点（「片方はうまくフォーマットがとれているが、片方はだいぶ崩れて
いる」という報告を受け、2つの実際の英文レジュメ（Goecke様・久保内様）を比較して原因を
特定した。v20260915_01で「1つの生ブロックが複数行にまたがるPDF」は行単位に正しく分割
できるようになったが、行頭が箇条書き記号（•・◦▪等）で始まる行を独立した項目として
扱う対策（v20260915_02の開発中に追加）が、Goecke様のレジュメでは効いたのに久保内様の
レジュメでは全く効かないという現象が見つかった）:

    - **【根本原因】箇条書き記号の位置がPDFによって異なり、行頭判定だけでは
      検出できない書式があった。** Goecke様のレジュメは箇条書き記号が各項目の
      「行頭」に付く一般的な書式（`•Worked as ...`）だったが、久保内様のレジュメは
      記号が前の項目の「行末」に付く書式だった（`Solve customer technical
      problems. ・` の次の行が `Create materials and tools for sales
      representatives. ・` …と続く）。既存の `_starts_with_bullet` は行頭しか
      見ていなかったため、久保内様のレジュメでは一度も検出されず、7個の独立した
      箇条書き項目が引き続き1つの122.2pt超の翻訳単位へ結合されたままだった。
      日本語訳はコンパクトな1文になりがちなため、この巨大な矩形の下半分が空白の
      まま残り、「フォーマットが崩れて見える」原因になっていた。
      → 行末が箇条書き記号で終わっているかを判定する `_ends_with_bullet` を追加し、
      直前のグループが箇条書き記号で終わっている場合も、次の行を結合しないように
      `_merge_paragraph_groups` のガード条件を拡張した（行頭・行末どちらの書式でも
      対応できる）。
      実データ（久保内様レジュメ）で、35個で固着していた翻訳単位が49個に増え、
      最大122.2ptあった巨大ブロックが60.5pt以下まで縮小することを確認した。

    実データ（久保内様の英文レジュメ）での効果:
...
v20260915_01での変更点（「改善はしているが、フォーマットがすべて消失しているように
見える」という報告を受け、実際の英文レジュメPDF（Word書き出し）で原因を特定した。
このPDFは、ページ全体（55行）がPyMuPDF上でたった1つの「生ブロック」として返る構成
だった。既存のコードはこの生ブロック単位で「X座標の隙間が一定以上離れていたら
別々の翻訳単位に分割する」処理を行っていたが、判定にY座標（行の情報）を一切使って
いなかったため、左揃えの本文（各行がほぼ同じX座標から始まる）ではX座標の隙間が
一度も閾値を超えず、55行すべてが1つの巨大な翻訳単位へ融合していた。結果として、
氏名・見出し・箇条書きのフォントサイズ・太字・寄せがすべて先頭行の値に統一され、
寄せの判定もページ全体を覆う矩形を見て「中央寄せ」と誤判定していた）:

    - **【根本原因】ブロック内のスパンをX座標だけで分割しており、行の情報を捨てていた。**
      PyMuPDFのブロックは、スライド系PDFでは「1ブロック≒1行」であることが多いが、
      Word等から書き出されたPDFでは「1ブロック＝ページ全体（複数行）」であることが
      ある。後者の場合、既存のX座標ギャップ分割（同一行内で離れた場所にある要素を
      分割する目的で導入したもの）が、行をまたいで無関係な複数行を1つに結合して
      しまっていた。
      → PyMuPDFが返す「行」単位を最初の分割単位として使うように変更した（従来は
      スパンごとに保持していなかった行番号を新たに記録するようにした）。同一行内で
      離れた位置にある要素を分割する、という本来の目的は維持しつつ、行をまたいだ
      誤結合が起きなくなった。複数行にまたがる段落の再結合は、引き続き
      `_merge_paragraph_groups`（フォントサイズ・色・左端・行間の一致を見て判断する、
      より厳密な処理）に任せている。
      実データで、ページ全体が2ブロックに融合していたものが、35個の正しく分離された
      翻訳単位（氏名・各見出し・各箇条書き等）に分割されることを確認した。
    - **複数行にまたがる段落が、ページ全体を見て「中央寄せ」と誤判定されていた。**
      `_guess_free_alignment` は左右の余白の対称性だけを見ていたため、左揃えの
      折り返し段落がページの大半の幅を占めると、たまたま左右の余白が近い値になり
      「中央寄せ」と誤判定されることがあった。2行以上にまたがるブロック（＝段落として
      結合されたもの）は寄せの推定を行わず常に左寄せとする、また単一行であっても行の
      幅がページ幅の78%を超える場合は中央寄せとみなさない、という2つのガードを追加した。
      実データで、35ブロック中34ブロックが正しく左寄せと判定されるようになった
      （残る1件は、罫線の無い2カラム風レイアウトの右側セルで、構造的に判別が難しい
      既知の限界として残る）。

    実データ（英文レジュメ、2ページ・55行）での効果:

    | 指標 | 修正前 | 修正後 |
    |---|---|---|
    | 抽出された翻訳単位の数 | 2個 | **35個** |
    | 保持されているフォントサイズの種類数 | 1種類 | **4種類（11/12/14/15pt）** |
    | 中央寄せに誤判定された自由配置ブロック | ほぼ全部 | **1/35件** |

v20260914_01での変更点（2つの重大な不具合を修正。実際のレジュメPDF（英語→日本語）で
「文章が完全に欠落している」「PDFを開いても文字が全く表示されない（開き直すと表示
される）」という2つの報告を受け、原因を特定した）:

    - **【最も重大】Gemini APIの応答が複数行にまたがると、2行目以降が丸ごと失われて
      いた。** `translate_batch_gemini()` の応答パーサーは「`[番号]` で始まる行だけ」を
      1行ずつ正規表現で拾っており、`[番号]` の付いていない行（＝その項目の2行目以降）
      は無条件に捨てていた。翻訳元テキストが「見出し＋箇条書き複数個」のような1つの
      ブロックのとき、Geminiは1項目の訳文を実際の改行付き箇条書きとして返すことが
      あり、その場合は最初の行（見出し部分）しか結果に反映されず、本文（箇条書きの
      ほとんど）が丸ごと消えていた。実際の英文レジュメを日本語へ翻訳したところ、
      「OVERVIEW」等の見出し語だけが残り、直後の4行の職務内容がすべて欠落する形で
      再現した（翻訳そのものは行われており、パース時に失われていたことをGemini
      応答を模したテストで確認済み）。このバグは初版（`ppt_translation_20260309_03.py`
      由来）から存在しており、英語→日本語・日本語→英語の両方向で、翻訳元テキストが
      複数の文・箇条書きを含む場合に発生しうる。
      → `[番号]` の出現位置どうしの間（＝次の `[番号]` の直前まで）をまるごとその
      項目の訳文として取り込むように変更した。1項目が複数行で返ってきても内容を
      失わない。
    - **PDFを開いても日本語（中国語・韓国語）の文字が全く表示されないことがあった。**
      翻訳先が日本語／中国語／韓国語のとき、`page.insert_textbox(fontname="japan", ...)`
      という、PDFへフォントを埋め込まない経路（Adobe-Japan1 のCID参照）で描画して
      いた。この経路で生成された文字は、閲覧環境に対応する日本語フォントが無い、
      または読み込みが間に合わない場合に表示されないことがある（実データの出力PDFで、
      使用フォント "Gothic" が embed=n/a＝埋め込みなしだったことを確認した。「開き直す
      と表示される」という報告は、ビューア側のフォント代替処理が後から追いついた
      ためと考えられる）。
      → 翻訳先が英文フォントで描けない言語（欧文フォント "helv" 以外）のときは、
      常にフォント埋め込みが保証されるTextWriter経由の描画（`_draw_mixed_text`）を
      使うようにした。これは20260821_02で「③」等の記号だけをこの経路に通した際に
      確認済みの効果（Droid Sans Fallbackとして実体をPDFへサブセット埋め込みする）を、
      日本語／中国語／韓国語への翻訳全体に広げたもの。

    実際のレジュメPDF（英語→日本語）で、上記2点を再現する条件（箇条書きを含む
    ブロック・複数行応答）の疑似翻訳を通した結果:
      - 修正前: 「見出しだけ残り本文の箇条書きが消える」ブロックが複数発生
      - 修正後: 全ブロックで箇条書きの本文が保持され、使用フォントもすべて埋め込み
        あり（'n/a' なし）になることを確認

v20260911_01での変更点（出力ファイル名の言語コードの統一）:
    - 中国語（簡体字）へ翻訳したときの末尾を `_zh` から **`_cn`** へ変更した。
      国名に沿った表記のほうが分かりやすい、というユーザーの指定による
      （ISO 639-1 の言語コードとしては "zh" が正しいが、運用上の分かりやすさを優先）。
    - 繁体字（`_tw`）の対応も一覧に加えた（GUIの選択肢には未追加）。
    - 日本語 `_ja` / 英語 `_en` は従来どおり変更なし。
      例: `資料.pdf` を英訳 → `資料_en.pdf`、中国語簡体字へ翻訳 → `資料_cn.pdf`
...
使い方:
    ...
    3. python pdf_translator_20260915_03.py
...
def translate_batch_gemini(texts, target_language="Japanese", batch_idx=0, logger=None):
    ...
    5. If a line contains a bullet/list marker symbol (e.g. •, ・, ◦, ▪, ◇, ‣, ∙,
       ○, ■, ➤, →, ･), keep that exact symbol as-is in the translation, in the same
       position relative to the text. Do not translate it, drop it, or replace it
       with different punctuation (e.g. do not turn "◇" into ":" or "-").
    ...
    for attempt in range(1, 4):  # 最大3回リトライ
        try:
            ...
            # [番号] の開始位置を全て探し、次の [番号] の直前までを丸ごとその項目の
            # 訳文とする。
            # v20260914_01での修正: 旧実装は「[番号]で始まる行だけ」を1行ずつ正規表現で
            # 拾っており、[番号]の付いていない行（＝その項目の2行目以降）は無条件に
            # 捨てていた。翻訳元テキストが「見出し＋箇条書き複数個」のような1ブロックの
            # とき、Geminiは1項目の訳文を実際の改行付き箇条書きとして返すことがあり、
            # その場合は最初の行（見出し部分）しか結果に反映されず、本文（箇条書きの
            # ほとんど）が丸ごと消えていた。実際の英文レジュメを日本語へ翻訳した際に、
            # 「OVERVIEW」等の見出し語だけが残り、直後の4行の職務内容がすべて欠落する
            # 形で発生することを確認した（翻訳自体は行われており、パース時に失われていた）。
            # 対策として、[番号]の出現位置どうしの間（＝次の[番号]の直前まで）を
            # まるごとその項目の訳文として取り込むようにした。1項目が複数行で返って
            # きても内容を失わない。
            markers = list(re.finditer(r'^\s*\[(\d+)\]\s*', response_text, flags=re.MULTILINE))
            for i, m in enumerate(markers):
                ...
                results[idx] = response_text[start:end].strip()
...
def _guess_free_alignment(bbox, box_rect, page_rect):
    ...
    # v20260915_01での追加: 左右の余白が近いだけでなく、行自体がページ幅の大半を
    # 占めていないことも中央寄せの条件に加えた。左寄せの1行が偶然ほぼページ幅いっぱい
    # まで伸びていると、左右の余白がどちらも小さいために「近い値＝中央寄せ」と
    # 誤判定されることがある（実データの英文レジュメで、職務タイトルの1行がこの形で
    # 誤判定されていた）。本当に中央寄せの見出しは、周囲に意味のある余白を残すはずなので、
    # 行の幅がページ幅の78%を超える場合は中央寄せとみなさない。
    fills_width = bbox.width > width * 0.78

    if left > width * 0.06 and not fills_width and abs(left - right) <= max(10.0, width * 0.035):
...
# 行頭・行末にあれば「この行は独立した箇条書きの1項目である」とみなす記号。
# 曖昧さを避けるため、通常の文字（アルファベット・数字・ハイフン等）は含めない。
_BULLET_CHARS = "•・◦▪‣∙○■➤→･"


def _starts_with_bullet(text):
    """行頭が箇条書き記号（•・◦▪等）で始まっているか"""
    t = (text or "").strip()
    return bool(t) and t[0] in _BULLET_CHARS


def _ends_with_bullet(text):
    """行末が箇条書き記号（•・◦▪等）で終わっているか

    実データ（久保内様の英文レジュメ）で確認: このPDFでは箇条書き記号が次の
    項目の「行頭」ではなく、前の項目の「行末」に付く書式だった
    （例: 'Solve customer technical problems. ・' の次の行が
    'Create materials and tools for sales representatives. ・'）。
    行頭判定（_starts_with_bullet）だけでは、この書式の箇条書きを検出できず、
    7項目が1つの巨大な翻訳単位へ結合されたまま残ってしまう不具合があった。
    """
    t = (text or "").rstrip()
    return bool(t) and t[-1] in _BULLET_CHARS
...
def _merge_paragraph_groups(groups):
    ...
    v20260915_02での修正（「改善はしているが、ページの後半が不自然に空白だらけになる」
    報告への対策。実際のレジュメPDFで再現・確認した）:
      本来は「1つの文が複数行に分割されている場合の再結合」だけを狙ったものだったが、
      判定基準（フォントサイズ・色・左端・行間が同じ）は、**別々の箇条書き項目**にも
      ほぼそのまま当てはまってしまう（各項目が同じ書式・同じインデントで並ぶのが
      箇条書きの性質そのものであるため）。その結果、7個の独立した箇条書き項目が
      1つの巨大な翻訳単位へ結合されるケースが実データで確認された。
      箇条書き項目はそれぞれ独立した文なので、1つに結合すると誤訳・意味の混同の
      リスクが増えるだけでなく、より深刻な副作用があった: 結合前は元の英語の高さの
      合計だった矩形（7項目ぶん、100pt超）に対し、Geminiへ「簡潔に訳す」よう指示して
      いることもあって、日本語訳はコンパクトな1つの段落になりがちで、矩形の下半分が
      丸ごと空白として残っていた。これが「フォーマットが崩れて見える」主要因の1つ
      だった（英語の記述が長く・箇条書きが多い資料ほど空白が目立ちやすく、資料に
      よって症状の見え方が大きく変わっていた）。
      → 行頭が箇条書き記号（•・◦▪等）で始まっている行は、**独立した1項目として
      扱い、前後の行と結合しない**ようにした。箇条書きの記号が無い、単なる文の
      折り返し（同じ文が2行に分かれているだけのケース）は、従来どおり結合される。
    """
    if not groups:
        return groups
    ...
    def text_of(recs):
        return "".join(r.get("text", "") for r in recs)

    ordered = sorted(groups, key=lambda g: (round(bbox_of(g).y0, 1), bbox_of(g).x0))
    merged = []
    for recs in ordered:
        # この行自体が箇条書きの新しい項目として始まっている場合、直前のグループへは
        # 結合しない（直前が箇条書きでも、ただの折り返しでも、常に独立させる）。
        # また、直前のグループが箇条書き記号で「終わっている」場合も結合しない。
        # PDFによっては記号が次項目の行頭ではなく前項目の行末に付く書式があり
        # （実データの久保内様レジュメで確認）、行頭判定だけでは検出できないため。
        if (merged and not _starts_with_bullet(text_of(recs))
                and not _ends_with_bullet(text_of(merged[-1]))):
            prev = merged[-1]
...
# 下線らしき塗り矩形とみなす条件（v20260915_03で追加）。
#   高さ: 下線は細い帯（実データで確認した値は0.6pt）なので、罫線や図形と誤認しない
#         よう十分に低いものだけを対象にする。
#   幅  : 表の罫線（ページ幅近くまで伸びる横線）を誤検出しないよう、対応するテキストの
#         幅に対して極端に広い矩形は除外する。
_UNDERLINE_MAX_HEIGHT = 2.0
_UNDERLINE_MIN_WIDTH = 4.0
_UNDERLINE_MAX_WIDTH_RATIO = 2.0


def _detect_underlines(page, infos):
    """テキストブロックの直下に、下線として描かれた塗り矩形が無いか調べ、
    見つかったブロックへ `info["underline"] = True` を付与する。

    v20260915_03での追加（「見出し・氏名の下線が消える」不具合への対策。実データの
    久保内様レジュメで原因を特定した）:
      原文PDFの下線は、フォントの装飾（span flags）ではなく、テキストの直下に
      別途描画された細い塗り矩形（`page.get_drawings()` で拾える罫線オブジェクト）
      だった。この矩形自体は墨消し（redaction）の対象外（`apply_redactions` に
      `graphics=PDF_REDACT_LINE_ART_NONE` を指定済みのため消去されない）だが、
      翻訳文を書き戻す前に描く墨消し用の塗りつぶし矩形（背景色で覆う「白塗り」）が
      ちょうどこの下線の上に重なって描かれるため、下線オブジェクト自体は残っていても
      見た目上は完全に隠れてしまっていた（実データで、下線の矩形オブジェクトが
      出力PDFのdrawingsに残っているにもかかわらず、目視では全く見えないことを確認）。
      この関数は「消す前」の元のページから下線らしき矩形を検出するだけで、実際の
      再描画は `_draw_text_fitted`／`_draw_mixed_text` が翻訳文の実際の描画幅に
      合わせて行う（元の下線の幅をそのまま使うと、翻訳後に文字幅が変わった場合に
      文字とズレるため）。
    """
    try:
        drawings = page.get_drawings()
    ...
    for d in drawings:
        ...
        if r is None:
            continue
        if r.height <= _UNDERLINE_MAX_HEIGHT and r.width >= _UNDERLINE_MIN_WIDTH:
            thin_rects.append(r)
    if not thin_rects:
        return
    ...
    for info in infos:
        if info.get("is_cell"):
            continue  # 表セルの罫線と誤検出しないよう対象外
        bbox = info["bbox"]
        for r in thin_rects:
            # 下線の位置はテキストのbboxの下端付近だが、フォントのディセンダー分だけ
            # bboxの内側（上）にめり込むことがある（実データで氏名の見出しで1.4pt
            # めり込んでいるのを確認）。フォントによってこのめり込み量が変わるため、
            # 許容範囲は上下とも余裕を持たせている。
            if not (bbox.y1 - 3.0 <= r.y0 <= bbox.y1 + 4.0):
                continue
            if r.width > bbox.width * _UNDERLINE_MAX_WIDTH_RATIO:
                continue  # 表の罫線など、テキストよりずっと広い矩形は下線ではない
            overlap = min(bbox.x1, r.x1) - max(bbox.x0, r.x0)
            if overlap >= min(bbox.width, r.width) * 0.6:
                info["underline"] = True
                break
...
def extract_translatable_blocks(doc, protect_graphics=False, target_pages=None, logger=None):
    ...
    for page_index in range(len(doc)):
        ...
        for b_idx, block in enumerate(page_dict.get("blocks", [])):
            ...
            for l_idx, line in enumerate(block.get("lines", [])):
                for span in line.get("spans", []):
                    ...
                    span_records.append({
                        "block_idx": b_idx,
                        # PyMuPDFが返す「行」の番号。v20260915_01で追加（下記参照）。
                        "line_key": (b_idx, l_idx),
        ...
        #
        # v20260915_01での修正（「英文レジュメを日本語へ翻訳すると、フォーマットが
        # すべて消失し、ページ全体が1つの中央寄せの塊になる」不具合への対策）:
        # 上記のX座標ギャップ分割を「PyMuPDFのブロック単位」でまとめて行っていたため、
        # Word等から書き出されたPDF（1つのブロックがページ全体・55行にわたる1つの
        # テキストフレームであることがある。スライド系PDFのように「1ブロック≒1行」
        # ではない）では、名前・見出し・箇条書きなど全く別々の行のスパンが1つの
        # ブロックへまとめて投入され、すべての行の左端がほぼ同じX座標（≒本文の左余白）
        # であるためX座標ギャップが一度も閾値を超えず、ページ全体が1つの巨大な
        # 翻訳単位に融合していた（実データで確認: 55行のレジュメが2ブロックに縮退し、
        # フォントサイズ・太字・寄せがすべて先頭行の値に統一され、寄せの判定も
        # ページ全体を覆う矩形を見て「中央寄せ」と誤判定していた）。
        # 対策として、X座標ギャップ分割はPyMuPDFの「行」単位でのみ行うようにした
        # （同一行内で離れた位置にある要素の分割、という本来の目的はそのまま
        # 果たしつつ、行をまたいだ誤結合を防ぐ）。複数行にまたがる段落の再結合は、
        # 引き続き後段の _merge_paragraph_groups（フォントサイズ・色・左端・行間の
        # 一致を見て判断する、より厳密な処理）に任せる。
        GAP_SPLIT_THRESHOLD = 30.0
        remaining_by_line = {}
        for rec in span_records:
            if id(rec) in assigned_ids:
                continue
            remaining_by_line.setdefault(rec["line_key"], []).append(rec)

        grouped_records = []
        for recs in remaining_by_line.values():
        ...
        # v20260915_03での修正（「箇条書き記号・波ダッシュ・括弧の位置がずれる」不具合
        # への対策。実データの久保内様レジュメで原因を特定した）:
        #   ここより上のこの時点で、各グループ（`current`）は1つのPyMuPDF「行」
        #   （同一line_key）のスパンだけで構成されており、`recs_by_x`（X座標昇順）を
        #   元に順序どおり積み上げてあるため、既に正しい左→右の読み順になっている。
        #   従来はここで改めて (y0, x0) 順に並べ替えていたが、箇条書き記号「・」
        #   「◇」や波ダッシュ「～」は本文と異なるフォント・サイズで描画されており、
        #   PyMuPDFが計算する実際の文字インクのbboxのy0が本文より1〜2pt程度ズレる
        #   ことがある。この並べ替えがそのズレを拾ってしまい、本来は行頭にあるはずの
        #   記号が行末や文中へ移動する不具合が発生していた（実データで確認:
        #   `・Solve customer technical problems.` が
        #   `Solve customer technical problems. ・` になる、
        #   `Mar. 2019 ～ Present` が `Mar. 2019 Present ～` になる、
        #   `(AVNET KK, KEC-Japan)` が `AVNET KK , KEC-Japan( )` になる、等）。
        #   同一行内では既に正しい順序なので、ここでの並べ替えは不要かつ有害と判断し、
        #   削除した。

        # 1行ずつ別ブロックになっている同一段落をまとめる（文脈の維持とフォント
        # サイズの不揃い解消のため）。詳細は _merge_paragraph_groups を参照。
        # `_merge_paragraph_groups` は行間の近さ・左端の一致などを見て、既に
        # 正しい上→下順に並んだグループ列（`ordered`）を作った上で連結するため、
        # 連結後の並び順も自動的に正しい（同じ理由でここでも並べ替えを行わない）。
        ...
        for recs in grouped_records:
            ...
            # 複数の元の行にまたがるブロック（_merge_paragraph_groups で結合された、
            # ＝段落として書かれた本文）は、寄せの推定を行わず常に左寄せとして扱う。
            # v20260915_01での追加: _guess_free_alignment はページ幅に対する左右余白の
            # 対称性だけを見て中央寄せかどうかを判定するが、左寄せの段落でも折り返しで
            # 行幅がページの大半を占めると、たまたま左右の余白が近い値になり「中央寄せ」
            # と誤判定されることがある（実データの英文レジュメで、複数行の箇条書き段落が
            # 軒並み中央寄せと誤判定され、見た目が崩れていた）。一方、_merge_paragraph_groups
            # は左端がほぼ揃っている（`abs(pb.x0 - cb.x0) <= ...`）行同士しか結合しないため、
            # 2行以上にまたがるブロックは実質的に必ず左揃え（または両端揃え）であり、
            # 中央寄せの多段見出しがこの形になることはまず無い。
            distinct_lines = len({round(r["bbox"].y0, 1) for r in recs})
            if distinct_lines >= 2:
    ...
    for page_index, infos in blocks_by_page.items():
        ...
        # 墨消しで隠れる前に、下線らしき矩形を検出しておく（詳細は関数のdocstring参照）
        _detect_underlines(page, infos)
...
# 下線を引く位置・太さ（ベースラインからの下方向オフセット・太さとも、フォントサイズに
# 対する比率）。v20260915_03で追加。元の下線の正確な位置に合わせる必要は無い
# （元の下線自体は墨消しで隠れて失われており、これは新規に引き直すものである）ため、
# 一般的な下線装飾として違和感のない値を採用した。
_UNDERLINE_OFFSET_RATIO = 0.12
_UNDERLINE_THICKNESS_RATIO = 0.045
_UNDERLINE_MIN_THICKNESS = 0.6


def _draw_underline_rect(page, x0, x1, baseline_y, fontsize, color):
    """指定した1行分のテキストの下に、下線の塗り矩形を描く（v20260915_03で追加）。

    `draw_line`（線描画）ではなく `draw_rect(fill=...)`（塗り矩形）を使うのは、
    元のPDFの下線自体が塗り矩形として描かれており（`_detect_underlines` 参照）、
    同じ表現方法に揃えるため。
    """
    if x1 <= x0:
        return
    thickness = max(_UNDERLINE_MIN_THICKNESS, fontsize * _UNDERLINE_THICKNESS_RATIO)
    y = baseline_y + fontsize * _UNDERLINE_OFFSET_RATIO
    rect = fitz.Rect(x0, y, x1, y + thickness)
    page.draw_rect(rect, color=None, fill=color)


def _draw_mixed_text(page, rect, text, fontsize, color, align,
                     bold=False, force=False, line_factor=None, underline=False):
    ...
    underline=True のときは、実際に描いた各行の幅に合わせて下線も描く
    （v20260915_03で追加。詳細は `_detect_underlines` のdocstring参照）。
    """
    ...
    for line in lines:
        ...
        for font, seg, seg_width in line:
            ...
            x += seg_width
        if underline and line_width > 0:
            underline_spans.append((line_x0, line_x0 + line_width, y))
        y += line_height

    writer.write_text(page, color=color)
    for x0, x1, baseline_y in underline_spans:
        _draw_underline_rect(page, x0, x1, baseline_y, fontsize, color)
    return rect.height - total_height


def _needs_mixed_fonts(text, base_fontname):
    """文字単位のフォント切り替え（埋め込み保証のあるTextWriter経由の描画）が必要か。

    v20260914_01での修正（「日本語へ翻訳した結果、PDFを開いても文字が全く表示されない
    （開き直すと表示される）」という報告への対策）:
      旧実装は base_fontname == "helv"（＝翻訳先が英語）のときだけ判定していた。
      翻訳先が日本語／中国語／韓国語のときは base_fontname が "japan"/"china-s"/"korea"
      になり、常に False を返していたため、page.insert_textbox(fontname="japan", ...) の
      経路（PDFへフォントを埋め込まない、Adobe-Japan1 のCID参照）で描画されていた。
      この経路で生成された文字は、閲覧環境に対応する日本語フォントが無い、または
      読み込みが間に合わない場合に**表示されない**ことがある（実データで確認: 出力PDFの
      "Gothic" フォントが embed=n/a=埋め込みなしだった）。
      翻訳先が欧文フォントで描けない言語（helv以外）のときは常にTextWriter経由の
      描画（_draw_mixed_text）にする。これは Droid Sans Fallback として実体を
      PDFへサブセット埋め込みするため、閲覧環境のフォント有無に依存しなくなる
      （20260821_02で「③」等の記号だけをこの経路に通した際に確認済みの効果を、
      日本語／中国語／韓国語への翻訳全体に広げたもの）。
    """
    if base_fontname != "helv":
        return True
    return not _fits_in_latin1(text)


def _draw_underline_for_textbox(page, rect, text, size, fontname, color, align):
    """`insert_textbox` 経路（欧文フォント・1行に収まる場合のみ）向けの下線描画。

    v20260915_03で追加。`insert_textbox` は内部でどう折り返したかを呼び出し側へ
    返してくれないため、`_draw_mixed_text` のように実際の行幅を使うことができない。
    そこで `fitz.get_text_length` で単一行として描いた場合の幅を見積もり、それが
    枠の幅に収まる（＝折り返されていない可能性が高い）場合に限って下線を引く。
    折り返された疑いがある場合は、幅がズレた下線を引くより「引かない」方が安全
    なため、その場合はスキップする。
    """
    if "\n" in text:
        return
    try:
        ...
        return
    if width <= 0 or width > rect.width + 0.5:
        return
    if align == fitz.TEXT_ALIGN_CENTER:
    ...
    _draw_underline_rect(page, x0, x0 + width, baseline_y, size, color)
...
def _draw_text_fitted(page, rect, text, font_size, fontname, color, align,
                      ...
                      logger=None, page_index=0, underline=False):
    ...
    underline=True の場合、元のテキストの直下に下線があったブロックとして、翻訳文の
    実際の描画幅に合わせた下線を引き直す（v20260915_03で追加。詳細は
    `_detect_underlines` のdocstring参照）。
    """
    ...
    def draw(target_rect, size, force=False):
        ...
        if mixed or force:
            return _draw_mixed_text(page, target_rect, text, size, color, align,
                                    bold=bold, force=force, underline=underline)
        # insert_textbox は収まらない場合、負値を返して何も描かない（＝試行しても安全）
        result = page.insert_textbox(target_rect, text, fontsize=size, fontname=fontname,
                                     color=color, align=align)
        if result >= 0 and underline:
            _draw_underline_for_textbox(page, target_rect, text, size, fontname, color, align)
        return result
    ...
    while size >= floor:
        if _draw_mixed_text(page, rect, text, size, color, align,
                            bold=bold, line_factor=_TIGHT_LINE_FACTOR,
...
# 出力ファイル名の末尾に付ける2文字の言語コード。
# 例: 資料.pdf を英語へ翻訳 → 資料_en.pdf
# 中国語は、国名に沿った分かりやすさを優先して "cn" を使う（ISO 639-1 の "zh" ではない）。
LANGUAGE_SUFFIX_MAP = {
    "Japanese": "ja",
    "English": "en",
