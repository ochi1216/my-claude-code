...
使い方:
    ...
    3. python pdf_translator_20260821_02.py
...
def extract_translatable_blocks(doc, protect_graphics=False, target_pages=None, logger=None):
    ...
    for page_index, infos in blocks_by_page.items():
        ...
        for info in infos:
            ...
            # 収まらなかったときに枠を下へ広げてよい限界。障害物の位置と背景色の
            # 変わり目の、手前の方を採る（無制限に広げると下の要素へ重なるため）。
            _, _, free_y1 = _free_space_limits(page_rect, info["bbox"], others)
            info["grow_limit_y"] = _clip_expansion_to_background(
                sampler, info["bbox"],
                fitz.Rect(info["box_rect"].x0, info["box_rect"].y0,
                          info["box_rect"].x1, max(free_y1, info["box_rect"].y1)),
                info["fill"], align=info["align"],
            ).y1
...
def _layout_mixed_text(text, width, fontsize, latin_font, cjk_font):
    ...
    戻り値: [[(font, 文字列, 幅), ...], ...]
    英字は単語単位、CJK文字は1文字単位で折り返す（日本語は語中でも改行できるため）。
    """
    lines = []
    for paragraph in (text or "").split("\n"):
        atoms = []
        for font, seg in _split_font_runs(paragraph, latin_font, cjk_font):
            if font is cjk_font:
                atoms.extend((font, ch) for ch in seg)
            else:
                atoms.extend((font, tok) for tok in re.findall(r"\s+|\S+", seg))

        current, used = [], 0.0
        for font, seg in atoms:
            seg_width = font.text_length(seg, fontsize)
            if current and seg.strip() and used + seg_width > width:
                lines.append(current)
                current, used = [], 0.0
            if not current and not seg.strip():
                continue  # 行頭に残った空白は捨てる
            current.append((font, seg, seg_width))
            used += seg_width
        lines.append(current)
    return lines
...
def _draw_mixed_text(page, rect, text, fontsize, color, align,
                     bold=False, force=False, line_factor=1.3):
...
def _draw_text_fitted(page, rect, text, font_size, fontname, color, align,
                      bold=False, page_rect=None, grow_limit=None,
    ...
    for page_index, items in pages_items.items():
        ...
        for (info, translated), _fill in zip(items, fills):
            ...
            _draw_text_fitted(
                ...
                grow_limit=info.get("grow_limit_y"),
