...
既知の制限（技術的な原理限界。100%保証はできない領域）:
    ...
    - 罫線で囲まれていない自由配置の図（チャートの凡例・軸ラベル等）は既定で保護
      （未翻訳のまま）される。誤検出でチェックボックスをOFFにして強制翻訳させた
      場合、その部分は再びレイアウトが崩れるリスクがある（＝これは実装のバグでは
      なく、可変長の翻訳文を固定レイアウトの図形に安全に収める一般解が存在しない
      という、PDFという形式そのものの制約）。
...
使い方:
    ...
    3. python pdf_translator_20260722_02.py
...
def _analyze_page_graphics(page, logger=None):
    ...
    for c in clusters.values():
        n_v, n_h = len(c["v"]), len(c["h"])
        # 単一の矩形（例: 棒グラフの1本）は4辺が互いに交差するため v=2,h=2 を満たして
        # しまうが、これは「表」ではなく単なる図形の枠線なので、実際にセルが2つ以上
        # 生まれる（＝行または列が複数ある）グリッドのみを表として扱う
        if n_v >= 2 and n_h >= 2 and (n_v - 1) * (n_h - 1) >= 2:
            grids.append((sorted(c["v"]), sorted(c["h"])))
...
def extract_translatable_blocks(doc, protect_graphics=True, logger=None):
...
def sample_background_color(page, rect, inside=False, logger=None):
    """背景色を1点サンプリングして近似取得する（失敗時は白）。

    inside=False（自由配置の段落テキスト向け）: 矩形の外側すぐ上（周囲のページ背景）を見る。
    inside=True （表セル向け）: 罫線の外に出るとセル外の背景を拾ってしまうため、
    セル矩形の内側（右端寄り。左寄せテキストの上に乗りにくい）をサンプリングする。
    """
    try:
        if inside:
            x0 = max(rect.x1 - 1.5, rect.x0)
            y0 = rect.y0 + max(min(rect.height / 2, 4), 0.5)
        else:
            x0 = max(rect.x0 - 2, 0)
            y0 = max(rect.y0 - 2, 0)
...
def translate_pdf_document_thread(file_path, target_language, progress_window, protect_graphics=True):
    """バックグラウンドで実行されるメイン処理"""
    logger = get_logger()
    doc = None
    try:
...
def select_file():
    ...
    protect_var = tk.BooleanVar(lang_win, value=True)
    protect_check = tk.Checkbutton(
        lang_win, text="表・グラフに重なるテキストを保護する（推奨）",
        variable=protect_var, font=("Arial", 9),
    )
    protect_check.pack(padx=20, pady=(5, 0))
    tk.Label(
        lang_win,
        text="※ 罫線グリッドとして認識できない図形（チャート等）に重なる文字は\n"
             "　 レイアウト崩れ防止のため翻訳せず原文のまま残します",
