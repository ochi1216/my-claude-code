"""pdf_translator_20260915_02.py の Gemini プロキシ移行部分の検証テスト。
...
実行方法:
    pip install PyMuPDF
    python3 tests/test_pdf_translator_20260915_02.py
"""
...
if _RESUME_SRC_CANDIDATES:
    ...
    # 元テキストが「見出し・箇条書き」構成（・を複数含む）のブロックでは、
    # 訳文も複数の箇条書きを保持していること（＝見出し1行だけに縮退しない）。
    # v20260915_02で「行末に・が付く独立した箇条書き項目を結合しない」修正を
    # 入れたことにより、複数の・を含むブロック自体が意図的に減っている
    # （各項目が別々の翻訳単位に分かれるようになったため）。そのため件数の
    # しきい値は2件から1件に緩めた。1件でも存在すれば、このテストが検証したい
    # 「複数行応答のパースで項目が欠落しないこと」は引き続き検証できる。
    _bulleted = [(src, tr) for src, tr in zip(_rtexts, _rtranslated) if src.count("・") >= 2]
    check("元が複数の箇条書きを含むブロックが存在する（検証の前提）", len(_bulleted) >= 1,
          f"該当件数={len(_bulleted)}")
    _lost = [(s[:20], t) for s, t in _bulleted if t.count("・") < 2]
    check("箇条書きを複数含む原文は、訳文でも複数の箇条書きが保持される"
