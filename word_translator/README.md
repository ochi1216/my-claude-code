...
1. 「ファイル選択」から翻訳したい `.docx` を選び、翻訳先言語（日本語／英語／中国語簡体字）を
   選んで「翻訳開始」を押す。
2. 完了すると同じフォルダに `元ファイル名_ja.docx` のように保存される
...
- **フェイルファストで中断したとき、出力先には「翻訳前の複製」が残る。** 翻訳を始める前に
  ...
  `_ja.docx` が英語のまま残るので、失敗した回のファイルは削除してよい
  （PowerPoint版も同じ挙動）。
- **出力ファイル名の言語サフィックスは 20260911_01 で短縮した**（`_gemini_japanese.docx`
  → `_ja.docx`）。20260812_02 以前が作ったファイルを上書きすることは無いため、同じ元ファイルを
  翻訳し直すと新旧2つのファイルが並ぶ。古い方は手で削除してよい。
  なお中国語簡体字は `_cn` を使っており、`pdf_translator` の `_zh`（ISOコード）とは異なる。
...
`20260306_01` / `20260812_01` / `20260812_02` / `20260911_01` の4版でテキスト・書式が
完全一致することも検証している（＝更新でWord文書の中身が変わっていないことの証拠）。
20260911_01 では加えて、3言語すべてが `_ja` / `_en` / `_cn` の名前で出力されることを
実ファイルで確認している。
