...
移行（20260812_01）で変わったものではなく、**20260306_01 から続く既存の仕様**である。
実機で気になったときに「移行のせいではない」と切り分けられるよう記載しておく。
...
- **失敗時のリトライが無い。** 通信に失敗したバッチ（10項目）は、その場で原文のまま残る。
  遮断下では最初のバッチだけ15秒のタイムアウトを待つため、「最初の10項目だけ英語のまま」
  という症状が出ることがある。
- **ログファイルは作らない。** エラーはコンソールへ表示されるだけなので、バッチファイルから
  起動していると窓が閉じて見えないことがある。
- **翻訳中の進捗バーはほとんど動かない**（`0` → `完了` の2段階のみ）。長い文書では
  固まったように見えるが、バックグラウンドで処理は進んでいる。
- **出力先ファイルのロックを事前検知しない。** 出力先をWordで開いたままだと、数分の翻訳が
  終わった後の保存時に初めてエラーになる。
...
- **一部の段落だけ英語のまま残る**: そのバッチの通信が失敗している。本ツールにはリトライが
  無いため、失敗したバッチは原文のまま残る（既存仕様）。もう一度実行し直す。
- **`ファイルが他のプログラム（Wordなど）で開かれています`**: 出力先ファイルをWordで
  閉じてから再実行する。
...
翻訳の書き戻しまでエンドツーエンドで確認する。旧版 `word_translation_20260306_01.py` と
