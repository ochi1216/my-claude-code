...
**追加ファイル:** `word_translation_20260911_01.py`, `tests/test_word_translation_20260911_01.py`

**更新ファイル:** `README.md`, `CHANGELOG.md`

ユーザー依頼により、**翻訳後のファイル名に付く言語サフィックスを短縮**した。ファイル名が
長くなりすぎるのを避けるため。

| 翻訳先 | 旧（20260812_02 まで） | 新（20260911_01 以降） |
|---|---|---|
| Japanese | `報告書_gemini_japanese.docx` | `報告書_ja.docx` |
| English | `報告書_gemini_english.docx` | `報告書_en.docx` |
| Chinese Simplified | `報告書_gemini_chinese.docx` | `報告書_cn.docx` |

- `_gemini` の部分も併せて削除した。
- 実装は `LANGUAGE_SUFFIX_MAP` と `lang_to_suffix()` を新設し、
  `translate_word_document_thread()` の出力パス組み立てをそこへ差し替えただけ。
  旧実装は `target_language.split()[0].lower()` をそのまま使っていた。
- 対応表に無い言語を指定した場合は、言語名の先頭2文字を小文字にしたものを使う
  （例: `Korean` → `ko`）。GUIの選択肢は3言語のみなので通常は使われないが、保険として置いた。

### 申し送り

- **出力されるファイル名が変わる。** 20260812_02 以前が作った `*_gemini_japanese.docx` を
  上書きすることは無いため、同じ元ファイルを翻訳し直すと**新旧2つのファイルが並ぶ**。
  古い方は手で削除してよい。
- **中国語簡体字は `cn` を使っている。** `pdf_translator` は ISO 639-1 の `zh` を使っており
  食い違うが、依頼どおり `cn` にしてある。将来ツール間で揃える場合はここを変更する。

### 変更していないもの

Gemini通信経路（互換シム）・リトライ・ロギング・進捗表示・フェイルファスト・ファイルロック
事前検知・Wordの書き戻し処理は 20260812_02 から**一切変更していない**。ASTによる関数単位の
ハッシュ比較で、24個の関数・クラス（`_CommonGeminiClient` / `init_gemini` /
`check_dependencies` / `translate_batch_gemini` / `translate_super_fast_parallel` /
`get_logger` / `WordProgressWindow` / `select_file` など）が 20260812_02 と**完全一致**する
ことを確認済み。変更したのは `translate_word_document_thread` の1つだけで、新規追加は
`lang_to_suffix` の1つだけ。

### テスト

`tests/test_word_translation_20260911_01.py`（159項目）を追加した。20260812_02 のテストを
土台に、新しい命名規則の検証を足している。

- `lang_to_suffix()` が `Japanese` → `ja` / `English` → `en` / `Chinese Simplified` → `cn` を
  返すこと。対応表がGUIの選択肢3言語をすべて網羅し、値がすべて2文字であること。
  対応表に無い言語では先頭2文字へフォールバックすること（前後の空白があっても落ちないこと）。
- **3言語それぞれで実際にDOCXを翻訳し、`_ja` / `_en` / `_cn` のファイルが出力されること。**
  古い長い名前のファイルを作らないこと。翻訳自体は従来どおり成功すること。
- 日本語以外では `run.font.name` を `游ゴシック` にしない既存仕様が維持されていること。
- **20260812_02 と 20260911_01 に同じ翻訳文を与えると、ファイル名が違うだけで出力DOCXの
  run 構成・テキスト・書式・フォント名が完全一致すること**（＝中身に影響が無いことの証拠）。
  移行前の `20260306_01` と比べても同じく完全一致する。
- 20260812_02 から引き継いだ検証（payload形状・`safetySettings`・モデル名の明示指定・
  共通モジュール探索・認証情報判定・リトライ3回・進捗コールバック・フェイルファスト・
  ファイルロック事前検知・ロギング）は**すべてそのまま通ること**。
...
2. 日本語に翻訳して **`元ファイル名_ja.docx`** で保存されること
3. 英語・中国語簡体字でも **`_en.docx` / `_cn.docx`** になること
4. 文書の中身（フォント・色・配置・表）が従来どおりであること
