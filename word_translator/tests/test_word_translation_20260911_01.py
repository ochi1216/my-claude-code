"""word_translation_20260911_01.py の検証テスト。

20260911_01 は 20260812_02 から**出力ファイル名の言語サフィックスだけ**を短縮した版。

    旧: 元ファイル名_gemini_japanese.docx
    新: 元ファイル名_ja.docx   （英語 _en / 中国語簡体字 _cn）

そのため本テストの主眼は次の2点。

  1. 新しい命名規則が3言語すべてで正しく効くこと（`lang_to_suffix()` と実出力の両方）
  2. **それ以外が何も変わっていないこと** ― Gemini通信経路（互換シム）・リトライ・
     ロギング・進捗表示・フェイルファスト・ファイルロック事前検知・Wordの書き戻し処理。
     20260812_02 のテストをそのまま引き継いで確認している。
...
実際に生成して翻訳文の書き戻しまでエンドツーエンドで検証する。
20260812_02 と 20260911_01 に同じ翻訳文を与えて出力を比較し、
**ファイル名以外は中身が完全に同一**であることを実物で示す。
tkinter だけスタブ化する。

実行方法:
    pip install python-docx
    python3 tests/test_word_translation_20260911_01.py
"""
...
TARGET = os.path.join(HERE, "..", "word_translation_20260911_01.py")
PREV_TARGET = os.path.join(HERE, "..", "word_translation_20260812_02.py")
...
# --- 【20260911_01 の本題】出力ファイル名の言語サフィックス --------------
check("lang_to_suffix: Japanese -> ja", mod.lang_to_suffix("Japanese") == "ja",
      f"got={mod.lang_to_suffix('Japanese')}")
check("lang_to_suffix: English -> en", mod.lang_to_suffix("English") == "en",
      f"got={mod.lang_to_suffix('English')}")
check("lang_to_suffix: Chinese Simplified -> cn",
      mod.lang_to_suffix("Chinese Simplified") == "cn",
      f"got={mod.lang_to_suffix('Chinese Simplified')}")
check("サフィックスがすべて2文字",
      all(len(v) == 2 for v in mod.LANGUAGE_SUFFIX_MAP.values()),
      f"map={mod.LANGUAGE_SUFFIX_MAP}")
check("対応表が GUI の選択肢3言語をすべて網羅している",
      set(mod.LANGUAGE_SUFFIX_MAP) == {"Japanese", "English", "Chinese Simplified"},
      f"keys={sorted(mod.LANGUAGE_SUFFIX_MAP)}")
check("対応表に無い言語は先頭2文字を小文字にして使う",
      mod.lang_to_suffix("Korean") == "ko", f"got={mod.lang_to_suffix('Korean')}")
check("対応表に無い言語で前後に空白があっても落ちない",
      mod.lang_to_suffix("  German  ") == "ge",
      f"got={mod.lang_to_suffix('  German  ')}")
...
try:
    ...
    out_docx = os.path.join(_docx_dir, "sample_ja.docx")
    check("出力ファイル名が 元ファイル名_ja.docx になる（サフィックス短縮）",
    ...
    # 今回変えたのは出力ファイル名の組み立てだけで、Word の書き戻し処理は
    # 触っていない。同じ翻訳文を与えれば、**ファイル名が違うだけで中身は
    # 完全に一致する**はずである。
    ...
    check("20260812_02 は従来どおり _gemini_japanese.docx を出力する(退行していない)",
          os.path.isfile(prev_out), f"exists={os.path.isfile(prev_out)}")
    check("20260911_01 は 20260812_02 と同じ長い名前では出力しない",
          not os.path.isfile(os.path.join(_docx_dir, "sample_gemini_japanese.docx")))
    prev_sig = _run_signature(prev_out)
    check("20260812_02 と 20260911_01 で出力DOCXの run 構成が一致する",
          [s["key"] for s in prev_sig] == [s["key"] for s in after_sig],
          f"prev={len(prev_sig)} / new={len(after_sig)}")
    check("20260812_02 と 20260911_01 で出力DOCXのテキストが完全一致する",
          [s["text"] for s in prev_sig] == [s["text"] for s in after_sig])
    check("20260812_02 と 20260911_01 で出力DOCXの書式が完全一致する",
          _fmt_only(prev_sig) == _fmt_only(after_sig))
    check("20260812_02 と 20260911_01 で出力DOCXのフォント名が完全一致する",
    ...
    if os.path.isfile(OLD_TARGET):
        ...
        check("移行前(20260306_01)は従来どおりの名前で出力される", os.path.isfile(old_out))
        old_sig = _run_signature(old_out)
        check("移行前と 20260911_01 で出力DOCXの run 構成が一致する",
              [s["key"] for s in old_sig] == [s["key"] for s in after_sig],
              f"old={len(old_sig)} / new={len(after_sig)}")
        check("移行前と 20260911_01 で出力DOCXのテキストが完全一致する",
              [s["text"] for s in old_sig] == [s["text"] for s in after_sig])
        check("移行前と 20260911_01 で出力DOCXの書式が完全一致する",
              _fmt_only(old_sig) == _fmt_only(after_sig))
        check("移行前と 20260911_01 で出力DOCXのフォント名が完全一致する",
    ...
    _hf_out = Document(os.path.join(_docx_dir, "header_footer_ja.docx"))
    ...
    # --- 3言語すべてで新しい命名規則どおりに出力されること ------------------
    # GUI の選択肢(日本語/英語/中国語簡体字)がそのまま target_language になる。
    for _lang, _suffix in (("Japanese", "ja"), ("English", "en"),
                           ("Chinese Simplified", "cn")):
        _lang_src = os.path.join(_docx_dir, f"doc_{_suffix}.docx")
        _build_sample_docx(_lang_src)
        MESSAGEBOX.CALLS.clear()
        mod.translate_word_document_thread(_lang_src, _lang, _FakeProgressWindow())
        _expected = os.path.join(_docx_dir, f"doc_{_suffix}_{_suffix}.docx")
        check(f"{_lang} の出力が _{_suffix}.docx になる", os.path.isfile(_expected),
              f"expected={os.path.basename(_expected)} / "
              f"actual={sorted(f for f in os.listdir(_docx_dir) if f.startswith('doc_'))}")
        check(f"{_lang} で古い長い名前のファイルを作らない",
              not any(f.startswith(f"doc_{_suffix}_gemini")
                      for f in os.listdir(_docx_dir)))
        check(f"{_lang} の翻訳自体は従来どおり成功する",
              any(c[0] == "info" for c in MESSAGEBOX.CALLS)
              and not any(c[0] == "error" for c in MESSAGEBOX.CALLS),
              f"{MESSAGEBOX.CALLS}")

    # 日本語以外ではフォント指定(游ゴシック)が入らない既存仕様も維持されていること
    _en_runs = _run_signature(os.path.join(_docx_dir, "doc_en_en.docx"))
    check("英語翻訳では run.font.name を 游ゴシック にしない(既存仕様)",
          all(r["name"] != "游ゴシック" for r in _en_runs),
          f"names={sorted({r['name'] for r in _en_runs})}")
    ...
    check("ロック検知時は出力ファイルを作らない",
          not os.path.isfile(os.path.join(_docx_dir, "locked_ja.docx")))

    # (2) 保存先(前回の出力)がロックされている場合
    lock_src2 = os.path.join(_docx_dir, "locked2.docx")
    _build_sample_docx(lock_src2)
    lock_out2 = os.path.join(_docx_dir, "locked2_ja.docx")
    ...
    _ff_out = os.path.join(_docx_dir, "failfast_ja.docx")
