r"""
VERSION 2026.0911.01
Word Gemini 翻訳ツール（書式完全保持版）

■ このバージョンでの変更（出力ファイル名の言語サフィックスを短縮）
翻訳後のファイル名に付く言語サフィックスを、言語名そのものから2文字の言語コードへ
短縮した。ファイル名が長くなりすぎるのを避けるため。

  旧（20260812_02 まで）: 報告書_gemini_japanese.docx
  新（20260911_01 以降）: 報告書_ja.docx

  Japanese          -> _ja
  English           -> _en
  Chinese Simplified -> _cn

`_gemini` の部分も併せて削除した。対応表に無い言語を指定した場合は、言語名の先頭2文字を
小文字にしたものを使う（`lang_to_suffix()`）。

【申し送り】**出力されるファイル名が変わる。** 20260812_02 以前が作った
`*_gemini_japanese.docx` を上書きすることは無いので、同じ元ファイルを翻訳し直すと
新旧2つのファイルが並ぶことになる。古い方は手で削除してよい。
なお `pdf_translator` は中国語簡体字に ISO コードの `zh` を使っているため、本ツールの
`cn` とは食い違う（依頼どおり `cn` にしてある）。

【この変更で触ったのは出力ファイル名の組み立てだけ。】Gemini通信経路（互換シム）・
リトライ・ロギング・進捗表示・フェイルファスト・ファイルロック事前検知・Wordの
書き戻し処理は 20260812_02 から一切変更していない。

■ 20260812_02 での変更（PowerPoint版の改善を逆輸入。以下は据え置き）
...
■ 使い方
  1. run_word_translator.bat を実行（または python word_translation_20260911_01.py）
  2. 翻訳したい .docx を選ぶ
  3. 翻訳先言語を選んで「翻訳開始」を押す
  4. 完了すると同じフォルダに `元ファイル名_ja.docx` のように保存される
     （英語なら `_en.docx`、中国語簡体字なら `_cn.docx`）
...
# --- 20260911_01 で追加: 出力ファイル名の言語サフィックス ---
# 20260812_02 までは target_language.split()[0].lower() をそのまま使っていたため、
# `_gemini_japanese.docx` のように長い名前になっていた。2文字の言語コードへ短縮する。
# ※ pdf_translator は中国語簡体字に ISO コードの `zh` を使っているが、本ツールは
#   依頼により `cn` を使う（食い違う点に注意）。
LANGUAGE_SUFFIX_MAP = {
    "Japanese": "ja",
    "English": "en",
...
def lang_to_suffix(target_language):
    ...
    対応表に無い言語は、言語名の先頭2文字を小文字にしたものを使う。"""
...
def translate_word_document_thread(file_path, target_language, progress_window):
    ...
    try:
        ...
        lang_code = lang_to_suffix(target_language)
        output_path = os.path.splitext(file_path)[0] + f"_{lang_code}.docx"
