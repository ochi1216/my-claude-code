# -*- coding: utf-8 -*-
"""analog_ic_se_strategy_organizer 統合ダッシュボード v20260716_01

タブ構成:
  📦 製品登録・検索   … 型番からステージ0(製品取り込み)を実行して登録、既存ケースライブラリの検索
  📊 ポートフォリオ俯瞰 … 蓄積製品を横断した俯瞰分析（カテゴリ分布・競合ギャップ・AI総評）
  🎯 製品ディープダイブ … 型番を入力→5ステージ一気通貫でHTMLレポートを生成

起動: streamlit run analog_ic_se_strategy_organizer_20260716_01.py

DESIGN_analog_ic_se_strategy_organizer.md 9章の設計に基づく初版実装。
"""
...
with tab_portfolio:
    ...
    if df.empty:
        ...
        gap_df = pd.DataFrame(gap_rows).sort_values("競合優位点の総数", ascending=False)
        if not gap_df.empty:
            st.dataframe(gap_df, width="stretch", hide_index=True)
        else:
            st.info("競合IC比較データがまだありません。")
