# -*- coding: utf-8 -*-
"""analog_ic_se_strategy_organizer 統合ダッシュボード v20260718_02

タブ構成:
  📦 製品登録・検索   … 型番からステージ0(製品取り込み)を実行して登録、既存ケースライブラリの検索
  📊 ポートフォリオ俯瞰 … 蓄積製品を横断した俯瞰分析（ホワイトスペース分析・ロードマップビュー・
                        カテゴリ分布・競合ギャップ・AI総評）
  🎯 製品ディープダイブ … 型番を入力→5ステージ一気通貫でHTMLレポートを生成

起動: streamlit run analog_ic_se_strategy_organizer_20260718_02.py
...
v20260718_01での変更点（MECE改善検討の優先度2）:
  - ステージ3(競合IC比較)に自社(Nexperia)を強制的に含めるよう ic_engine.py を変更
    （地域代表選定でNexperiaが漏れる欠陥の修正）。自社がそのカテゴリでまだ製品を
    持たない場合は無理に検索させず「未参入」と明示する
  - ベンチマーク対象(TI)自身が競合候補プールに紛れ込み「TI vs TI」の重複比較に
    なりうる不具合を修正（ic_schema.load_competitors/pick_regional_representativesに
    exclude_names引数を追加）
  - ステージ4(次世代スペック提案)の主語をTIから自社(Nexperia)に転換。提案の
    parameter_keyをcategory_schemaのkeyに正規化し、比較表の行と機械的に対応させた
  - HTMLレポート(ic_report.py)を「各社バラバラのカード」から「パラメータ行×企業列の
    統合比較表1枚」に再構成。TI・各競合企業・自社(現行)・🏆自社提案(次世代)を
    横一列で比較できるようにし、自社提案列を強調表示した
  - 🎯製品ディープダイブタブの実行結果表示で、廃止した"kpi"キー参照が残っていた
    バグを修正（kpi_label参照に変更）

v20260718_02での変更点（MECE改善検討の優先度3）:
  - 📊ポートフォリオ俯瞰タブに「🗺️ ロードマップビュー」セクションを追加。
    ic_index.roadmap_priorities()により、ホワイトスペース分析（市場全体の手薄さ）×
    自社の現状ポジション（未参入ほど優先度を上げる）×分析済みの鮮度（未分析ほど
    優先度を上げる）を組み合わせた「次に着手すべきカテゴリ」優先順位表を追加。
    分析済み製品がある場合は、カテゴリ別の解析日時×最優先提案の時系列散布図も表示する。
    いずれもLLM呼び出し不要（コスト0）。TIの推定リフレッシュ周期等、競合の開発動向
    シグナルは対象外（別データソースが必要な今後の拡張候補。DESIGN 14.6章参照）
"""
...
with tab_portfolio:
    ...
    st.markdown("### 🗺️ ロードマップビュー（次に着手すべきカテゴリ）")
    st.caption(
        "ホワイトスペース分析（市場全体の手薄さ）に、自社の現状ポジション（未参入ほど優先度を上げる）と、"
        "そのカテゴリを最後にいつ深掘り分析したか（未分析ほど優先度を上げる）を組み合わせた"
        "「ロードマップ優先度」です。競合の新製品開発動向・TIのリフレッシュ周期は含まれていません"
        "（別のデータソースが必要な今後の拡張候補です）。")

    roadmap = ic_index.roadmap_priorities(data_dir=DATA_DIR)
    roadmap_df = pd.DataFrame([{
        "カテゴリ": r["label"],
        "ロードマップ優先度": r["roadmap_score"],
        "手薄度": r["whitespace_score"],
        "自社の現状": (OWN_STATUS_LABELS.get(r["own_company_status"], "—")
                    if r["own_company_status"] is not None else "（自社未設定）"),
        "分析済み件数": r["analyzed_count"],
        "直近の分析日": (r["last_analyzed_at"] or "未分析")[:19] if r["last_analyzed_at"] else "未分析",
        "直近の型番": ", ".join(r["recent_part_numbers"]) or "—",
    } for r in roadmap])
    st.dataframe(roadmap_df, width="stretch", hide_index=True)

    if not df.empty:
        st.markdown("#### 📅 分析済み製品の時系列（カテゴリ別）")
        timeline_details = load_portfolio_details(built_at)
        timeline_rows = []
        for d in timeline_details:
            meta = d.get("metadata", {})
            classifiers = d.get("classifiers", {})
            s4 = d.get("content", {}).get("stage4_next_gen_proposal", {})
            top = None
            if isinstance(s4, dict) and s4.get("proposed_specs"):
                top = s4["proposed_specs"][0]
            timeline_rows.append({
                "型番": meta.get("part_number", ""),
                "カテゴリ": CATEGORY_LABELS.get(classifiers.get("category"), classifiers.get("category")),
                "解析日時": meta.get("analyzed_at", ""),
                "最優先提案": (top.get("kpi_label") or top.get("parameter_key", "")) if top else "—",
                "提案目標値": top.get("target_value", "") if top else "—",
                "優先度": top.get("priority", "不明") if top else "不明",
            })
        timeline_df = pd.DataFrame(timeline_rows)
        if not timeline_df.empty:
            timeline_df["解析日時_dt"] = pd.to_datetime(timeline_df["解析日時"], errors="coerce")
            timeline_df = timeline_df.dropna(subset=["解析日時_dt"])
        if not timeline_df.empty:
            import plotly.express as px
            fig_roadmap = px.scatter(timeline_df, x="解析日時_dt", y="カテゴリ", color="優先度",
                                      hover_data=["型番", "最優先提案", "提案目標値"])
            fig_roadmap.update_traces(marker=dict(size=14))
            fig_roadmap.update_layout(height=360, xaxis_title="解析日時", yaxis_title="カテゴリ")
            st.plotly_chart(fig_roadmap, width="stretch")
        else:
            st.info("解析日時が読み取れる分析済み製品がまだありません。")
    ...
    if "last_result" in st.session_state:
        ...
        if isinstance(s4, dict) and s4.get("executive_summary"):
            ...
            for spec in s4.get("proposed_specs", [])[:3]:
                label = spec.get("kpi_label") or spec.get("parameter_key", "")
                st.markdown(f"**{label}**（優先度: {spec.get('priority', '')}）: "
