...
# ステージ4: 次世代スペック提案（JSONモード、grounding不要 = 既存事実の統合のため）
STAGE4_NEXT_GEN = SYSTEM_PREFIX + """
# Task
これまでの市場分析・キーカスタマー推定・競合IC比較の結果を統合し、
TI製品「{part_number}」（カテゴリ: {category}）をベースに次に開発すべきデバイスの
KPI・追加機能を、根拠・実現性リスク・優先度付きで提案してください。
...
{{
  ...
  "proposed_specs": [
    {{
      "kpi": "改善すべきKPI名（例: 静止電流(Iq)）",
      "current_ti_value": "現行TI製品の値",
      "target_value": "提案する目標値",
      "rationale": "根拠(どの分析結果に基づくか)(150文字程度)",
      "competitive_gap_addressed": "どの競合とのギャップを埋めるか",
