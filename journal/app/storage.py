...
EMOTION_LABELSを喜・怒・無・哀・楽の5種類にした。「喜」ばかりが
選ばれてしまい、記録として偏っていた（強い感情が無い状態を表す
選択肢が無かった）ことへの対応。Hirebiシートのスキーマ・既存の
記録済み行には影響しない（感情ラベルは文字列としてそのまま保存
されているため、増やしたラベルは今後の記録から使えるようになる
だけで、過去データの読み替えは不要）
...
# 「無」（何も無い・凪の状態）を追加。喜のボタンばかり選ばれてしまい、
# 「特に強い感情は無い」を表す選択肢が無かったことへの対応
# （喜怒哀楽の間、真ん中に配置する）
