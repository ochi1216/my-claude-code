...
class PopupWindow:
    ...
    def show(self) -> None:
        ...
        # 背景色はウィンドウと同じBG_COLORにしているため、Tkinter既定の
        # フォーカスリング(highlightthickness)が縁取りとして目立ってしまう
        # 問題があったため、highlightthickness=0で明示的に消している。
        # 文字色(ACCENT_COLOR)はタグ未選択の暗い背景では読めるが、タグ選択後の
        # 淡い背景(pale_bg)に対してはコントラスト比1.56とWCAG基準を大きく
        # 下回っていたため、fg=Trueで登録しテーマ切替に追従させる
        # （name_label等、他の要素と同じ扱いにする）
        toggle_font = tkfont.Font(family="Yu Gothic UI", size=10)
        self.lkpt_toggle_btn = tk.Button(
            self.window, text="✏️ 振り返りを書く（LKPT）", bg=BG_COLOR, fg=ACCENT_COLOR,
        ...
        self.lkpt_toggle_btn.pack(pady=(2, 4))
        self._register_themed(self.lkpt_toggle_btn, bg=True, fg=True)
