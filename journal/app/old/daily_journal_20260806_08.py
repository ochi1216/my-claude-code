...
ACTION_BADGE_BG = "#f5b8b0"  # 未着手アクション件数バッジの地色。dashboard.pyの
                              # ACTION_STATUS_COLORS[未着手]（赤系）に寄せつつ、
                              # ポップアップ内では彩度を落として浮きすぎないようにする
...
QUICK_ACTION_PLACEHOLDER = "気になったことをすぐ書き留める"
...
class PopupWindow:
    ...
    def show(self) -> None:
        ...
        self.pending_action_count = self._fetch_pending_action_count()
        ...
        quick_action_frame.pack(pady=(4, 2), padx=20, fill="x")
        ...
        quick_action_btn = tk.Button(
            quick_action_frame, text="＋", bg=LKPT_TOGGLE_BG, fg=BUTTON_TEXT_COLOR,
            activebackground=ACCENT_COLOR, relief="flat", bd=0, highlightthickness=0,
            font=tkfont.Font(family="Yu Gothic UI", size=11, weight="bold"),
            width=3, cursor="hand2", command=self._add_quick_action,
        )
        quick_action_btn.pack(side="left", padx=(6, 0))
        ...
        # 未着手アクション件数を表示するバッジ。クリックすると1クリックで
        # ダッシュボードのACTIONモードへ直接遷移する（従来は「📊 DB」ボタンで
        # LKPTモードを開いてからACTIONへ切り替える2クリックが必要だった）。
        # 0件の時はクリックしても意味が無いため出さない
        badge_font = tkfont.Font(family="Yu Gothic UI", size=9, weight="bold")
        self.action_badge_btn = tk.Button(
            self.window, text="", bg=ACTION_BADGE_BG, fg=BUTTON_TEXT_COLOR,
            activebackground=ACCENT_COLOR, activeforeground=BUTTON_TEXT_COLOR,
            relief="flat", bd=0, highlightthickness=0, font=badge_font, cursor="hand2",
            padx=10, pady=3, command=lambda: self._open_dashboard(mode="ACTION"),
        )
    ...
    def _fetch_pending_action_count(self) -> int:
        """未着手アクションの件数を取得する（取得失敗時は0扱いでバッジを出さない）。"""
        try:
            return sum(
                1 for a in get_actions() if a["status"] == ACTION_STATUS_PENDING
            )
        except Exception as e:
            print(f"⚠️ 未着手アクション件数の取得に失敗しました: {e}")
            return 0

    def _update_action_badge(self) -> None:
        """
        未着手アクション件数バッジの表示を更新する。
        0件の間はクリックしても無意味なため、ボタン自体を表示しない。
        """
        if self.pending_action_count > 0:
            self.action_badge_btn.config(
                text=f"📌 未着手アクション {self.pending_action_count}件 →",
            )
            self.action_badge_btn.pack(pady=(0, 4))
        else:
            self.action_badge_btn.pack_forget()
    ...
    def _add_quick_action(self) -> None:
        ...
        if ok:
            ...
            self.quick_action_entry.config(fg=PLACEHOLDER_COLOR)
            self.quick_action_entry.insert(0, QUICK_ACTION_PLACEHOLDER)
