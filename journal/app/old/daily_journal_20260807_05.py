...
class PopupWindow:
    ...
    def show(self) -> None:
        ...
        # 登録・キャンセル・DBはアイコンのみの表示にする。
        # 前回、DBだけ別フォントで大きくして見た目の大きさを合わせようと
        # したが、フォントサイズが3つでバラバラになった結果、行の高さが
        # 一番大きいDBに引っ張られ、他の2つとの縦位置・大きさのバランスが
        # かえって崩れてしまった（指摘を受けて撤回）。3つとも同じ
        # font・pady（=同じボタンの箱の大きさ）に統一し、まず「揃っている」
        # ことを優先する
    ...
    def _on_tag_clicked(self, tag_name: str, color_code: str) -> None:
        """
        タグチップのクリックを振り分ける。既に選択中のタグをもう一度
        クリックした場合はトグルで未選択に戻し、それ以外は新規選択として扱う。
        """
