# -*- coding: utf-8 -*-
"""
daily_journal_20260815_01.py
学びジャーナル - ホットキー起動の入力ポップアップUI
Version: 0.40.0

v0.40.0での変更点：
タグを再クリックした際に中項目を無条件で空上書きしていたバグを修正。
ポップアップを閉じてAMEND_WINDOW_MINUTES(5分)以内に開き直し、同じタグを
もう一度クリックすると（show()がselected_tagをNoneに戻すため、これも
「タグクリック」に見える）、直前に書いた行の中項目が空文字列で
上書きされていた。その結果、直後にOtherを選ぶと「前回のコメントを
初期値にする」ロジックがこの空になった行を素通りしてしまい、
もっと過去の（別の日の）Otherコメントを拾って再表示・再書き込みして
しまう不具合があった（例："本日たった今書いたコメント"が消え、
"前日昼のコメント"に化ける）。
"""
