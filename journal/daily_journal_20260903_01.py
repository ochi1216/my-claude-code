...
喜・怒・無・哀・楽の5択にした（storage.py v0.14.0のEMOTION_LABELS拡張
と対になる変更）。「喜」ばかりが選ばれてしまい記録が偏っていた、という
指摘への対応。「無」は他の4感情と同じ配置（1行に横並び）で真ん中に置き、
色は他4色と衝突しない中間グレー(#9aa4b0)、アイコンは既存4アイコンと
同じ様式（色つき円＋濃紺の目鼻、assets/emotion_mu.png）で新規作成した。
EMOTION_LABELS・EMOTION_BUTTON_COLORS・EMOTION_ICONS・EMOTION_ICON_FILES
の4か所がすべてデータ駆動のため、ボタン生成ループ自体への変更は無い。

v0.43.0での変更点（daily_journal_20260831_02.pyからの差分）：
Outlook予定表連携（会議中の自動ポップアップ抑制・終了した会議の自動記録・
分類の声かけ）に対応した。ポップアップ表示部分での変更は次の1点のみ。

- ウィンドウの一番上に「会議の分類」バナーを追加した。TimeLogに
  タグ未確定（会議(未分類)）のまま自動記録された会議が残っている場合、
  show()を通るたびに一番古い1件を表示し、タグ一覧と同じ配色のボタンで
  分類できるようにする（storage.classify_pending_meeting()を呼ぶ）。
  「後で（次にJournalを開いた時にまた聞く）」を選ぶと、その回だけ
  バナーを閉じ、TimeLog側は分類待ちのまま残す。
  会議中の自動抑制・終了会議の自動記録・分類タイミングの判定ロジック
  本体はscheduler.py / outlook_calendar.py / storage.py側にあり、
  このファイルはバナーの表示とタグ選択の受け付けだけを担う。
- run()から、既存のstart_scheduler_loop()に加えて
  scheduler.start_meeting_sync_loop()も起動するようにした
  （Outlook側で問題が起きても定時リマインド機能には影響しない設計）。

v0.42.0での変更点：
2つ目のグローバルホットキー Ctrl+Shift+T（タスク特化起動）を追加した。
時間帯プレビュー・選択中バッジ・タグ・ひれぶり・LKPTを「📓 Journal」
という1つのアコーディオンボタンにまとめ、これを開閉することで表示・
非表示を切り替えられるようにした（以前独立していた「LKPT」トグルは
この新しいJournalトグルに統合され、廃止した）。

- Ctrl+Shift+J（従来通り）: Journalパネルを開いた状態で起動する
- Ctrl+Shift+T（新規）: Journalパネルを畳んだ状態で起動し、
  「08:00-08:05」のような時刻表示も含めて隠れ、タスク一覧が主役になる

ボタンとパネルの構成自体はどちらのホットキーでも同じで、既定の開閉
状態だけが違う。Journalボタンはいつでも押して開閉できる。

ウィンドウの高さ（開いた時／畳んだ時）は、固定の目安値を積み上げて
見積もるのではなく、実際にウィジェットを組み立てた後にwinfo_reqheight()
で実測して求めるようにした（タグの数やタスクの文字サイズ設定が変わっても
正しい高さが自動的に求まる）。
...
LKPT_TOGGLE_BG = "#cbb8f5"  # 📓 Journalトグルの地色（名前はLKPT単体が
                             # 独立トグルだった頃の名残。値は変えていない
                             # ため定数名はそのまま残す）。CANCEL/DBのボタン
                             # 群とは異なる色相にして、視認性のある独立した
                             # チップとして目立たせる（他ボタンと被らないラベンダー）

# Outlook連携：分類待ちの会議を知らせるバナーの地色。暗い基調の中で
# 「対応が要る」ことが一目で伝わるよう、他のどのボタンとも被らない
# 明るい琥珀色にする
MEETING_CLASSIFY_BG = "#f2c078"
...
# 一覧の中でも見分けが付くようにする。「無」は他のどの感情でもない
# ニュートラルな状態を表すため、あえて彩度を落としたグレーにする
EMOTION_BUTTON_COLORS = {
    "喜": "#f4c542",
    "怒": "#ef5350",
    "無": "#9aa4b0",
    "哀": "#5c8fd6",
...
# ボタンの見た目だけを絵文字アイコンにする（「喜」「怒」等の漢字表示が
# 単調で見劣りする、との指摘への対応）。Excelに書き込む値・内部の判定は
# 引き続きEMOTION_LABELSの漢字のまま変更しない（過去に記録した行との
# 表記ゆれを防ぐため。表示だけをここで差し替える）
EMOTION_ICONS = {
    "喜": "😄",
    "怒": "😠",
    "無": "😐",
...
# タスク一覧の文字サイズ調整（見出し右端のA－/A＋/↺ボタン用）。
# 老眼で見づらい／逆にもっと情報量を見たい、という好みの差があるため、
# 都度リサイズできるようにする。範囲はあまり広げすぎず、300px幅の
# ポップアップでも1行カードのレイアウトが崩れない程度に収める
ACTION_FONT_SIZE_DEFAULT = 10
ACTION_FONT_SIZE_MIN = 8
ACTION_FONT_SIZE_MAX = 16
ACTION_FONT_SIZE_STEP = 1
...
HOTKEY = "ctrl+shift+j"
# タスク特化起動（📓 Journalパネルを畳んだ状態で開き、タスク一覧を主役に
...
_WM_HOTKEY = 0x0312
# 通常起動・タスク特化起動、2つのホットキーを別々のIDで登録し、
# WM_HOTKEY受信時にmsg.wParam（発火したホットキーのID）で
# どちらが押されたかを区別する
_HOTKEY_ID_NORMAL = 1
_HOTKEY_ID_FOCUS = 2
...
def _hotkey_listener_loop() -> None:
    """
    専用スレッドで通常起動(HOTKEY)とタスク特化起動(HOTKEY_FOCUS)、
    2つのグローバルホットキーを登録し、WM_HOTKEYメッセージを待ち受ける。
    hWnd=NoneでRegisterHotKeyを呼ぶ場合、登録した同一スレッドでGetMessage
    ループを回す必要があるため、両方の登録とメッセージ待受を同じ関数内で行う。
    """
    user32 = ctypes.windll.user32
    registered_ids = []
    for hotkey_id, hotkey in (
        (_HOTKEY_ID_NORMAL, HOTKEY), (_HOTKEY_ID_FOCUS, HOTKEY_FOCUS),
    ):
        modifiers, vk = _parse_hotkey(hotkey)
        if not user32.RegisterHotKey(None, hotkey_id, modifiers | _MOD_NOREPEAT, vk):
    ...
    if not registered_ids:
        print("❌ ホットキーを1つも登録できなかったため、待受を開始しません。")
        return
...
def register_global_hotkey() -> None:
    """
    通常起動・タスク特化起動、両方のグローバルホットキーを登録する
    （専用スレッドでメッセージループを待受）。
    """
    thread = threading.Thread(target=_hotkey_listener_loop, daemon=True)
    thread.start()
...
class PopupWindow:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self._height_expanded = False    # 上記トグルの現在の状態
        self.time_preview_label = None   # 「何時-何時になるか」の事前表示（見出し位置・大文字）
        ...
        # どちらも、このshow()の中の同じチェックだけで対応できる
        # （スケジューラ側の声かけ機能とは別経路で、常にここで確認する）
        self.pending_meeting = None        # dict | None  今回表示する分類待ちの会議
        self._meeting_classify_frame = None
    ...
    def show(self, task_focus: bool = False) -> None:
        ...
        self.subitem_free_entry = None
        # 📓 Journalパネル（時間・タグ・ひれぶり・LKPT）の既定の開閉状態。
        ...
        # Outlook連携：分類待ちの会議（タグ未確定のままTimeLogに自動記録
        # された会議）があれば、開いた瞬間に真っ先に尋ねる。一番古い1件
        # だけを今回の対象にする（複数残っていても一度に1件ずつ、次に
        # 開いた時にまた確認する）。取得に失敗しても普段通り開けるように
        # 必ずNoneにフォールバックする
        try:
            ...
            pending_meetings = []
        self.pending_meeting = pending_meetings[0] if pending_meetings else None
        ...
        self._popup_width = 300
        # この時点ではまだ中身のウィジェットが無く、正しい高さを求められない。
        # 幅だけ先に固定しておく（折り返し幅がずれると、後でwinfo_reqheight()
        # を測る際に実際より低く見積もられてしまうため）。高さ・位置(y)は
        # 全ウィジェットを組み立てた後、実測してから一度だけ正しい値に
        # 設定し直す（本メソッド末尾の「実際の高さを測って確定する」を参照）
        screen_w = self.window.winfo_screenwidth()
        screen_h = self.window.winfo_screenheight()
        x = screen_w - self._popup_width - 20
        self.window.geometry(f"{self._popup_width}x{screen_h - 40}+{x}+10")

        # Outlook連携：分類待ちの会議があれば、ウィンドウの一番上に
        # 「会議の分類」バナーを常設する。手動でJournalを開いた場合も、
        # PCを再起動して次にJournalを開いた場合も、このshow()を必ず
        # 通るため、入口をここ1箇所に集約できる（スケジューラ側の
        # 自動声かけは、これとは別に「開かせるタイミング」だけを判断する）
        self._meeting_classify_frame = tk.Frame(self.window, bg=MEETING_CLASSIFY_BG)
        if self.pending_meeting is not None:
            self._meeting_classify_frame.pack(fill="x")

        meeting_header_font = tkfont.Font(family="Yu Gothic UI", size=9, weight="bold")
        tk.Label(
            self._meeting_classify_frame, text="📅 この会議の記録を分類してください",
            bg=MEETING_CLASSIFY_BG, fg=BUTTON_TEXT_COLOR, font=meeting_header_font,
            anchor="w",
        ).pack(fill="x", padx=14, pady=(10, 2))

        subject_text = ""
        if self.pending_meeting is not None:
            m = self.pending_meeting
            time_range = ""
            if m["start"] and m["end"]:
                time_range = f"{m['start'].strftime('%H:%M')}–{m['end'].strftime('%H:%M')} "
            subject_text = f"{time_range}{m['subject']}"
        meeting_info_font = tkfont.Font(family="Yu Gothic UI", size=10, weight="bold")
        tk.Label(
            self._meeting_classify_frame, text=subject_text,
            bg=MEETING_CLASSIFY_BG, fg=BUTTON_TEXT_COLOR, font=meeting_info_font,
            anchor="w", wraplength=self._popup_width - 28, justify="left",
        ).pack(fill="x", padx=14, pady=(0, 8))

        # タグ選択は、下のタグ一覧と同じ2列グリッドにする（見慣れた配置の方が
        # 「これはタグを選ぶ操作だ」と直感的に伝わるため）
        meeting_tag_row = tk.Frame(self._meeting_classify_frame, bg=MEETING_CLASSIFY_BG)
        meeting_tag_row.pack(fill="x", padx=14, pady=(0, 6))
        for col in range(2):
            meeting_tag_row.columnconfigure(col, weight=1, uniform="meetingtag")
        meeting_tag_font = tkfont.Font(family="Yu Gothic UI", size=10, weight="bold")
        for i, (tag_name, color_code) in enumerate(tags):
            row, col = divmod(i, 2)
            btn = tk.Button(
                meeting_tag_row, text=tag_name, bg=color_code,
                fg=_readable_text_color(color_code), relief="flat", bd=0,
                font=meeting_tag_font, cursor="hand2", pady=5,
                command=lambda t=tag_name: self._classify_pending_meeting(t),
            )
            btn.grid(row=row, column=col, sticky="ew", padx=3, pady=3)

        tk.Button(
            self._meeting_classify_frame, text="後で（次にJournalを開いた時にまた聞く）",
            bg=MEETING_CLASSIFY_BG, fg=BUTTON_TEXT_COLOR, relief="flat", bd=0,
            font=tkfont.Font(family="Yu Gothic UI", size=8, underline=True),
            cursor="hand2", command=self._skip_pending_meeting,
        ).pack(pady=(0, 10))
        ...
        # 既定の開閉状態に合わせて、ここで一度だけpackしておく（既に開いた
        # 状態で立ち上げる通常起動の場合、後段のwinfo_reqheight()実測の
        # ためにも中身が組み立てられている必要がある）。実際の開閉状態の
        # 反映は、全ウィジェットを組み立てた後の「実際の高さを測って
        # 確定する」でまとめて行う
        self._journal_frame.pack(fill="x")

        # 見出し「📝 LKPT」は表示せず、その位置に時間帯プレビューそのものを
        # 大きく太字で置く（モックアップで確認済みの案）。「いま押すと何時から
        # 何時までになるか」を、タグを押す前から常時示す。v1.30.0でタグ押下＝
        # 即記録になった際に廃止された「タグ選択時プレビュー」の復活版だが、
        # タグ選択と結び付ける必要は無い（記録される範囲はどのタグを押すかに
        # 関係なく「前回の終了時刻(または勤務開始時刻)〜いま」だけで決まる
        # ため）。Journalパネルの一番上に置く
        time_main_font = tkfont.Font(family="Yu Gothic UI", size=20, weight="bold")
        self.time_preview_label = tk.Label(
            self._journal_frame, text="", bg=BG_COLOR, fg=TEXT_COLOR,
            font=time_main_font, wraplength=self._popup_width - 20, justify="center",
        )
        self.time_preview_label.pack(pady=(2, 0))
        self._register_themed(self.time_preview_label, bg=True, fg=True)

        time_dur_font = tkfont.Font(family="Consolas", size=9)
        self.time_preview_dur_label = tk.Label(
            self._journal_frame, text="", bg=BG_COLOR, fg=PLACEHOLDER_COLOR,
            font=time_dur_font,
        )
        self.time_preview_dur_label.pack(pady=(1, 4))
        self._register_themed(self.time_preview_dur_label, bg=True)

        # 縦幅を画面いっぱいに広げる／元に戻すトグル（MS To Doの展開ボタンと
        # 同じ動作）。中央揃えのタイトルとは独立させたいのでplace()で
        # ウィンドウ右上に固定する。ウィンドウ幅が300pxしかないため、
        # タイトルの短さ（中央に寄る）を利用して隅の余白に収める
        expand_font = tkfont.Font(family="Yu Gothic UI", size=11)
        self.expand_height_btn = tk.Button(
            ...
            relief="flat", bd=0, font=expand_font, cursor="hand2",
            command=self._toggle_height_expand,
        )
        self.expand_height_btn.place(relx=1.0, x=-6, y=8, anchor="ne")
        self._register_themed(self.expand_height_btn, bg=True)
        ...
        self._register_themed(self.version_label, bg=True)
        ...
        self.selected_label = tk.Label(
            self._journal_frame, text="未選択", bg=BG_COLOR, fg=ACCENT_COLOR,
        ...
        # 「未選択」バッジの下に添える一言。時間帯プレビュー本体（時刻・所要
        # 時間）は上の見出し位置に出ているため、ここには短い説明文だけを置く。
        # 2時間超の空きがある場合は警告色にする
        preview_comment_font = tkfont.Font(family="Yu Gothic UI", size=9)
        self.time_preview_comment_label = tk.Label(
            self._journal_frame, text="", bg=BG_COLOR, fg=PLACEHOLDER_COLOR,
            font=preview_comment_font, wraplength=self._popup_width - 30, justify="center",
        )
        self.time_preview_comment_label.pack(pady=(0, 4))
        self._register_themed(self.time_preview_comment_label, bg=True)
        ...
        # 指摘への対応）
        self.tag_frame = tk.Frame(self._journal_frame, bg=BG_COLOR)
        ...
        for i, (tag_name, color_code) in enumerate(tags):
            ...
            # innerをfill="x"にせず中身の幅だけでpackすると、チップ自体は
            # grid+uniformで同じ幅に揃っていても、pack()の既定挙動（左右
            # 中央寄せ）によって「R19」のような短い名前と「JP Site」の
            # ような長い名前とで●の開始位置がずれ、列内で「並んでいない」
            # ように見える不具合があった。fill="x"にしてチップいっぱいに
            # 広げたうえで中身は左詰め（pack側はside="left"のまま）にする
            # ことで、名前の長さに関係なく●の位置が列ごとに揃うようにする
            inner = tk.Frame(chip, bg=BG_COLOR)
            inner.pack(pady=7, fill="x")
        ...
        # 中項目（何をしていたか）ボタンは、タググループ全体（2列チップの
        # 並び）の下にまとめて表示する。以前は選んだタグの行の直後に
        # 割り込ませていたが、「大項目タグをまとめて上に揃えた方が
        # スッキリする」との指摘を受け、タグ一覧を最後まで表示してから
        # その下に固定表示する構成に変更した
        # （時間帯プレビューはv1.30.0でここから廃止され、現在は別の行
        # （タグ一覧より前、time_preview_label）として復活している）
        self.tag_detail_frame = tk.Frame(self._journal_frame, bg=BG_COLOR)
        ...
        # _journal_frameの中ではなくウィンドウ直下に置く。Journalパネルを
        # 畳んだ状態で開き直した場合でも、タグ未選択でも「直前の記録」を
        # 示す必要があるため（タスク特化起動でパネルを畳んでいても、
        # 直前にTIMEを記録した旨はここで分かる）
        ...
        action_section_label = tk.Label(
            action_header_frame, text="タスク", bg=BG_COLOR,
            fg=PLACEHOLDER_COLOR, font=tkfont.Font(family="Yu Gothic UI", size=9, weight="bold"),
            anchor="w",
        )
        action_section_label.pack(side="left")
        self._register_themed(action_section_label, bg=True)

        # タスク一覧の文字サイズをその場で拡大・縮小・既定に戻せる小さな
        # ボタン列。「－」「＋」自体のフォントサイズを字面より一回り
        # 小さく／大きくしておくことで、読まなくても効果が直感的に伝わる
        # ようにする（多くの閲覧アプリの文字サイズ操作と同じ見せ方）
        font_ctrl_frame = tk.Frame(action_header_frame, bg=BG_COLOR)
        ...
        action_font_shrink_btn.pack(side="left")
        self._register_themed(action_font_shrink_btn, bg=True)
        ...
        action_font_grow_btn.pack(side="left")
        self._register_themed(action_font_grow_btn, bg=True)
        ...
        action_font_reset_btn.pack(side="left")
        self._register_themed(action_font_reset_btn, bg=True)
    ...
    def _resize_window(self, height: int) -> None:
        """
        ウィンドウの高さだけを変える。幅は呼び出し時点の実際の幅を保つ。
        以前は常にself._popup_width（既定の300px固定値）を使っていたため、
        ユーザーがウィンドウ端をドラッグして幅を手動で広げていても、
        LKPTの開閉や縦幅トグルを押すたびに既定の300pxへ強制的に
        戻ってしまっていた。winfo_width()で今の幅を読み、それをそのまま
        指定することで手動リサイズを保つ。
    ...
    def _toggle_journal(self) -> None:
        ...
        self._cancel_autoclose(permanent=True)
        self.journal_expanded = not self.journal_expanded
        # 縦幅いっぱいに展開中だった場合、Journal開閉の方を優先して解除する
        # （両立させるとどちらのボタン表示が正か分からなくなるため）
        if self._height_expanded:
    ...
    def _classify_pending_meeting(self, tag: str) -> None:
        """会議分類バナーでタグを選んだ時、TimeLogの該当行を確定タグへ更新する。

        classify_pending_meeting()は楽観ロック（開始・終了が記録時と一致するか）
        を内部で確認するため、想定外の変更があった場合はFalseが返る。その場合は
        バナーを消さず、状態欄にエラーを出して次回show()時の再試行に委ねる。
        """
        if self.pending_meeting is None:
            return
        m = self.pending_meeting
        try:
            ok = classify_pending_meeting(
                m["row"], tag, expect_start=m["start"], expect_end=m["end"],
            )
        except Exception as e:
            print(f"⚠️ 会議の分類に失敗しました: {e}")
            ok = False

        if not ok:
            if self.status_label is not None:
                self.status_label.config(text="⚠️ 会議の分類に失敗しました（もう一度お試しください）")
            return

        if self.status_label is not None:
            self.status_label.config(text=f"📅 会議「{m['subject']}」を「{tag}」として記録しました")

        # 他にも分類待ちが残っている場合があるが、一度に尋ねるのは1件だけ
        # （show()冒頭のコメントの通り、残りは次にJournalを開いた時にまた尋ねる）
        self.pending_meeting = None
        if self._meeting_classify_frame is not None:
            self._meeting_classify_frame.pack_forget()

    def _skip_pending_meeting(self) -> None:
        """「後で」を選んだ時、今回だけバナーを隠す（分類はせず、次回show()でまた聞く）。"""
        if self._meeting_classify_frame is not None:
            self._meeting_classify_frame.pack_forget()
        self.pending_meeting = None
    ...
    def _update_time_preview(self) -> None:
        """
        「いま押すと何時から何時までになるか」を、タグを押す前から示す。
        append_time_log()と同じロジック(_compute_time_range_v2)で先読みする
        peek_next_time_range_v2()を使うため、実際に記録される内容と一致する。
        v1.30.0でタグ押下＝即記録になった際に廃止されたプレビューの復活版。

        3つのウィジェットに分けて表示する（モックアップで確認済みの構成）。
        - time_preview_label: 見出し位置。時刻そのものを大きく太字で
          （「📝 LKPT」という文字は表示しない）
        - time_preview_dur_label: その下に所要時間を極小で
        - time_preview_comment_label: 「未選択」バッジの下に、短い一言だけ

        押した後（status_labelに結果が出た後）も、次に押した場合の見込みとして
        呼び直す（開いたまま時間が経つと「前回の終了時刻〜いま」がずれて
        古くなるため）。
        """
        if self.time_preview_label is None:
            return
        try:
            kind, start, end = peek_next_time_range_v2()
        except Exception as e:
            print(f"⚠️ 時間帯プレビューの計算に失敗しました: {e}")
            self.time_preview_label.config(text="")
            self.time_preview_dur_label.config(text="")
            self.time_preview_comment_label.config(text="", fg=PLACEHOLDER_COLOR)
            return

        if kind == "anchor":
            self.time_preview_label.config(text="—", fg=PLACEHOLDER_COLOR)
            self.time_preview_dur_label.config(text="")
            self.time_preview_comment_label.config(
                text="基準点として記録されます", fg=PLACEHOLDER_COLOR,
            )
            return

        if kind == "gap":
            # append_time_log()と同じ境界計算（未記録／実作業の2行に分かれる）。
            # ここでの表示はズレると意味が無いため、分岐先の書き込みロジックと
            # 同じ式を使う
            cutoff = end - timedelta(hours=MAX_TIMELOG_GAP_HOURS)
            span_start, span_end = cutoff, end
            comment_text = "これより前は未記録になります"
            comment_color = TIME_GAP_WARN_COLOR
        else:
            span_start, span_end = start, end
            comment_text = "この範囲で記録されます"
            comment_color = PLACEHOLDER_COLOR

        minutes = int((span_end - span_start).total_seconds() // 60)
        self.time_preview_label.config(
            text=f"{span_start.strftime('%H:%M')}–{span_end.strftime('%H:%M')}",
            fg=TEXT_COLOR,
        )
        self.time_preview_dur_label.config(text=_format_duration(minutes))
        self.time_preview_comment_label.config(text=comment_text, fg=comment_color)

    def _on_emotion_tapped(self, emotion: str) -> None:
        """
        ひれぶり（喜怒無哀楽）ボタンのタップを1件記録する。
        LKPTと違い確認や取り消しの手順を持たない。押し間違えても、
        次の瞬間に別の感情を押せば増える一方なので実害が小さく、
        「訂正できる」より「何もしないより軽く残せる」を優先している。
        タグは選択中であれば引き継ぐが、必須にはしない
        （タグを選ばずにポップアップを開いた瞬間の気分でも記録できるように）。
        """
        self._cancel_autoclose(permanent=True)
        try:
            ok, stamp = append_hirebi(self.selected_tag or "", emotion)
        except Exception as e:
            print(f"❌ ひれぶりの記録に失敗しました: {e}")
            ok, stamp = False, None

        if self.emotion_feedback_label is None:
            return
        if ok:
            icon = EMOTION_ICONS.get(emotion, "")
            self.emotion_feedback_label.config(
                text=f"{icon} {emotion} を記録しました（{stamp.strftime('%H:%M')}）"
            )
        else:
            self.emotion_feedback_label.config(text="⚠️ 記録に失敗しました")
        self.window.after(
            1800, lambda: self.emotion_feedback_label.config(text=""),
        )
    ...
    def _on_tag_clicked(self, tag_name: str, color_code: str) -> None:
        ...
        # ポップアップを閉じて開き直した直後は、show()がselected_tagを
        # Noneに戻すため、直前と同じタグをもう一度クリックしても
        # 「新しいタグの選択」に見えてしまう。これが直前に書いた行と
        # 同じタグ＝「記録を見に来ただけ／作業を継続している」を指して
        # いて、かつまだ修正可能時間内なら、中項目を空で上書きしては
        # いけない。空上書きすると、直前にOtherへ書いた自由記述が
        # 消えるだけでなく、「前回のコメントを初期値にする」ロジック
        # （_select_subitem）がこの空になった行を素通りして、もっと
        # 過去の別のコメントを拾って再表示・再書き込みしてしまう
        if self._timelog_token_live() and self.timelog_tag == tag_name:
            return
    ...
    def _build_subitem_buttons(self, tag_name: str) -> None:
        ...
        # フォーカスが当たったら自動クローズを恒久的に止める（quick_action_entry・
        # LKPT欄と同じパターン）。これが無いと、Otherを選んで自由記述を書いている
        # 最中でも3.5秒の自動クローズが働き続け、入力途中でウィンドウが
        # 閉じてしまう不具合があった
        self.subitem_free_entry.bind(
            "<FocusIn>", lambda e: self._cancel_autoclose(permanent=True),
        )
    ...
    def _select_subitem(self, item: str) -> None:
        ...
        if item == OTHER_SUBITEM_LABEL:
            # 前回このタグの「Other」に書いた自由記述を初期値として出す。
            # 「触らなければ前回と同じ作業の継続、書き換えれば別の作業」を
            # 意味させたいという要望への対応。まだ何も打っていない場合だけ
            # 埋める（同じタグ選択のまま一度Otherを離れてまた戻ってきた場合、
            # 直前に自分が打った内容を上書きしないように）
            if not self.subitem_free_var.get():
                try:
                    last = get_last_other_comment(self.selected_tag)
                except Exception as e:
                    print(f"⚠️ 前回のOtherコメント取得に失敗しました: {e}")
                    last = ""
                if last:
                    self.subitem_free_var.set(last)
            self.subitem_free_entry.pack(fill="x", pady=(6, 0), ipady=3)
            self.subitem_free_entry.focus_set()
            self.subitem_free_entry.icursor("end")
        else:
            self.subitem_free_entry.pack_forget()
        print(f"🗂️ 中項目選択: {item}")
        # 中項目を選んだ時点で、直前に書いたTimeLog行を上書き更新する。
        # Otherの場合はまず"Other"（前回の自由記述があればそれ）として確定させ、
        # 自由記述が変更された時点であらためて上書きする
        # （記録が未確定のまま宙に浮く瞬間を作らない）
    ...
    def _update_action_font_size_label(self) -> None:
        """タスク一覧の文字サイズ操作ボタンの間に、今のサイズを数値で示す。"""
    ...
    def _increase_action_font_size(self) -> None:
        """タスク一覧の文字を1段階大きくする。"""
        self._cancel_autoclose(permanent=True)
        self.action_font_size = min(
            ACTION_FONT_SIZE_MAX, self.action_font_size + ACTION_FONT_SIZE_STEP,
        )
        self._update_action_font_size_label()
        self._render_action_rows()
    ...
    def _decrease_action_font_size(self) -> None:
        """タスク一覧の文字を1段階小さくする。"""
        self._cancel_autoclose(permanent=True)
        self.action_font_size = max(
            ACTION_FONT_SIZE_MIN, self.action_font_size - ACTION_FONT_SIZE_STEP,
        )
        self._update_action_font_size_label()
        self._render_action_rows()
        self._persist_action_font_size()
    ...
    def _offer_forecast_seed(self, task_text: str) -> None:
        """
        タスクを登録した直後に「なぜ今これを？ → 読みにする」を出す。

        タスクの裏には必ず読みがあるので、そこを捕まえれば新しい習慣を
        作らずに読みを書き始められる。押さなければ数秒で消えるだけなので、
        「打つ→Enter」の速い動線は一切変わらない。
        """
        if self.forecast_seed_label is None:
            return
        self.forecast_seed_label.config(text="なぜ今これを？ → 読みにする")
    ...
    def _open_forecasts(self) -> None:
        ...
        try:
            ...
            print(f"❌ 読みの一覧を開けませんでした: {e}")

    def _seed_forecast_from_task(self, task_text: str) -> None:
        """
        いま書いたタスクを打ち手として、読みの入力を開く。

        タスクを書く時点で頭の中では必ず何かを読んでいる
        （「ベンダーに催促する」の裏には「放っておくと間に合わない」がある）。
        ゼロから読みを捻り出すのは難しいので、その読みを捕まえる入口にする。
...
def run() -> None:
    ...
    print(
        f"🚀 ポップアップUIを起動しました。"
        f"ホットキー(通常:{HOTKEY} / タスク特化:{HOTKEY_FOCUS})待受中..."
    )
    root.mainloop()
