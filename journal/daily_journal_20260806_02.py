...
from storage import get_tag_master, peek_next_time_range, record_check_in
...
TIME_PREVIEW_COLOR = "#a8c8f0"  # タグ選択直後に出る「何時から何時まで」プレビューの色
...
class PopupWindow:
    ...
    def show(self) -> None:
        ...
        row_height = 34
        # タイトル・selected_label(太字バッジ)・時間帯プレビュー・LKPT入力4行・
        # 登録/キャンセルボタン・余白の合計目安
        chrome_height = 340  # 時間帯プレビューが2行に折り返す場合の余白を含む
        width = 300
        height = max(360, chrome_height + row_height * len(tags))
        ...
        self.time_preview_label = tk.Label(
            self.window, text="", bg=BG_COLOR, fg=TIME_PREVIEW_COLOR, font=preview_font,
            wraplength=width - 30, justify="center",
        )
        self.time_preview_label.pack(pady=(2, 8), padx=15)
        ...
        # 表示側(ラベル・ボタン等)はすべてYu Gothic UIを明示指定しているが、
        # 入力欄だけfontを指定し忘れておりTkの既定フォントで描画されていた
        # （サイズも書体も浮いて見える原因）。表示側と揃える。
        entry_font = tkfont.Font(family="Yu Gothic UI", size=10)
        first_entry = None
