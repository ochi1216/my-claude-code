...
class PopupWindow:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.time_preview_label = None   # 「いま押すと何時-何時になるか」の事前表示
    ...
    def show(self) -> None:
        ...
        row_height = 34
        # タイトル・selected_label(太字バッジ)・時間帯プレビュー行・
        # ひれぶり(喜怒哀楽)ボタン行・中項目ボタン・LKPTトグルボタン・
        # 登録/キャンセル/DBボタン・＋アクション欄・
        ...
        # 時間帯プレビュー：「いま押すと何時から何時までになるか」を、
        # タグを押す前から常時示す。v1.30.0でタグ押下＝即記録になった際に
        # 廃止された「タグ選択時プレビュー」の復活版だが、タグ選択と結び付ける
        # 必要は無い（記録される範囲はどのタグを押すかに関係なく「前回の
        # 終了時刻(または勤務開始時刻)〜いま」だけで決まるため）。タグ一覧より
        # 前、開いた瞬間に見える位置に置く。status_label（押した後の結果）とは
        # 別の行にし、押す前と押した後の情報を混同しないようにする
        ...
        self.time_preview_label.pack(pady=(0, 4))
    ...
    def _update_time_preview(self) -> None:
        ...
        if kind == "anchor":
            body = "🕐 いま押すと、本日の基準点として記録されます"
        else:
            if kind == "gap":
                # append_time_log()と同じ境界計算（未記録／実作業の2行に
                # 分かれる）。ここでの表示はズレると意味が無いため、
                # 分岐先の書き込みロジックと同じ式を使う
                cutoff = end - timedelta(hours=MAX_TIMELOG_GAP_HOURS)
                span = f"{cutoff.strftime('%H:%M')}–{end.strftime('%H:%M')}"
                minutes = int((end - cutoff).total_seconds() // 60)
            else:
                span = f"{start.strftime('%H:%M')}–{end.strftime('%H:%M')}"
                minutes = int((end - start).total_seconds() // 60)
            body = f"🕐 いま押すと {span} として記録されます（{_format_duration(minutes)}）"
            if kind == "gap":
                body += "\n※ それより前は「未記録」として残ります"
        self.time_preview_label.config(text=body)
