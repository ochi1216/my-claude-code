# -*- coding: utf-8 -*-
"""
daily_journal_20260831_01.py
学びジャーナル - ホットキー起動の入力ポップアップUI
Version: 0.41.0

v0.41.0での変更点：
1. LKPTを既定で開いた状態で立ち上げるようにした（以前は畳んだ状態が既定）。
2. LKPTの開閉・縦幅トグルでウィンドウをリサイズする際、幅を既定値
   (300px)へ、高さをcollapsed_height/expanded_heightという絶対値へ
   強制的に戻していたのをやめた。今の実際の幅をそのまま使い、高さは
   LKPT欄ぶんの差分だけを足し引きするようにし、ユーザーが手動で
   リサイズした大きさを保つ。
3. タスク一覧の文字サイズを、見出し「タスク」の右端のA－/A＋/↺ボタンで
   その場で拡大・縮小・既定値へのリセットができるようにした。選んだ
   サイズはtimelog_rowなどと同様、閉じて開き直しても保たれる。
"""
...
# 「魂のひれぶり」信号（喜・怒・哀・楽）のボタン色。タグ4色・LKPTトグルの
...
class PopupWindow:
    ...
    def show(self) -> None:
        ...
        self.subitem_free_entry = None
        # LKPT（振り返り）は開いた状態で立ち上げる。以前は畳んだ状態が既定
        # だったが、書きたい時にまずアコーディオンを開く1手間がかえって
        # 面倒という指摘を受け、既定を開にした
        self.lkpt_expanded = True
        ...
        row_height = 34
        # 時間帯プレビュー(見出し位置の大文字＋所要時間)・selected_label
        # (太字バッジ)・バッジ下コメント・ひれぶり(喜怒哀楽)ボタン行・
        # 中項目ボタン・LKPTトグルボタン・登録/キャンセル/DBボタン・
        # ＋アクション欄・たまっているアクション一覧（固定4行分）・
        # 余白の合計目安
        # （LKPT欄自体は畳んだ状態の高さで、展開時はlkpt_block_heightを追加する）
        chrome_height = 730
        # LKPT欄の開閉ぶんの高さの差分。_toggle_lkpt()でウィンドウを
        # リサイズする際、絶対値ではなくこの差分を今の高さに足し引きする
        # ことで、ユーザーが手動でリサイズした幅・高さを保つ（差分をここに
        # インスタンス変数として持たせておかないと、開閉のたびに再計算元の
        # tags件数などが必要になってしまう）
        self.lkpt_block_height = 4 * 40 + 20
        self._popup_width = 300
        ...
        self.expanded_height = min(
            self.collapsed_height + self.lkpt_block_height,
        ...
        # LKPTを開いた状態で立ち上げるため、初期高さもcollapsed_heightでは
        # なくexpanded_heightにする（畳んだ状態のまま開くと、後段で
        # _lkpt_frame.pack()した時に中身がウィンドウからはみ出す）
        self.window.geometry(f"{self._popup_width}x{self.expanded_height}+{x}+{y}")
        min_width = int(self._popup_width * 2 / 3)
        self.window.minsize(min_width, self.expanded_height)
        ...
        # 直前の要素（ひれぶりのフィードバック行）との間に固定の間隔を
        # 空けて配置する。横幅は他のボタン列（登録/キャンセル/DB）や
        # タグ一覧・入力欄と揃うよう、中央寄せの小さなピルではなく
        # fill="x"で全幅にする
        ...
        # LKPTは既定で開いた状態にするため、ここで一度だけ組み立てる。
        # ウィンドウの高さは既にexpanded_heightで作成済みのため、
        # _resize_window()は呼ばずpack()だけでよい
    ...
    def _toggle_height_expand(self) -> None:
        ...
        おくのではなく、その時点のLKPT開閉状態（lkpt_expanded）が本来
        求める高さ（expanded_height / collapsed_height）を都度計算して使う。
        こうしておけば、展開中にLKPT欄を開閉されても矛盾が起きない
        （_toggle_lkpt側でもこのフラグを解除している）。
        """
        self._cancel_autoclose(permanent=True)
        ...
        if self._height_expanded:
            ...
            base_height = self.expanded_height if self.lkpt_expanded else self.collapsed_height
    ...
    def _toggle_lkpt(self) -> None:
        ...
        # 縦幅いっぱいに展開中だった場合、LKPT開閉の方を優先して解除する
        ...
        # 「今のウィンドウ高さ」にLKPT欄ぶんの差分(lkpt_block_height)だけを
        # 足し引きする。絶対値へ飛ばす実装だと、ユーザーが手動でウィンドウを
        # リサイズしていても、LKPTを開閉するたびにその高さが既定値へ
        # 戻ってしまっていた
        ...
        if self.lkpt_expanded:
            ...
            new_height = min(current_height + self.lkpt_block_height, screen_h - 40)
            self._resize_window(new_height)
        else:
            self._lkpt_frame.pack_forget()
            self.lkpt_toggle_btn.config(text="LKPT")
            new_height = max(
                self.collapsed_height, current_height - self.lkpt_block_height,
            )
            self._resize_window(new_height)
