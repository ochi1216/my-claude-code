...
class PopupWindow:
    ...
    def show(self) -> None:
        ...
        # 以前はfill="both", expand=Trueにしていたため、タグ一覧が窓の余白を
        # 吸収し、直下の「選択中」バッジとの間に大きな空白ができてしまって
        # いた。「間を空けずに配置」の指示に沿い、タグ一覧自身の自然な高さ
        # だけを占めるようにする（expandを外す）
        tag_frame = tk.Frame(self.window, bg=BG_COLOR)
        tag_frame.pack(pady=(4, 0), fill="x", padx=20)
        ...
        # 中項目（何をしていたか）ボタンと時間帯プレビューは、以前は
        # タグ一覧全体の下の固定位置に表示していたため「選んだタグから
        # 離れている」という指摘を受けた。tag_detail_frameをtag_frameの
        # 子（タグの行と同じ階層）として持たせ、_select_tag()の中で
        # pack(after=選択した行)することで、選んだタグの行の直後
        # （＝次のタグとの間）に動的に差し込む
        self.tag_detail_frame = tk.Frame(tag_frame, bg=BG_COLOR)
        ...
        # タグ一覧（中項目が差し込まれた状態も含む）の直下、間を空けずに固定表示する
        self.selected_label.pack(pady=(2, 0))
        ...
        action_section_label = tk.Label(
            self.window, text="📌 たまっているアクション", bg=BG_COLOR,
        ...
        action_vbar.pack(side="left", fill="y")
    ...
    def _select_tag(self, tag_name: str, color_code: str) -> None:
        ...
        # 中項目＋時間帯プレビューのブロックを、選んだタグの行の直後に
        # 差し込み直す（別のタグに切り替えた場合は、一旦外してから
        # 新しい行の直後に付け直すことで移動させる）
        self.tag_detail_frame.pack_forget()
        self.tag_detail_frame.pack(
            after=self.tag_rows[tag_name], fill="x", pady=(4, 4),
        )
