...
class MailSummarizer:
    ...
    def summarize_review_month(self, year: int, month: int, threads: dict, meetings: list,
        ...
        Tier1/Tier2関与・G2小分類・報告ランク・会議紐付けまで、この関数の中で annotate
        してからキャッシュする(以前はgenerate_review_data側で毎回annotateしていたが、
        「対象月チェックボックスで選ばれた月だけ再取得・再分析し、それ以外の月は
        キャッシュからそのまま読み込む」機能に対応するため、annotate済みの状態を
        月単位でキャッシュへ保存し、threads/meetingsが無くてもそのまま使えるようにした)。
    ...
    def load_review_month_cache(self, yyyymm: str) -> list:
    ...
    def generate_review_data(self, monthly_threads: dict, monthly_meetings: dict,
                              all_target_months: list, progress_callback=None) -> dict:
        """振り返りタブ(四半期パフォーマンスレビュー)用のデータを生成する。
        monthly_threads/monthly_meetings: {"YYYYMM": ...} 形式で、対象月チェックボックスが
        ONだった月(=今回メールを再取得・再分析する月)のみを含む。
        all_target_months: チェックボックス一覧に表示されている全ての月("YYYYMM"のリスト、
        ON/OFF問わず)。monthly_threadsに含まれない月は、load_review_month_cacheで
        キャッシュ済みのannotate結果をそのまま読み込む(メール再取得・AI再分析はしない)。
        これにより「更新したい月だけチェックして再生成」が可能になる。
        ...
        for i, yyyymm in enumerate(months):
            ...
            if yyyymm in monthly_threads:
                achievements = self.summarize_review_month(
                    year, month, monthly_threads[yyyymm], monthly_meetings.get(yyyymm, []),
                    progress_callback=None
                )
            else:
                achievements = self.load_review_month_cache(yyyymm)
    ...
    def _ui_review_tab(self):
        ...
        ttk.Label(
            ...
            text="※ チェックの付いた月だけ、現行メールボックスとオンラインアーカイブの両方から自分が送信したメール\n"
                 "(含・関連スレッド)と主催した会議を取得し、AIが「実績」単位に統合し直します。チェックを外した月は\n"
        ...
        def task():
            try:
                ...
                review_data = self.summarizer.generate_review_data(
                    monthly_threads, monthly_meetings, all_months,
                    progress_callback=lambda c, t, msg: self.root.after(0, lambda: self._set_status(msg, current=c, total=t))
                )
