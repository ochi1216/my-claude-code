...
def get_physical_work_area():
    ...
    try:
        ...
        prev_context = user32.SetThreadDpiAwarenessContext(DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2)
        if not prev_context:
            print("🔧[Outlook配置] SetThreadDpiAwarenessContextに失敗しました"
                  "(Windows 10 1607より前の可能性)。物理ピクセル座標の取得を諦め、仮想座標にフォールバックします。")
            return None
        try:
            ...
            user32.SetThreadDpiAwarenessContext(prev_context)
    except Exception as e:
...
class MailManagerGUI:
    ...
    def __init__(self):
        ...
        # 本ツール(Tkinter, DPI非対応)自身の配置には、winfo_screenwidth()等と
        # 同じ「仮想(拡大率で割った)座標系」の作業領域を使う。
        ...
        half_w = wa_width // 2
        left_w = half_w - gap // 2
        # 高さを作業領域ぴったりにすると暗黙の最大化を誘発するため、わずかに低くする。
        target_height = wa_height - self.WINDOW_HEIGHT_MARGIN
        fit_window_to_work_area(self.root, left_w, target_height, wa_left, wa_top)

        # OutlookはDPI Aware(物理ピクセル座標)で動作するため、Outlookへ渡す
        # 座標は物理ピクセル単位の作業領域から計算する(仮想座標のままだと、
        # 拡大率150%の環境で実際の画面の2/3の範囲にしか収まらず、Windowsの
        # 暗黙の最大化トリガーとの組み合わせで繰り返し最大化される不具合が
        # 実機ログで確認された)。取得できない場合は仮想座標へフォールバックする
        # (この場合、拡大率100%の環境であれば仮想座標=物理座標なので問題ない)。
        ...
        p_half_w = p_width // 2
        outlook_left = p_left + p_half_w + (gap - gap // 2)
        outlook_width = p_right - outlook_left
        outlook_height = p_height - self.WINDOW_HEIGHT_MARGIN

        print(f"🖥 画面配置: work_area(仮想, 取得={'成功' if work_area else '失敗→画面全体を使用'})="
              f"({wa_left},{wa_top})-({wa_right},{wa_bottom}) / "
              f"本ツール=x:{wa_left} w:{left_w} h:{target_height} / "
              f"work_area(物理, 取得={'成功' if phys_work_area else '失敗→仮想座標を流用'})="
              f"({p_left},{p_top})-({p_right},{p_bottom}) / "
              f"Outlook=x:{outlook_left} w:{outlook_width} h:{outlook_height}")
