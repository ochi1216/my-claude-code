...
### 追加・修正
	...
	**振り返りタブ: HTMLサマリを対象者ごとに別ファイルへ分割生成する仕様に変更**。従来は選択した全対象者(Ochi+スタッフ)の実績を1つのHTMLファイルに人物チップ付きでまとめて表示していたが、四半期パフォーマンスレビューという用途上、本人以外の評価内容が同じファイルに混在するのは望ましくないとのユーザー指摘を受け、対象者ごとに独立したHTMLファイル(`Review_{対象者名}_{タイムスタンプ}.html`)を生成する方式に変更した。複数人まとめたHTMLは生成しない。「📈 振り返りを生成」「🎨 フォーマットのみ再生成」の両方で対応し、フォーマットのみ再生成についても、既存の`review_last_result.json`(メール再取得・AI再解析なしで再利用する保存済みデータ)から対象者ごとに別々のHTMLへ分割生成できるようにした(この用途のため`review_last_result.json`に新たに`persons`フィールドを追加。旧バージョンで保存されたファイルには無いため、その場合はOchiのみとして扱う後方互換を実装)。
...
### 変更関数
	...
	`HTMLReportGenerator.generate_review_report`（`filter_person`引数を追加。指定時はachievementsをその対象者のみに絞り込み、ファイル名・ページタイトル・ヘッダーの期間表示に対象者名を反映する。手動追加フォームの対象者プルダウンも、その対象者のみに固定する）
	...
	`MailManagerGUI._reformat_review`（保存済みの`persons`を読み込み、対象者ごとに`generate_review_report`をループ呼び出しするよう変更。複数人まとめたHTMLは生成しない）
