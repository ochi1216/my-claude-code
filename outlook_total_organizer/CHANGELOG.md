...
### 追加・修正
	**Outlookが画面右半分に配置されない問題の真因を特定し修正した。原因は座標の「二重適用」**。実機ログとスクリーンショットを突き合わせたところ、決定的な矛盾が見つかった。ログでは`GetWindowRect`が指定どおり`(1926, 0, 1914, 2080)`を返しているのに、画面上のOutlookの左端は画面幅の約75%の位置にあった。`2889 ÷ 3840 = 75.2%`であり、`1926 × 1.5 = 2889`と完全に一致する。
	原因は、**本ツールのプロセスがDPI非対応であること**(`_05`で、文字が小さく詰まって見える不具合を直すために意図的にそうした)による座標の仮想化。DPI非対応プロセスがWin32 APIを呼ぶと、Windowsは渡された座標を拡大率倍して実ウィンドウに適用し、`GetWindowRect`は逆に実座標を拡大率で割って返す。`_20260820_01`で「物理ピクセル(3840基準)で計算する」ようにしたにもかかわらず、その物理値をDPI非対応のスレッドから`SetWindowPos`へ渡していたため、Windowsによってさらに1.5倍されていた。
	**この不具合が自分自身を隠していたことが、本件の原因究明が異常に長引いた最大の理由である**。`SetWindowPos`と`GetWindowRect`が同じ仮想座標系で動作するため、「1926を設定→1926が返る」と完全に整合し、15回の再チェックすべてが`drifted=False`になる。ログ上はどう見ても成功しているのに画面だけがズレる、という状態だった。座標系の不一致(`_20260820_01`)・COM層の不安定さ(`_20260820_02`)・Z順序フラグの矛盾(`_20260820_03`)と、途中で見つけた不具合はいずれも実在するものだったが、この最後の一枚が残っていたために症状が消えなかった。
	修正として`ThreadDpiAware`(コンテキストマネージャ)を新設し、**実際にWin32 APIでウィンドウを操作する区間だけスレッドをDPI Awareに切り替える**ようにした。これにより物理ピクセルがそのまま適用される。Tkinterを動かすメインスレッドとは別のワーカースレッドで完結するため、GUIの描画(拡大率に応じた自動拡大表示)には一切影響しない。
	あわせて**配置計算を物理ピクセルに一本化**した。従来は本ツール側を仮想座標、Outlook側を物理座標で別々に計算しており、左右のペインの幅・高さが揃わない原因になっていた。物理ピクセルで等幅・等高に割り付けたうえで、Tkinterへ渡す値だけを最後に仮想座標へ換算する。ご要望に合わせて隙間(`WINDOW_SPLIT_GAP`)を12→30pxに変更した。

### 変更関数
	`get_physical_work_area`（新設した`ThreadDpiAware`を使うようリファクタ。挙動は同じ）
	`OutlookMailManager.arrange_outlook_window`（本体を`_arrange_outlook_window_impl`へ分離し、`ThreadDpiAware`で囲んで呼ぶよう変更。座標を物理ピクセルのまま扱えるようにした）
	`MailManagerGUI.__init__`（配置計算を物理ピクセルに一本化。左右ペインを等幅・等高で割り付け、Tkinterへ渡す値のみ仮想座標へ換算する。診断ログに拡大率・ペインサイズを追加）

### 新規追加：
	クラス `ThreadDpiAware`（with文の区間だけスレッドをDPI Awareにするコンテキストマネージャ。切り替え可否は`active`プロパティで判定できる）
	メソッド `OutlookMailManager._arrange_outlook_window_impl`（`arrange_outlook_window`の本体。DPI Awareなスレッドコンテキスト下で実行される前提）

### 削除：
	なし

変更ファイル：
	`outlook_total_organizer_20260821_02.py`（`_20260821_01`からのコピー＋今回の変更。`_20260821_01`はそのまま残置）

変更しないこと（宣誓）：
	`_set_window_bounds_win32`/`_get_window_state_win32`/`_find_explorer_hwnd`の実装自体(Z順序修正・状態取得・HWND特定)には触れていない。呼び出される際のスレッドのDPIコンテキストが変わっただけである。`fit_window_to_work_area`(本ツール自身のタイトルバー補正)、`_bind_hour_spinbox_state`(hスピンボックスのグレーアウト)、`show_thread_in_explorer`(件名クリックでのOutlook検索遷移)にも触れていない。

動作確認時の注意：
	本ツールはWindows専用のため、本セッションの実行環境では実際のDPI仮想化挙動は再現できない。`python3 -m py_compile`による構文チェック、`_20260821_01`との`diff`で変更範囲を確認済み。
	Outlook非依存のスタンドアロン`python3`ハーネスで、**拡大率150%のWindowsの座標仮想化そのものを再現するフェイクWin32層**(スレッドのDPI Awareness状態に応じて`SetWindowPos`が座標を1.5倍する/しない、`GetWindowRect`が1.5で割る/割らないを切り替える)を実装し、(1)`SetWindowPos`がDPI Awareな状態で呼ばれ、実ウィンドウの物理位置が指定どおりになること、(2)**同じフェイク層でDPI非対応のまま呼ぶと物理位置が2889になり、テスト自体が本不具合を検出できること**(担保テスト)、(3)Outlookの右端が物理作業領域の右端に一致し画面外へはみ出さないこと、(4)左右のペインが等幅(誤差1px以内)・隙間30px・高さがタスクバーを除いた作業領域より小さいこと、(5)`get_physical_work_area`呼び出し後にDPI非対応へ戻りTkinter描画に影響しないこと、を検証し全て合格した。`_20260821_01`時点の既存回帰テスト(hスピンボックスのグレーアウト、Z順序修正、HWND特定診断、件名検索の全角括弧対応)も再実行し、合格を維持していることを確認した。
	**実機での確認をお願いしたい**: (1)Outlookが画面右半分に正しく配置されること(これまでのように75%の位置から始まらないこと)。(2)本ツールとOutlookの幅・高さが揃い、間に30px程度の隙間が空いていること。(3)両ウィンドウともタスクバーに重ならないこと。(4)本ツールの文字の大きさがこれまでどおりであること(DPI切り替えはワーカースレッド限定のため影響しないはずだが、念のため確認いただきたい)。起動時ログの`🖥 画面配置:`行に拡大率とペインサイズが出力されるので、あわせて共有いただけると確実に判断できる。

## VERSION 20260821_01

### 追加・修正
	**期間プルダウンが「任意時間(h)」以外のとき、隣接するhスピンボックスをグレーアウト(操作不能)にするUI改善**。ユーザーからのご指摘: 「検索/整理」タブの「1 h」入力欄は、期間プルダウンで「任意時間(h)」を選んだときしか使わないのに、常に編集可能な見た目になっており紛らわしい。同じ構成(期間プルダウン+hスピンボックス)を持つ「プロジェクト俯瞰」「スタッフ俯瞰」タブにも適用する方針とした(ユーザー確認済み)。
	`_bind_hour_spinbox_state`を新設し、プルダウンの値(StringVar)が「任意時間(h)」でないときはスピンボックスを`disabled`にする。ユーザーがCombobox上で選択を変更した場合だけでなく、コードから`.set()`でプルダウンの値を変更した場合(将来的な追加があった場合)にも追従できるよう、Comboboxのイベントではなく`textvariable`(StringVar)の`trace_add("write", ...)`で判定している。ウィジェット生成直後にも一度呼び出し、初期状態(既定値が「任意時間(h)」以外の3タブすべて)から正しくグレーアウトされた状態で表示されるようにした。

### 変更関数
	`MailManagerGUI._ui_search_tab`（`spin_search_h`生成後に`_bind_hour_spinbox_state`を呼ぶよう追加）
	`MailManagerGUI._ui_project_tab`（`spin_proj_h`生成後に同様に追加）
	`MailManagerGUI._ui_staff_tab`（`spin_staff_h`生成後に同様に追加）

### 新規追加：
	メソッド `MailManagerGUI._bind_hour_spinbox_state(self, combo_var, spinbox)`

### 削除：
	なし

変更ファイル：
	`outlook_total_organizer_20260821_01.py`（`_20260820_04`からのコピー＋今回の変更。`_20260820_04`はそのまま残置。日付が変わったためバージョン番号は`20260821_01`から再開）

変更しないこと（宣誓）：
	期間プルダウンの選択肢一覧や、選択された期間をもとに実際の検索条件を組み立てるロジック(`days_map`等)には一切触れていない。今回はスピンボックスの見た目(操作可否)のみの変更。Outlookウィンドウ配置関連のコード(`_find_explorer_hwnd`等)には触れていない。

動作確認時の注意：
	`python3 -m py_compile`による構文チェック、`_20260820_04`との`diff`で変更範囲が今回の追加分のみであることを確認済み。Outlook非依存のスタンドアロン`python3`ハーネスで、フェイクのStringVar/Spinboxを用いて、(1)初期値が「任意時間(h)」以外の場合は生成直後からグレーアウトされること、(2)初期値が「任意時間(h)」の場合は生成直後から操作可能なこと、(3)プルダウンの値を「任意時間(h)」に変更すると即座に操作可能になること、(4)「任意時間(h)」から他の値に戻すと再びグレーアウトされること、(5)3タブすべてで`_bind_hour_spinbox_state`が呼ばれていること、を検証し全て合格した。既存の回帰テスト(Z順序修正・HWND特定診断)も再実行し、合格を維持していることを確認した。
	実機での確認をお願いしたい点: (1)「検索/整理」「プロジェクト俯瞰」「スタッフ俯瞰」の3タブとも、起動直後（既定値が「任意時間(h)」以外）はhスピンボックスがグレーアウトされていること。(2)期間プルダウンで「任意時間(h)」を選ぶと即座に操作可能になり、数値を入力できること。(3)「任意時間(h)」から他の選択肢に戻すと再びグレーアウトされること。

## VERSION 20260820_04（診断用ビルド）

### 追加・修正
	**「表示結果が寸分の狂いなく全く変わらない」との報告を受け、操作対象のウィンドウの実体を確認する診断を追加した**。`_20260820_03`(`SWP_NOZORDER`フラグの矛盾を修正)投入後も画面が一切変化しないとのことで、ログ上は毎回`SetWindowPos`が成功し位置・サイズも安定しているにもかかわらず実際の見た目が変わらないという食い違いから、**そもそも`_find_explorer_hwnd`が特定しているHWNDが、実際に画面に見えているOutlookのウィンドウとは別物である可能性**を疑った。
	`_find_explorer_hwnd`に、(1)COM側で取得した`Explorer.Caption`の値、(2)HWNDをどちらの経路(`FindWindowW`によるクラス名+Caption一致、または`GetForegroundWindow`によるフォールバック)で特定したか、(3)特定したHWNDの実際のウィンドウタイトル(`GetWindowTextW`)とクラス名(`GetClassNameW`)、をログ出力するようにした。これにより、次回の実機テストで「操作しているウィンドウは本当にOutlookの目に見える受信トレイ画面か」を直接確認できる。

### 変更関数
	`OutlookMailManager._find_explorer_hwnd`（Caption・特定経路・実際のウィンドウタイトル/クラス名をログ出力するよう変更。HWND特定ロジック自体は変更なし）
...
変更ファイル：
	`outlook_total_organizer_20260820_04.py`（`_20260820_03`からのコピー＋今回の変更。`_20260820_03`はそのまま残置）

変更しないこと（宣誓）：
	`_set_window_bounds_win32`（`_20260820_03`で修正したZ順序）、`_get_window_state_win32`、`arrange_outlook_window`の全体フロー、座標計算ロジックには一切触れていない。今回はログ追加のみで、HWND特定ロジック自体(FindWindowW→GetForegroundWindowフォールバックの順序・条件)は変更していない。

動作確認時の注意：
	本ツールはWindows専用のため、本セッションの実行環境では実際のウィンドウタイトル・クラス名は取得できていない。`python3 -m py_compile`による構文チェック、`_20260820_03`との`diff`で変更範囲が`_find_explorer_hwnd`内のログ追加のみであることを確認済み。
	Outlook非依存のスタンドアロン`python3`ハーネスで、(1)ログ追加後もHWND特定ロジック自体の返り値が変わっていないこと(デグレなし)、(2)`FindWindowW`一致時・`GetForegroundWindow`フォールバック時それぞれで、特定方法・実際のウィンドウタイトル・クラス名が正しくログ出力されること、(3)タイトル・クラス名の取得自体が失敗してもHWNDは正しく返され例外にならないこと、を検証し全て合格した。`_20260820_03`時点の既存回帰テスト(Z順序修正、Win32直接操作の全体フロー)も再実行し、合格を維持していることを確認した。
	**実機での確認をお願いしたい**。起動時コンソールログに新たに出力される`Explorer.Caption(COM側で取得した値)`と`HWND特定方法=... / 実際のウィンドウタイトル=... / クラス名=...`の行を共有いただきたい。実際のウィンドウタイトルがOutlookの受信トレイ画面のタイトル(例:「受信トレイ - ...@...com - Outlook」)と一致していれば操作対象は正しく、それでも画面が変わらない場合は別の原因(例: 複数モニターにまたがる座標指定の問題、DWM/コンポジションの問題等)を疑う必要がある。逆にタイトルが空・別物・Outlookと無関係なものであれば、HWND特定ロジック自体に問題があることが確定する。

## VERSION 20260820_03

### 追加・修正
	**「Outlookが他のウィンドウ(ブラウザ等)の後ろに隠れる」不具合を修正**。`_20260820_02`(Win32 API直接操作への切り替え)投入後、診断ログでは位置・サイズとも15回連続で完全に安定していたにもかかわらず、実際の画面では本ツールと同じ高さ・幅で並ばず、Outlookの手前に別のウィンドウ(ブラウザ・Claude Codeのウィンドウ等)が挟まって見える、というスクリーンショットをご提供いただいた。ログ上の「成功」と実際の見た目が食い違っていたことから、位置・サイズの計算ではなく、**ウィンドウの重なり順(Z順序)**の問題であると判断し、コードを再点検した。
	原因は`_set_window_bounds_win32`内の単純な実装ミス。`SetWindowPos`に「最前面へ」を意味する`HWND_TOP`を`hWndInsertAfter`として渡しつつ、同時に`SWP_NOZORDER`(「重なり順を変更しない」)フラグも渡していた。Win32 APIの仕様上、`SWP_NOZORDER`が立っていると`hWndInsertAfter`パラメータは無視されるため、**この2つの指定は自己矛盾しており、実際には重なり順が一切変更されていなかった**。そのため、Outlookの位置・サイズ自体は正しく設定されていた(ログの`Left=1926...`が示すとおり)にもかかわらず、起動前から手前にあった他のアプリのウィンドウの後ろに隠れたままになっていた。「本ツールとOutlookの高さが揃っていない」という見え方も、実際には高さの計算がズレていたのではなく、Outlookが他ウィンドウに隠れて下端が見えなかったことによる誤認だったと考えられる(仮想/物理座標換算で比較すると、両ウィンドウの外枠高さの差は数px程度に収まっている)。
	`SWP_NOZORDER`フラグを取り除き(重なり順を実際に変更する)、あわせて`SetForegroundWindow`も呼んで前面化を確実にした。

### 変更関数
	`OutlookMailManager._set_window_bounds_win32`（`SetWindowPos`から`SWP_NOZORDER`フラグを削除。`SetForegroundWindow`の呼び出しを追加）
...
変更ファイル：
	`outlook_total_organizer_20260820_03.py`（`_20260820_02`からのコピー＋今回の変更。`_20260820_02`はそのまま残置）

変更しないこと（宣誓）：
	`_find_explorer_hwnd`（HWND特定ロジック）、`_get_window_state_win32`（状態取得ロジック）、`arrange_outlook_window`の全体フロー（HWND特定→配置→再チェック）、`get_physical_work_area`等の座標計算ロジックには一切触れていない。今回の修正はウィンドウの重なり順(Z順序)のフラグ指定ミス1点のみ。

動作確認時の注意：
	本ツールはWindows専用（ctypes.windll依存）のため、本セッションの実行環境では実際のウィンドウの重なり順は検証できていない。`python3 -m py_compile`による構文チェック、`_20260820_02`との`diff`で変更範囲が`_set_window_bounds_win32`内の数行のみであることを確認済み。
	Outlook非依存のスタンドアロン`python3`ハーネスで、`SetWindowPos`へ実際に渡される`flags`パラメータに`SWP_NOZORDER`(0x0004)のビットが立っていないこと(重なり順が実際に変更される状態になっていること)、`hWndInsertAfter`に正しく`HWND_TOP`(0)が渡されていること、`SetForegroundWindow`も呼ばれることを検証し合格した。`_20260820_02`時点の既存回帰テスト(Win32直接操作の全体フロー、物理/仮想座標の計算)も再実行し、合格を維持していることを確認した。
	**実機での確認をお願いしたい**。今回は位置・サイズの計算ではなく重なり順のみの修正のため、これまでの座標計算(物理ピクセル単位のOutlook配置)が正しく機能していれば、Outlookが他ウィンドウに隠れず、本ツールと並んで表示されることを期待している。可能であれば、起動前に手前にブラウザ等のウィンドウがある状態で試していただき、Outlookがその手前に出てくることを確認いただきたい。

## VERSION 20260820_02

### 追加・修正
	**Outlook COMの`Explorer.WindowState`等のプロパティ経由でのウィンドウ操作を全面的に廃止し、Win32 APIでの直接操作に切り替えた**。`_20260820_01`(DPI座標系のズレを修正)を実機投入いただいたところ、Outlookへ渡す座標(幅・高さ)自体は正しくなった(物理ピクセル単位の作業領域を正しく取得できていた)ものの、症状(繰り返し最大化される)は再発した。ログを精査すると、`_apply_explorer_geometry`完了直後は`WindowState=2`(最小化)、1秒後の再チェックでも`WindowState=2`のまま、さらに1.2秒後には`WindowState=0`だが`Left=-11 Top=-11`という最大化特有の生の座標に変化しており、**こちらが一切指示していないにもかかわらず、Outlook内部でWindowStateが数秒おきに0→2(最小化)→0(実際には最大化)と勝手に遷移し続けている**ことが分かった。座標系(仮想/物理)の問題を修正した後もこの現象が変わらず起きたことから、原因はCOM座標の計算ミスではなく、**`Explorer`オブジェクトのCOMプロパティによる状態管理そのものが、この環境で不安定である**ことだと判断した。
	対策として、Outlook COMの`WindowState`/`Left`/`Top`/`Width`/`Height`プロパティを一切使わない設計に変更した。`Explorer`オブジェクトは、Outlookの起動・既存ウィンドウの検出・HWND特定用の`Caption`取得にのみ使用し、実際の位置・サイズの設定と状態確認は、`FindWindowW`でHWNDを特定したうえで**Win32 API(`ShowWindow`/`SetWindowPos`/`GetWindowRect`/`IsZoomed`/`IsIconic`)を直接呼び出す**方式に統一した。これは人間がマウスでウィンドウを操作するのと同じOSレベルの操作であり、Outlook COM層の(不安定であることが判明した)内部状態管理を完全にバイパスする。

### 変更関数
	`OutlookMailManager.arrange_outlook_window`（COMプロパティのリトライ・診断ログをすべて廃止し、HWND特定→Win32 APIでの配置→Win32 APIでの状態監視・再設定、という設計に全面書き換え）

### 新規追加：
	メソッド `OutlookMailManager._find_explorer_hwnd(self, explorer) -> int`（`_restore_if_actually_maximized`のHWND特定ロジックを独立させたもの）
	メソッド `OutlookMailManager._set_window_bounds_win32(self, hwnd, left, top, width, height) -> bool`（`ShowWindow(SW_RESTORE)`→`SetWindowPos`でウィンドウを直接配置する）
	メソッド `OutlookMailManager._get_window_state_win32(self, hwnd)`（`GetWindowRect`/`IsZoomed`/`IsIconic`から位置・サイズと異常状態(最大化/最小化)を取得する）

### 削除：
	メソッド `OutlookMailManager._apply_explorer_geometry`（COMプロパティのリトライ実装。Win32直接操作に置き換えたため不要）
	メソッド `OutlookMailManager._restore_if_actually_maximized`（`_find_explorer_hwnd`+`_set_window_bounds_win32`に統合・置き換え）
	`arrange_outlook_window`内のDisplay前のCOMプロパティ設定処理(`explorer.WindowState = 0`等)、および関連する詳細診断ログ(Display前後の状態出力等)。Win32での配置に一本化したため、COM層の状態を細かく追跡する必要が無くなった。

変更ファイル：
	`outlook_total_organizer_20260820_02.py`（`_20260820_01`からのコピー＋今回の変更。`_20260820_01`はそのまま残置）

変更しないこと（宣誓）：
	`get_primary_work_area`/`get_physical_work_area`（DPI座標系の計算。`_20260820_01`で修正済みで、今回のOutlook側の症状とは別の問題だったため変更不要と判断）、`fit_window_to_work_area`（本ツール自身のタスクバー回避）、`WINDOW_SPLIT_GAP`/`WINDOW_HEIGHT_MARGIN`の値には触れていない。`show_thread_in_explorer`（件名クリックでのOutlook検索遷移）にも触れていない。

動作確認時の注意：
	本ツールはWindows専用（win32com/ctypes.windll依存）のため、本セッションの実行環境では実際のWin32 API呼び出しは検証できていない。`python3 -m py_compile`による構文チェック、`_20260820_01`との`diff`で変更範囲を確認済み。
	Outlook非依存のスタンドアロン`python3`ハーネスで、(1)`_apply_explorer_geometry`/`_restore_if_actually_maximized`が完全に削除され、新設した3メソッドに置き換わっていること、(2)`_set_window_bounds_win32`が`ShowWindow(SW_RESTORE)`→`SetWindowPos`の順で正しいパラメータで呼ばれること、(3)`_get_window_state_win32`が`GetWindowRect`/`IsZoomed`/`IsIconic`から正しく状態を組み立てること、(4)**実機ログの「勝手に最小化⇔最大化を繰り返す」状態を再現した最悪ケース**でも、`arrange_outlook_window`が毎回正しい目標位置・サイズで`SetWindowPos`を呼び続け、最終的に安定した状態に収束すること、を検証し全て合格した。`_20260820_01`時点の座標計算(仮想/物理work area)・`show_thread_in_explorer`関連の既存回帰テストも再実行し、合格を維持していることを確認した。
	**実機での確認を強くお願いしたい**。今回はCOM層を経由しない、より低レベルで直接的なアプローチのため、これまでの一連の不具合(最大化される・手動解除しても戻される)が解消されることを期待している。診断ログは引き続き出力される(内容はWin32ベースの値に変更)ので、ログを共有いただければ確実に判断できる。万一今回も同様の症状が再発する場合、Outlook COM/Win32レベルの問題ではなく、Outlookアドインや企業ポリシー等、さらに別の要因を疑う必要がある。

## VERSION 20260820_01

### 追加・修正
	**「何度直しても最大化に戻される」問題の、おそらく最終的な根本原因を特定した**。`_20260819_01`の高さマージン(8px)対策後も同じ症状(`Left=-11 Top=-11 Width=3862 Height=2110`という、実際に最大化されたウィンドウの生の座標)が再発するとのご報告を受け、ユーザーにモニター構成を確認したところ、プライマリモニターが**3840×2160・拡大率150%**であることが判明した。
	`3840 ÷ 1.5 = 2560`、`2160 ÷ 1.5 = 1440`。これは、これまでのログで一貫して観測されていた作業領域`(0,0)-(2560,1392)`の数値と正確に一致する。
	原因: `_05`で、文字が小さく詰まって見える不具合を直すために本ツール(Tkinter)のDPI Awareness宣言を撤去した。その結果、本ツールは「DPI非対応」の古いアプリとしてWindowsに扱われ、`winfo_screenwidth()`や`SystemParametersInfoW`が返す座標は、**拡大率で割った仮想座標**になる(3840×2160の物理モニターが2560×1392として見える)。一方**Outlook自体はDPI Aware(物理ピクセル座標)で動作するアプリ**であり、COM経由で`Explorer.Left/Top/Width/Height`に仮想座標の値をそのまま渡すと、Outlook側はこれを物理ピクセル座標として解釈してしまう。結果として、意図した「画面右半分」よりずっと小さい範囲(物理的には画面の2/3程度)にウィンドウを収めようとすることになり、この座標系のズレが、Windowsの暗黙の最大化トリガー(`_20260819_01`で対応した、高さが作業領域ぴったりだと最大化とみなされる挙動)と組み合わさって、Outlookの位置・サイズ設定と実際の最大化復元が延々と噛み合わない状態(`_restore_if_actually_maximized`でShowWindow(SW_RESTORE)しても、次の瞬間ズレた座標で再設定されまた最大化と誤認される、という循環)を引き起こしていたと考えられる。
	対策として`get_physical_work_area`を新設した。`SetThreadDpiAwarenessContext`でこの関数の呼び出し中だけスレッド単位でDPI Awareへ切り替えたうえで作業領域を取得することで、物理ピクセル単位の正しい値を得る。呼び出し後は元のコンテキストへ戻すため、本ツール自身(Tkinter)の描画には影響しない(プロセス全体のDPI Awareness宣言を復活させるわけではないので、`_05`で修正した文字が小さくなる不具合は再発しない)。本ツール自身の配置には従来どおり仮想座標(`wa_*`)を使い、Outlookの配置座標だけを物理座標(`p_*`)から計算するよう分離した。拡大率・解像度をハードコードしていないため、異なる拡大率や解像度の環境でも実行時に正しい値を取得できる設計とした。

### 変更関数
	`MailManagerGUI.__init__`（Outlookへ渡す座標の計算を、本ツール自身と共通の仮想座標(`work_area`)から、`get_physical_work_area()`による物理座標(`phys_work_area`)ベースの独立した計算に変更。本ツール自身の配置ロジックは変更なし）

### 新規追加：
	関数 `get_physical_work_area()`（`SetThreadDpiAwarenessContext`でスレッド単位にDPI Awareへ切り替えたうえで物理ピクセル単位の作業領域を取得する。取得できない場合はNoneを返す）

### 削除：
	なし

変更ファイル：
	`outlook_total_organizer_20260820_01.py`（`_20260819_01`からのコピー＋今回の変更。`_20260819_01`はそのまま残置）

変更しないこと（宣誓）：
	`_restore_if_actually_maximized`（OS側の実状態確認・復元処理）、`_apply_explorer_geometry`のリトライロジック、`fit_window_to_work_area`（本ツール自身のタイトルバー補正）、`WINDOW_SPLIT_GAP`/`WINDOW_HEIGHT_MARGIN`の値自体には触れていない。本ツール自身(Tkinter)の座標計算・配置ロジックにも触れていない(仮想座標のまま、`_05`で確立した「拡大率100%相当の座標系で正しく動く」状態を維持)。

動作確認時の注意：
	本ツールはWindows専用（win32com/ctypes.windll依存）のため、本セッションの実行環境では実際の`SetThreadDpiAwarenessContext`呼び出しやマルチモニター・拡大率環境は検証できていない。`python3 -m py_compile`による構文チェック、`_20260819_01`との`diff`で変更範囲が今回の追加分のみであることを確認済み。
	Outlook非依存のスタンドアロン`python3`ハーネスで、**ユーザーの実際の環境(3840×2160・拡大率150%)を再現**するフェイク(DPI非対応時は仮想座標2560×1392相当、DPI Aware化後は物理座標3840×2088相当を返す)を用意し、(1)`get_physical_work_area()`が正しく物理座標を返すこと、(2)呼び出し後にDPI Awarenessコンテキストが元に戻ること(本ツール自身の描画への影響が無いこと)、(3)`SetThreadDpiAwarenessContext`が失敗する環境(古いWindows等)や`ctypes.windll`自体が無い環境でも例外を投げずNoneへフォールバックすること、(4)`__init__`のソース上で本ツールには仮想座標、Outlookには物理座標が使われていることを検証し、全て合格した。`_20260819_01`時点の既存回帰テスト(OS側実状態の確認・復元、`_set_retry`の例外変数バグ修正)も再実行し、合格を維持していることを確認した。
	**実機での確認を強くお願いしたい**。今回はユーザーからご提供いただいたモニター構成の情報に基づく、定量的に裏付けのある修正(2560×1392と3840×2160÷1.5が完全に一致することを確認済み)のため、これまでで最も確度が高いと考えている。期待としては、診断ログの`Outlook=x:... w:... h:...`の`w`(幅)がこれまでのおよそ1.5倍の値になり(拡大率150%の逆数分)、再チェックのたびに`Left=-11...`のような異常値や最大化の再発が**起きなくなる**ことを想定している。診断ログは引き続き出力されるので、ログを共有いただければ確実に判断できる。

## VERSION 20260819_01

### 追加・修正
	**「何度手動で解除しても最大化に戻される」という、繰り返し発生していた根本原因を特定し修正**。`_20260818_05`で追加したOS側の実状態確認(`_restore_if_actually_maximized`)は正しく機能し、`ShowWindow(SW_RESTORE)`・Left/Top/Width/Heightの設定は一度成功したが、その直後の再チェックで**再び**最大化状態(`Left=-11 Top=-11 Width=3862 Height=2110`という、実際に最大化されたウィンドウの見えないリサイズ枠を含む生の座標の特徴的な値)に戻っており、以降のチェックのたびに何度SW_RESTOREで復元しても、次のチェックでまた同じ異常値に戻る、という循環が続いていた。
	原因は**このツール自身が指定していた目標の高さ**にあった。目標の高さは作業領域(タスクバーを除いた画面の高さ)ぴったりの値(例: 1392px)を指定していたが、**Windowsは、ウィンドウの高さを画面の作業領域の高さにぴったり一致させるリサイズを、単なるリサイズ操作であっても暗黙的に「垂直方向の最大化」とみなして自動的にOSレベルの最大化状態にしてしまう**(ユーザーがWin+Shift+↑で行う操作と同じ内部処理が、プログラムによるリサイズでも作動してしまう)。そのため、私たちが「正しい位置・サイズ」として設定した値そのものが、設定した瞬間に再び最大化を引き起こしており、`_restore_if_actually_maximized`で復元しても、次の再チェックで同じ目標サイズを再設定すればまた最大化に戻ってしまう、という無限ループになっていた。「手動で何度解除してもまた最大化される」というご報告は、実際にはOutlook自身の挙動ではなく、**本ツールが繰り返し同じ「暗黙の最大化トリガーになるサイズ」を設定し続けていたことによる自業自得**だったことになる。
	対策として、目標の高さを作業領域の高さぴったりではなく、`WINDOW_HEIGHT_MARGIN`(8px)だけ小さくした。これにより、ウィンドウの下端に8px程度の余白ができる代わりに、Windowsの暗黙の最大化トリガーを回避する。本ツール自身のウィンドウ・Outlookの両方に同じ高さを使うよう統一した(本ツール側でも同じ現象が起きうるため、Outlookだけでなく両方に適用)。

### 変更関数
	`MailManagerGUI.__init__`（高さの計算を`wa_height`直接指定から`target_height = wa_height - WINDOW_HEIGHT_MARGIN`に変更。本ツール(`fit_window_to_work_area`)・Outlook(スレッド引数)の両方に適用）

### 新規追加：
	クラス属性 `MailManagerGUI.WINDOW_HEIGHT_MARGIN`（作業領域の高さから差し引く余白。既定8px）

### 削除：
	なし

変更ファイル：
	`outlook_total_organizer_20260819_01.py`（`_20260818_05`からのコピー＋今回の変更。`_20260818_05`はそのまま残置。日付が変わったためバージョン番号は`20260819_01`から再開）

変更しないこと（宣誓）：
	`_restore_if_actually_maximized`（OS側の実状態確認・復元処理自体）、診断ログの内容・出力箇所には触れていない。これらは正しく機能していたため変更不要と判断した。`WINDOW_SPLIT_GAP`（横方向の隙間）のロジックにも触れていない。

動作確認時の注意：
	本ツールはWindows専用のため、本セッションの実行環境では実際のWindowsの暗黙の最大化挙動は検証できていない。`python3 -m py_compile`による構文チェック、`_20260818_05`との`diff`で変更範囲が今回の追加分のみであることを確認済み。
	Outlook非依存のスタンドアロン`python3`ハーネスで、`__init__`のソースから高さの計算式を抽出し、(1)`WINDOW_HEIGHT_MARGIN`(8px)が定義されていること、(2)`target_height`の計算式が存在し、本ツール・Outlookの両方に(作業領域の高さそのものではなく)`target_height`が渡されていること、(3)実機ログの値(1392)で計算すると実際に余白が生まれる(1384になる)ことを検証し合格した。`_20260818_05`時点の既存回帰テスト(OS側実状態の確認・復元、`_set_retry`の例外変数バグ修正)も再実行し、合格を維持していることを確認した。
	**実機での確認をお願いしたい**。今回は実機ログで確認された明確な因果関係(高さが作業領域ぴったり→暗黙の最大化トリガー)に基づく修正のため、期待としては、再チェックのたびに`Left=-11 Top=-11...`のような異常値や「実OS側で最大化を検出」のログが**繰り返し出なくなり**、一度`_apply_explorer_geometry`が成功したら、そのまま安定して右半分に収まり続けることを想定している。診断ログは引き続き出力されるので、今回も可能であればログを共有いただき、想定通りかご確認いただきたい。

## VERSION 20260818_05

### 追加・修正
	**「Outlookの最大化が直らない」根本原因が、実機ログにより確定した**。`_20260818_04`の診断ログをご提供いただき、決定的な手がかりを得た。
	```
	既存のActiveExplorerを検出: WindowState=0 Left=2030 Top=59 ...
	Left=1286 設定失敗(タイムアウト, 試行34回, 経過10.2秒): 最後の例外=(... '最大化または最小化されているため、
	エクスプローラーまたはインスペクターのサイズまたは位置を変更できません。' ...)
	```
	**`Explorer.WindowState`はCOM上「0(通常)」を返し続けているにもかかわらず、`Left`の設定は34回リトライしても一度も成功しなかった**。これは、Outlookの`WindowState`プロパティ(COMレベルの状態管理)と、実際のOSレベルのウィンドウ状態が食い違っていることを意味する。おそらくWindowsのスナップ機能等、Outlook自身のUI操作を経ない経路でウィンドウが最大化されると、COM側の`WindowState`プロパティがこれに追従しない。これまでの一連の修正(`_04`のリトライ、`_20260818_02`のDisplay前設定・再チェック短縮)は、すべてCOMの`WindowState`プロパティだけを見て「直った」と判定していたため、この食い違いを一度も検出・対処できていなかった。
	対策として、`_restore_if_actually_maximized`を新設し、**Win32 API(`FindWindowW`/`GetForegroundWindow`/`IsZoomed`/`ShowWindow`)で実際のOSウィンドウ(HWND)を直接特定し、本当に最大化されている場合は`ShowWindow(SW_RESTORE)`で直接復元する**処理を追加した。HWNDの特定は、まずExplorerの`Caption`(タイトルバー文言)とOutlookの既知のウィンドウクラス名`rctrl_renwnd32`で`FindWindowW`により一致するウィンドウを探し、失敗した場合は`Explorer.Activate()`で前面化した直後に`GetForegroundWindow()`で取得する、の2段構えとした。この処理を、(1)`_apply_explorer_geometry`でLeft等を試みる直前、(2)起動後の再チェックループでドリフトを検知し再設定する直前、の2箇所で呼び出すようにした。

### 変更関数
	`OutlookMailManager._apply_explorer_geometry`（Left/Top/Width/Heightを試みる直前に`_restore_if_actually_maximized`を呼ぶよう追加）
	`OutlookMailManager.arrange_outlook_window`（起動後の再チェックループで、ドリフト検知時の再設定直前に`_restore_if_actually_maximized`を呼ぶよう追加）

### 新規追加：
	メソッド `OutlookMailManager._restore_if_actually_maximized(self, explorer) -> bool`（Win32 APIで実OS側のウィンドウ状態を直接確認し、本当に最大化されていれば`ShowWindow(SW_RESTORE)`で復元する）

### 削除：
	なし

変更ファイル：
	`outlook_total_organizer_20260818_05.py`（`_20260818_04`からのコピー＋今回の変更。`_20260818_04`はそのまま残置）

変更しないこと（宣誓）：
	`_20260818_03`/`_04`で追加した診断ログ(内容・出力箇所・15秒間の再チェック監視)には触れていない。`fit_window_to_work_area`（タスクバー回避）、`get_primary_work_area`/`WINDOW_SPLIT_GAP`（左右分割・隙間）のロジックにも触れていない。

動作確認時の注意：
	本ツールはWindows専用（win32com/ctypes.windll依存）のため、本セッションの実行環境では実際のWin32 API呼び出し(`FindWindowW`/`IsZoomed`/`ShowWindow`等)は検証できていない。`python3 -m py_compile`による構文チェック、`_20260818_04`との`diff`で変更範囲が今回の追加分のみであることを確認済み。
	Outlook非依存のスタンドアロン`python3`ハーネスで、`ctypes.windll`を動的にフェイクへ差し替え、**実機ログと同一の状況(COM上のWindowStateは0を返し続けるが、実OS側が最大化されておりLeft等の設定が常に例外になる)を再現**し、(1)`_restore_if_actually_maximized`が実OS側の最大化を検出して`ShowWindow(SW_RESTORE)`を呼び出すこと、(2)その結果、`_apply_explorer_geometry`全体を通してLeft/Top/Width/Heightが最終的に正しく設定されること、(3)HWNDを特定できない場合や`ctypes.windll`自体にアクセスできない場合でも例外を投げずFalseにフォールバックすること、を検証し全て合格した。`_20260818_04`時点の既存回帰テスト(診断ログの出力・Display前設定の確認・`_set_retry`の例外変数バグ修正)も再実行し、合格を維持していることを確認した。
	**実機での確認をお願いしたい**。今回は仮説ではなく実機ログに基づく修正のため、期待としては(1)`🔧[Outlook配置] 実OS側で最大化を検出...ShowWindow(SW_RESTORE)で復元します`というログが出て、(2)その後`Left=... 設定成功`に変わり、(3)Outlookが実際に画面右半分の通常ウィンドウとして表示される、という結果を想定している。引き続き診断ログは出力されるので、今回もログ全文を共有いただけると、想定通りだったか・別の要因が残っているかを確実に判断できる。

## VERSION 20260818_04（診断用ビルド・バグ修正）

### 追加・修正
	**`_20260818_03`の診断ログ自体に含まれていたバグを修正**。実機で取得いただいたログにより、`Outlookウィンドウ配置エラー: cannot access local variable 'e' where it is not associated with a value` という、Outlook自体とは無関係な**このツール側のバグ**が判明した。
	原因は`_apply_explorer_geometry`内の`_set_retry`ヘルパー。`except Exception as e:`でリトライループ内の例外を捕まえていたが、Python 3は`except ... as e:`のブロックを**抜けた時点で変数`e`を自動的に破棄する**(参照サイクル防止のための言語仕様)。ループを抜けた後(ブロックの外)で`e`を参照して失敗理由をログ出力しようとしていたため、この参照自体が`UnboundLocalError`を引き起こし、**`Left`(横位置)の設定を試みている最中に処理全体が異常終了していた**。
	この結果、いただいたログの回では、Outlookは既に起動済み・既に通常ウィンドウ状態(`WindowState=0`)だったにもかかわらず、目的の位置(画面右半分)への再配置が一度も完了せず、元の位置のまま(前面に出す`Activate()`も未実行)で終わっていたことが分かった。ユーザーからも「今回は最大化もされず、既存のOutlookが前面に押し上げられることもなかった」との報告があり、この診断と一致する。
	修正は、例外メッセージを`except`ブロックの中で文字列として`last_error`変数に退避しておき、ブロックを抜けた後はその文字列を参照するように変更した。診断目的のログ出力自体(内容・頻度)は変更していない。

### 変更関数
	`OutlookMailManager._apply_explorer_geometry`（`_set_retry`内で、例外オブジェクト`e`を`except`ブロックの外で参照していた箇所を、ブロック内で文字列化した`last_error`変数の参照に修正）
...
変更ファイル：
	`outlook_total_organizer_20260818_04.py`（`_20260818_03`からのコピー＋今回の変更。`_20260818_03`はそのまま残置）

変更しないこと（宣誓）：
	`_20260818_03`で追加した診断ログの内容・出力箇所・再チェック監視時間(15秒)には触れていない。今回のバグ修正により、これまで実行されていなかった`Left`/`Top`/`Width`/`Height`の設定・`Activate()`・起動後の再チェックループが、次回のテストでは実際に実行されるようになるはずである。

動作確認時の注意：
	本ツールはWindows専用のため、本セッションの実行環境では実際のログ内容は確認できていない。`python3 -m py_compile`による構文チェック、`_20260818_03`との`diff`で変更範囲が`_set_retry`内の3行のみであることを確認済み。
	Outlook非依存のスタンドアロン`python3`ハーネスで、**実機で発生した状況(あるプロパティの設定が常に失敗し続ける)を再現**し、(1)`UnboundLocalError`が再発せず`_apply_explorer_geometry`が最後まで完走すること、(2)実機で発生した「cannot access local variable」というエラーメッセージが再発しないこと、を検証し合格した。`_20260818_03`時点の既存回帰テスト(Display前設定の確認等)も再実行し、合格を維持していることを確認した。
	**実機で再度ログの取得をお願いしたい**。手順は`_20260818_03`のときと同様(`python outlook_total_organizer_20260818_04.py > outlook_log.txt 2>&1`でファイルへ記録するか、コンソールで直接確認)。今回は`Left`/`Top`/`Width`/`Height`の設定・`Activate()`・再チェックループが実際に実行されるログが出るはずなので、そのうえでOutlookの最大化・位置の挙動がどうなるかをご確認いただきたい。

## VERSION 20260818_03（診断用ビルド）

### 追加・修正
	**「Outlookの最大化が直らない、数回手動解除してもまた最大化される」という報告を受け、これ以上の憶測ベースの修正を避け、実際の挙動を正確に記録する診断ログを追加した**。`_04`(WindowStateタイミング対応)・`_20260818_02`(Display前設定・再チェック短縮)と複数回対応してきたが、いずれも実機で解消しなかった。原因の当たりを外し続けている以上、次に必要なのは新しい仮説ではなく、実際に何が起きているかの生ログである。
	`arrange_outlook_window`と`_apply_explorer_geometry`の各段階(Explorer取得直後・Display前設定・Display直後・`_apply_explorer_geometry`の各プロパティ設定試行・完了時点の実測値・起動後の再チェック各回)すべてに`🔧[Outlook配置]`という接頭辞付きのログを追加した。あわせて、これまで3秒間だった起動後の再チェック監視を、診断のため一時的に15秒間・毎秒ログ出力に延長した(3秒間では捉えきれていない、より遅いタイミングでの再最大化を観測するため)。
	**本バージョンは診断目的のビルドであり、恒久版ではない**。ログで原因が特定でき次第、根本原因に対応した修正版を発行し、ログ出力・15秒間の延長監視は削除する。

### 変更関数
	`OutlookMailManager._apply_explorer_geometry`（各プロパティ設定の試行・成功・失敗、WindowState読み取り確認、完了時点の実測値をログ出力するよう変更。ロジック自体(リトライ条件・タイムアウト)は変更なし）
	`OutlookMailManager.arrange_outlook_window`（Explorer取得直後・Display前設定・Display直後・起動後の再チェック各回の状態をログ出力するよう変更。再チェックの監視時間を診断のため3秒→15秒に延長。ロジック自体(ドリフト検知・再設定条件)は変更なし）
...
変更ファイル：
	`outlook_total_organizer_20260818_03.py`（`_20260818_02`からのコピー＋今回の変更。`_20260818_02`はそのまま残置）

変更しないこと（宣誓）：
	`fit_window_to_work_area`（タスクバー回避）、`get_primary_work_area`/`WINDOW_SPLIT_GAP`（左右分割・隙間）、Display前の設定を試みること自体には触れていない。ログ出力の追加と、再チェック監視時間の延長(診断目的、一時的)のみ。

動作確認時の注意：
	本ツールはWindows専用のため、本セッションの実行環境では実際のログ内容は確認できていない。`python3 -m py_compile`による構文チェック、`_20260818_02`との`diff`で変更範囲がログ追加と監視時間延長のみであることを確認済み。Outlook非依存のスタンドアロン`python3`ハーネスで、ログ出力の書式自体(各段階で意図した内容が出力されること)を確認した。
	**実機でのご確認方法**: (1)本ツールをコンソール（ターミナル・コマンドプロンプト）から`python`で起動し、ウィンドウを閉じずに操作を続ける。(2)Outlookが起動〜最大化〜(手動で解除〜再度最大化、を含め)何が起きるかそのまま観察する。(3)起動から**1〜2分程度経過するまで**コンソールを閉じずに、`🔧[Outlook配置]`で始まる行を**すべて**コピーしていただきたい。特に、(a)「Display前のWindowState/位置設定」が成功しているか例外か、(b)「Display()直後の状態」でWindowStateが0になっているか1(最大化)のままか、(c)再チェックの各回で`drifted=True`になっている回があるか、その`WindowState`実測値が何になっているか、(d)15秒の監視終了後(「完了」ログの後)に再度最大化される場合、その時刻とご自身の操作(手動で解除した、等)のタイミングの対応関係、をお知らせいただきたい。このログ一式があれば、次こそ確定的な原因究明・修正に進められる。
