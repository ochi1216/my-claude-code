...
class MailManagerGUI:
    ...
    def __init__(self):
        ...
        # 高さを作業領域ぴったりにすると暗黙の最大化を誘発するため、
        # 両ウィンドウともわずかに低くする(WINDOW_HEIGHT_MARGIN参照)。
        ...
        print(f"🖥 画面配置: work_area(取得={'成功' if work_area else '失敗→画面全体を使用'})="
              ...
              f"Outlook=x:{outlook_left} w:{outlook_width} h:{target_height}")
        ...
        threading.Thread(
            ...
            args=(outlook_left, wa_top, outlook_width, target_height),
            daemon=True,
        ).start()
