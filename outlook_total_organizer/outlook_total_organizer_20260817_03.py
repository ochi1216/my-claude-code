...
def set_process_dpi_aware():
    """本プロセスをDPI Awareにする(ディスプレイの拡大率設定の影響を受けず、
    実ピクセル座標で画面サイズ・ウィンドウ位置を扱えるようにする)。
    Outlook自身はDPI Aware(実ピクセル座標)で動作するため、Python側がDPI
    Awareでないまま拡大率100%以外の環境で動かすと、winfo_screenwidth()等が
    返す値とOutlookが解釈する座標がずれ、本ツールとOutlookの左右分割配置に
    重なり/隙間のずれが生じる。ウィンドウ生成(tk.Tk())より前に呼ぶ必要がある。
    失敗しても致命的ではないため例外は握りつぶす(その場合は拡大率100%相当の
    座標系で動作し、100%環境なら特に問題は出ない)。"""
    try:
...
class OutlookMailManager:
    ...
    def arrange_outlook_window(self, left: int, top: int, width: int, height: int,
        ...
        try:
            ...
            # 最大化/最小化中はLeft/Top/Width/Heightの設定が反映されないため、
            # 先に通常ウィンドウへ戻してから位置とサイズを設定する。
            explorer.WindowState = 0  # olNormalWindow
            explorer.Left = left
            explorer.Top = top
            ...
            # ウィンドウ状態(最大化等)を自分で復元することがある。これにより
            # 手動で最大化解除が必要になる不具合が実機で確認されたため、
            # 少し待って状態がずれていないか確認し、ずれていれば再設定する。
            for _ in range(4):
                time.sleep(0.5)
                try:
                    drifted = (
                        explorer.WindowState != 0
                        or explorer.Left != left
                        or explorer.Top != top
                        or explorer.Width != width
                        or explorer.Height != height
                    )
                except Exception:
                    break
                if not drifted:
...
class MailManagerGUI:
    ...
    def __init__(self):
        # tk.Tk()より前に呼ぶ必要がある。DPI拡大率が100%以外の環境で、Python側と
        # Outlook側の座標解釈がずれて分割位置がずれる(重なり/隙間)のを防ぐ。
        ...
        self.root.geometry(f"{left_w}x{wa_height}+{wa_left}+{wa_top}")
