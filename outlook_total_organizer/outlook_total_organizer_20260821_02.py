...
class ThreadDpiAware:
    """with文の中だけ、呼び出し元スレッドをDPI Aware(物理ピクセル座標)にする。

    本ツールのプロセスはDPI非対応のままにしてある(_05参照。プロセス全体を
    DPI Awareにすると、Tkinterが拡大率に応じた自動拡大表示の対象外になり、
    文字が小さく詰まって見える不具合が出るため)。

    その副作用として、**このプロセスから呼ぶWin32 APIはすべて「仮想座標」で
    やり取りされる**。つまりSetWindowPosへ渡した座標はWindowsによって拡大率
    倍されて実際のウィンドウに適用され、GetWindowRectは実際の座標を拡大率で
    割った値を返す。実機ログで「SetWindowPosで1926を指定→GetWindowRectも
    1926を返す→ログ上は完全に整合しているのに、画面上のOutlookは物理2889px
    (=1926×1.5)の位置にある」という、不具合が自分自身を隠してしまう状態に
    なっていた(この自己隠蔽が、本件の原因究明が長引いた最大の理由)。

    ウィンドウの配置は物理ピクセルで計算しているので、実際にWin32 APIを叩く
    区間だけこのクラスでスレッドをDPI Awareに切り替え、座標系を一致させる。
    Tkinterを動かすメインスレッドとは別のワーカースレッドで使う限り、GUIの
    描画には影響しない。切り替えに失敗した場合(Windows 10 1607より前など)は
    何もしない(従来どおり仮想座標として扱われる)。"""
    ...
    def __enter__(self):
        try:
            ...
            self._prev = prev if prev else None
            if self._prev is None:
                print(f"🔧[Outlook配置] スレッドのDPI Aware化に失敗しました{self._label}"
                      "(Windows 10 1607より前の可能性)。仮想座標のまま処理を続行します。")
        except Exception as e:
            self._prev = None
            print(f"🔧[Outlook配置] スレッドのDPI Aware化でエラー{self._label}: {e}。"
                  "仮想座標のまま処理を続行します。")
        return self

    @property
    def active(self) -> bool:
        """実際にDPI Awareへ切り替えられたか(=物理ピクセル座標で扱えるか)。"""
...
def get_physical_work_area():
    """Outlookの配置先座標を計算するための、物理ピクセル単位のプライマリモニター
    作業領域を返す。

    本ツール(Tkinter)は_05でプロセス全体のDPI Awareness宣言を撤去している
    (宣言するとTkinterがディスプレイの拡大率に応じた自動拡大表示の対象外になり、
    文字が小さく詰まって見える不具合があったため)。そのため本ツール自身は
    「DPI非対応」のレガシーアプリとしてWindowsに扱われ、get_primary_work_area()
    等が返す座標は拡大率で割った仮想座標になる(例: 拡大率150%の3840x2160
    モニターは2560x1392として見える)。

    一方Outlook自体はDPI Aware(物理ピクセル座標)で動作するアプリのため、
    COM経由でExplorer.Left/Top/Width/Heightに仮想座標の値をそのまま渡すと、
    Outlook側は物理ピクセル座標として解釈してしまい、意図した位置・サイズの
    1/拡大率倍の範囲にしかウィンドウが収まらない(実機ログで、Outlookへ
    設定しようとした値と無関係に、モニター全体に近い異常なLeft/Top/Width/Height
    (最大化時特有の生の座標)へ勝手に変化し続ける現象として現れていた)。

    SetThreadDpiAwarenessContextで、この関数の実行中だけスレッド単位で
    DPI Awareへ切り替えたうえでget_primary_work_area()を呼び直すことで、
    物理ピクセル単位の作業領域を取得する。呼び出し後は元のコンテキストへ
    戻すため、プロセス全体・Tkinter側の描画には影響しない。
    Windows 10 1607より前など、この機能が無い環境では失敗しNoneを返す
    (呼び出し元は仮想座標をそのまま使うフォールバックとなる)。"""
    try:
        with ThreadDpiAware("(作業領域の取得)") as dpi:
            if not dpi.active:
                return None
            phys = get_primary_work_area()
            print(f"🔧[Outlook配置] 物理ピクセル単位の作業領域: {phys}")
            return phys
    except Exception as e:
        print(f"🔧[Outlook配置] 物理ピクセル単位の作業領域取得でエラー: {e}。仮想座標にフォールバックします。")
        return None
...
class OutlookMailManager:
    ...
    def _find_explorer_hwnd(self, explorer) -> int:
        """ExplorerオブジェクトからOSレベルのウィンドウハンドル(HWND)を特定する。
        まずCaption(タイトルバー文言)とOutlookの既知のウィンドウクラス名
        rctrl_renwnd32でFindWindowWにより一致するウィンドウを探し、失敗した
        場合はExplorer.Activate()で前面化した直後にGetForegroundWindow()で
        取得する、の2段構え。取得できない場合は0を返す(例外は投げない)。"""
        try:
            user32 = ctypes.windll.user32
        ...
        try:
            ...
            caption = None
        print(f"🔧[Outlook配置] Explorer.Caption(COM側で取得した値): {caption!r}")
        found_by = None
        try:
            if caption:
                hwnd = user32.FindWindowW("rctrl_renwnd32", caption)
                if hwnd:
                    found_by = "FindWindowW(クラス名+Caption一致)"
        except Exception:
            hwnd = 0
        if not hwnd:
            try:
                ...
                if hwnd:
                    found_by = "GetForegroundWindow(Activate直後のフォアグラウンド)"
            except Exception:
                hwnd = 0
        if hwnd:
            # 操作対象が本当に画面上のOutlookウィンドウと一致しているかを直接
            # 確認できるよう、実際のウィンドウタイトル・クラス名をログに残す。
            try:
            ...
            try:
                ...
                class_name = f"(取得失敗: {e})"
            print(f"🔧[Outlook配置] HWND特定方法={found_by} / 実際のウィンドウタイトル={title!r} / "
                  f"クラス名={class_name!r}")
        return hwnd

    def _set_window_bounds_win32(self, hwnd, left: int, top: int, width: int, height: int) -> bool:
        """Win32 API(ShowWindow/SetWindowPos)でウィンドウの位置・サイズを直接設定する。
        実機ログで、Outlook COMの`Explorer.WindowState`/`.Left`/`.Top`/`.Width`/
        `.Height`プロパティ経由の設定を繰り返すと、`WindowState`が0(通常)→2(最小化)→
        0→(実際には最大化)という具合に、こちらの設定と無関係にOutlook内部の状態が
        数秒おきに勝手に遷移し続け、何度補正しても収束しない事象が確認された
        (COM層の内部状態管理そのものが不安定と考えられる)。
        そのため、Outlook COMのプロパティは一切使わず、人間がマウスでウィンドウを
        操作するのと同じOSレベルのWin32 API(ShowWindow/SetWindowPos)で直接
        位置・サイズを設定する。失敗しても例外は投げない(Falseを返す)。"""
        try:
            user32 = ctypes.windll.user32
            SW_RESTORE = 9
            user32.ShowWindow(hwnd, SW_RESTORE)
            time.sleep(0.2)
            # HWND_TOP(最前面)を指定しつつSWP_NOZORDER(重なり順を変更しない)を
            # 同時に渡していたため、実際には重なり順が一切変更されず、Outlookが
            # 手前にある他のウィンドウ(ブラウザ等)の後ろに隠れたままになる不具合が
            # 実機で確認された。SWP_NOZORDERは指定しない(重なり順を実際に変更する)。
            HWND_TOP = 0
            ok = user32.SetWindowPos(hwnd, HWND_TOP, left, top, width, height, 0)
            if not ok:
                print(f"🔧[Outlook配置] SetWindowPosが失敗しました(hwnd={hwnd})")
            try:
            ...
            print(f"🔧[Outlook配置] Win32でのウィンドウ配置でエラー: {e}")
            return False

    def _get_window_state_win32(self, hwnd):
        """指定HWNDの位置・サイズ(Left, Top, Width, Height)と、最大化/最小化の
        いずれかに該当するかをWin32 APIで直接取得する。取得できない場合はNoneを返す。"""
        try:
            user32 = ctypes.windll.user32
            rect = ctypes.wintypes.RECT()
            if not user32.GetWindowRect(hwnd, ctypes.byref(rect)):
                return None
            abnormal = bool(user32.IsZoomed(hwnd)) or bool(user32.IsIconic(hwnd))
            return (rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, abnormal)
        except Exception as e:
            print(f"🔧[Outlook配置] ウィンドウ状態(Win32)の取得でエラー: {e}")
            return None
    ...
    def arrange_outlook_window(self, left: int, top: int, width: int, height: int,
        ...
        位置・サイズの実際の設定は、Outlook COMの`Explorer.WindowState`/`.Left`等の
        プロパティを経由せず、HWNDを特定したうえでWin32 API(ShowWindow/SetWindowPos)
        で直接行う(`_set_window_bounds_win32`)。COM経由での設定を繰り返し試みた
        過去のバージョンでは、WindowStateが数秒おきに勝手に最小化⇔最大化を繰り返し
        続け収束しない事象が実機ログで確認されたため。Explorerオブジェクト自体は、
        Outlookの起動・既存ウィンドウの検出・HWND特定のためのCaption取得にのみ使う。"""
        t_start = time.time()
        print(f"🔧[Outlook配置] 開始 (target left={left} top={top} width={width} height={height})")
        deadline = t_start + wait_seconds
        try:
            pythoncom.CoInitialize()
            # 引数(left/top/width/height)は物理ピクセルで計算された値である。
            # 本プロセスはDPI非対応のため、このままWin32 APIを呼ぶとWindowsが
            # 座標を拡大率倍して適用してしまい、指定の1.5倍の位置・大きさになる
            # (実機で発生していた不具合。GetWindowRectも同じ仮想座標を返すため
            # ログ上は正しく見えてしまい発見が遅れた)。このワーカースレッドの
            # あいだだけDPI Awareに切り替え、物理ピクセルのまま扱えるようにする。
            with ThreadDpiAware("(ウィンドウ配置)"):
                return self._arrange_outlook_window_impl(
                    outlook_app=win32com.client.Dispatch("Outlook.Application"),
                    left=left, top=top, width=width, height=height,
                    deadline=deadline, t_start=t_start,
                )
        except Exception as e:
    ...
    def _arrange_outlook_window_impl(self, outlook_app, left, top, width, height,
                                      deadline, t_start) -> bool:
        """arrange_outlook_windowの本体。呼び出し元でDPI Awareなスレッド
        コンテキストが確立された状態で実行される(座標は物理ピクセル)。"""
        explorer = None
        try:
            outlook = outlook_app
            ...
            hwnd = self._find_explorer_hwnd(explorer)
            if not hwnd:
                print("🔧[Outlook配置] HWNDを特定できず、Win32での配置を諦めます")
                return False
            print(f"🔧[Outlook配置] HWND特定: {hwnd} (経過{time.time()-t_start:.1f}秒)。"
                  f"Win32 APIで直接配置します")

            self._set_window_bounds_win32(hwnd, left, top, width, height)
            state = self._get_window_state_win32(hwnd)
            print(f"🔧[Outlook配置] 配置直後の実測値(Win32): {state}")

            # 起動直後は、Win32での配置後もOutlook自身が状態を変化させることが
            # あるため、しばらく監視して、ずれていれば再設定する。
            recheck_deadline = time.time() + 15.0
            check_no = 0
            while time.time() < recheck_deadline:
                time.sleep(1.0)
                check_no += 1
                cur = self._get_window_state_win32(hwnd)
                if cur is None:
                    print(f"🔧[Outlook配置] 再チェック#{check_no}: ウィンドウ状態を取得できませんでした")
                    break
                cur_left, cur_top, cur_width, cur_height, abnormal = cur
                drifted = (
                    abnormal
                    or cur_left != left
                    or cur_top != top
                ...
                print(f"🔧[Outlook配置] 再チェック#{check_no}(経過{time.time()-t_start:.1f}秒): "
                      f"Left={cur_left} Top={cur_top} Width={cur_width} Height={cur_height} "
                      f"最大化/最小化={abnormal} drifted={drifted}")
                if not drifted:
                    continue
                self._set_window_bounds_win32(hwnd, left, top, width, height)
            ...
            # CoUninitializeは呼び出し元(arrange_outlook_window)のfinallyで
            # 一度だけ行う。ここで重ねて呼ぶとCOMの参照カウントが狂うため行わない。
            print(f"Outlookウィンドウ配置エラー: {e}")
            return False
...
class MailManagerGUI:
    # 本ツールとOutlookの間に、デスクトップが少しだけ覗く隙間を残すための余白。
    # 物理ピクセル単位(ご要望の「30ptぐらい」に合わせた値)。
    WINDOW_SPLIT_GAP = 30
    # ウィンドウの高さを作業領域の高さぴったりに設定すると、Windowsが単なる
    # リサイズ操作を暗黙的に「最大化」とみなしてしまうことが実機ログで確認された
    # (Win+Shift+↑の「垂直方向の最大化」に近い挙動)。これを避けるため、
    # 高さをわずかに小さくする余白(px)。
    WINDOW_HEIGHT_MARGIN = 8
    ...
    def __init__(self):
        ...
        # 配置計算は「物理ピクセル」に一本化する。左右のペインを完全に同じ幅・
        # 高さで割り付けたうえで、Tkinter(DPI非対応)へ渡す値だけを最後に仮想
        # 座標へ換算する。仮想座標と物理座標を混在させたまま計算していた従来の
        # 実装では、本ツール側とOutlook側で基準がずれ、幅・高さが揃わなかった。
        ...
        phys_work_area = get_physical_work_area()
        if phys_work_area:
            p_left, p_top, p_right, p_bottom = phys_work_area
        else:
            # 取得できない環境では仮想座標=物理座標とみなす(拡大率100%相当)。
            p_left, p_top, p_right, p_bottom = wa_left, wa_top, wa_right, wa_bottom
        p_width = p_right - p_left
        p_height = p_bottom - p_top

        # 仮想座標から物理座標への倍率(=ディスプレイの拡大率)。
        scale = (p_width / wa_width) if wa_width else 1.0
        if scale <= 0:
            scale = 1.0

        gap = self.WINDOW_SPLIT_GAP
        # 左右のペインを等幅にする。高さを作業領域ぴったりにするとWindowsが
        # 暗黙的に最大化とみなすため、わずかに低くする。
        pane_w = (p_width - gap) // 2
        pane_h = p_height - self.WINDOW_HEIGHT_MARGIN

        outlook_left = p_left + pane_w + gap
        outlook_width = p_right - outlook_left   # 端数を吸収(pane_wとの差は最大1px)
        outlook_height = pane_h

        # 本ツール(Tkinter)へ渡す値は仮想座標へ換算する。
        tool_x = wa_left
        tool_y = wa_top
        tool_w = int(round(pane_w / scale))
        tool_h = int(round(pane_h / scale))
        fit_window_to_work_area(self.root, tool_w, tool_h, tool_x, tool_y)

        print(f"🖥 画面配置: work_area(仮想)=({wa_left},{wa_top})-({wa_right},{wa_bottom}) "
              f"work_area(物理, 取得={'成功' if phys_work_area else '失敗→仮想座標を流用'})="
              f"({p_left},{p_top})-({p_right},{p_bottom}) 拡大率={scale:.2f} / "
              f"ペイン(物理)=w:{pane_w} h:{pane_h} 隙間={gap} / "
              f"本ツール(仮想)=x:{tool_x} y:{tool_y} w:{tool_w} h:{tool_h} / "
              f"Outlook(物理)=x:{outlook_left} y:{p_top} w:{outlook_width} h:{outlook_height}")
        ...
        threading.Thread(
            ...
            args=(outlook_left, p_top, outlook_width, outlook_height),
            daemon=True,
        ).start()
    ...
    def _bind_hour_spinbox_state(self, combo_var, spinbox):
        """期間プルダウンが「任意時間(h)」のときだけhスピンボックスを操作可能にし、
        それ以外の選択時はグレーアウト(disabled)する。プルダウンの値変更(ユーザー
        操作・プログラムからの変更どちらも)を検知できるよう、Combobox自体の
        イベントではなくtextvariable(StringVar)のtraceで判定する。"""
        def _update(*_):
            spinbox.config(state="normal" if combo_var.get() == "任意時間(h)" else "disabled")
        combo_var.trace_add("write", _update)
        _update()
    ...
    def _ui_search_tab(self):
        ...
        self._bind_hour_spinbox_state(self.v_prd, self.spin_search_h)
    ...
    def _ui_project_tab(self):
        ...
        self._bind_hour_spinbox_state(self.var_proj_period, self.spin_proj_h)
    ...
    def _ui_staff_tab(self):
        ...
        self._bind_hour_spinbox_state(self.var_staff_period, self.spin_staff_h)
        ttk.Label(f_period, text="h").pack(side=tk.LEFT)
