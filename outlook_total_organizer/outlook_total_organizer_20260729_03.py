...
<div class="nav-fab-container">
    ...
    def generate_cockpit_v2_report(self, cockpit_data, total_input, total_output, reformat_mode=False) -> str:
        ...
        try:
            ...
            # 表示2: プロジェクト別。プロジェクト再分類UI(cockpitReassignProject)の移動先として
            # 使うため、0件のプロジェクトも空のグループとして描画しておく
            ...
            for proj in project_names:
                ...
                rows = "".join(_render_queue_row(item) for item in items_in_proj)
...
function cockpitRecomputeCounts() {{
    document.querySelectorAll('.q-project-group, .q-cat-group').forEach(group => {{
