...
class OutlookMailManager:
    ...
    def arrange_outlook_window(self, left: int, top: int, width: int, height: int,
        ...
        try:
            ...
            while not explorer and time.time() < deadline:
                try:
                    ...
                    if not explorer:
                        ...
                        # 設定しておくことで、最大化状態が画面に一度も現れないようにする。
                        # 一部のOutlookバージョンではDisplay前のプロパティ設定に対応していない
                        # 可能性があるため、失敗しても表示自体は続行し、Display後の
                        # _apply_explorer_geometry(下記)による補正に委ねる。
                        try:
            ...
            # ウィンドウ状態(最大化等)を自分で復元することがある。これに対応する
            # ための再チェックだが、実機で「手動で最大化を解除してもすぐ全画面に
            # 戻される」不具合が報告された。原因は、ずれを検知するたびに
            # _apply_explorer_geometry(内部で最大10秒リトライする)を丸ごと
            # 呼び直していたため、最悪ケースで合計約40秒間ウィンドウ状態を
            # 上書きし続けてしまい、その間の手動操作がことごとく上書きされて
            # いたこと。この再チェックの目的はあくまで「起動直後の短い時間差
            # だけ」を補うことなので、合計時間を数秒に限定し、各回の再設定も
            # リトライなしの1回だけの軽い上書きにとどめる。
            recheck_deadline = time.time() + 3.0
            while time.time() < recheck_deadline:
                ...
                try:
                    ...
                    pass  # ここではリトライしない(次のチェック周期に委ね、早く切り上げる)
            return True
        except Exception as e:
