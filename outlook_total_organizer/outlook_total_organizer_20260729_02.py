    ...
    def generate_cockpit_v2_data(self, project_threads: dict, project_knowledge: dict,
        ...
        for i, proj in enumerate(project_list):
            ...
            for card in action_cards:
                ...
                queue_items.append({
                    # プロジェクト再分類UIで上書きされていれば、そちらを優先する。
                    "project": project_overrides.get(cid, proj),
                    "conversation_id": cid,
                    ...
                    # 進捗3択(進行/完了/無視)ボタンから、Actionタブと同じ/update_action_statusを
                    # 一括で叩いてカードの状態を更新するために保持する。
...
<div class="nav-fab-container">
    ...
    def generate_cockpit_v2_report(self, cockpit_data, total_input, total_output, reformat_mode=False) -> str:
        ...
        try:
            ...
            STATE_LABELS = (("in_progress", "進行"), ("done", "完了"), ("ignored", "無視"))
            ...
            def _render_queue_row(item, rank):
                ...
                # action_keysをJSに渡し、進捗3択(進行/完了/無視)ボタンからActionタブと同じ
                ...
                # 生成直後のキューは常に「進行」状態(open_actionsだけが入っているため)。
                state_btns = "".join(
                    f'<button data-val="{val}" class="q-state-btn{" active" if val == "in_progress" else ""}" '
                    f'onclick="cockpitSetState(this, \'{cid_safe}\', {keys_json}, \'{val}\', {self.server_port})">{label}</button>'
                ...
                return f'''
                <div class="q-row" data-cid="{cid_safe}" data-progress="in_progress" data-project="{proj_safe}">
                    {rank_html}
                    <div class="q-body">
                    ...
                    <div class="q-right">
                        ...
                        <div class="q-state-btns">{state_btns}</div>
                    </div>
                </div>'''
            ...
            # 0件のプロジェクトも空のグループとして描画しておく(.q-project-rowsが空の場合は
            # CSSの:empty::afterで「該当なし」を表示する)。
            by_project = {}
            for item in queue:
            ...
            for proj in project_names:
                ...
                <div class="q-project-group" data-project="{proj_safe}">
                    ...
                    <div class="q-project-rows">{rows}</div>
                </div>''')
...
.q-state-btns{{display:flex;justify-content:flex-end;gap:4px;margin-top:6px;}}
.q-state-btn{{font-size:.68rem;border:1px solid var(--border);border-radius:12px;padding:3px 9px;
  background:var(--chip);color:var(--ink2);cursor:pointer;white-space:nowrap;}}
.q-state-btn.active[data-val="in_progress"]{{background:var(--blue);color:#fff;border-color:var(--blue);}}
.q-state-btn.active[data-val="done"]{{background:var(--good);color:#fff;border-color:var(--good);}}
.q-state-btn.active[data-val="ignored"]{{background:var(--muted);color:#fff;border-color:var(--muted);}}
.q-filters{{display:flex;align-items:center;gap:8px;margin-bottom:10px;}}
.q-filters-label{{font-size:.72rem;color:var(--muted);}}
.q-filter-btn{{font-size:.76rem;font-weight:600;border:1px solid var(--border);border-radius:14px;
  padding:4px 12px;background:var(--surface);color:var(--ink2);cursor:pointer;}}
.q-filter-btn.active{{background:var(--blue);color:#fff;border-color:var(--blue);}}
...
<div class="wrap">
  ...
  <div class="q-filters">
    <span class="q-filters-label">状態:</span>
    <button class="q-filter-btn active" data-val="in_progress" onclick="cockpitToggleFilter(this)">進行</button>
    <button class="q-filter-btn" data-val="done" onclick="cockpitToggleFilter(this)">完了</button>
    <button class="q-filter-btn" data-val="ignored" onclick="cockpitToggleFilter(this)">無視</button>
  </div>
...
<script>
async function cockpitSetState(btn, cid, keys, val, port) {{
    // 状態(進行/完了/無視)は、両ビュー(解放スコア順・プロジェクト別)にある同じcidの
    // 行すべてに反映する(1つのカードが2箇所にレンダリングされているため)。
    document.querySelectorAll(`.q-row[data-cid="${{cid}}"]`).forEach(row => {{
        row.dataset.progress = val;
        row.querySelectorAll('.q-state-btn').forEach(b => b.classList.toggle('active', b.dataset.val === val));
    }});
    try {{
        for (const key of keys) {{
            await fetch(`http://localhost:${{port}}/update_action_status`, {{
                method: 'POST', headers: {{'Content-Type': 'application/json'}},
                body: JSON.stringify({{action_key: key, progress: val}})
            }});
        }}
    }} catch (e) {{ console.error(e); }}
    cockpitApplyFilters();
}}
function cockpitToggleFilter(btn) {{
    btn.classList.toggle('active');
    cockpitApplyFilters();
}}
function cockpitApplyFilters() {{
    const activeStates = Array.from(document.querySelectorAll('.q-filter-btn.active')).map(b => b.dataset.val);
    document.querySelectorAll('.q-row').forEach(row => {{
        row.style.display = activeStates.includes(row.dataset.progress) ? '' : 'none';
    }});
    // プロジェクト別ビューの件数バッジは、フィルタ後に実際に表示されている件数で更新する。
    document.querySelectorAll('.q-project-group').forEach(group => {{
...
async function cockpitReassignProject(select, cid, port) {{
    ...
    // 解放スコア順ビュー・プロジェクト別ビュー、両方の同じcidの行のタグ・セレクタを揃える。
    document.querySelectorAll(`.q-row[data-cid="${{cid}}"]`).forEach(row => {{
    ...
    // プロジェクト別ビューの行は、実際に新しいプロジェクトのグループへ移動させる。
    document.querySelectorAll(`#queue-project .q-row[data-cid="${{cid}}"]`).forEach(row => {{
        const targetRows = document.querySelector(
            `#queue-project .q-project-group[data-project="${{CSS.escape(newProject)}}"] .q-project-rows`
        );
...
document.addEventListener('DOMContentLoaded', cockpitApplyFilters);
</script>
</body>
</html>"""
...
class MailManagerGUI:
    ...
    def _reformat_cockpit_v2(self):
        ...
        保存時点のqueueのうち、action_status.jsonの最新値で全アクションが完了/無視に
        なっている項目は除外して描画する(Actionタブ側や進捗3択ボタンでの変更を、
        再取得なしでキューに反映するため)。また、カード上のプロジェクト再分類UIでの
        変更(cockpit_v2_project_overrides.json)も最新値で反映し直す。"""
        import tkinter.messagebox as messagebox
