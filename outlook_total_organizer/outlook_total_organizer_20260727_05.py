        ...
        try:
            ...
            for card in sorted_cards:
                ...
                for idx, a in enumerate(card.get("actions", [])):
                    ...
                    # トグルボタンの記号だけで「開けばコメントが入っているかどうか」がわかるようにする:
                    # 塗り▼(閉/開く)▲(開/閉じる)=コメントあり、白抜き▽(閉/開く)△(開/閉じる)=コメントなし。
                    has_comment = bool(comment.strip())
                    comment_toggle_symbol = "▼" if has_comment else "▽"

                    # レイアウト: 1件目の進捗ボタンはタイトル行(card-header)に同居させるため、
                    # ここでは保持するだけでitems_htmlには出さない。2件目以降は、進捗ボタンの
                    # 表示場所がないため、専用の「左空欄+進捗ボタン」の行をアクション行の上に追加する。
                    if idx == 0:
                        first_action_prog_btns = prog_btns
                        progrow_html = ""
                    else:
                        progrow_html = f'''
                        <div class="action-item-extra-progrow">
                    ...
                    <div class="action-item" data-key="{key}" data-progress="{progress}" data-priority="{priority}" data-has-comment="{"true" if has_comment else "false"}">
                        {progrow_html}
                        <div class="action-item-detail-row">
                            <button type="button" class="comment-toggle-btn" onclick="toggleActionComment(this)" title="コメント欄の表示/非表示">{comment_toggle_symbol}</button>
                ...
                # フラグ/受信日時/1件目の進捗ボタンを1行にまとめる(flex-wrap:nowrap、タイトル文字列側が
                # 縮んで吸収するため折り返さない)。2件目以降のアクションは、進捗ボタンの表示場所が
                # ないため専用の「左空欄+進捗ボタン」の行を各.action-itemの先頭に追加する。
                cards_html.append(f'''
                <div class="action-card js-action" data-key="{cid}" data-ts="{latest_ts}" data-unread="{unread_flag}" data-r19="{r19_flag}" data-progresses="{progresses_attr}" data-priorities="{priorities_attr}">
                    <div class="card-inner" style="border-left:4px solid {badge_col};">
                        <div class="card-header">
                            ...
                            <div class="prog-btns">{first_action_prog_btns}</div>
                        </div>
                        {"".join(items_html)}
                    </div>
                </div>''')
            ...
            <head>
                ...
                .action-item-extra-progrow {{ display:flex; justify-content:flex-end; margin-bottom:6px; }}
                .action-item-detail-row {{ display:flex; align-items:center; gap:8px; flex-wrap:wrap; }}
                .comment-toggle-btn {{ flex-shrink:0; border:none; background:none; cursor:pointer; font-size:0.95em; line-height:1.4; color:#64748b; padding:0 2px; }}
                .comment-toggle-btn:hover {{ color:#2563eb; }}
                .action-item-detail {{ flex:1 1 auto; min-width:0; font-size:0.95em; color:#334155; }}
                ...
                .prog-btns, .prio-btns {{ display:flex; gap:4px; flex-wrap:nowrap; flex-shrink:0; }}
            ...
            function applyActionFilters() {{
                ...
                allCards.forEach(card => {{
                    ...
                    card.querySelectorAll('.action-item').forEach((item, idx) => {{
                        totalItemCount++;
                        ...
                        // 1件目の進捗ボタンはタイトル行(card-header)に同居しているため、
                        // 1件目のアクション自体がフィルタで非表示になったら、タイトル行の
                        // 進捗ボタンも一緒に隠す。そうしないと、表示中の別アクションの
                        // 進捗ボタンと並んで、隠れた1件目の状態が残ったまま二重に見えてしまう。
                        if (idx === 0) {{
                            const headerProg = card.querySelector('.card-header .prog-btns');
                            if (headerProg) headerProg.style.display = itemMatch ? '' : 'none';
                        }}
                    }});
