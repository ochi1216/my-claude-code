...
def rank_review_achievement(is_confirmed: bool, goal_keys: list, tier1_present: bool,
                             tier2_count: int, site_wide: bool, has_quantitative: bool) -> str:
    """MAG Leaderへの報告優先順位を決定木で判定する(前回の統括コックピットv2で失敗した
    加重和スコア方式は採用しない)。
    Step1: 成果が確定(完了/判断を下した)していなければB。
    Step2: 3ゴール(G1/G2/G3)のいずれにも紐付かなければB。
    Step3: Tier1関与・Tier2 2名以上・サイト全体影響・定量効果、のいずれか1つでもあればS、
           無ければA。"""
    if not is_confirmed:
        return "B"
    if not goal_keys:
        return "B"
    if tier1_present or tier2_count >= 2 or site_wide or has_quantitative:
        return "S"
    return "A"
...
def review_activity_qualifies(mails: list, user_smtp_address: str) -> bool:
...
class OutlookMailManager:
    ...
    def _get_archive_inbox_and_sent(self, namespace):
        """オンラインアーカイブのルート配下から、受信トレイ・送信済みアイテムに相当する
        フォルダを名前一致で探す(オンラインアーカイブはGetDefaultFolderのようなショートカット
        が使えないため、直下フォルダを名前で照合する)。見つからない場合は(None, None)。"""
        root = self._find_online_archive_root(namespace)
        if not root:
            return None, None
        inbox = self._find_subfolder_by_names(root, ["受信トレイ", "Inbox"])
        sent = self._find_subfolder_by_names(root, ["送信済みアイテム", "Sent Items"])
        return inbox, sent
    ...
    def get_review_mails_for_month(self, year: int, month: int, progress_callback=None) -> list:
        ...
        try:
            ...
            archive_inbox, archive_sent = self._get_archive_inbox_and_sent(namespace)
...
class MailSummarizer:
    ...
    def summarize_review_month(self, year: int, month: int, threads: dict, meetings: list,
                                progress_callback=None) -> list:
        """振り返りタブ用: 暦月1か月分の対象スレッド(review_activity_qualifiesで機械フィルタ
        済み)を、AIで複数スレッドの束=「実績」単位に統合する(L2活動→L3実績のAI要約)。
        ...
        cache_file = os.path.join(REVIEW_CACHE_DIR, f"{yyyymm}.json")
        ...
        if os.path.exists(cache_file):
            try:
                with open(cache_file, 'r', encoding='utf-8') as f:
                    cached = _json.load(f)
                if is_closed_month or cached.get("item_count") == item_count:
        ...
        for cid, t in threads.items():
            ...
            for m in t['mails']:
                body = self._clean(self._clean_body_for_ai(m['body']))
                mail_texts.append(f"【送信者:{m['sender_name']}】\n本文:{body}\n")
            content = "".join(mail_texts)[:8000]
            thread_blocks.append(f"=== スレッドID: {cid} / 件名: {t.get('topic','')} ===\n{content}\n")
        ...
        prompt = f"""
        以下は、ある月に実際にメールでやり取りされた複数のスレッドである。あなた(メール送信者本人)が
        四半期パフォーマンスレビューでMAG Leader(上司)に報告するための「実績」単位に統合・要約せよ。
        ...
        - goal_keys: この実績が紐づくゴールを0〜複数個、配列で指定。以下の3つのいずれか:
          "G1_project"(個別プロジェクトの遂行) / "G2_site"(Japan Siteの基盤整備。
          Finance/Procurement/Legal/IT/Export/Import/HR/人事/調達/PO/支払い/設備/安全など
          コーポレート機能に関する活動) / "G3_r04"(R19等の既存プロジェクトの進め方を、
          より整った"R04"のやり方(リスク管理・FMEA等のプロセス)に合わせる変革活動)。
          1つのプロジェクト実績が「案件を前に進めた」と同時に「進め方をR04式に変えた」場合は、
          G1_projectとG3_r04の両方を入れてよい(1実績=1ゴールに限定しない)。
          いずれのゴールにも該当しない場合は空配列でよい。
        ...
        achievements = res.get("achievements", []) if res and not res.get("_error") else []
        for a in achievements:
            a["year_month"] = yyyymm

        with open(cache_file, 'w', encoding='utf-8') as f:
            _json.dump({"item_count": item_count, "achievements": achievements}, f, ensure_ascii=False, indent=2)
    ...
    def generate_review_data(self, monthly_threads: dict, monthly_meetings: dict,
        ...
        monthly_threads: {"YYYYMM": threads_dict}(各月、review_activity_qualifiesで
        機械フィルタ済みのgroup_by_thread結果)。monthly_meetings: {"YYYYMM": [meeting,...]}。
        各月をsummarize_review_monthでAI統合したうえで、Tier1/Tier2関与・G2小分類・
        報告ランク・会議の紐付けをOutlook非依存の純粋関数(classify_review_tier等)で
        機械的に付加する。手動編集(非表示・ランク上書き・手動追加・文言修正)も
        review_manual_items.jsonからここでマージする。"""
        now = datetime.now()
        all_achievements = []
        months = sorted(monthly_threads.keys())
        for i, yyyymm in enumerate(months):
            year, month = int(yyyymm[:4]), int(yyyymm[4:])
            threads = monthly_threads[yyyymm]
            meetings = monthly_meetings.get(yyyymm, [])
            if progress_callback:
                progress_callback(i, len(months), f"🎯 {year}年{month}月を統合中...")
            achievements = self.summarize_review_month(year, month, threads, meetings, progress_callback=None)
            bundled_meetings = bundle_recurring_meetings(meetings)
    ...
    def apply_review_manual_overrides(self, all_achievements: list) -> list:
        ...
        for a in all_achievements:
            a["achievement_id"] = _aid(a)

        hidden_ids = set(manual.get("hidden", []))
        all_achievements = [a for a in all_achievements if a["achievement_id"] not in hidden_ids]

        rank_overrides = manual.get("rank_overrides", {})
        for a in all_achievements:
            aid = a["achievement_id"]
            if aid in rank_overrides:
                a["rank"] = rank_overrides[aid]
                a["rank_label"] = REVIEW_RANK_LABELS.get(rank_overrides[aid], rank_overrides[aid])
                a["rank_is_manual"] = True

        text_overrides = manual.get("text_overrides", {})
        for a in all_achievements:
            ov = text_overrides.get(a["achievement_id"])
            if ov:
    ...
    def generate_review_report(self, review_data, period_label, total_input, total_output, reformat_mode=False) -> str:
        ...
        try:
            ...
            def _render_row(a):
                ...
                return f'''
                <div class="rv-row" data-aid="{aid_safe}" data-rank="{rank}">
                    <div class="rv-body">
                        <div class="rv-title-line">
                            <span class="rv-rank-badge rv-rank-{rank}">{rank_label_safe}</span>
                            <span class="rv-title">{title_safe}</span>
                        </div>
...
<div class="wrap">
  ...
  <details class="rv-add-box">
    ...
    <div class="rv-add-grid">
      ...
      <div class="rv-add-goals">
        <label><input type="checkbox" id="raG1"> G1 プロジェクト遂行</label>
        <label><input type="checkbox" id="raG2"> G2 サイト基盤整備</label>
        <label><input type="checkbox" id="raG3"> G3 R04フロー適合</label>
      </div>
  ...
  <div class="footer-note">
    メールで送信済みの活動のみが自動集計の対象です(受信のみのスレッドは対象外)。
    ランクは「成果確定 → G1/G2/G3のいずれかに紐づく → Tier1関与/部門横断2名以上/Japan Site全体/定量効果のいずれかでS、無ければA」の順で機械的に判定しています。
    合成スコアではないため、常にチップで根拠を確認できます。手動での判断とは異なる場合は🙈非表示・ランク変更で調整してください。
  </div>
</div>
<script>
...
async function reviewAddManual(port) {{
    const goalKeys = [];
    if (document.getElementById('raG1').checked) goalKeys.push('G1_project');
    if (document.getElementById('raG2').checked) goalKeys.push('G2_site');
    if (document.getElementById('raG3').checked) goalKeys.push('G3_r04');
    const payload = {{
        action: 'add',
...
class MailManagerGUI:
    ...
    def _ui_review_tab(self):
        ...
        ttk.Label(top_f, text="対象期間:").pack(side=tk.LEFT, padx=(0, 5))
        self.v_review_prd = tk.StringVar(value="3か月")
        cmb = ttk.Combobox(
            top_f, textvariable=self.v_review_prd, state="readonly", width=10,
            values=["1か月", "2か月", "3か月", "4か月", "5か月", "6か月"]
        )
        cmb.pack(side=tk.LEFT, padx=(0, 10))

        self.btn_run_review = ttk.Button(top_f, text="📈 振り返りを生成", command=self._run_review)
        ...
        ttk.Label(
            main,
            text="※ 指定期間分、現行メールボックスとオンラインアーカイブの両方から自分が送信したメール(含・関連スレッド)と\n"
                 "主催した会議を月ごとに取得し、AIが「実績」単位に統合してMAG Leaderへの報告優先順位(S/A/B)を付けます。\n"
                 "2か月より前はオンラインアーカイブから取得するため、初回生成は時間がかかることがあります。過去の月は\n"
                 "一度分析すると内容が変わらないためキャッシュされ、次回以降は当月分のみ再分析します。\n"
                 "受信のみで自分から送信していないスレッドは対象外です(振り返りは「実行したこと」を見るためです)。",
            foreground="gray", justify=tk.LEFT
        ).pack(anchor=tk.W)

    def _get_review_months(self):
        """選択期間(1〜6か月)から、当月を含む対象の(年,月)タプルを古い順で返す。"""
        n = {"1か月": 1, "2か月": 2, "3か月": 3, "4か月": 4, "5か月": 5, "6か月": 6}.get(self.v_review_prd.get(), 3)
        now = datetime.now()
        months = []
    ...
    def _run_review(self):
        """選択期間の各暦月について、現行メールボックス+オンラインアーカイブから
        送信メール中心にスレッドを取得し(review_activity_qualifiesで機械フィルタ)、
        AIで実績単位に統合してHTMLレポートを生成・表示する。"""
        self.btn_run_review.config(state=tk.DISABLED, text="⏳ 取得・分析中...")
        self._set_status("📈 振り返りを生成中...", start_timer=True)

        def task():
            try:
                months = self._get_review_months()
                monthly_threads = {}
                monthly_meetings = {}
                progress_cb = lambda c, t, msg: self.root.after(0, lambda: self._set_status(msg))
                ...
                for (y, m) in months:
                    ...
                    qualifying = {
                        ...
                        if review_activity_qualifies(t['mails'], self.outlook.user_smtp_address)
                    }
                    meetings = self.outlook.get_review_calendar_events(y, m, progress_callback=progress_cb)
                    yyyymm = f"{y:04d}{m:02d}"
                    monthly_threads[yyyymm] = qualifying
                    monthly_meetings[yyyymm] = meetings

                review_data = self.summarizer.generate_review_data(
                    monthly_threads, monthly_meetings, self.outlook.user_smtp_address,
                    progress_callback=lambda c, t, msg: self.root.after(0, lambda: self._set_status(msg))
                )
                start_label = f"{months[0][0]}年{months[0][1]}月"
                end_label = f"{months[-1][0]}年{months[-1][1]}月"
                period_label = f"直近{len(months)}か月（{start_label}〜{end_label}）"
