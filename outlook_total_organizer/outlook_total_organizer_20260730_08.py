...
def review_staff_function_labels(staff_names_involved: list) -> list:
    ...
    for name in staff_names_involved:
        funcs = REVIEW_STAFF_FUNCTIONS.get(name.lower(), [])
...
class MailSummarizer:
    ...
    def summarize_review_month(self, year: int, month: int, threads: dict, meetings: list,
                                staff_names: list = None, force_refresh: bool = False,
                                progress_callback=None) -> list:
    ...
    def generate_review_data(self, monthly_threads: dict, monthly_meetings: dict,
        ...
        ONだった月(=今回メールを再取得・再分析する月)のみを含む。この月は、既存キャッシュの
        有無・状態に関わらず必ずforce_refresh=Trueで再分析する(「チェックした月は必ず
        更新されるはず」という直感に反して、過去月のキャッシュ無条件再利用ロジックにより
        実際には更新されないケースがあったため、チェック=強制更新に統一した)。
        ...
        staff_namesはproject_knowledge["staffs"]の登録名一覧(スタッフ成果の機械判定用)。
        ...
        for i, yyyymm in enumerate(months):
            ...
            if yyyymm in monthly_threads:
                achievements = self.summarize_review_month(
                    ...
                    staff_names=staff_names, force_refresh=True, progress_callback=None
                )
            else:
        ...
        try:
            ...
            def _render_row(a):
                ...
                if a.get("thread_entry_id"):
                    ...
                    open_link_html = (
                        f'<a class="rv-open-link" href="http://localhost:{self.server_port}/open?id={a["thread_entry_id"]}&topic={topic_encoded}" '
                        f'target="_blank" title="Outlookでスレッドを検索して開く">🚀 Outlook</a>'
                    )
...
.rv-open-link{{font-size:.7rem;text-decoration:none;background:#eff6ff;color:#2563eb;
  border:1px solid #bfdbfe;border-radius:12px;padding:3px 9px;white-space:nowrap;}}
.rv-open-link:hover{{text-decoration:underline;}}
...
<div class="wrap">
  ...
  <div class="footer-note">
    ...
    ランクは「成果確定していなければ🔵進行中 → G1/G2/G3のいずれにも紐づかなければ🅑B → Tier1関与/部門横断2名以上/Japan Site全体/定量効果/スタッフの成果を牽引、のいずれかで🅢S、無ければ🅐A」の順で機械的に判定しています。
