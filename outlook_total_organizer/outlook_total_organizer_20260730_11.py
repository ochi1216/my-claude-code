...
def _default_review_person_goals() -> dict:
    ...
    キーは、既存のREVIEW_STAFF_FUNCTIONS・classify_review_staff_involvementが使う
    表記("Oi Yuto"等のフルネーム)に合わせてある(project_knowledge["staffs"]の実際の
    登録キー表記と完全一致しない場合はget_review_person_goal_defsの部分一致で吸収する)。"""
    return {
        "Ochi": {
            "G1_project": {
...
def get_review_person_goal_defs(person: str, goals_data: dict = None) -> dict:
    ...
    project_knowledge["staffs"]の登録キー表記が雛形のキー表記(例:"Oi Yuto")と完全一致しない
...
class MailSummarizer:
    ...
    def summarize_review_month(self, year: int, month: int, threads: dict, meetings: list,
        ...
        # 抽出対象の説明を変える(REVIEW_STAFF_FUNCTIONSのファンクション表示に準拠)。
        minutes_instruction = ""
        if has_minutes_for_person:
            func_labels = REVIEW_STAFF_FUNCTIONS.get(person.lower(), [])
            if "PM" in func_labels:
                focus_desc = "プロジェクト全体の進捗(PM観点のサマリ部分)"
            elif func_labels:
                focus_desc = f"{person}さん自身の{'/'.join(func_labels)}としての進捗パート"
            else:
                focus_desc = f"{person}さんの担当領域に関する進捗パート"
