...
class MailManagerGUI:
    def __init__(self):
        ...
        # 配置する。Outlook側の起動・COM操作(起動していなければ起動を含む)は時間が
        # かかりうるため、本ウィンドウの表示を妨げないようワーカースレッドで行う。
        screen_w = self.root.winfo_screenwidth()
        screen_h = self.root.winfo_screenheight()
        left_w = screen_w // 2
        self.root.geometry(f"{left_w}x{screen_h}+0+0")
        ...
        threading.Thread(
            ...
            args=(left_w, 0, screen_w - left_w, screen_h),
            daemon=True,
        ).start()
