        ...
        try:
            ...
            for card in sorted_cards:
                ...
                for a in card.get("actions", []):
                    ...
                    # 1カードを2行で収める: 1行目(タイトル行、カード共通)の右端にフラグ/受信日時、
                    # 2行目(誰から→誰へのアクション行、アクションごと)の右端に進捗/優先度ボタンを配置する。
                    items_html.append(f'''
                ...
                # フラグ/受信日時をまとめ、進捗/優先度ボタンは各.action-itemの誰から→誰へ行の右端に配置する。
                # これにより1カードが基本2行(タイトル行＋アクション行)に収まる。
                cards_html.append(f'''
            ...
            <head>
                ...
                .action-item {{ display:flex; flex-direction:column; gap:6px; padding:8px 16px 12px; border-top:1px dashed #e2e8f0; }}
                ...
                .action-item-controls {{ display:flex; align-items:center; gap:10px; flex-shrink:0; flex-wrap:wrap; margin-left:auto; }}
                .action-deadline {{ margin-left:8px; font-size:0.85em; color:#b45309; }}
                .action-comment-row {{ display:flex; margin-left:22px; }}
