...
- **文字コード**：`.bas`/`.cls`ファイルはUTF-8で作成しているが、日本語版Outlook
  のクラシックVBAはANSI(Shift-JIS)としてファイルを読み込むため、そのまま
  インポートするとコード中の日本語文字列（メッセージボックスの文言等）が
  文字化けする。配布時は `iconv -f UTF-8 -t CP932` 等でShift-JISに変換した
  版を渡すこと。
- **ビューの複製方法**：`View`オブジェクトに`SaveAs`メソッドは存在せず、
  実行時エラー438になる。正しくは `Folder.Views.Add(Name, ViewType, SaveOption)`
  で新規ビューを作成し、元のビューの `View.XML` プロパティ（表示項目・
  並び順・フィルター等を含む定義一式）を複製先にコピーしてから `.Save` する。
  `modSearchViewController.RegisterCurrentFolder` はこの方式に修正済み。
