...
# -*- coding: utf-8 -*-
"""
Document Search Manager  v20260903_01
=====================================
...
PO_CONFIG_PATH   = BASE_DIR.parent / "po_database_organizer" / "config.json"
...
def _load_config() -> Dict[str, Any]:
    ...
    tenant_id / client_id が未設定の場合は、同一リポジトリ内の
    po_database_organizer/config.json から自動的に借用する
    （読むだけで、書き換えは一切行わない）。
    """
    ...
    # tenant_id / client_id のフォールバック（po_database_organizer から借用）
    if not cfg.get("tenant_id") or not cfg.get("client_id"):
        if PO_CONFIG_PATH.exists():
            try:
                with open(PO_CONFIG_PATH, "r", encoding="utf-8") as f:
                    po_cfg = json.load(f)
                borrowed = []
                if not cfg.get("tenant_id") and po_cfg.get("tenant_id"):
                    cfg["tenant_id"] = po_cfg["tenant_id"]
                    borrowed.append("tenant_id")
                if not cfg.get("client_id") and po_cfg.get("client_id"):
                    cfg["client_id"] = po_cfg["client_id"]
                    borrowed.append("client_id")
                if borrowed:
                    print(f"🔗 po_database_organizer/config.json から "
                          f"{' / '.join(borrowed)} を流用しました。")
            except Exception as e:
                print(f"⚠️  po_database_organizer/config.json の読み込みに失敗しました: {e}")
        else:
            print(f"⚠️  流用元が見つかりません: {PO_CONFIG_PATH}")

    if not cfg.get("tenant_id") or not cfg.get("client_id"):
        print("\n❌ tenant_id / client_id が特定できませんでした。\n"
              "   config.json に設定するか、po_database_organizer/config.json を\n"
              "   同じフォルダ階層に配置してください。\n")
        raise SystemExit(1)
...
<header>
  ...
  <div class="sub">社内ドキュメント横断検索 &nbsp;/&nbsp; v20260903_01 (Phase 1: SharePoint)</div>
</header>
...
def main() -> None:
    ...
    print("=" * 70)
    print("📚 Document Search Manager  v20260903_01  (Phase 1: SharePoint)")
    print("=" * 70)
