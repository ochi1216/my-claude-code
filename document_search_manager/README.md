# Document Search Manager

社内のドキュメント管理システムを、**同一キーワードで横断検索**するツール。

| 選択肢 | 対象 | 状態 |
|---|---|---|
| `0. All` | 有効な全系統を並列検索（既定） | ✅ 稼働（SharePoint ＋ Nexus ＋ Enovia） |
| `1. SharePoint` | 社内SharePoint全社横断検索 | ✅ **Phase 1で実装済み** |
| `2. Nexus` | Shareflex品質文書サイト（SF_QualityDocumentsProd） | ✅ **Phase 2で実装済み** |
| `3. Enovia` | 3DEXPERIENCE / ENOVIA | ✅ **Phase 3で実装済み**（利用にはEnoviaへのログインが必要。後述） |

未実装の系統があれば、UI上に「実装予定」と明示されます（黙って0件を返しません）。

**「タイトルだけを検索する」**（既定オン）をオフにすると、文書の本文まで
検索します。オンのままのほうが目的の文書に辿り着きやすく、オフにすると
件数が大きく増えます。

**「前方一致で検索する」**（既定OFF、SharePoint/Nexusのみ）をオンにすると、
入力した語で始まるファイル名・フォルダ名も検索結果に含めます
（例："P01" で "P010024_Lorry" もヒットするようになります）。
ただし**短い語（既定4文字未満）には適用しません**。短い語に前方一致を
適用すると、無関係な結果が大量にヒットしてしまうことが実機検証で
判明したためです（詳しくは後述の「設定項目」と `DESIGN_NOTES.md` 参照）。

**「フォルダのみを検索する」**（既定OFF、SharePointタブのみ）をオンにすると、
取得済みの検索結果をその場でフォルダだけに絞り込みます（再検索はしません）。
該当フォルダが多く、上限件数を超えて存在する場合は「さらに取得」もあわせて
お使いください。

Nexusタブには**有効期限**の列があり、期限を過ぎた標準書には **「期限切れ」**、
60日以内に切れるものには **「まもなく」** のバッジが付きます。
有効期限の列で「この日以前」に今日の日付を入れると、
**期限切れの標準書だけを抽出**できます。

画面は**系統ごとのタブ**になっており、**タブごとに表の列構成が切り替わります**。
SharePointとNexusでは持っている情報が違うためです（Nexusの実体はShareflexで、
フォルダ名が内部ハッシュになる一方、SharePointには無い標準Indexを持ちます）。

## 必要要件

- Python 3.9以上
- 対象SharePointへの閲覧権限（**新規のEntra ID権限申請は不要**）
- 「要約」機能を使う場合のみ：`pip install python-docx python-pptx PyMuPDF` と、
  環境変数 `GEMINI_API_KEY` または `GEMINI_PROXY_URL` のいずれか
  （word_translator等の翻訳ツールと共通。詳しくは後述「文書の要約」参照）。
  未設定でも検索・出力・ダウンロード等、他の機能には影響しない。

## 認証について（重要）

本ツールは **`po_database_organizer` と同一のEntra IDアプリ登録をそのまま流用**します。
要求スコープは **`Sites.Read.All` のみ**で、新規のGraph権限申請は一切発生しません。

- `config.json` の `tenant_id` / `client_id` を空のままにしておくと、既存ツールの
  `config.json` から自動的に借用します（**読むだけ**で書き換えません）。探索順は
  `../po_database_organizer/config.json` → `../onenote_report_generator/config.json` で、
  キー名は小文字（`tenant_id`）・大文字（`TENANT_ID`）の両方に対応します。
- 既存ツールの `config.json` がどちらにも無い場合は、`config.json` の `credentials_from` に
  フルパスを指定するか、`tenant_id` / `client_id` を直接記入してください（後述）。
- トークンキャッシュは本フォルダ内の `token_cache.json` に分離して保存します
  （`po_database_organizer` と同時に起動しても競合しません）。そのため初回だけ、
  本ツール用のデバイスコード認証が1回必要です。

**EnoviaはGraph / Entra IDとは完全に別の認証**（3DEXPERIENCEのCAS・Cookie）
です。詳しくは次の「Enoviaへのログイン」を参照してください。

## Enoviaへのログイン（重要）

Enovia検索を使うには、**画面の「Enoviaにログイン」ボタン**を1回押す必要があります。

1. 「Enoviaにログイン」ボタンを押すと、**会社PCに入っているEdge**が別ウィンドウで
   開きます（Playwright用の新しいブラウザをダウンロードすることはありません）。
2. 開いた画面でEnoviaにログインします。**1日1回など、既にログイン済みであれば
   そのままEnoviaのトップ画面が表示されます。**
3. ログインできたら、**そのウィンドウを閉じてください。** 閉じたタイミングで
   ログイン情報（Cookie）が `enovia_session.json` に保存され、以後の検索は
   ブラウザを起動せず高速に行われます。
4. Cookieには有効期限があります。検索がエラーになったら、もう一度
   「Enoviaにログイン」を実行してください。

**`enovia_session.json` は `.gitignore` で除外されており、リポジトリには
コミットされません。** 会社PCのEdgeプロファイル（`enovia_profile/` フォルダ）も
同様です。

**Playwrightが使えない環境の場合**：`config.json` の `enovia_auth_mode` を
`"manual"` にし、`enovia_manual_cookie` にブラウザのF12で確認した
`Cookie` ヘッダーの値をそのまま貼り付けてください（Cookieが切れるたびに
手で貼り直す必要があります）。

## セットアップ手順

1. 依存パッケージをインストールする。

   ```
   pip install -r requirements.txt
   ```

2. `config.example.json` を `config.json` にコピーする（中身は編集不要でそのまま動きます）。

   ```
   copy config.example.json config.json
   ```

3. 起動する。`run_document_search_manager.bat` をダブルクリックするか、次を実行する。

   ```
   python document_search_manager_20260903_16.py
   ```

   初回はターミナルにDevice Code Flowの認証コード（URLとコード）が表示されるので、
   表示されたURLをブラウザで開いてサインインする。

4. `http://127.0.0.1:5020` が自動で開く。キーワードを入力して「検索」を押す。

> `run_document_search_manager.bat` は先頭で `cd /d %~dp0` を実行するため、
> ショートカットや別フォルダから起動しても正しく動作します。

## 画面の使い方

- **検索欄**にキーワードを入力し、Enter または「検索」ボタン。
- **検索対象**を `0. All` 〜 `3. Enovia` から選択（既定は `0. All`）。
- **取得件数**は「関連度上位 10 / 25 / 50 / 100 / 200 / 500 件」から選択。
  ここでいう「関連度」は **SharePoint検索が算出するレリバンス（一致度）** です。
  **画面での並べ替えは「取得した範囲の中での並べ替え」**である点にご注意ください
  （例：関連度上位10件を取得して最終更新日で降順にしても、
  「全社で最も新しい文書」ではなく「関連度上位10件の中で最も新しい文書」になります）。
  網羅的に見たい場合は件数を増やしてください。
  初期選択値は `config.json` の `default_max_results` で決まります
  （`config.example.json` の既定は開発段階向けに **10件**。運用時は100等に変更してください）。
  無制限取得は行いません（体感速度を優先しているため）。
- **タイトルをクリック**するとファイルが直接開きます。
- **「サイト」列**をクリックすると、その文書が保管されているサイトが開きます。
- **「フォルダ」列**をクリックすると、その文書が保管されているフォルダが開きます。
  - 表示は `/40. Bench Validation/01. Validation_Plan` のような階層パスです。
  - 先頭の `/` は **`Shared Documents` などの既定ライブラリのルート**を表します
    （どのサイトにもある名前で冗長なため短縮しています）。`PO` のような固有の
    ライブラリ名はそのまま表示されます。
  - **マウスを載せるとフルパス**が表示されます。Excel / CSV にもフルパスを出力します。
- **列見出しをクリック**すると並べ替えできます。クリックのたびに
  昇順（▲）→ 降順（▼）→ 解除、と切り替わります。
- **列見出し右の「⋮」**で、その列の絞り込みができます。
  - **最終更新日**は「この日以降」「この日以前」の日付範囲で指定します（片方だけでも可）。
  - **それ以外の列**はチェックボックスで複数選択します
    （例: 種別で `pptx` と `xlsm` の両方を選ぶ）。候補には件数が表示され、
    候補が多い場合は入力欄で絞り込めます。
  - 絞り込みが効いている列は「⋮」がアクセント色になります。
  - 「フィルタ・ソート解除」ボタンで一括解除できます。
- **タイトル列の右のチェックボックス**で複数のファイルを選び、
  「選択ファイルをZIPで取得」でまとめてダウンロードできます。
  見出しのチェックボックスで表示中の行を一括選択できます。
- **タブは、同じ条件（キーワード・上限件数・タイトル限定）ならキャッシュから
  即座に表示されます。** タブを行き来するたびに毎回待たされる問題への対処です。
  データが更新されている可能性がある場合は、**「🔄 最新の情報に更新」**
  ボタンでキャッシュを無視して取得し直せます（キャッシュはブラウザを
  再読み込みすると自動的に消えます）。
- **「さらに取得」ボタン**は、上限件数を超えて該当件数がある場合にだけ
  有効になります。押すと取得件数が選択肢の最大値（500件）まで引き上がり、
  再取得します。**まずは少なめの件数で素早く表示し、必要なときだけ多く
  取りに行く**という考え方です。
- **疎通診断**ボタンで、各系統が利用可能かを確認できます。
  キーワード不要・1件分の最小リクエストで、接続可否と動作モードだけを確認します
  （結果テーブルは更新されず、実行ログにのみ表示されます）。
- **Enoviaにログイン**ボタンで、Enovia検索に必要なログイン情報を取得します。
  **社外（自宅等）から会社PCで使う場合は、先に会社のVPNに接続してください。**
  Enoviaは社内ネットワークからしか名前解決できません。未接続の場合は
  「サーバー名を解決できませんでした（DNS）」と表示されます。
  **必ず次の順で行ってください。**
  1. Enoviaにログインする（SSOで自動的に入る場合はそのまま）
  2. **画面上部の検索ボックスで1回検索**し、結果が出るのを確認する
  3. そのウィンドウを閉じる（ここでログイン情報が保存されます）

  **2番を省略すると検索は必ず失敗します。** 検索API（`federated`）は
  3DSpaceとは別のセッションを使っており、**1回検索して初めて発行される**ため
  です（2026-09-11に実機で確認）。保存後のログに
  「うち検索に使えるもの ○ 件」と出ます。**0件だった場合はやり直してください。**
- **Enovia検索診断**ボタンで、Enoviaのタイトル限定検索に使えそうな構文
  （全文検索 / `title:` / `ds6w:label:`）の件数を比較できます。
  Enoviaのタイトル限定構文は実機で未確定のため、確認できるまでは
  「タイトルだけを検索する」がオンでもEnoviaは常に全文検索を行います。
- **Enovia型診断**ボタン（S04の事前確認用）は、既定の絞り込みに関係なく
  **全種別（191型）**で検索し、型ごとの件数（取得できた範囲のサンプル集計。
  Enovia画面のファセット件数とは一致しない場合があります）・返ってくる
  属性の充足率・代表1件の全属性・「Enoviaで開く」URLを実行ログに表示します。
- **Enoviaの検索対象**（`3. Enovia` タブでのみ表示）で、Documentタイプ以外
  （Wiring Diagram・Kit等）も検索対象に含めるかを選べます（S04本実装、
  v20260924_01）。
  - **①Documentのみ**（既定）… 従来どおり。
  - **②よく使う型**（Document・Project Space）… ECOなど多数該当する型に
    埋もれず、Project Spaceを見やすくする絞り込みです。
  - **③全種別** … Wiring Diagram・Kit等、Document以外もすべて表示します。
    部品番号での横断検索など、①②では検索漏れが起こる場合にお使いください。
  - 選んだ内容は次回起動時にも引き継がれます（前回の選択を記憶）。
  - Enoviaへの検索クエリ自体は変わりません（既に191種類の型を要求済みの
    ため、絞り込みは表示側だけで行っています）。
- **フォルダ探索診断**ボタンは、フォルダ探索機能の**動作確認**です。
  検索結果の最初のフォルダ（行を選んでいればその行）について、配下を
  再帰的にたどれるかを実際に試し、実行ログに判定を出します。
  読み取りのみで、何も書き換えません。探索がうまくいかないときの
  切り分けに使います（2026-09-10に会社PCで①②③すべて成功を確認済み）。
- **Enoviaタブ**では、Document Number / Title / **Enovia型** / Revision / State /
  Description / 作成者 / Doc Owner / 最終更新者 / 最終更新日 / 作成日 /
  フォルダ / 種別 / Enoviaで開く、の列構成になります。**同じDocument Number
  でもリビジョン違いは別行のまま表示します**（Enovia画面と一致させるため）。
  `0. All` タブでは、タイトルに `(Rev.N)` を添えて見分けられるようにしています。
  - **Enovia型**列は、Enoviaの型（`ds6w:what/ds6w:type`。例:
    `nex_WiringDiagram`）を、`nex_` 接頭辞を除いて大文字の前にスペースを
    入れた表示名（例: `Wiring Diagram`）にして表示します。実機のEnovia画面
    表示と一致することを2件（Kit・Wiring Diagram）で確認済みですが、
    全191型で検証したものではありません。既存の「種別」列（拡張子）とは
    別の情報です。
  - 型によっては「フォルダ」「最終更新者」が元々空欄になることを実機で
    確認しています。該当する行のタイトルには **ⓘ** の注記アイコンが付き、
    マウスを乗せると取得漏れではなく型の仕様である旨が表示されます。
  **Enoviaは一括ZIPダウンロードの対象外です**（文書の状態によってはダウンロードURLが
  拒否されるため）。「Enoviaで開く」からEnovia画面上で操作してください。
- **「要約」ボタン**（SharePoint / Nexusの`.docx`/`.pptx`/`.xlsx`/`.xlsm`/`.pdf`）で、
  文書を開く前にAIによる3段階の要約をポップアップ表示します。詳しくは
  後述「文書の要約（AI・Gemini経由）」を参照してください。
- **Excel出力 / CSV出力**で結果を `exports/` フォルダに保存します。
  **画面で絞り込み・並べ替えた状態がそのまま出力されます。**
  - **Excel**：タイトル・サイト・フォルダの各セルが**ハイパーリンク**になっており、
    クリックでそれぞれファイル・サイト・フォルダが開きます。オートフィルタ付き。
  - **CSV**：ハイパーリンクを持てない形式のため、
    「ファイルリンク」「サイトリンク」「フォルダリンク」をURLの列として出力します。

### フォルダの扱い

検索結果には、ファイルだけでなく**フォルダ自体**もヒットします。これらは除外せず、
**種別列に「フォルダ」と表示して区別**しています。

- **理由**：フォルダ名にキーワードが含まれていても、その中のファイルには含まれない、
  というケースがあります。この場合、**フォルダの存在そのものが有用な情報**になります。
- ファイルだけを見たいときは、**種別フィルタで「フォルダ」のチェックを外して**ください。
  逆に「フォルダ」だけを選べば、一致したフォルダの一覧になります。
- フォルダ行のタイトルをクリックすると、そのフォルダが開きます。
- フォルダは一括ダウンロードの対象外です（チェックボックスが無効になります）。
- 常に除外したい場合は、`config.json` の `exclude_folders` を `true` にしてください。

### サイト内検索（特定のSharePointサイトの中だけを探す）

「1. SharePoint」タブに「**対象サイト**」の行があります。サイトを選ぶと、
**そのサイトの中だけ**を検索します（検索条件に `path:"<サイトのURL>"` を
足しています）。「0. All」「2. Nexus」「3. Enovia」タブには影響しません。

**サイトの選び方（3通り）**

1. **サイトを探す**：サイト名の一部（例: `P010008`、`Lorry`）を入れて
   「サイトを探す」を押すと、候補が出ます。クリックで選ぶと、そのまま
   検索します。
   - 表示名をまるごと入れて見つからないとき（例:「P010008 - Jeep pkg-spin」）は、
     **先頭の語（番号など）で自動的に探し直します**。ログにその旨が出ます。
2. **検索結果から選ぶ**：検索結果の「サイト」列の 🔍 を押すと、その文書が
   あるサイトの中を検索します（「0. All」タブで押した場合は、SharePointタブに
   切り替わります）。
3. **よく使うサイト**：選んだサイトのチップの ☆ を押すと登録され（★になります）、
   「★ よく使うサイト」から選べるようになります。もう一度押すと解除です。
   登録内容はPC内の `favorite_sites.json` に保存されます（最大50件）。

**使い方のポイント**

- **キーワードを空にして検索すると、サイト内の一覧**になります。
- 同じ番号・似た名前のサイトがよくあるため、チップと候補には
  **名前と一緒にURL上の場所（`/sites/...`）** を出しています。選ぶ前に確認してください。
- 「タイトルだけを検索する」「フォルダのみを検索する」「さらに取得」
  「Excel出力」「フォルダ探索」は、サイト内検索でもそのまま使えます。
- チップの × で指定を外すと、全社検索に戻ります。
- **選んだサイトは次回起動時には引き継ぎません**（別の件を探すときに、
  気づかずに前のサイトに絞られたままになる事故を防ぐため）。前回サイト内検索
  だった場合は、起動時のログでお知らせします。
- 権限不足で「サイト単位検索モード」（`fallback_site_urls`）になっている環境では、
  サイト内検索は使えません（エラーでお知らせします）。

### フォルダ探索（配下を再帰的にたどる）

検索でヒットしたフォルダの**中身をまとめて見たい**ときに使います。
対象はSharePointのみです（Nexus / Enoviaは対象外）。

**手順**

1. `1. SharePoint` タブで検索します。「フォルダのみを検索する」を使うと
   フォルダだけに絞り込めます。
2. 見たいフォルダの行の**「探索」列にチェック**を付けます（複数選べます）。
   ファイルの行にはチェックが付きません。
3. **「📂 選択フォルダを探索」**ボタンを押します。進捗が実行ログに流れます。
4. 探索が終わると **`4. フォルダ探索` タブ**が現れ、結果が表示されます。

**2つの表示**

- **ツリー表示**（既定）：階層のまま表示します。フォルダを先に、その後は名前順です。
  フォルダ左の記号（▾ / ▸）で開閉できます。
  階層が崩れるため、絞り込み・並べ替えは使えません。
- **一覧表示**：階層を平らにします。**種別・名前・最終更新日・作成者・場所での
  絞り込みと並べ替え**が、検索結果と同じように使えます。サイズは数値として
  並べ替えます（「9 KB」と「10 MB」を取り違えません）。

どちらの表示でも、**ファイル名をクリックするとその場でファイルが開きます**。

**探索結果に対してできること**

- チェックを付けて**選択ファイルをZIPで取得**
- **Excel出力 / CSV出力**（画面で絞り込んだ状態がそのまま出ます）
- **要約**（`.docx` / `.pptx` / `.xlsx` / `.xlsm` / `.pdf`）

**深さ（Lv）の指定と、▶で先を読み込むツリー**

- 「選択フォルダを探索」の左の欄で、**一度に読み込む深さ**を選べます
  （**Lv1〜Lv4、標準はLv2**）。チェックしたフォルダが**Lv0**、そのすぐ下が
  **Lv1**、もう一つ下が**Lv2**です。選んだ深さは次回起動時も使います。
- 選んだ深さまでは、すべて開いた状態で表示します。その深さにあるフォルダは
  「▶ 📁 名前（5件・未読込）」と閉じた状態で出ます。**▶を押すと、その中身
  （次の1階層）だけを読み込んで開きます**。読み込んだ中のフォルダも、同じように
  ▶で開けます（何階層でも）。一度読み込んだフォルダの開閉は、通信せずに切り替わります。
- 中身が0件のフォルダには▶が出ません。件数はあったのに開いたら0件だった場合は
  「（空）」と表示します。
- ▶で読み込んだ行も、ZIP取得・Excel/CSV出力・要約の対象になります。
- 深く掘るほど時間がかかるため、まずLv2で全体をつかみ、見たいフォルダだけ
  ▶で開くのがおすすめです（以前は常に5階層まで掘っていました）。

**上限とその意味**

- 最初の一括読み込みは**2000件**で打ち切ります。巨大なライブラリを指定したときに
  Graphへの呼び出しが数千回に膨らみ、止められなくなるのを防ぐためです。
  ▶で開く分は1フォルダずつなので、この上限の対象外です。
- 上限に当たった場合は、実行ログに**打ち切った理由**が出ます。
  そのときは、深さを浅くするか、より下の階層のフォルダを起点に選び直してください。
- 幅優先でたどるため、打ち切られても**浅い階層は全部見えている**状態になります。
- 権限等で開けないフォルダがあっても探索は止まりません。開けなかった
  フォルダだけが理由付きで実行ログに出ます。

**注意**

- 探索結果は検索結果とは**別に保持**されます。タブを行き来しても、
  どちらも失われません。
- `4. フォルダ探索` タブでは「🔄 最新の情報に更新」「さらに取得」は出ません
  （どちらも検索専用の操作のためです）。探索し直すときは
  `1. SharePoint` タブに戻ってやり直してください。
- 探索結果はツールを再起動すると消えます。

### 文書の要約（AI・Gemini経由）

検索結果を開かなくても、内容の見当が付くように、AI（Gemini）による
3段階の要約をポップアップ表示する機能です（対象は
SharePoint / Nexus の `.docx` / `.pptx` / `.xlsx` / `.xlsm` / `.pdf`）。

- ①**Executive Summary**：この文書が何についてのものかを300字程度で説明。
- ②**文書の構造**：章立て（見出しや話題のまとまり）ごとの概要一覧。
- ③**Japan Site Managerへの示唆**：この文書をもとに業務を遂行する上での
  羅針盤（活用すべきこと／考慮すべき注意点／さらに問いを深めるべき方向性）。

文書が英語であっても、必ず日本語に翻訳した要約になります。

**セットアップ**（要約機能を使わない場合は不要）:

1. `pip install python-docx python-pptx`（`.xlsx`/`.xlsm`の抽出には
   `openpyxl`を使うが、これはExcel出力機能で既に必須のため追加インストール
   不要）
2. 環境変数 `GEMINI_API_KEY`（直接呼び出し用）または `GEMINI_PROXY_URL`
   （自宅PCプロキシ経由用）のいずれかを設定する
   （word_translator等の翻訳ツールと共通の設定です。既に設定済みの場合は
   そのまま使えます）。
3. 共通モジュール `gemini_client.py` が会社PC上のどこかに配置されている
   必要があります（本ツールは1〜3階層上の `common` フォルダを自動で
   探します。見つからない場所にある場合は環境変数 `GEMINI_COMMON_DIR` で
   フォルダを直接指定してください）。

**対象外の行と理由**：

| 行の種類 | 理由 |
|---|---|
| フォルダ | 本文を持たないため |
| Enovia | 検索の実体・認証方式が全く別（Exalead系API）のため未対応 |
| `.docx`/`.pptx`/`.xlsx`/`.xlsm`以外（.pdf等） | 現時点のスコープ外（今後対応予定） |

対象外の行では「要約」ボタンが最初から無効になっており、
マウスを載せると理由が表示されます。

**その他の設計上の注意点**：

- 文書本文は「会社PC→（自宅PCプロキシ経由の場合あり）→Google Gemini」へ
  送信されます。機密性の高い文書を扱う場合はご留意ください。
- 本文が極端に短い場合（画像スキャンのdocx等）は、誤った要約を避けるため
  要約自体を行わず、その旨を表示します。
- **回路図など図面が主体のPDFは、要約を行わずに理由を表示します。**
  詳しくは後述「図面が主体のPDFの扱い」を参照してください。
- **本文が長い場合（既定4万字超）は、その場でAIを呼ばず「本文が◯字あり、
  上限（◯字）を超えています。先頭部分だけで要約を進めますか？」という
  確認を先に表示します。** 「続ける」を選ぶとその場で先頭部分だけの要約を
  実行します（越智さんのフィードバックへの対応：切り詰めに気づかず
  部分的な要約だけを見てしまうのを防ぐため）。
- 同じ文書は結果がキャッシュされ、2回目以降は待たずに表示されます
  （文書が更新されると自動的に再要約します。ツール再起動でキャッシュは
  消えます）。「続ける」を選んで実行した要約もキャッシュされるため、
  同じ文書を再度開いたときは確認なしでそのまま表示されます。
- 初回の要約生成には、文書のダウンロード・本文抽出・AI呼び出しを含め
  数十秒かかることがあります。
- 本文抽出は、`.docx`は見出し・段落のテキスト、`.pptx`はスライドの
  タイトル・本文テキスト、`.xlsx`/`.xlsm`は各シートのセルの値（数式は
  最後に保存された計算結果）のみが対象です。マクロのコード自体は
  対象外です。**表・グラフ・画像中の文字列、
  `.pptx`の発表者ノートは対象外**です。

### 図面が主体のPDFの扱い

回路図のPDFで「要約」を押すと、要約の代わりに次の案内が出ます。

> 📐 このPDFは図面が主体と判断しました。本文の要約は行いません。

**なぜそうしているか**

- 回路図PDFは**スキャン画像ではなくベクター図面**で、部品番号（R23）・
  ネット名（VDD_3V3）・ピン名・定数が**すべて「文字」として埋め込まれています**。
- そのため十万字を超える文字が取り出せますが、**文章ではなく単語の羅列**です。
  AIに渡しても内容は分からず、待ち時間だけがかかります。
- **ファイルを開いてご覧いただく方が確実で、早い**という判断です。
  案内の中の「ファイルを開く」ボタンからそのまま開けます。

**どう判定しているか**

抽出した本文について、次の2つを実測します。**両方**に当てはまるときだけ
図面と判断します（片方だけでは、箇条書き中心の文書や表を巻き込むため）。

- 文末記号（。．? !）の密度が低い ── 文章なら30〜60字に1つは現れます
- 1行あたりの平均文字数が少ない ── 図面の文字は単語1つで1行になります

判定した根拠は実測値として画面に表示されるので、妥当かどうかを確認できます。

**要約したい場合／誤判定だった場合**

- 案内の中の**「それでも要約する」**を押せば、羅列のまま要約を実行できます
  （意味のある要約にならない可能性が高い操作です）。
- 誤判定が続く場合は、`config.json` の次の値で調整できます。
  値を小さくするほど「図面と見なしにくく」なります。

| キー | 既定値 | 意味 |
|---|---|---|
| `pdf_drawing_check_min_chars` | 5000 | これ未満の短いPDFは判定しない |
| `pdf_drawing_sentence_ratio` | 0.005 | 200字に1つ未満なら文章ではないとみなす |
| `pdf_drawing_avg_line_chars` | 12 | 1行平均12字未満なら羅列とみなす |

### 系統別ステータスの見方

| 表示 | 意味 |
|---|---|
| 🟢 | 検索成功（1件以上ヒット） |
| 🟡 | 検索は成功したが0件 |
| 🔴 | エラー（メッセージに理由を表示）。Enoviaが未ログインのときもここに出ます |
| ⚪ | 未実装 |

1つの系統がエラーになっても、他の系統の結果は必ず表示されます（部分成功方式）。

## トラブルシューティング

### 起動時に「ポートが使用中です」と出る

**v20260924_03以降は、多くの場合は自動で解消され、この案内自体が出ません。**
起動時にポートが使用中だった場合、そのプロセスが**自分自身
（Document Search Manager）の旧起動**だと確認できたときは、自動的に
終了させてから起動を続けます（画面に🔧のログが出ます）。

それでもこの案内が出た場合は、以下のいずれかです。

- ポートを使用しているプロセスを**1つに特定できなかった**（複数見つかった等）
- そのプロセスが自分自身だと**確認できなかった**（無関係なプロセスの可能性が
  あるため、あえて自動終了しません）
- `config.json` の `auto_kill_port_conflict` を `false` にしている

案内どおり、開いているコマンドプロンプトをすべて閉じるか、
`taskkill /F /IM python.exe` を実行してから、もう一度起動してください。

### 疎通診断で「権限不足」と出る

Graphの `/search/query` が `Sites.Read.All` では拒否された場合です。
**権限を追加申請する必要はありません。** `config.json` の `fallback_site_urls` に
検索したいサイトのURLを列挙すると、サイト単位検索モードに切り替わります。

```json
"fallback_site_urls": [
  "https://nexperia.sharepoint.com/sites/JapanDesign",
  "https://nexperia.sharepoint.com/sites/SF_QualityDocumentsProd"
]
```

### 検索結果の「開く」リンクが開けない

社内ブラウザが `.mcas.ms`（Microsoft Defender for Cloud Apps）経由でないと
アクセスできない場合があります。`config.json` の以下を `true` にしてください。

```json
"rewrite_host_to_mcas": true
```

### `tenant_id / client_id が特定できませんでした` と出る

既存ツール（`po_database_organizer` / `onenote_report_generator`）の `config.json` が
どちらも存在しない環境です。コンソールに探索したパスがすべて表示されるので、
次のどちらかで対処してください。**新規のEntra ID権限申請は不要です。**

**(1) 直接記入する（推奨）** — 既存ツールと同じEntra IDアプリ登録の値を書く。

```json
"tenant_id": "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
"client_id": "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
```

**(2) 既存の設定ファイルの場所を教える** — 別フォルダにある場合。
バックスラッシュは2つ重ねてください。

```json
"credentials_from": "C:\\\\Users\\\\nx023836\\\\Documents\\\\PythonScripts\\\\my-claude-code\\\\po_database_organizer\\\\config.json"
```

### Enoviaの検索がエラーになる（ログイン情報がありません 等）

「Enoviaにログイン」を実行していないか、Cookieの有効期限が切れています。
画面の「Enoviaにログイン」ボタンを押し、Edgeでログイン後にウィンドウを
閉じてください。会社PCの制限で `channel="msedge"` が使えない場合は、
`config.json` の `enovia_auth_mode` を `"manual"` にしてCookieを直接
貼り付ける方式に切り替えてください（前述「Enoviaへのログイン」参照）。

### `SSLCertVerificationError` / `unable to get local issuer certificate` と出る

**会社のネットワークにあるSSL検査（SSLインスペクション）が原因です。**
社内向け通信（`federated.plm.nexperia.com`）の中身を検査するために、社内の
証明書に差し替えられますが、Pythonはこの証明書を標準では信頼していません
（Edgeなどのブラウザは、Windowsの設定を通じて信頼しています）。
Graph API（SharePoint/Nexus）はこの検査の対象外のため、Enoviaだけで
発生することがあります。以下のコマンドで解消します（コード変更は不要です）。

```
pip install pip-system-certs
```

インストール後、ツールを再起動してください。

### 「要約」ボタンを押すと「Gemini共通モジュール(gemini_client.py)を読み込めませんでした」と出る

`gemini_client.py` が見つかっていません。本ツールはこのスクリプトの
1〜3階層上にある `common` フォルダを自動で探します。それでも
見つからない配置の場合は、環境変数 `GEMINI_COMMON_DIR` で
`gemini_client.py` が置かれているフォルダを直接指定してください。
検索・出力・ダウンロード等、要約以外の機能には影響しません。

### 「要約」ボタンを押すと「Gemini認証情報が設定されていません」と出る

環境変数 `GEMINI_API_KEY`（直接呼び出し用）または `GEMINI_PROXY_URL`
（自宅PCプロキシ経由用）のどちらも設定されていません。
word_translator等の翻訳ツールで使っているものと同じ環境変数です。
`setx` で設定した場合は、コマンドプロンプトを開き直してから
ツールを再起動してください。

### 「要約」ボタンを押すと「本文をほとんど抽出できませんでした」と出る

画像スキャンされたdocx（本文がすべて画像）など、文字として本文を
ほとんど取り出せない文書です。誤った要約を防ぐための安全策として、
意図的に要約を行っていません（仕様どおりの動作です）。

### Enoviaの件数がEnovia画面と合わない

`federated/search` の `nhits` は、絞り込み前の全種別（Document以外を含む）の
合計です。「Enoviaの検索対象」が①Documentのみ／②よく使う型のときは、
画面の行がその型だけに絞られるため `nhits` より少なくなるのは仕様です
（画面・出力にも注記が出ます）。Document以外（Wiring Diagram・Kit等）も
含めて確認したい場合は「③全種別」に切り替えてください（③選択時は
`nhits` と表の行数が一致します）。それでも大きく食い違う場合は、
`enovia_types`（対象タイプ一覧）がEnovia画面の検索範囲と一致しているかを
ご確認ください。

## 設定項目（`config.json`）

| キー | 既定値 | 説明 |
|---|---|---|
| `tenant_id` / `client_id` | 空 | 空なら既存ツールの `config.json` から借用（po → onenote の順） |
| `credentials_from` | 空 | 流用元の `config.json` をフルパスで明示指定（最優先で参照） |
| `graph_scopes` | `["...Sites.Read.All"]` | 要求スコープ。**変更すると新規権限申請が必要になるため、原則変更しない** |
| `flask_port` | `5020` | 画面のポート（既存ツールの5000/5010と衝突しない値） |
| `auto_kill_port_conflict` | `true` | 起動時にポートが使用中で、それが自分自身の旧起動だと確認できた場合、自動終了して起動を続けるか。`false`で従来どおり案内のみに戻せる |
| `default_max_results` | `10` | APIで件数指定が無い場合のサーバー側の既定値。画面の初期値は常に「上位10件」（前回の作業状態があればそれを優先） |
| `exclude_folders` | `false` | 検索結果からフォルダ自体を除外するか。既定は除外せず種別列で区別する |
| `restore_last_search` | `true` | 再起動時に前回の検索状態を復元するか |
| `max_download_files` | `50` | 一括ダウンロードの1回あたりの上限件数 |
| `download_timeout_sec` | `120` | 1ファイルあたりのダウンロード待ち時間（秒） |
| `search_fields` | 7項目 | Graph Searchに要求する検索マネージドプロパティ。無効な名前があれば自動でfields無しに切り替わる |
| `hard_max_results` | `500` | 取得件数の上限 |
| `provider_timeout_sec` | `180` | 1系統あたりのタイムアウト秒数。超えた系統は 🔴 になるが、他系統の結果は失われない |
| `nexus_site_url` / `nexus_folder_path` | Nexus用 | この2つを組み合わせたURLを、Graph Search の KQL `path:` に渡して検索範囲をNexusに限定する |
| `nexus_list_id` | Nexus用 | 実機調査で判明したShareflexのリストID。現在は記録用（Graph経由の検索では未使用） |
| `nexus_extra_fields` | `[]` | Nexus検索のときだけ追加要求する検索マネージドプロパティ（Shareflex固有の列）。**テナントに存在しない名前を書くと検索がHTTP 400になる**ため、実機で確認できたものだけを足す |
| `title_only_default` / `title_field` | `true` / `title` | 「タイトルだけを検索する」の初期状態と、絞り込みに使う検索プロパティ名 |
| `prefix_search_default` | `false` | 「前方一致で検索する」の初期状態（SharePoint/Nexusのみ有効） |
| `prefix_search_min_length` | `4` | 前方一致を適用する最小文字数。これ未満の語には`*`を付けない（短い語での暴走対策） |
| `nexus_resolve_people` | `true` | 人物列の数値ID（`<列名>LookupId`）を氏名に解決するか |
| `expiry_warn_days` | `60` | 有効期限のあと何日以内を「まもなく」と表示するか。0で期限切れのみ |
| `nexus_view_url` | Nexus画面URL | 「Nexusで開く」リンクの土台。`&@qmDocumentNo=<文書番号>` を付けて1件に絞り込んだ状態で開く |
| `nexus_deeplink_field` / `nexus_deeplink_title_field` | `qmDocumentNo` / `qmDocumentTitle` | 上記リンクで使う列フィルタの内部列名 |
| `nexus_scope_mode` | `path` | Nexus検索の絞り込み方式（`path` / `site` / `none`）。画面の「Nexus検索診断」で実測して決める |
| `nexus_enrich_metadata` | `true` | Nexusの標準Indexをリスト項目から取得するか |
| `nexus_enrich_max` / `nexus_enrich_workers` | `200` / `6` | Index取得の上限件数と並列数（1件につき1リクエスト増える） |
| `nexus_field_map` | `{}` | 列名の自動判別が外れた場合の明示指定。実在する列名は起動後の検索でコンソールに出る |
| `dedupe_nexus_from_sharepoint` | `true` | Nexusと同時に検索したときに、SharePoint側の重複行を除外するか。`1. SharePoint` 単独のときは（取りこぼしを防ぐため）除外しない |
| `rewrite_host_to_mcas` | `false` | リンクを `.mcas.ms` 経由に書き換えるか |
| `fallback_site_urls` | `[]` | 全社検索が使えない場合のサイト単位検索対象 |
| `enovia_base_url` | `.../3dspace` | EnoviaのベースURL。「Enoviaで開く」リンクの土台にもなる |
| `enovia_search_url` | `federated/search` のURL | Enoviaの検索APIエンドポイント |
| `enovia_auth_mode` | `playwright` | `playwright`（Edgeでログイン）または `manual`（Cookie手貼り） |
| `enovia_manual_cookie` | 空 | `enovia_auth_mode="manual"` のときに使うCookieヘッダーの値 |
| `enovia_browser_channel` | `msedge` | ログイン時に起動するブラウザ（会社PC既存のEdgeを使う） |
| `enovia_login_timeout_sec` | `300` | ログイン待ちの制限時間（秒）。超えてもその時点のCookieで保存を試みる |
| `enovia_types` | 191種類 | 検索対象とするEnoviaのタイプ一覧。将来Project Space等を増やす場合はここに追記する |
| `enovia_type_scope_default` | `"document"` | S04（v20260924_01）: 画面の「Enoviaの検索対象」の初期値（`"document"`/`"major"`/`"all"`）。前回選んだ内容が保存されていれば、そちらが優先される |
| `enovia_major_types` | `["Document", "Project Space"]` | スコープ「②よく使う型」で表の対象とする型（`ds6w:what/ds6w:type` の生値） |
| `enovia_document_type_only` | `true` | **旧キー（S04以前）**。`enovia_type_scope_default` が未設定のときのみ後方互換で読み替える（`true`→`"document"` / `false`→`"all"`）。新規設定は `enovia_type_scope_default` を使う |
| `enovia_page_size` | `100` | Enovia検索の1ページあたりの取得件数 |
| `enovia_max_raw_pages` | `20` | 型フィルタで大半が対象外になるキーワードでも、際限なく叩き続けないための上限ページ数 |
| `summary_max_chars` | `40000` | 文書の要約で使う本文の切り詰め上限（文字数）。超えた場合は要約前に画面で確認を求め、続行を選ぶと先頭のみを要約する |
| `summary_min_chars` | `50` | これ未満しか本文を抽出できなかった場合、要約自体を行わない下限（画像のみのファイル等での誤った要約を防ぐガード） |

## 生成物とGit管理

以下は `.gitignore` で除外しており、リポジトリにはコミットされません。

- `config.json`（テナントID等を含む）
- `token_cache.json`（認証トークン）
- `session_state.json`（前回の検索キーワード・絞り込み条件）
- `cache/`, `exports/`, `downloads/`（検索キーワード・結果・取得したファイル本体）
- `enovia_session.json`（EnoviaのログインCookie）, `enovia_profile/`（Edgeプロファイル）
- `favorite_sites.json`（よく使うサイト。社内のサイト名・URLを含む）

コミット対象は `config.example.json` のみです。

### バージョン管理と `old/` フォルダ

起動時に、**自分より古いバージョンのスクリプトは自動的に `old/` フォルダへ移動**します。
フォルダ直下には常に最新版だけが残ります。

- 過去のバージョンは削除されず、`old/` に保持されます（リポジトリにもコミット済み）。
- リポジトリ側の構成も同じであるため、`git pull` で旧版が復活して重複することはありません。
- 自分より新しいバージョンは移動しません（旧版を起動したときに最新版を
  退避させてしまわないため）。

## 既知の制限（現時点のスコープ外）

- **文書の要約（AI）は SharePoint / Nexus の `.docx` / `.pptx` / `.xlsx` /
  `.xlsm` のみ対応。** フォルダ・Enovia・`.pdf`は現時点で非対応（今後
  対応予定）。`.pptx`は表・グラフ・画像中の文字列、発表者ノートは抽出
  対象外。`.xlsx`/`.xlsm`はグラフ・画像・条件付き書式・マクロのコードは
  抽出対象外で、数式セルは最後に保存された計算結果のみを読み取る
  （キャッシュが無い場合は空欄扱い）。
- **Enoviaの一括ZIPダウンロードは未対応。** 詳細画面の `WebPublish URL` は
  文書の状態（`Allow Web publish: No` 等）次第で拒否されることを実機で
  確認済みのため、設計から外した。「Enoviaで開く」からEnovia画面上で
  操作する。
- **Enoviaのタイトル限定検索は非対応と確定しました（実機確認済み、2026-09-04）。**
  「タイトルだけを検索する」をオンにしても、Enoviaは常に全文検索を行います。
  候補構文（`title:` / `ds6w:label:`）はいずれも0件で、フィールド名が
  検索エンジンに認識されていないことを確認済みです（DESIGN_NOTES 3-3参照）。
- 検索結果に**本文スニペットは含めない**（一覧表示までが今回のスコープ）。
- **Nexusの標準Indexは Nexusタブでのみ表示**します（SharePointには無い列のため）。
  列名の自動判別が外れた場合は、起動後の検索でコンソールに出る実在の列名を見て
  `nexus_field_map` に設定してください。
- **Index 7列の対応づけは実機で確定済み**です（Nexus画面と突き合わせて一致を確認）。
  `qmDocumentNo` / `nxOldDocumentNo` / `qmDocumentTitle` / `qmEditor`(Doc Author) /
  `qmConfirmer`(Doc Owner) / `nxApplicable` / `nxFunctionalOrg`。
  **各セルにマウスを載せると「どの内部列から採った値か」が表示されます。**
  **「Nexus列診断」ボタン**を押すと、先頭1件についてNexusが持っている
  全ての列と値、および対応づけが実行ログに出ます。
- **「Nexusで開く」は、その1件だけに絞り込んだ状態でNexusを開きます。**
  Document Number の列フィルタ（`@qmDocumentNo=`）を使っています。
  全文検索ではありません（全文検索だと、他の文書の本文に参照文献として
  書かれた番号にも当たり、1件に絞り込めないため）。
- **キーワードを引用符で囲むと完全一致のフレーズ検索**になり、件数が大きく減ります
  （実測: `validation plan` で117件 → `"validation plan"` で14件）。
  Nexus画面の Full text search は既定でAND検索なので、比較するときは引用符を外してください。
- 検索結果のランキングはGraphの返却順に従う。SharePoint画面の並び順とは
  一致しない場合がある。

## 開発者向け

- **`DESIGN_NOTES.md`** … 調査で判明した事実・設計判断の理由・既知の罠。
  このツールを次に改修するときは**まずここを読む**。
- **`tests/`** … 検証ハーネス（ネットワーク不要）。

  ```
  python tests/run_tests.py     # 663項目の自動検証
  python tests/ui_check.py      # ブラウザ操作テスト（Playwright必要・任意）
  ```

  テストは**最新バージョンの本体を自動検出**するため、
  バージョンを上げてもテスト側の修正は不要です。
- **`.claude/skills/document-search-tool-dev/`**（リポジトリ直下）…
  Claude Code でこのツールを改修するときの開発手順を定義したスキル。

## 開発計画（全体像）

| Phase | 内容 | 状態 |
|---|---|---|
| Phase 1 | SharePoint全社検索 MVP | ✅ 完了（v20260903_08、実機動作確認済み） |
| Phase 2 | Nexus検索追加（Graph Searchを `nexus_folder_path` に限定）＋重複排除の有効化 | ✅ 完了（件数はNexus画面と一致することを実測で確認） |
| Phase 2.5 | 系統別タブ＋Nexus標準Index＋タイトル限定検索＋Nexusリンク | ✅ 完了（v20260903_15、実機で対応づけの一致まで確認済み） |
| Phase 3 | Enovia検索追加（`federated/search` を実測して実装） | ✅ 完了（v20260904_01、実機でログイン・件数突き合わせ・リンク開通まで確認済み） |
| S04 | Enovia検索拡張（Document以外の型にも対応、スコープ3択） | ✅ 完了（v20260924_01、実機診断（NEX13160）でWiring Diagram/Kit両型の属性充足率・リンク開通を確認済み） |
| S05 | SharePoint検索のサイト内検索（サイト名で探す／検索結果から選ぶ／よく使うサイト） | 🚧 実装済み・実機確認待ち（v20260925_03。診断2つは実機で合格済み） |
| Phase 4 | 3系統統合の磨き込み（名寄せ・検索履歴・お気に入り） | 未着手 |
