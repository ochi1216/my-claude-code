...
- **Enovia型診断**ボタン（S04・Document以外の型への対応検討用）は、
  既定の絞り込み（Documentのみ）に関係なく**全種別（191型）**で検索し、
  型ごとの件数（取得できた範囲のサンプル集計。Enovia画面のファセット件数
  とは一致しない場合があります）・返ってくる属性の充足率・代表1件の
  全属性・「Enoviaで開く」URLを実行ログに表示します。まだ診断のみで、
  画面上でDocument以外の型を検索・表示する機能そのものは未実装です。
