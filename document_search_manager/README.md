...
- **「要約」ボタン**（SharePoint / Nexusの`.docx`/`.pptx`/`.xlsx`）で、
...
1. `pip install python-docx python-pptx`（`.xlsx`の抽出には`openpyxl`を
   使うが、これはExcel出力機能で既に必須のため追加インストール不要）
...
| `.docx`/`.pptx`/`.xlsx`以外（.pdf等） | 現時点のスコープ外（今後対応予定） |
...
- 本文抽出は、`.docx`は見出し・段落のテキスト、`.pptx`はスライドの
  タイトル・本文テキスト、`.xlsx`は各シートのセルの値（数式は最後に
  保存された計算結果）のみが対象です。**表・グラフ・画像中の文字列、
  `.pptx`の発表者ノートは対象外**です。
...
- **文書の要約（AI）は SharePoint / Nexus の `.docx` / `.pptx` / `.xlsx` のみ
  対応。** フォルダ・Enovia・`.pdf`は現時点で非対応（今後対応予定。
  依頼順: docx→pptx→xlsx→pdf）。`.pptx`は表・グラフ・画像中の文字列、
  発表者ノートは抽出対象外。`.xlsx`はグラフ・画像・条件付き書式は
