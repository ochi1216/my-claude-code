...
- **サイト絞り込み診断**ボタン（`1. SharePoint`タブでのみ表示、S05の事前
  確認用）は、特定のSharePointサイトに絞り込んで検索できるかを試します。
  タブ内に現れる「対象サイト（診断用）」欄に、サイト名（例:
  `P010024_Lorry`）またはURL（`/teams/`・`/personal/`系サイトはこちら）を
  入力してから押してください。組み立てたKQL・該当件数・先頭数件のタイトル/
  URLを実行ログに表示します。**まだ診断のみで、画面のメイン検索にサイト
  絞り込みを組み込む機能そのものは未実装です。** `"`等の記号を含む入力は
  安全のためエラーにします。ブラウザからコピーしたURLや名前はエラーに
  なりやすいので、サイト名は手で入力してください。
- **サイト名検索診断**ボタン（「対象サイト（診断用）」欄の右、S05の事前
  確認用）は、欄に入れたサイト名の一部（例: `Lorry`）でサイトを探します。
  2つの方式（`/search/query`のサイト検索と`/sites?search=`）を同時に試し、
  方式ごとに候補のサイト名とURLを実行ログに表示します。日本語のサイト名も
  入力できます。**これも診断のみです。**
...
| S05 | SharePoint検索のサイト絞り込み（`P010024_Lorry`のような特定サイトへの絞り込み） | 🚧 進行中（v20260924_02「サイト絞り込み診断」は実機で合格。v20260925_02で「サイト名検索診断」を追加。実機確認後に本実装へ） |
