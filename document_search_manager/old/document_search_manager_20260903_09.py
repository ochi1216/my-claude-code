...
# -*- coding: utf-8 -*-
"""
Document Search Manager  v20260903_09
=====================================
...
<header>
  ...
  <div class="sub">社内ドキュメント横断検索 &nbsp;/&nbsp; v20260903_09 (Phase 2: SharePoint + Nexus)</div>
</header>
...
def api_export():
    ...
    csv_headers = ["ソース", "タイトル", "作成者", "最終更新日", "種別",
    ...
    # Excelでは URL を別列に並べず、タイトル・サイト・フォルダの各セル自体を
    # ハイパーリンクにする（画面と同じ感覚でクリックして開けるようにするため）。
    xlsx_headers = ["ソース", "タイトル", "作成者", "最終更新日", "種別",
                    "サイト", "フォルダ"]
    # 列番号(1始まり) → その列に張るリンクを取り出す関数
    link_columns = {
        2: lambda r: r.url,
        6: lambda r: r.site_url,
        7: lambda r: r.folder_url,
    }

    def xlsx_row_of(r: SearchResult):
        return [r.source, r.title, r.author, r.last_modified, r.doc_type,
                r.site, (r.folder_full or r.folder)]
    ...
    for column, width in zip("ABCDEFG", (12, 56, 24, 14, 10, 22, 40)):
        ws.column_dimensions[column].width = width

    ws.freeze_panes = "A2"
    ws.auto_filter.ref = f"A1:G{ws.max_row}"      # Excel側でも絞り込めるようにする
...
def main() -> None:
    ...
    print("=" * 70)
    print("📚 Document Search Manager  v20260903_09  (Phase 2: SharePoint + Nexus)")
    print("=" * 70)
