...
# -*- coding: utf-8 -*-
"""
Document Search Manager  v20260925_02
=====================================
...
【本バージョン(S05 Phase3-0b: サイト名検索診断)のスコープ】
...
class SharePointProvider(SearchProvider):
    ...
    def diagnose_site_search(self, term: str, size: int = 10) -> Dict[str, Any]:
        ...
        どちらの方式も新しい権限申請は不要な想定だが、実機では未確認。
        ...
        # ① Graph Search API（entityTypes = site）
        row: Dict[str, Any] = {"label": "① /search/query（entityTypes=site）",
                               "endpoint": "POST /search/query"}
        try:
            resp = http_req.post(
                f"{GRAPH_V1}/search/query", headers=headers, timeout=timeout,
        ...
        # ② Sites API のキーワード検索
        row = {"label": "② /sites?search=", "endpoint": "GET /sites?search="}
        try:
            resp = http_req.get(f"{GRAPH_V1}/sites", headers=headers,
                                params={"search": term}, timeout=timeout)
            row["status"] = resp.status_code
            if resp.status_code == 200:
                resources = (resp.json().get("value") or [])[:size]
...
<header>
  ...
  <div class="sub">社内ドキュメント横断検索 &nbsp;/&nbsp; v20260925_02 (サイト名検索診断)</div>
</header>
...
<main>
  <div class="panel">
    <div class="row">
      ...
      <button id="btnSiteScopeDiag" class="ghost" title="S05: SharePointの特定サイトに絞り込んで検索できるか（KQLのpath:演算子）を事前確認します。まだメイン検索には反映しません（診断のみ）。下の「対象サイト（診断用）」欄にサイト名かURLを入力してからお使いください。">サイト絞り込み診断</button>
    </div>
    <div class="row">
    ...
    <div class="row" id="siteScopeDiagRow" hidden>
      <label class="opt" title="S05: 「サイト絞り込み診断」ボタン専用の入力欄です。まだメイン検索には反映されません。短縮名（例: P010024_Lorry）またはフルURL（https://nexperia.sharepoint.com/sites/P010024_Lorry 等。/teams/・/personal/系サイトはフルURLで入力してください）を指定できます。">
        対象サイト（診断用）:
        <input type="text" class="wide" id="siteScopeInput"
               placeholder="例: P010024_Lorry または https://nexperia.sharepoint.com/sites/P010024_Lorry">
      </label>
      <button id="btnSiteSearchDiag" class="ghost" title="S05: 左の欄に入れたサイト名の一部（例: Lorry）でサイトを探し、候補のサイト名とURLを実行ログに出します。2つの方式を同時に試し、どちらが今の権限で使えるかを確認します（診断のみ・メイン検索には反映しません）。">サイト名検索診断</button>
    </div>
...
function setBusy(busy) {
  ...
  document.getElementById("btnSiteScopeDiag").disabled = busy;
  document.getElementById("btnSiteSearchDiag").disabled = busy;
...
function setTarget(target) {
  ...
  // S05 Phase3-0: サイト絞り込み診断もSharePointタブでのみ表示する
  // （診断専用であり、メイン検索にはまだ反映しない）。
...
function runSearch() {
  ...
  var key = cacheKey(p.keyword, p.target, p.maxResults, p.titleOnly, p.prefixSearch, p.typeScope);
...
document.getElementById("btnRefresh").addEventListener("click", function () {
  var p = currentSearchParams();
  if (!p.keyword) { log("キーワードが空です。"); return; }
  delete searchCache[cacheKey(p.keyword, p.target, p.maxResults, p.titleOnly, p.prefixSearch, p.typeScope)];
...
// S05 Phase3-0: サイト絞り込み診断。KQLのpath:演算子による絞り込みが
// 実際に機能するかを実測する（メイン検索にはまだ反映しない）。
document.getElementById("btnSiteScopeDiag").addEventListener("click", function () {
  var keyword = document.getElementById("keyword").value.trim();
  var site = document.getElementById("siteScopeInput").value.trim();
  if (!keyword && !site) {
    log(ICON_INFO + " キーワードと対象サイト（診断用）のどちらかを入力してください。");
    return;
  }
  setBusy(true);
  log(ICON_INFO + " サイト絞り込み診断を実行します...");
  fetch("/api/site_scope_diag", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
    ...
    .then(function (data) {
      if (data.error) { log(ICON_ERROR + " " + data.error); return; }
      if (!data.ok) {
        log(ICON_ERROR + " " + data.message);
        if (data.query) { log("      KQL: " + data.query); }
        return;
      }
      log(ICON_OK + " 該当 " + data.total + " 件（サンプル取得 " + data.sample_count
          + " 件" + (data.more ? "・続きあり" : "") + "）");
      log("      KQL: " + data.query);
      if (data.scope_url) { log("      絞り込みURL: " + data.scope_url); }
      (data.samples || []).forEach(function (s) {
        log("      " + (s.is_folder ? "📁 " : "📄 ") + s.title
            + "（" + (s.site || "サイト不明") + "） " + s.url);
      });
      log(ICON_INFO + " ①該当件数は想定どおりか ②サイトのトップURLだけでも絞り込めているか"
          + " ③意図しない文字（\" 等）を含む入力がエラーになるか、をご確認ください。");
    })
    .catch(function (e) { log(ICON_ERROR + " 通信エラー: " + e); })
    .finally(function () { setBusy(false); });
});

// S05 Phase3-0b: サイト名検索の診断。サイト名の一部からサイトの候補を
// 探せるかを2つの方式で試し、方式ごとの結果をそのまま並べる。
document.getElementById("btnSiteSearchDiag").addEventListener("click", function () {
  var term = document.getElementById("siteScopeInput").value.trim();
  if (!term) {
    log(ICON_INFO + " 「対象サイト（診断用）」欄にサイト名の一部（例: Lorry）を入力してください。");
    return;
  }
  setBusy(true);
  log(ICON_INFO + " サイト名検索診断を実行します（2つの方式で比較）...");
  fetch("/api/site_search_diag", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    ...
    .then(function (data) {
      ...
      log(ICON_INFO + " 検索語: " + data.term);
      (data.methods || []).forEach(function (m) {
        if (!m.ok) {
          log(ICON_ERROR + " " + m.label + " → HTTP " + m.status + " " + (m.message || ""));
          return;
        }
        var cands = m.candidates || [];
        log(ICON_OK + " " + m.label + " → 候補 " + cands.length + " 件（total " + m.total + "）");
        cands.forEach(function (c) {
          log("      🌐 " + (c.name || "(名前なし)") + "  " + (c.url || "(URLなし)"));
        });
        if (m.fields && m.fields.length) {
          log("      返ってきた項目: " + m.fields.join(", "));
        }
      });
      log(ICON_INFO + " ①目的のサイトが候補に出たか ②URLが正しいか"
          + " ③2つの方式のどちらが使えたか、をご確認ください。");
    })
    .catch(function (e) { log(ICON_ERROR + " 通信エラー: " + e); })
    .finally(function () { setBusy(false); });
});
...
def main() -> None:
    ...
    print("=" * 70)
    print("📚 Document Search Manager  v20260925_02  (サイト名検索診断)")
    print("=" * 70)
