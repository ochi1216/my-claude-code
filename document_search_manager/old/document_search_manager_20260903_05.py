...
<header>
  ...
  <div class="sub">社内ドキュメント横断検索 &nbsp;/&nbsp; v20260903_05 (Phase 1: SharePoint)</div>
</header>
...
function refreshSelectAll() {
  ...
  if (!box) { return; }
  box.checked = viewResults.length > 0 && viewResults.every(function (r) {
...
function renderHead() {
  ...
  COLUMNS.forEach(function (col) {
    ...
    if (col.type === "select") {
      ...
      all.checked = viewResults.length > 0 && viewResults.every(function (r) {
        return selected[r._idx];
      });
...
function buildCell(r, col) {
  ...
  if (col.key === "__select") {
    ...
    box.disabled = !r.url;
    box.title = r.url ? "一括ダウンロードの対象にする" : "リンクが無いため選択できません";
...
applyDefaults().then(restoreState);
</script>
</body>
</html>
"""
...
def api_download():
    ...
    results = [r for r in _pick_results(idx_param) if r.url]
    if not results:
        return jsonify({"error": "ダウンロード対象が選択されていません。"}), 400
