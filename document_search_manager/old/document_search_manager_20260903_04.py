        ...
        for site_url in (self.cfg.get("fallback_site_urls") or []):
            ...
            for item in (resp.json().get("value") or []):
                ...
                results.append(SearchResult(
                    ...
                    doc_type=_pick(_extension_of(name), _extension_of(_name_from_url(url))),
...
<header>
  ...
  <div class="sub">社内ドキュメント横断検索 &nbsp;/&nbsp; v20260903_04 (Phase 1: SharePoint)</div>
</header>
      ...
      <table id="resultTable">
        <thead>
          <tr>
            ...
            <th>作成者</th><th>最終更新日</th><th>種別</th><th>サイト</th>
          </tr>
        </thead>
        <tbody id="resultBody">
...
function renderResults(results) {
  ...
  results.forEach(function (r) {
    ...
    tdNum.textContent = r.document_number || "";
    ...
    if (r.url) {
      ...
      tdTitle.appendChild(aFile);
    } else {
      tdTitle.textContent = r.title || "(タイトルなし)";
    }
    ...
    tdDate.textContent = r.last_modified || "";
    tr.appendChild(tdDate);

    // 種別
    var tdType = document.createElement("td");
    tdType.className = "nowrap";
    tdType.textContent = r.doc_type || "";
    tr.appendChild(tdType);

    // サイト（クリックで保管先サイトを開く）
    var tdSite = document.createElement("td");
    tdSite.className = "nowrap";
    if (r.site_url) {
...
function applyDefaults() {
  fetch("/api/config")
    ...
    .then(function (c) {
      ...
      var want = String(c.default_max_results || "");
      for (var i = 0; i < sel.options.length; i++) {
        if (sel.options[i].value === want) { sel.selectedIndex = i; return; }
      }
    })
    .catch(function () { /* 取得できなくても既定のままで動作する */ });
}
...
def api_export():
    ...
    headers = ["ソース", "Document Number", "タイトル", "作成者",
               "最終更新日", "種別", "サイト", "ファイルリンク", "サイトリンク"]

    def row_of(r: SearchResult):
        return [r.source, r.document_number, r.title, r.author,
                r.last_modified, r.doc_type, r.site, r.url, r.site_url]
    ...
    for r in results:
        ws.append(row_of(r))
    for column, width in zip("ABCDEFGHI", (12, 18, 52, 20, 14, 10, 24, 60, 44)):
