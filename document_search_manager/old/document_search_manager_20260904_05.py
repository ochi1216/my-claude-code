...
# -*- coding: utf-8 -*-
"""
Document Search Manager  v20260904_05
=====================================
...
<header>
  ...
  <div class="sub">社内ドキュメント横断検索 &nbsp;/&nbsp; v20260904_05 (要約機能 Phase B: .pptx対応)</div>
</header>
...
var SUMMARIZABLE_EXTENSIONS = ["docx", "pptx"];
...
# ここに1行足すだけでよい（Phase B: docxに続きpptxを追加。順次pdf/xlsxを追加予定）。
SUMMARIZABLE_EXTENSIONS = ("docx", "pptx")
...
def _extract_pptx_text(content: bytes, max_chars: int) -> "tuple[str, bool]":
    """pptxのバイト列から本文テキストを抽出する。(本文, 切り詰めたか) を返す。
...
def _extract_text_for_summary(doc_type: str, content: bytes, max_chars: int) -> "tuple[str, bool]":
    """拡張子に応じた本文抽出関数へ振り分ける。"""
...
def api_summarize():
    ...
    try:
        text, truncated = _extract_text_for_summary(
            result.doc_type, content, int(_cfg.get("summary_max_chars", 40000)))
    except Exception as e:
...
def main() -> None:
    ...
    print("=" * 70)
    print("📚 Document Search Manager  v20260904_05  (要約機能 Phase B: .pptx対応)")
    print("=" * 70)
