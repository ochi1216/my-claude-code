...
# -*- coding: utf-8 -*-
"""
Document Search Manager  v20260903_12
=====================================
...
NEXUS_FIELD_CANDIDATES = {
    ...
    "title":             ["qmDocumentTitle", "Document Title", "DocumentTitle",
                          "Title"],
    # Doc Author / Doc Owner に対応する内部名は**まだ確定していない**。
    # Graph は人物列を <名前>LookupId（数値ID）でしか返さないため、
    # _resolve_people() でIDを氏名に解決したうえで候補を探す。
    # 解決できた人物列を優先し、それが無いときだけ他の候補に回る。
    # どの列から採ったかは index_sources に記録し、画面のツールチップに出す。
    "doc_author":        ["qmEditor", "Doc Author", "DocAuthor",
                          "nxDocReviewer", "Author"],
    "doc_owner":         ["qmOwner", "Doc Owner", "DocOwner",
                          "qmConfirmer", "qmApprover"],
...
class NexusProvider(SharePointProvider):
    ...
    def _user_name(self, token: str, lookup_id: str) -> str:
        ...
        self._people_cache[lookup_id] = ""      # 失敗しても再試行しない
        ...
        if resp.status_code != 200:
            ...
            self._people_note = (f"ユーザー情報リストの参照が HTTP {resp.status_code}。"
                                 f"人物列は空欄のままになります。")
            self._people_disabled = True
            return ""
        ...
        try:
            ...
            return ""
        name = _pick(fields.get("title"), fields.get("displayname"), fields.get("name"))
...
<header>
  ...
  <div class="sub">社内ドキュメント横断検索 &nbsp;/&nbsp; v20260903_12 (Phase 2.5: タブ構成 + Nexus Index)</div>
</header>
...
function buildCell(r, col) {
  ...
  if (col.key === "nexus_url") {
    td.className = "nowrap";
    if (r.nexus_url) {
      // Shareflexは検索条件をURLに載せないため、リンクだけでは
      // 「この1件だけを検索した状態」にできない。そこで、クリック時に
      // 検索語（既定はDocument Number）をクリップボードへ入れてから開き、
      // Nexus側の Full text search に貼るだけで済むようにする。
      ...
      aNexus.title = "「" + term + "」をコピーしてNexusを開きます。"
                   + "Nexusの Full text search に貼り付けてください。";
      aNexus.textContent = "Nexusで開く";
      aNexus.addEventListener("click", function () {
        if (navigator.clipboard && term) {
          navigator.clipboard.writeText(term).then(function () {
            log(ICON_INFO + " 「" + term + "」をコピーしました。"
                + "Nexusの Full text search に貼り付けてください。");
          }, function () { /* コピーできなくてもNexusは開く */ });
        }
      });
...
def main() -> None:
    ...
    print("=" * 70)
    print("📚 Document Search Manager  v20260903_12  (Phase 2.5: タブ構成 + Nexus Index)")
    print("=" * 70)
