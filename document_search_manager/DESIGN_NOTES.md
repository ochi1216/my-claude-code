...
#### S04（Enovia型拡張）進行中 — 診断機能まで実装（v20260911_03）
...
- **設計監査（Phase2）で承認済みの方針**：スコープを3択にする
  （①Documentのみ(既定)／②よく使う型(Document＋Project Space) ／
  ③全種別）。理由：②を挟まないと、③では上位N件がECO等で埋もれ、
  越智さんが最優先とするProject Spaceが見えにくくなる
  （実機実測：`Application`でDocument 263件に対しECO 256件など）。
- **未確認事項（推測で実装しない）**：①型ごとにどの属性が返るか
  ②`emxNavigator.jsp?objectId=`がDocument以外でも開くか
  ③型の英語名とEnovia画面の日本語ラベルの対応
  ④型ごとに列構成を変えるべきか。
- **v20260911_03（本バージョン）** ではこれらを潰すため、
  「Enovia型診断」ボタンのみを追加した（全種別で検索し、型別件数・
  属性充足率・生属性ダンプ・`emxNavigator.jsp`URLを表示。判定は断定しない）。
  **スコープ3択・Type列・注記の出し分けといった本実装はまだ行っていない。**
  会社PCでの実行結果を見てから、次バージョンで本実装に入る。
