...
【本バージョン(Phase 2)のスコープ】
  ...
  - Enovia はUI上に枠だけ用意し、「未実装」と明示する（黙って0件を返さない）。
...
NEXUS_FIELD_CANDIDATES = {
    "document_number":   ["Document Number", "DocumentNumber", "DocNumber",
                          "DocumentNo", "DocNo"],
    "old_system_id":     ["OldSystemIdentifier", "Old System Identifier",
                          "OldSystemId", "OldSystemID"],
    "title":             ["Document Title", "DocumentTitle", "Title"],
    "doc_author":        ["Doc Author", "DocAuthor"],
    "doc_owner":         ["Doc Owner", "DocOwner"],
    "applicable_to":     ["Applicable To", "ApplicableTo"],
    "department":        ["Department"],
    "top_level_process": ["Top Level Process", "TopLevelProcess"],
    "document_type":     ["Document Type", "DocumentType"],
}
...
def _pick_nexus_fields(fields: Any, cfg: Dict[str, Any]) -> Dict[str, str]:
    ...
    for raw_key, raw_value in fields.items():
        lookup.setdefault(_normalize_field_key(raw_key), raw_value)
    ...
    for slot, candidates in NEXUS_FIELD_CANDIDATES.items():
        ...
        for name in names:
            text = _field_text(lookup.get(_normalize_field_key(name)))
            if text:
                picked[slot] = text
                break
    return picked
...
def _nexus_deep_link(cfg: Dict[str, Any], keyword: str) -> str:
    """Shareflexの画面を「検索済みの状態」で開くURLを組み立てる。

    SharePoint標準のフォルダビュー（Forms/AllItems.aspx）は、Shareflexが
    自サイトのトップへ強制遷移させるため使えない（実機で確認済み）。
    Shareflex自身の View.aspx に &q= を付けた形なら、そのまま開ける。
    """
    ...
    return f"{base}{joiner}q={quote(text, safe='')}"
...
class NexusProvider(SharePointProvider):
    ...
    def _enrich_one(self, token: str, result: SearchResult):
        ...
        picked = _pick_nexus_fields(fields, self.cfg)
    ...
    def _enrich(self, results: List[SearchResult]) -> str:
        ...
        # 実在する列名を1度だけログに出す。config の nexus_field_map を
        # 決めるための materials になるので、推測せずに済む。
        if found_keys and not NexusProvider._field_names_logged:
            NexusProvider._field_names_logged = True
            print("🔎 Nexusのリスト項目が返した列名（内部名）: "
                  + ", ".join(sorted(str(k) for k in found_keys)))
    ...
    def search(self, keyword: str, max_results: int) -> Dict[str, Any]:
        ...
        # Shareflexの画面を検索済み状態で開くディープリンクを張る。
        # 文書番号があればそれで、無ければタイトルで検索した状態にする。
        for result in kept:
            result.nexus_url = _nexus_deep_link(
                self.cfg, _pick(result.document_number, result.title))
    ...
    def diagnose(self, keyword: str) -> List[Dict[str, Any]]:
        ...
        variants = [
            ("① 現行方式（フォルダで限定）", self._build_query(keyword, "path")),
            ("② サイトで限定", self._build_query(keyword, "site")),
            ("③ 限定なし（全社）", self._build_query(keyword, "none")),
        ]
        if stripped != keyword:
            variants.append(("④ 引用符を外す＋フォルダで限定",
                             self._build_query(stripped, "path")))
        else:
            variants.append(("④ 完全一致（引用符あり）＋フォルダで限定",
...
<header>
  ...
  <div class="sub">社内ドキュメント横断検索 &nbsp;/&nbsp; v20260903_10 (Phase 2.5: タブ構成 + Nexus Index)</div>
</header>
...
function buildCell(r, col) {
  ...
  if (col.key === "nexus_url") {
    ...
    if (r.nexus_url) {
      ...
      aNexus.title = "Nexusの画面をこの文書で検索した状態で開く";
...
def api_export():
    ...
    # 結果がすべてNexusなら、Nexusの標準Indexを出す
    # （SharePointの「サイト／フォルダ」列はNexusでは意味を成さないため）。
    ...
    if nexus_only:
        ...
        widths = [18, 18, 52, 22, 22, 24, 24, 14, 10, 14]
    else:
        spec = [
...
def main() -> None:
    ...
    print("=" * 70)
    print("📚 Document Search Manager  v20260903_10  (Phase 2.5: タブ構成 + Nexus Index)")
    print("=" * 70)
