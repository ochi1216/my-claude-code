...
# -*- coding: utf-8 -*-
"""
Document Search Manager  v20260925_03
=====================================
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .ttoggle {
    ...
    color: var(--muted); user-select: none; font-size: 10px;
  }
  .ttoggle:hover { color: var(--accent); }
  .ttoggle.empty { cursor: default; }
  .ttoggle.empty:hover { color: var(--muted); }
...
<header>
  ...
  <div class="sub">社内ドキュメント横断検索 &nbsp;/&nbsp; v20260925_03 (サイト内検索)</div>
</header>
  ...
  <div class="panel">
    <div class="row" style="justify-content: space-between;">
      ...
      <div class="row">
        ...
        <button id="btnExplore" class="mini" hidden disabled title="「探索」に印を付けたフォルダの配下を、再帰的にたどって一覧にします（最大5階層 / 2000件）。">選択フォルダを探索</button>
...
function pollExplore(job) {
  fetch("/api/explore_status?job=" + encodeURIComponent(job))
    ...
    .then(function (st) {
      ...
      log(ICON_OK + " 探索完了: フォルダ " + st.folders + " 件 / ファイル "
          + st.files + " 件（走査 " + st.scanned + " フォルダ）");
...
document.getElementById("btnExplore").addEventListener("click", function () {
  ...
  log(ICON_FOLDER + " フォルダ探索を開始します（" + idx.length + " フォルダ / "
      + "上限 5階層・2000件）。件数が多いと数分かかることがあります。");
  fetch("/api/explore", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ idx: idx.join(",") }),
  })
    .then(function (resp) { return resp.json(); })
    .then(function (data) {
...
EXPLORE_MAX_DEPTH = 5      # 起点フォルダを0とした深さ。5階層目まで開く
...
def _explore_run(job_id: str, roots: List[SearchResult], token: str,
                 timeout: int) -> None:
    """幅優先で配下をたどる。上限に当たったら、そこで打ち切って理由を残す。
    ...
    while queue:
        ...
        for child in children:
            ...
            found.append(row)
            if row.is_folder:
                if depth < EXPLORE_MAX_DEPTH:
                    queue.append((drive_id, row.item_id, depth + 1,
                                  child_path, root))
                elif not stopped:
                    stopped = (f"深さの上限（{EXPLORE_MAX_DEPTH} 階層）に達した"
                               "フォルダがあり、その先は開いていません。")
    ...
    print(f"🌲 フォルダ探索: 起点 {len(roots)} 件 / 走査 {scanned} フォルダ / "
...
def api_explore():
    ...
    with _explore_lock:
        ...
        _explore_jobs[job_id] = {
            ...
            "roots": [r.title for r in roots],
        }

    thread = threading.Thread(target=_explore_run,
                              args=(job_id, roots, token, timeout), daemon=True)
    thread.start()
    return jsonify({"job": job_id, "roots": [r.title for r in roots]})
...
def main() -> None:
    ...
    print("=" * 70)
    print("📚 Document Search Manager  v20260925_03  (サイト内検索)")
    print("=" * 70)
