...
# -*- coding: utf-8 -*-
"""
Document Search Manager  v20260925_04
=====================================
...
【本バージョン(フォルダ探索の深さ指定)のスコープ】
  - 「選択フォルダを探索」の左に深さ（Lv1〜Lv4、標準Lv2）を選ぶ欄を追加した。
    チェックしたフォルダがLv0、そのすぐ下がLv1、もう一つ下がLv2（越智さんの
    定義）。選んだLvまでを一括で取り、以前の「常に5階層まで」より速くした。
    選んだLvは session_state.json に保存し、次回起動時も使う。
  - 指定Lvにあるフォルダは「▶ 📁 名前（5件・未読込）」と閉じて表示し、▶を
    押すと次の1階層だけを /api/explore_children で読み込む（何階層でも可）。
    読み込んだ行はサーバー側の探索結果の末尾に同じ順で足し、足し始めた位置を
    返す。画面は位置が一致するときだけ足す（ずれると、ZIP・Excel・要約で別の
    ファイルが選ばれるため）。読み込み済み・古い探索結果のフォルダは読み込まない。
  - ツリーの開閉記号を大きくした（10px→15px、押せる範囲24px四方）。以前は
    小さくて見づらかった（越智さんのご指摘）。▶は文字として描く指定を付けた
    （Windowsで絵文字として描かれないように）。

【前バージョン(S05本実装: サイト内検索)のスコープ】
  - 「1. SharePoint」タブに「対象サイト」の行を追加した。サイトを選ぶと、
    そのサイトの中だけを検索する（KQLに path:"<サイトのトップURL>" を足す。
    トップURLだけで絞り込めることは2026-09-25に実機で確認済み）。
    キーワードが空なら、サイト内の一覧になる。
  - サイトの選び方は、越智さんの決定どおり2つ＋お気に入り。
      A. 「サイトを探す」：サイト名の一部から候補を出して選ぶ。/search/query
         （entityTypes=site）を使い、失敗時だけ /sites?search= に切り替える。
         0件なら先頭の語（番号など）で自動的に探し直す（表示名まるごとでは
         見つからない実例があったため。実機で確かめた方法だけを使う）。
      B. 検索結果の「サイト」列の🔍：その文書があるサイトの中を検索する。
      ☆: よく使うサイトを登録し、「★ よく使うサイト」から選べる
         （favorite_sites.json。社内の情報を含むためGitには載せない）。
  - 同じ番号・似た名前のサイトが複数ある（実機で確認）ため、チップと候補には
    名前と一緒にURL上の場所（/sites/...）を必ず表示する。
  - サーバー側で、届いたサイトURLを必ず検査してから使う（使える文字・社内の
    ホストのみ・/sites/ か /teams/・トップURLに揃える）。だめな文字があれば
    その文字を表示する（9/25に原因が分からず手間取った反省）。
  - 選んだサイトはキャッシュの条件に含める（論点B2）。次回起動時には引き継がず
    （S05 Q1=A案）、前回サイト内検索だった場合はその旨をログで知らせる。
  - 診断用の入力欄と「サイト絞り込み診断」「サイト名検索診断」ボタンは画面から
    外した（上のボタン列のはみ出しも1つ減る）。サーバー側の診断処理は残してある。

【前バージョン(S05 Phase3-0b: サイト名検索診断)のスコープ】
  - S05本実装（SharePointタブで「サイトを探す→選んだサイトの中で
    ファイル・フォルダを検索」）の設計監査で越智さんの承認を得た。
    S03以来の手順どおり、まず**未確認事項②「サイト名での検索が今の権限で
    使えるか」を確かめる診断だけ**を追加する。本実装はまだ行っていない。
  - 未確認事項①（`path:`でサイト内に絞れるか）は、2026-09-25に会社PCで
    解決済み（`title:TEST_Product path:"…/sites/P010024_Lorry"`で
    `50.Test_Product_Engineering`フォルダが1件ヒット）。
  - 「サイト名検索診断」ボタン（SharePointタブの「対象サイト」行）で、
    ① POST /search/query（entityTypes=site）② GET /sites?search= の
    2方式を同時に試し、方式ごとに候補（サイト名・URL）と返ってきた項目名を
    そのまま表示する（判定は断定しない）。どちらも Sites.Read.All の範囲の
    想定で、新しい権限申請は行わない。
  - 検索語は、日本語のサイト名にも対応できるよう英数字の制限はかけず、
    KQLを壊し得るダブルクォートと、コピー時に混ざる目に見えない文字
    （ゼロ幅スペース等）だけを取り除く。

【前バージョン(実機バグ修正: netstat等のデコード失敗)のスコープ】
  - v20260924_03で追加した「ポート使用中の自動解消」が、越智さんの会社PCで
    起動時に必ずクラッシュする不具合を修正した。
  - 原因：`netstat -ano`の出力に含まれる日本語の列見出し（「ローカル
    アドレス」等、環境依存でCP932/Shift-JISエンコード）を、Pythonが
    UTF-8前提でデコードしようとして失敗していた。デコードに失敗すると
    `subprocess.run`の内部スレッドで例外が握りつぶされ、`result.stdout`が
    `None`のまま返ってくることがあり、それを`.splitlines()`しようとして
    `AttributeError`でクラッシュしていた（`netstat`/`wmic`/PowerShell/
    `taskkill`の呼び出し全箇所が同じ原因で影響を受け得た）。
  - 対処：デコード失敗時に例外を出さず代替文字に置き換える
    `errors="replace"`を全箇所に付け、あわせて`result.stdout`/`stderr`が
    `None`になり得る前提でコードを書き直した（見出し行の文字化けは判定に
    使わないため実害が無い）。
  - **開発環境（Linuxコンテナ）では日本語ロケールの文字化けが再現せず、
    検証で検出できていなかった不具合。** 検証不足だったことをここに明記する。

【前バージョン(ポート使用中の自動解消)のスコープ】
  - 越智さんのご要望：「毎回起動時にポートが使用中になる。使用中なら
    Killして起動できるようにしてほしい」（ほぼ常に、閉じ忘れた自分自身の
    旧起動が原因）。
  - ポートが使用中だった場合、そのプロセスが**自分自身
    （Document Search Manager）の旧起動だと確認できたときだけ**自動終了し、
    起動を続ける。python.exe/pythonw.exeで、コマンドラインに
    `document_search_manager`を含む場合のみを「自分自身」と判定する。
  - **確認できない場合（無関係なプロセスの可能性）は、従来どおり起動を
    中止して手動対処を案内する。** 無関係なプロセスを誤って終了させる事故を
    避けるため、あえて自動化しない（安全側に倒す設計。設計提案時に
    越智さんと合意済み）。
  - Windows標準コマンド（netstat / wmic、wmicが無い環境ではPowerShellへ
    自動フォールバック / taskkill）のみで実現し、新規の依存パッケージは
    追加しない。設定`auto_kill_port_conflict`（既定true）で無効化もできる。

【前バージョン(S05 Phase3-0: サイト絞り込み診断)のスコープ】
  - 越智さんの要望：「P010024_LorryというSharePoint Siteの中の
    TEST_Product_Engineeringという名前のフォルダを検索したい」が、今の
    全社横断検索だけでは実現できない（サイトのトップページに手動で移動して
    探索する必要がある）。
  - Graph Search APIのKQL `path:"<絶対URL>"` によるサイト絞り込みは、
    Nexus検索が既に実装・実績あり（`NexusProvider._build_query()`）。ただし
    対象サイトはconfig.jsonに固定されており、**ユーザーが検索のたびに
    自由入力する**ケースでの実績ではない。敵対的プロダクトマネージャー
    レビューで、自由入力をKQLへそのまま埋め込むリスク（B1）・キャッシュキー
    への反映漏れ（B2）等が指摘され、S04で確立した「診断→本実装」の手順を
    踏むことになった。
  - **このバージョンでは診断機能のみ**を追加する（「サイト絞り込み診断」
    ボタン）。サイト名/URLの許可文字ホワイトリスト検証・`path:`によるKQLの
    組み立て・該当件数と先頭数件の実測を行い、結果をそのまま表示する
    （判定は断定しない）。**メイン検索への組み込み（3択スコープ等の本実装）
    はまだ行っていない。** 会社PCでの実行結果を見てから、次バージョンで
    本実装に入る。

【前バージョン(S04: Enovia全種別対応)のスコープ】
  - v20260911_03で追加した「Enovia型診断」の実機結果（NEX13160での実測、
    Wiring Diagram/Kit両型とも属性充足率・「Enoviaで開く」リンクの開通を
    確認済み）を踏まえ、Document以外の型を検索対象に含められるようにする
    本実装を行う（DESIGN_NOTES 3-3のS04節、docs/NEXT_TASK.md S04節）。
  - Enovia検索スコープを3択にした：①Documentのみ（既定）／②よく使う型
    （Document＋Project Space）／③全種別。検索クエリ自体（federated/search
    へのリクエスト）は変更せず、応答をどう絞り込んで表示するかだけを変える
    （新しいAPI連携・権限申請は不要）。
  - スコープは、越智さんの選択（Q1=B案）に従い「前回選んだものを記憶し、
    次回起動時も引き継ぐ」方式にした（`/api/state`に保存）。既定値は
    `enovia_type_scope_default`（config.json、既定"document"）。
  - Enovia結果テーブルに新しい列「Enovia型」を追加した。既存の「種別」列
    （拡張子）とは別概念のため、列を分けて混同を避けた。型の表示名は、
    `nex_`接頭辞を除去し大文字の前にスペースを挿入する機械的な変換ルールで
    生成する（`nex_Kit`→`Kit`、`nex_WiringDiagram`→`Wiring Diagram`が実機の
    Enovia画面表示と一致することを確認済み）。
  - 主要な列（作成者・Doc Owner・最終更新者・フォルダ等）が型によっては
    構造的に空欄になることを実機診断で確認したため、該当行のタイトルに
    注記アイコンを付ける（列の作り分けはしない）。
  - 既存の`enovia_document_type_only`（bool）は後方互換のため読み替えて
    使用する（true→"document" / false→"all"）。実機のconfig.jsonに残って
    いても動作する。

【前バージョン(S04 Phase3-0: Enovia型診断)のスコープ】
  - S04（Enovia検索拡張。Document以外の型も検索・表示できるようにする）の
    設計監査（Phase 2）で越智さんの承認を得た後、まず「診断機能だけ」を出す
    という手順（S03で確立した型）に従い、このバージョンでは診断機能のみ
    を追加した。
  - 追加した「Enovia型診断」ボタンは、全種別（191型）で検索を実行し、
    (1) 型別の件数内訳（取得できた範囲のサンプル集計。Enoviaの絞り込み
        ファセットのような全件集計ではないことを明示する）
    (2) 型ごとの代表1件について、返ってきた属性を**そのまま全部**表示
    (3) 型ごとに、ツールが使う主要属性が何件中何件で値を持つか（充足率）
    (4) 型ごとに「Enoviaで開く」URLを1本ずつ表示
    を返す。判定は断定せず、実測値をそのまま並べる
    （DESIGN_NOTES 3-2「不明点の潰し方」参照。過去に自作の診断レポートが
    2回誤診した教訓に基づく）。秘密情報（Cookie・トークン）は表示しない。

【前バージョン(ログイン失敗の切り分け)のスコープ】
  - Enoviaログインの失敗を、**ブラウザの起動失敗**と**接続の失敗**に切り分け、
    それぞれに合った対処を案内する。従来はどちらの場合も「Edgeの起動に
    失敗しました」と表示し、実際にはDNS解決の失敗（VPN未接続等）だったのに
    Edgeの設定を疑わせてしまっていた。

【前バージョンのスコープ】
  - Enoviaのログインで、**セッションCookieを取り逃がしていた不具合**を修正した。
    ウィンドウを閉じた「後」にCookieを取り出していたため、メモリ上にしか
    無いセッションCookie（JSESSIONID / SERVERID）が既に破棄されていた。
    待機中に1秒ごとに取得し、最良の内容を保存するように変えた。
  - federated.plm.nexperia.com は dspace とは別のセッションを持つため、
    **ログイン後にEnovia画面で1回検索する**必要がある。画面とログで案内する。

【前バージョンまでのスコープ】
  - SharePointタブの検索結果で、種別が「フォルダ」の行に「探索」チェックを
    付けて複数選択し、その配下を再帰的にたどって一覧化する（Phase A）。
    上限は 深さ5階層 / 2000件（越智さん承認済み）。対象はSharePointのみ。
  - 探索結果は「4. フォルダ探索」タブに表示する。ツリー表示と一覧表示を
    切り替えでき、一覧表示では既存の絞り込み・並べ替えがそのまま使える。
    ファイル名クリックでその場からファイルを開ける（ツリー・一覧とも）。
  - 探索結果に対しても、既存の選択→ZIP取得・Excel/CSV出力・要約が使える
    （Phase B）。検索結果と同じ SearchResult 型で扱うための設計。
  - 要約の対象形式に .pdf を追加した（.docx/.pptx/.xlsx/.xlsm/.pdf）。
  - 回路図など**図面が主体のPDF**は、文字は大量に取り出せても要約に使えない
    断片（部品番号・ネット名）のため、Geminiを呼ばずに理由を伝える。
...
# S05: よく使うサイト（☆で登録）。社内のサイト名・URLを含むためGitに載せない。
FAVORITE_SITES_PATH = BASE_DIR / "favorite_sites.json"
FAVORITE_SITES_MAX  = 50

# Enoviaはtenant_id/client_id方式の認証を使わない（Graphとは無関係の別システム）ため、
# Cookie・ブラウザプロファイルは token_cache.json とは別ファイルで管理する。
# Enoviaの検索が実際に投げられる先。ログイン時に、このホスト宛のCookieを
# 捕まえられたかどうかで成否を判断する（tools/enovia_evidence_report.py と同じ考え方）。
#   - dspace    … 3DSpace本体のセッション
#   - federated … 検索API。**dspaceとは別のセッション**で、Enovia画面で
#                 1回検索して初めて発行される（2026-09-11に実機で確認）
ENOVIA_SESSION_HOSTS = ("dspace.plm.nexperia.com", "federated.plm.nexperia.com")
...
# ─────────────────────────────────────────────────────────────
# S04 Phase3-0: Enovia型診断（DESIGN_NOTES 3-3のS04節、未確認事項①③④用）
#
# Document以外の型（ECO / 問題 / スケッチ 等）で、ツールが実際に使っている
# 属性（_item_to_result が読む属性）が値を持つかどうかを、型ごとに実測する。
# (属性キー, 画面表示用ラベル) の組。_item_to_result の読み取り箇所と対応させる。
# ─────────────────────────────────────────────────────────────
ENOVIA_TYPE_DIAG_ATTRS = [
    ("resourceid",                                "resourceid（Enoviaで開くURLに必須）"),
    ("ds6w:identifier",                            "Document Number"),
    ("ds6w:label",                                 "Title"),
    ("ds6w:description",                           "Description"),
    ("ds6wg:revision",                             "Revision"),
    ("ds6w:what/ds6w:status",                      "State"),
    ("ds6w:who/ds6w:responsible/ds6w:originator",  "作成者"),
    ("ds6w:who/ds6w:responsible",                  "Doc Owner"),
    ("ds6w:who/ds6w:lastModifiedBy",                "最終更新者"),
    ("ds6w:when/ds6w:modified",                    "最終更新日"),
    ("ds6w:when/ds6w:created",                     "作成日"),
    ("ds6w:where/ds6w:context/ds6w:folder",        "フォルダ"),
    ("ds6w:what/ds6w:docExtension",                "拡張子"),
]


# ブラウザが返す代表的な失敗の原因と、越智さんが取れる対処。
# エラー文字列で分類する（Playwrightは原因を構造化して返さないため）。
ENOVIA_NAV_ERROR_HINTS = [
    ("ERR_NAME_NOT_RESOLVED",
     "サーバー名を解決できませんでした（DNS）。社内ネットワークまたはVPNに"
     "接続されていない可能性が高いです。",
     "・VPNに接続されているかご確認ください。\n"
     "・普段お使いのEdgeで同じURLを開けるか試してください。\n"
     "・コマンドプロンプトで nslookup {host} を実行すると切り分けられます。"),
    ("ERR_CONNECTION_TIMED_OUT",
     "サーバー名は解決できましたが、接続がタイムアウトしました。",
     "・VPNの接続状態と、社内ネットワークへの到達性をご確認ください。"),
    ("ERR_CONNECTION_REFUSED",
     "サーバーに接続を拒否されました。",
     "・URLが正しいか（config.json の enovia_base_url）をご確認ください。"),
    ("ERR_CERT",
     "サーバー証明書の検証に失敗しました。",
     "・社内のSSLインスペクション環境で起こることがあります。"
     "IT部門にご相談ください。"),
    ("ERR_INTERNET_DISCONNECTED",
     "ネットワークに接続されていません。",
     "・有線／無線の接続状態をご確認ください。"),
    ("ERR_PROXY",
     "プロキシ設定の問題で接続できませんでした。",
     "・会社PCのプロキシ設定をご確認ください。"),
]


def _enovia_navigation_error_message(base_url: str, error: Exception) -> str:
    """ログイン画面を開けなかったときの案内文を組み立てる。

    「Edgeの起動に失敗しました」と一括りにすると、実際にはネットワークの
    問題なのにEdgeの設定を疑わせてしまう（実際にそうなった）。
    ブラウザが返す原因コードで分類し、対処まで書く。
    """
    text = str(error)
    host = base_url.split("//")[-1].split("/")[0] if "//" in base_url else base_url
    for code, reason, action in ENOVIA_NAV_ERROR_HINTS:
        if code in text:
            return (f"Enoviaのログイン画面を開けませんでした（{code}）。\n"
                    f"{reason}\n{action.format(host=host)}\n"
                    f"対象URL: {base_url}")
    return (f"Enoviaのログイン画面を開けませんでした。\n"
            f"対象URL: {base_url}\n元のエラー: {text[:300]}")


def _enovia_launch_error_message(error: Exception) -> str:
    """ブラウザ自体を起動できなかったときの案内文。"""
    return (f"ログイン用のEdgeを起動できませんでした: {error}\n"
            "会社PCのポリシーで channel=\"msedge\" が使えない場合は、"
            "config.json の enovia_auth_mode を \"manual\" にして"
            "Cookieを直接貼り付ける方式に切り替えてください。")


def _enovia_cookie_domain_covers(cookie_domain: str, host: str) -> bool:
    """そのCookieが、指定ホストへのリクエストに送られるかを判定する。

    先頭のドットの有無にかかわらず、そのドメインと配下のサブドメインに
    送られる（ブラウザの挙動と同じ）。
    """
...
def _enovia_session_cookie_count(cookies: List[Dict[str, Any]]) -> int:
    """検索に実際に使えるCookie（dspace / federated 宛）の件数を数える。"""
    count = 0
    for cookie in cookies or []:
        domain = str((cookie or {}).get("domain") or "")
        if any(_enovia_cookie_domain_covers(domain, h) for h in ENOVIA_SESSION_HOSTS):
            count += 1
    return count
...
DEFAULT_CFG: Dict[str, Any] = {
    ...
    # 起動時にポートが使用中だった場合、そのプロセスが「自分自身の旧起動」
    # だと確認できたときだけ自動終了して起動を続ける（既定true）。
    # 確認できない場合（無関係なプロセスの可能性）は、従来どおり起動を
    # 中止して案内する。falseにすると自動終了を一切行わず、常に案内のみ
    # （旧来の挙動）に戻せる。Windows以外では常に無効（対象コマンドが無いため）。
    "auto_kill_port_conflict": True,
    "page_size": 25,
    ...
    # S04: スコープ3択（"document" / "major" / "all"）。既定は現状維持の
    # "document"。旧キー enovia_document_type_only（bool）は後方互換のため
    # _load_config() で読み替える（true→"document" / false→"all"）。
    "enovia_type_scope_default": "document",
    "enovia_major_types": ["Document", "Project Space"],
    ...
    # 回路図PDFはスキャン画像ではなくベクター図面で、部品番号・ネット名・
    # ピン名がすべて「文字」として埋め込まれている。そのため数十万文字が
    # 正しく取り出せてしまうが、文章ではないため要約しても意味を成さない。
    # 下の2つの指標がどちらも閾値を下回るときに「図面が主体」と判定する。
    #   - 文末記号の密度：文章なら 30〜60 字に1つは 。．? ! が現れる
    #   - 1行あたりの平均文字数：図面の文字は単語1つで1行になる
    # 判定を厳しく（＝図面と見なしにくく）したい場合は値を小さくする。
    "pdf_drawing_check_min_chars": 5000,   # これ未満の短いPDFは判定しない
    "pdf_drawing_sentence_ratio": 0.005,   # 200字に1つ未満なら文章ではない
    "pdf_drawing_avg_line_chars": 12,      # 1行平均12字未満なら羅列とみなす
}
...
def _load_config() -> Dict[str, Any]:
    ...
    # S04: 旧キー enovia_document_type_only（bool）が実機のconfig.jsonに
    # 残っていて、新キー enovia_type_scope_default が無い場合はここで
    # 読み替える。新キーが明示されていれば、そちらを優先する（無視しない）。
    if "enovia_document_type_only" in loaded and "enovia_type_scope_default" not in loaded:
        migrated = "document" if loaded.get("enovia_document_type_only", True) else "all"
        cfg["enovia_type_scope_default"] = migrated
        print(f"ℹ️  config.json の enovia_document_type_only（旧キー）を "
              f"enovia_type_scope_default=\"{migrated}\" として読み替えました。"
              f"新しいキーへの移行を推奨します（config.example.json 参照）。")
...
class SearchResult:
    ...
    # ── Enovia全種別対応（S04, v20260924_01）─────────────────
    enovia_type: str = ""        # ds6w:what/ds6w:type の表示用ラベル（例: "Wiring Diagram"）
    enovia_type_raw: str = ""    # 同上、変換前の生値（例: "nex_WiringDiagram"）
    enovia_type_sparse: bool = False  # 主要列が軒並み空欄の型か（画面で注記を出す）
    # ── フォルダ探索（v20260910_02）─────────────────────
    # 検索結果と同じ型で扱うことで、絞り込み・並べ替え・選択・ZIP取得・
    # 要約といった既存の仕組みをそのまま再利用できる（DESIGN_NOTES 3-6）。
    item_id: str = ""            # driveItem の id（ツリーの節点の識別子）
    parent_item_id: str = ""     # 親フォルダの item_id（起点フォルダは空）
    drive_id: str = ""           # 所属ドライブの id（子要素の取得に必要）
    depth: int = 0               # 起点フォルダを 0 とした階層の深さ
    size: int = 0                # ファイルの大きさ（バイト。フォルダは0）
    child_count: int = 0         # フォルダが中に持つ件数（Graphの申告値）
    root_label: str = ""         # どの起点フォルダから見つかったか（表示用）
    path_text: str = ""          # 起点フォルダからの相対パス（表示用）
...
# ─────────────────────────────────────────────────────────────
# S05: SharePointのサイト絞り込み（診断のみ、v20260924_02）
#
# 敵対的プロダクトマネージャーレビューのB1で指摘されたとおり、
# _nexus_scope_url() はconfig.jsonに固定された信頼済みの値にしか使われて
# おらず、「ユーザーが検索のたびに自由入力する文字列」をKQLへ埋め込む
# ことは、このコードベースで一度も検証されていない。そのため、ここでは
# 許可文字のホワイトリストを設け、それ以外を含む入力はKQLへ渡さずに
# エラーとして扱う。
# ─────────────────────────────────────────────────────────────
SITE_SCOPE_ALLOWED_RE = re.compile(r'^[A-Za-z0-9_\-./:]+$')


def _validate_site_scope_input(text: str) -> str:
    """サイト名/URL入力を検証する。空文字（絞り込みなし）はそのまま通す。

    許可文字は英数字・_ - . / : のみ（KQLの制御文字である `"` 等を
    含む入力は拒否する）。前後の空白は取り除く。
    """
    text = str(text or "").strip()
    if not text:
        return ""
    if not SITE_SCOPE_ALLOWED_RE.match(text):
        # 2026-09-25 実機：どの文字がだめなのか分からず、原因の特定に手間取った
        # （コピー時に混ざった目に見えない文字だった）。だめな文字を示す。
        bad = []
        for ch in text:
            if not SITE_SCOPE_ALLOWED_RE.match(ch) and ch not in bad:
                bad.append(ch)
        shown = "、".join(_describe_char(ch) for ch in bad[:5])
        raise ValueError(
            f"サイト名/URLに使用できない文字が含まれています（{shown}）。"
            "使用できるのは英数字・_ - . / : のみです"
            "（コピーしたURLに余分な文字が混ざっていないかご確認ください）。"
        )
    return text


def _describe_char(ch: str) -> str:
    """エラー表示用に1文字を説明する。目に見えない文字は名前で示す。"""
    if ch == " ":
        return "空白"
    if ch == "　":
        return "全角空白"
    if unicodedata.category(ch) in ("Cc", "Cf", "Zs"):
        return f"目に見えない文字 U+{ord(ch):04X}"
    return f"「{ch}」"


# サイト内検索（S05本実装）で受け付けるサイトURLの種類。
# /personal/ は個人のOneDriveで、サイト検索の候補にも出ないため対象外。
SITE_SCOPE_KINDS = ("sites", "teams")


def _normalize_site_scope_url(url: str, cfg: Dict[str, Any]) -> str:
    """サイト内検索に使うサイトURLを検査し、サイトのトップURLに揃える。

    画面から届くURLはGraphが返したもの（サイト名検索の候補・検索結果の
    サイト列・お気に入り）だが、ブラウザ経由で届く値は改ざんされ得るため、
    `path:"..."`に埋め込む前に必ずここを通す。
      - 使える文字は `_validate_site_scope_input` と同じ（だめな文字は表示する）
      - https で、ホストが sharepoint_host（または .mcas.ms 経由の mcas_host）
      - パスが /sites/<名前> または /teams/<名前> で始まる
      - それより深いパス（ライブラリやフォルダ）は切り落とし、トップに揃える
    mcas_host 経由のURL（rewrite_host_to_mcas がオンのときの表示用リンク）は、
    検索条件としては sharepoint_host に戻す（インデックス上のURLはこちらのため）。

    文字の検査は、**トップURLに揃えた後の文字列（＝実際にKQLへ入る部分）**に
    対して行う。サイト名より後ろ（`/Shared Documents/...` の空白や `?id=...`）は
    捨てるので、そこに空白や記号があっても拒否しない。
    """
    ...
    sp_host = str(cfg.get("sharepoint_host") or "").strip().lower()
    mcas_host = str(cfg.get("mcas_host") or "").strip().lower()
    if parsed.scheme.lower() != "https" or not host:
        raise ValueError("サイトのURLは https:// で始まる必要があります。")
    if host not in {h for h in (sp_host, mcas_host) if h}:
        raise ValueError(f"社内のSharePoint（{sp_host}）以外のURLは指定できません。")
    parts = [p for p in parsed.path.split("/") if p]
    if len(parts) < 2 or parts[0].lower() not in SITE_SCOPE_KINDS:
        raise ValueError("サイトのURLは /sites/<サイト名> か /teams/<サイト名> の形で"
                         "指定してください。")
    # path:"..." に入るのはこの文字列だけ。ここで `"` 等を必ず拒否する。
    return _validate_site_scope_input(f"https://{sp_host}/{parts[0]}/{parts[1]}")


def _site_path_of(url: str) -> str:
    """表示用に、サイトURLから /sites/<名前> の部分だけを取り出す。"""
    parts = [p for p in urlparse(str(url or "")).path.split("/") if p]
    return "/" + "/".join(parts[:2]) if len(parts) >= 2 else str(url or "")


def _build_site_scope_url(site_input: str, cfg: Dict[str, Any]) -> str:
    """サイト名/URL入力から、KQLの `path:` に渡す絶対URLを組み立てる。

    `http(s)://` で始まる場合はフルURLとしてそのまま使う（`/teams/`・
    `/personal/` 系サイトなど、`/sites/` 以外の形式に対応するため）。
    それ以外は短縮名（例: "P010024_Lorry"）とみなし、config.json の
    sharepoint_host を使って `/sites/` 配下のURLに展開する。
    空文字は空文字のまま返す（呼び出し側が「絞り込みなし」として扱う）。
    """
    text = _validate_site_scope_input(site_input)
    if not text:
        return ""
    if text.lower().startswith("http://") or text.lower().startswith("https://"):
        return text.rstrip("/")
    host = str(cfg.get("sharepoint_host") or "").strip().rstrip("/")
    if not host:
        return ""
    return f"https://{host}/sites/{text.strip('/')}"


# サイト名検索（S05 Phase3-0b）で受け付ける検索語の最大文字数。
SITE_SEARCH_TERM_MAX = 100


def _sanitize_site_search_term(text: str) -> str:
    """サイト名検索の検索語を整える。

    サイト名には日本語や空白が入り得るため、`_validate_site_scope_input`
    （英数字と一部記号のみ許可）は使えない。代わりに、KQLの句を壊し得る
    ダブルクォート（全角の“”を含む）と、制御文字・書式文字（コピー時に
    混ざるゼロ幅スペース等。2026-09-25の実機で入力エラーの原因になった）
    だけを取り除き、連続する空白を1つにまとめる。長すぎる入力は切り詰める。
    空になった場合は空文字を返す。
    """
    text = str(text or "")
    text = "".join(
    ...
    return text[:SITE_SEARCH_TERM_MAX]
...
class SharePointProvider(SearchProvider):
    ...
    # S05: サイト内検索の対象サイト（トップURL。空なら全社検索）。
    # SearchManager が「1. SharePoint」タブの検索のときだけ設定する。
    # NexusProvider もこの属性を継承するが、常に空のまま使われる。
    site_scope_url: str = ""
    ...
    def _query_string(self, keyword: str) -> str:
        ...
        S05: 対象サイトが指定されていれば `path:"<サイトのトップURL>"` を足す。
        サイトのトップURLだけで、そのサイト内に絞り込めることは実機で確認済み
        （2026-09-25、P010024_Lorry）。キーワードが空ならサイト内の一覧になる。
        """
        expr = self._keyword_expr(keyword) if str(keyword or "").strip() else ""
        if self.site_scope_url:
            return f'{expr} path:"{self.site_scope_url}"'.strip()
        return expr
    ...
    def search(self, keyword: str, max_results: int) -> Dict[str, Any]:
        ...
        if self._mode == "site-drive":
            if self.site_scope_url:
                # 予備のサイト単位検索（fallback_site_urls）は path: を使えないため、
                # 黙って全社検索の結果を返さず、はっきり伝える。
                raise RuntimeError(
                    "サイト内検索は、全社横断検索（/search/query）が使える環境でのみ"
                    "動きます。現在はサイト単位検索モードのため使えません。")
            return self._search_by_site_drive(token, keyword, max_results)

        outcome = self._search_by_search_api(token, keyword, max_results)
        if self.site_scope_url:
            outcome["note"] = (f"サイト内検索: {_site_path_of(self.site_scope_url)}"
    ...
    # ── S05 Phase3-0: サイト絞り込み診断（未確認事項①②③の実測用） ──
    def diagnose_site_scope(self, keyword: str, site_input: str,
                            title_only: Optional[bool] = None,
                            prefix_search: Optional[bool] = None) -> Dict[str, Any]:
        """KQLの `path:` によるサイト絞り込みを実際に試し、結果をそのまま返す。

        本実装（画面のメイン検索への組み込み）の前に、以下を実測で
        確かめるための診断専用メソッド（メイン検索の挙動は一切変えない）。
          ① サイト名/URLでの絞り込みが実際に機能するか
          ② サイトのトップURLだけを指定した場合に0件にならないか
             （`path:` が前方一致かどうか）
          ③ 組み立てたKQLが意図した形になっているか
        判定は断定せず、該当件数・先頭数件・組み立てたKQLをそのまま返す。
        """
        keyword = str(keyword or "").strip()
        was_title_only, was_prefix = self.title_only, self.prefix_search
        if title_only is not None:
            self.title_only = bool(title_only)
        if prefix_search is not None:
            self.prefix_search = bool(prefix_search)
        try:
            scope_url = _build_site_scope_url(site_input, self.cfg)
        except ValueError as e:
            self.title_only, self.prefix_search = was_title_only, was_prefix
            return {"ok": False, "message": str(e)}

        try:
            keyword_expr = self._keyword_expr(keyword) if keyword else ""
            if scope_url and keyword_expr:
                query = f'{keyword_expr} path:"{scope_url}"'
            elif scope_url:
                query = f'path:"{scope_url}"'
            else:
                query = keyword_expr

            if not query:
                return {"ok": False, "message": "キーワードとサイトのどちらも空です。"}

            token = self._token()
            status, body, err = self._call_search_api(token, query, frm=0, size=10)
            if status != 200 or body is None:
                return {"ok": False, "message": f"HTTP {status}: {err}",
                        "query": query, "scope_url": scope_url}
            ...
            for i, hit in enumerate(hits):
                ...
                samples.append({
                    ...
                    "site": result.site, "is_folder": result.is_folder,
                })

            return {
                "ok": True, "query": query, "scope_url": scope_url,
                "total": total, "sample_count": len(samples), "samples": samples,
                "more": bool(container.get("moreResultsAvailable")),
            }
        finally:
            self.title_only, self.prefix_search = was_title_only, was_prefix

    def _site_search_request(self, term: str, size: int) -> Dict[str, Any]:
        """サイト名の検索に使う認証ヘッダー等を用意する（2方式で共通）。"""
        token = self._token()
    ...
    def _site_search_by_query(self, req: Dict[str, Any]) -> Dict[str, Any]:
        """① POST /search/query（entityTypes = site）でサイトを探す。

        方式ごとの結果（ok / status / total / candidates / fields / message）を
        返す。例外は投げない。関連度の高い順に並ぶ。
        """
        row: Dict[str, Any] = {"label": "① /search/query（entityTypes=site）",
                               "endpoint": "POST /search/query"}
        try:
            resp = http_req.post(
                f"{GRAPH_V1}/search/query", headers=req["headers"], timeout=req["timeout"],
                json={"requests": [{"entityTypes": ["site"],
                                    "query": {"queryString": req["term"]},
            ...
            if resp.status_code == 200:
                ...
                containers = (body.get("value") or [{}])[0].get("hitsContainers") or [{}]
                container = containers[0]
                resources = [h.get("resource") or {} for h in (container.get("hits") or [])]
                row.update(ok=True, total=container.get("total", len(resources)),
                           candidates=self._site_candidates(resources),
                           fields=sorted(resources[0].keys()) if resources else [])
            else:
                row.update(ok=False, message=resp.text[:300])
        except Exception as e:
            row.update(ok=False, status=0, message=str(e))
        return row

    def _site_search_by_sites_api(self, req: Dict[str, Any]) -> Dict[str, Any]:
        """② GET /sites?search=<語> でサイトを探す（①が使えないときの予備）。"""
        row: Dict[str, Any] = {"label": "② /sites?search=", "endpoint": "GET /sites?search="}
        try:
            resp = http_req.get(f"{GRAPH_V1}/sites", headers=req["headers"],
                                params={"search": req["term"]}, timeout=req["timeout"])
            row["status"] = resp.status_code
            if resp.status_code == 200:
                resources = (resp.json().get("value") or [])[:req["size"]]
                row.update(ok=True, total=len(resources),
                           candidates=self._site_candidates(resources),
                           fields=sorted(resources[0].keys()) if resources else [])
            else:
                row.update(ok=False, message=resp.text[:300])
    ...
    # ── S05 Phase3-0b: サイト名検索の診断 ─────────────────────
    def diagnose_site_search(self, term: str, size: int = 10) -> Dict[str, Any]:
        """サイト名の一部からサイトを探せるかを、2つの方式で実際に試す。

        本実装（「サイトを探す」→候補から選ぶ）の前に、今の権限
        （Sites.Read.All のみ）で使えるかを実測するための診断専用メソッド。
          ① POST /search/query（entityTypes = site）
          ② GET  /sites?search=<語>
        判定は断定せず、方式ごとに件数・候補（名前とURL）・返ってきた
        項目名をそのまま並べる。メイン検索の挙動は一切変えない。
        2026-09-25 実機：両方式とも使え、候補も同じだった（Lorry / P010008 / Jeep）。
        """
        term = _sanitize_site_search_term(term)
        if not term:
            return {"ok": False, "message": "サイト名（の一部）を入力してください。"}
        try:
            req = self._site_search_request(term, size)
        except Exception as e:
            return {"ok": False, "message": f"認証に失敗しました: {e}"}
        methods = [self._site_search_by_query(req), self._site_search_by_sites_api(req)]
        return {"ok": True, "term": term, "methods": methods}

    # ── S05 本実装: サイト名検索（「サイトを探す」ボタン） ─────────
    def search_sites(self, term: str, size: int = 15) -> Dict[str, Any]:
        """サイト名の一部からサイトの候補を探す。

        ① /search/query（entityTypes=site）を使う。今の文書検索と同じ仕組みで、
        関連度の高い順に並ぶため。①が失敗したとき（拒否・通信エラー等）だけ
        ② /sites?search= に自動で切り替える（どちらも実機で使えることを確認済み）。

        0件のときは、先頭の語だけで探し直す。表示名をまるごと入れると
        見つからない実例があったため（2026-09-25：「P010008 - Jeep pkg-spin」は
        0件、「P010008」なら見つかった）。ハイフンを空白に置き換える等の、
        実機で確かめていない加工はしない。
        """
        term = _sanitize_site_search_term(term)
        if not term:
        ...
        def run(r: Dict[str, Any]) -> Dict[str, Any]:
            row = self._site_search_by_query(r)
            if not row.get("ok"):
                fallback = self._site_search_by_sites_api(r)
                fallback["fallback_from"] = row
                row = fallback
            return row
        ...
        if row.get("ok") and not row.get("candidates") and first and first != term:
            retried_with = first
            row = run(dict(req, term=first))

        if not row.get("ok"):
            return {"ok": False, "term": term,
                    "message": (f"サイトを検索できませんでした（HTTP {row.get('status')}）: "
                                f"{row.get('message', '')}")}
        candidates = [c for c in (row.get("candidates") or []) if c.get("url")]
        return {"ok": True, "term": term, "retried_with": retried_with,
                "method": row.get("label", ""), "candidates": candidates}

    @staticmethod
    def _site_candidates(resources: List[Dict[str, Any]]) -> List[Dict[str, str]]:
        """Graphが返したサイト情報から、画面に出す「名前・URL・説明」を取り出す。

        名前は displayName（画面の表示名）を優先し、無ければ name を使う。
        同じ名前の別サイトを見分けられるよう、URLは必ず添える。
        """
        out: List[Dict[str, str]] = []
        for res in resources:
            out.append({
                "name": _pick(res.get("displayName"), res.get("name")),
                "url": _pick(res.get("webUrl")),
                "description": _pick(res.get("description"))[:120],
            })
        return out
...
class EnoviaProvider(SearchProvider):
    ...
    def _collect_cookies_while_open(self, context, page,
                                    timeout_sec: float) -> "tuple[list, bool]":
        """ウィンドウが開いている間、Cookieを取り続けて最良の内容を返す。

        【この処理が必要な理由（2026-09-11に実機で確認した不具合）】
        3DSpace / federated のセッションは、いずれも**セッションCookie**
        （JSESSIONID / SERVERID など、有効期限を持たないもの）で維持されている。
        セッションCookieはブラウザのメモリ上にしか無く、**ウィンドウを閉じた
        時点で破棄される**。
        従来の実装は「閉じるのを待つ → Cookieを取り出す」の順だったため、
        取り出したときには既に消えており、ディスクに保存済みの無関係な
        Cookie（Bing/M365等）だけが保存されていた。その結果、検索は必ず
        invalid_grant になっていた。

        そこで、閉じるのを待つ間に1秒ごとに取得し、**検索に使えるCookieを
        最も多く含んでいた時点の内容**を採用する。「最も多く」で選ぶのは、
        越智さんがEnovia画面で検索した瞬間に federated 側のセッションが
        増えるため、その後で減ることはあっても、最良点を逃さないようにするため。

        戻り値は (Cookieの一覧, 利用者がウィンドウを閉じたか)。
        """
        ...
        while time.monotonic() < deadline:
            ...
            try:
                ...
                # 終了処理に入ると取得できなくなる。直前の内容を活かす。
                closed = True
                break

            if current:
                latest = current
                count = _enovia_session_cookie_count(current)
                if count > best_count:
                    best_count = count
                    best = current
    ...
    def login_interactive(self) -> Dict[str, Any]:
        ...
        try:
            with sync_playwright() as pw:
                ...
                try:
                    ...
                    return {"ok": False, "message": _enovia_launch_error_message(e)}
                ...
                try:
                    ...
                    try:
                        ...
                        nav_error = _enovia_navigation_error_message(base_url, e)
                    if nav_error is None:
                        cookies, closed_by_user = self._collect_cookies_while_open(
                            context, page, timeout_ms / 1000.0)
                finally:
                    try:
                        context.close()
                    except Exception:
            ...
            return {"ok": False,
                    "message": f"ログイン用のブラウザ操作に失敗しました: {e}"}

        if nav_error:
            return {"ok": False, "message": nav_error}
        ...
        usable = _enovia_session_cookie_count(cookies)
        message = (f"Enoviaのログイン情報を保存しました"
                   f"（Cookie {len(cookies)} 件 / うち検索に使えるもの {usable} 件）。")
        if usable == 0:
            # ここで黙って成功扱いにすると、検索して初めて失敗が分かる。
            # 原因と対処をこの場で伝える（2026-09-11の不具合の教訓）。
            message += ("\n⚠️ 検索に使えるCookieが取得できていません。"
                        "検索は失敗します。Enoviaの画面が表示された後、"
                        "**画面上部の検索ボックスで1回検索してから**"
                        "ウィンドウを閉じてください。")
        elif not any(_enovia_cookie_domain_covers(str(c.get("domain") or ""),
                                                  "federated.plm.nexperia.com")
                     for c in cookies):
            message += ("\n⚠️ 検索API（federated）のCookieが取得できていません。"
                        "Enoviaの画面で1回検索してからウィンドウを閉じると取得できます。")
    ...
    # S04: nex_ 接頭辞を除去し、小文字/数字の直後の大文字の前に空白を挿入する
    # 機械的な変換ルール。対応表は持たず、実機のEnovia画面表示（Type列）との
    # 一致を2件（nex_Kit→Kit、nex_WiringDiagram→Wiring Diagram）実測で確認
    # 済みだが、全191型で検証したわけではない（DESIGN_NOTES 3-3参照）。
    _TYPE_LABEL_BOUNDARY_RE = re.compile(r"(?<=[a-z0-9])(?=[A-Z])")

    @classmethod
    def _enovia_type_label(cls, raw: str) -> str:
        if not raw:
            return ""
        text = raw[len("nex_"):] if raw.startswith("nex_") else raw
        spaced = cls._TYPE_LABEL_BOUNDARY_RE.sub(" ", text).strip()
        return spaced or raw

    def _type_allowed(self, enovia_type: str) -> bool:
        """S04: スコープ3択（"document" / "major" / "all"）による絞り込み。

        検索クエリ自体（federated/searchへのリクエスト）は変えず、応答から
        どの行を表に残すかだけをここで判定する。
        """
        scope = getattr(self, "type_scope", None) or \
            self.cfg.get("enovia_type_scope_default", "document")
        if scope == "all":
            return True
        if scope == "major":
            major_types = self.cfg.get("enovia_major_types") or ["Document", "Project Space"]
            return enovia_type in major_types
        return enovia_type == "Document"  # scope == "document"（既定）

    def _item_to_result(self, item: Dict[str, Any], rank: int) -> Optional[SearchResult]:
        attrs = self._attrs_of(item)
        enovia_type_raw = _field_text(attrs.get("ds6w:what/ds6w:type"))
        if not self._type_allowed(enovia_type_raw):
            return None  # D2/S04: スコープで絞る（191型のクエリ自体は変えない）
        ...
        return SearchResult(
            ...
            is_folder=False,
            rank=rank,
            enovia_type=self._enovia_type_label(enovia_type_raw),
            enovia_type_raw=enovia_type_raw,
            # S04/B6: 実機診断で「フォルダ」「最終更新者」が型によっては
            # 構造的に空欄になることを確認した。両方とも空欄の行は、
            # 取得漏れではなく型の仕様である旨を画面側で注記する。
            enovia_type_sparse=(not folder and not last_modified_by),
        )
    ...
    def search(self, keyword: str, max_results: int) -> Dict[str, Any]:
        ...
        if scope == "document":
            ...
            note_parts.append(
                "件数はEnoviaの検索対象全種別の合計です。表の行はよく使う型"
                "（Document・Project Space）に絞っているため、件数より少なくなります。"
            )
        # scope == "all" のときは、該当件数と表の行数が一致するため注記なし。
        if self.title_only:
            note_parts.append(
        ...
        while len(results) < max_results and pages < max_pages:
            ...
            for item in items:
                ...
                if result is None:
                    continue  # スコープ対象外の型（D2/S04の絞り込み）
                rank += 1
                results.append(result)
    ...
    # ── S04 Phase3-0: 型診断（未確認事項①③④の実測） ──────────
    def diagnose_types(self, keyword: str, max_pages: int = 5) -> Dict[str, Any]:
        """全種別（191型）で検索し、型ごとに件数・属性・URLを実測する。

        S04（Enovia型拡張）の本実装に入る前の診断機能。DESIGN_NOTES 3-3の
        S04節にある未確認事項①（型ごとに返る属性）③（英語名と日本語ラベルの
        対応）④（型ごとに列構成を変えるべきか）を、この結果を見て判断する。

        既定の `enovia_document_type_only` の値には関係なく、常に全型を対象に
        取得する（この診断の目的そのものがDocument以外を見ることのため）。
        取得は `max_pages` ページで打ち切るサンプル調査であり、Enovia画面の
        ファセット件数（全件集計）とは性質が異なることを結果に明記する。
        判定は断定しない（候補を挙げる）。Cookie・トークンは扱わない。
        """
        keyword = str(keyword or "").strip()
        if not keyword:
            return {"ok": False, "message": "キーワードが指定されていません。"}
        ...
        type_samples: Dict[str, List[Dict[str, Any]]] = {}   # 型 → 代表1件の生属性配列
        type_present: Dict[str, Dict[str, int]] = {}          # 型 → {属性キー: 値あり件数}
        type_url: Dict[str, str] = {}
        base_url = str(self.cfg.get("enovia_base_url") or "").rstrip("/")
        ...
        while pages < max_pages:
            ...
            status, data, err = self._call_search(session, body)
            if status != 200 or data is None:
                error_message = f"HTTP {status}: {err}"
                break

            infos = data.get("infos") or {}
            nhits = infos.get("nhits", nhits) or nhits
            next_start_token = infos.get("next_start")
            items = data.get("results") or []
            if not items:
                break
            ...
            for item in items:
                attrs = self._attrs_of(item)
                enovia_type = _field_text(attrs.get("ds6w:what/ds6w:type")) or "(型不明)"

                type_counts[enovia_type] = type_counts.get(enovia_type, 0) + 1
                present = type_present.setdefault(enovia_type, {})
                for attr_key, _label in ENOVIA_TYPE_DIAG_ATTRS:
                    if _field_text(attrs.get(attr_key)):
                        present[attr_key] = present.get(attr_key, 0) + 1

                if enovia_type not in type_samples:
                    # 属性を推測で選ばず、返ってきたものをそのまま並べる。
                    type_samples[enovia_type] = list(item.get("attributes") or [])
                    resourceid = _field_text(attrs.get("resourceid"))
                    if resourceid and base_url:
                        type_url[enovia_type] = (
                            f"{base_url}/common/emxNavigator.jsp?"
                            f"objectId={quote(resourceid, safe='')}")

            if next_start_token is None:
                break

        rows = []
        for enovia_type, count in sorted(type_counts.items(),
                                         key=lambda kv: kv[1], reverse=True):
            present = type_present.get(enovia_type, {})
            required = [{"label": label,
                        "present": present.get(attr_key, 0),
                        "total": count}
                       for attr_key, label in ENOVIA_TYPE_DIAG_ATTRS]
            rows.append({
                "type": enovia_type,
                "count": count,
                "sample_attrs": type_samples.get(enovia_type, []),
                "required": required,
                "example_url": type_url.get(enovia_type, ""),
            })

        return {
            "ok": not error_message or bool(rows),
            "message": error_message,
            "keyword": keyword,
            "nhits": nhits,
            "fetched_raw": fetched_raw,
            "pages_fetched": pages,
            "truncated": bool(next_start_token) and pages >= max_pages,
            "rows": rows,
        }
...
class SearchManager:
    ...
    def search(self, keyword: str, target: str, max_results: int,
               title_only: bool = False, prefix_search: bool = False,
               type_scope: Optional[str] = None,
               site_scope: str = "") -> Dict[str, Any]:
        """検索を実行する。site_scope（S05）は「1. SharePoint」タブのときだけ効く。

        対象サイトはSharePointプロバイダにだけ渡し、検索が終わったら必ず空に
        戻す。戻さないと、起動時の疎通診断や別タブの検索まで、気づかないうちに
        そのサイト内に絞られてしまうため。呼び出し側（/api/search）で検査済みの
        トップURLが渡される前提。
        """
        sharepoint = self.providers.get(TARGET_SHAREPOINT)
        scope = site_scope if (target == TARGET_SHAREPOINT and site_scope) else ""
        if sharepoint is not None:
            sharepoint.site_scope_url = scope
        try:
            return self._search(keyword, target, max_results, title_only=title_only,
                                prefix_search=prefix_search, type_scope=type_scope)
        finally:
            if sharepoint is not None:
                sharepoint.site_scope_url = ""

    def _search(self, keyword: str, target: str, max_results: int,
                title_only: bool = False, prefix_search: bool = False,
                type_scope: Optional[str] = None) -> Dict[str, Any]:
        ...
        for provider in providers:
            ...
            # type_scope はEnoviaProviderにのみ意味を持つ属性（S04）。他の
            # プロバイダには属性が付くだけで、参照されないため無害。
            provider.type_scope = type_scope
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* S05: 対象サイトのチップと候補一覧。display を指定するクラスには、
     必ず [hidden] の行を対で書く（DESIGN_NOTES 5-8、過去2回の再発）。 */
  .sitechip { display: inline-flex; align-items: center; gap: 6px;
              border: 1px solid var(--accent); border-radius: 14px;
              padding: 2px 4px 2px 10px; background: var(--panel2); }
  .sitechip[hidden] { display: none; }
  .sitecands { display: flex; flex-wrap: wrap; gap: 6px; align-items: center;
               margin: 6px 0 0 2px; }
  .sitecands[hidden] { display: none; }
  button.sitego { margin-left: 6px; padding: 0 6px; }
  ...
  .summary-confirm-row { display: flex; gap: 10px; margin-top: 14px; }
  ...
  /* フォルダ探索（ツリー表示）。階層は左の余白の幅だけで表す。
     罫線を引くとフィルタ・並べ替えのある一覧表示と見た目が食い違うため。 */
  .tindent { display: inline-block; height: 1px; vertical-align: middle; }
  /* 開閉の記号。以前は10pxで小さく押しにくかった（越智さんのご指摘、
     2026-09-25）ため、記号を大きくし、押せる範囲も広げた。 */
  .ttoggle {
    display: inline-block; width: 24px; height: 24px; line-height: 24px;
    margin-right: 2px; text-align: center; vertical-align: middle;
    cursor: pointer; color: var(--text); user-select: none;
    font-size: 15px; border-radius: 5px;
  }
  .ttoggle:hover { color: var(--accent); background: var(--panel2); }
  .ttoggle.lazy { color: var(--accent); }
  .ttoggle.loading { cursor: wait; }
  .ttoggle.empty { cursor: default; }
  .ttoggle.empty:hover { color: var(--text); background: transparent; }
  .tnote { color: var(--muted); font-size: 12px; margin-left: 6px; }
  select.lvsel { padding: 4px 8px; font-size: 12px; }
  .ticon { display: inline-block; margin-right: 5px; }
  .viewtoggle { display: inline-flex; gap: 2px; margin-right: 6px; }
  /* display を指定した要素では、hidden属性（ブラウザ既定の display:none）が
     負けてしまい、隠したはずのものが表示され続ける。過去に同じ原因の不具合を
     出しているため（DESIGN_NOTES 5-8）、display指定と対で必ず書く。 */
  .viewtoggle[hidden] { display: none; }
  .viewtoggle button.on { background: var(--accent); color: #fff; border-color: var(--accent); }
  .explorenote { color: var(--muted); font-size: 12px; margin: 8px 0 0 2px; }
</style>
</head>
<body>
<header>
  <h1>Document Search Manager</h1>
  <div class="sub">社内ドキュメント横断検索 &nbsp;/&nbsp; v20260925_04 (探索の深さ指定)</div>
</header>
...
<main>
  <div class="panel">
    <div class="row">
      ...
      <button id="btnEnoviaLogin" class="ghost" title="Edgeが開きます。Enoviaにログイン（SSOで自動的に入る場合はそのまま）した後、画面上部の検索ボックスで1回検索してから、そのウィンドウを閉じてください。検索を1回行わないと、検索API（federated）のログイン情報が発行されません。">Enoviaにログイン</button>
      <button id="btnEnoviaDiag" class="ghost" title="Enoviaのタイトル限定検索に使えそうな構文をいくつか試し、件数を比較します（構文は実機未確認のため）。">Enovia検索診断</button>
      <button id="btnEnoviaTypeDiag" class="ghost" title="Enoviaの全種別（Document以外を含む191型）で検索し、型ごとの件数・返ってくる属性・「Enoviaで開く」URLを実測します（S04: Document以外の型に対応する前の事前確認）。">Enovia型診断</button>
      <button id="btnExploreDiag" class="ghost" title="フォルダ探索機能の実装前確認です。検索結果の最初のフォルダについて、配下を再帰的にたどれるかを実際に試します（読み取りのみ）。">フォルダ探索診断</button>
    </div>
    <div class="row">
    ...
    <div id="siteScopeBox" hidden>
      <div class="row">
        <span class="opt" title="SharePointタブだけで使えます。サイトを選ぶと、そのサイトの中だけを検索します（キーワードが空ならサイト内の一覧）。選んだサイトは次回起動時には引き継ぎません。">対象サイト:</span>
        <span class="sitechip" id="siteChip" hidden>
          📍 <strong id="siteChipName"></strong>
          <span class="muted" id="siteChipPath"></span>
          <button type="button" class="mini" id="btnFavToggle" title="よく使うサイトに登録／解除します">☆</button>
          <button type="button" class="mini" id="btnClearSite" title="サイトの指定を外して、全社検索に戻します">×</button>
        </span>
        <span class="muted" id="siteNone">指定なし（全社検索）</span>
        <input type="text" id="siteSearchInput" placeholder="サイト名の一部（例: P010008 / Lorry）"
               title="サイト名の一部を入れて「サイトを探す」を押すと、候補が出ます。表示名をまるごと入れて見つからないときは、先頭の語（番号など）で自動的に探し直します。">
        <button type="button" id="btnSiteSearch" class="ghost">サイトを探す</button>
        <select id="favoriteSites" title="☆で登録したサイトから選びます">
          <option value="">★ よく使うサイト</option>
        </select>
      </div>
      <div class="sitecands" id="siteCandidates" hidden></div>
    </div>
    <div class="row" id="enoviaScopeRow" hidden>
      <label class="opt" title="Enoviaの検索対象を型で絞り込みます。①Documentのみ：既定の絞り込み（従来どおり）。②よく使う型：Document・Project Spaceに絞ります。③全種別：Wiring Diagram・Kit等、Document以外もすべて表示します（部品番号での横断検索など）。選んだ内容は次回起動時にも引き継がれます。">
        Enoviaの検索対象:
        <select id="enoviaScope">
          <option value="document" selected>①Documentのみ</option>
          <option value="major">②よく使う型（Document・Project Space）</option>
    ...
    <div class="tabs" id="tabs">
      ...
      <button type="button" id="tabExplore" data-target="explore" hidden><span class="tabmark">4.</span> フォルダ探索</button>
    </div>
  ...
  <div class="panel">
    <div class="row" style="justify-content: space-between;">
      ...
      <div class="row">
        <span class="viewtoggle" id="viewToggle" hidden>
          <button id="btnTreeView" class="mini on" title="階層のまま、フォルダの入れ子構造で表示します（絞り込み・並べ替えは使えません）。">ツリー表示</button>
          <button id="btnFlatView" class="mini" title="階層を平らにして一覧表示します。種別・名前・最終更新日での絞り込みと並べ替えが使えます。">一覧表示</button>
        </span>
        <select id="exploreLevel" class="lvsel" hidden title="どの深さまで一度に読み込むかを選びます。チェックしたフォルダがLv0、そのすぐ下がLv1、もう一つ下がLv2です。その先のフォルダは、ツリーの▶で開いたときに1階層ずつ読み込みます。選んだ深さは次回起動時も使います。">
          <option value="1">深さ Lv1</option>
          <option value="2" selected>深さ Lv2（標準）</option>
          <option value="3">深さ Lv3</option>
          <option value="4">深さ Lv4</option>
        </select>
        <button id="btnExplore" class="mini" hidden disabled title="「探索」に印を付けたフォルダの配下を、左で選んだ深さ（標準Lv2）まで一覧にします（上限2000件）。その先は、ツリーの▶で開いたときに読み込みます。">選択フォルダを探索</button>
    ...
    <div class="explorenote" id="exploreNote" hidden></div>
    <div class="tablewrap">
...
var ICON_FOLDER  = "\u{1F4C2}";
var ICON_DRAWING = "\u{1F4D0}";
var ICON_FILE    = "\u{1F4C4}";
var TRI_OPEN     = "\u{25BC}";
var TRI_CLOSED   = "\u{25B6}\u{FE0E}";
var TRI_LOADING  = "\u{23F3}";
...
var COLUMN_SETS = {
  ...
  sharepoint: [
    { key: "title",         label: "タイトル",             type: "set"    },
    { key: "__explore",     label: "探索",                type: "explore"},
    { key: "__select",      label: "選択",                type: "select" },
    { key: "author",        label: "作成者",               type: "set"    },
    { key: "last_modified", label: "最終更新日",           type: "date"   },
    { key: "doc_type",      label: "種別",                type: "set"    },
  ...
  // フォルダ探索の結果。ツリー表示・一覧表示のどちらもこの列構成を使う
  // （表示の仕方だけを変え、扱うデータは同じ1組にする）。
  explore: [
    { key: "__select",      label: "選択",                type: "select" },
    { key: "title",         label: "名前",                type: "set"    },
    { key: "doc_type",      label: "種別",                type: "set"    },
    { key: "last_modified", label: "最終更新日",           type: "date"   },
    { key: "size",          label: "サイズ",               type: "num"    },
    { key: "author",        label: "作成者",               type: "set"    },
    { key: "path_text",     label: "場所",                type: "set"    },
    { key: "__summary",     label: "要約",                type: "link"   }
  ],
  ...
  enovia: [
    ...
    // S04: Enoviaの「型」（ds6w:what/ds6w:type）専用の列。既存の「種別」列
    // （下の doc_type、拡張子）とは別概念のため、混同を避けて分けている。
    { key: "enovia_type",      label: "Enovia型",            type: "set"    },
...
// ── フォルダ探索の状態 ───────────────────────────────────
var exploreSelected = {};   // 検索結果側で「探索」に印を付けた行 {_idx: true}
var exploreData = [];       // 直近の探索結果（サーバーから受け取ったそのまま）
var exploreTreeMode = true; // true=ツリー表示 / false=一覧表示
var collapsed = {};         // ツリー表示で閉じている節点 {item_id: true}
var treeChildren = {};      // 親item_id → 子行の配列（開閉記号の要否の判定に使う）

function isExploreView() { return shownTarget === "explore"; }
function isTreeView() { return isExploreView() && exploreTreeMode; }
...
// 親子関係から、ツリー表示の並び順（深さ優先）を組み立てる。
// 閉じている節点の配下は結果に含めない（＝画面から消える）。
//
// 種別の絞り込み（例: PDFだけ見たい）は、ツリー表示でも使えるようにする。
// ただし、条件に合う行だけを残すと親フォルダが消えて階層が壊れるため、
// **条件に合う行の祖先フォルダは必ず残す**。これが「ツリーでも種別だけは
// 絞り込める」という承認済み設計の実現方法（DESIGN_NOTES 3-6）。
function buildTreeOrder(rows) {
  ...
  rows.forEach(function (r) {
    byId[r.item_id] = r;
    if (!r.parent_item_id) { roots.push(r); return; }
    if (!byParent[r.parent_item_id]) { byParent[r.parent_item_id] = []; }
    byParent[r.parent_item_id].push(r);
  });
  treeChildren = byParent;

  var keep = null;
  if (isFiltered("doc_type")) {
    keep = {};
    rows.forEach(function (r) {
      if (!rowPasses(r)) { return; }
      keep[r.item_id] = true;
      var parent = byId[r.parent_item_id];
      while (parent && !keep[parent.item_id]) {
        keep[parent.item_id] = true;
        parent = byId[parent.parent_item_id];
      }
    });
  }

  function sortNodes(list) {
    // フォルダを先に、そのあと名前順。エクスプローラーと同じ並びにする。
  ...
  var out = [];
  function walk(node) {
    if (keep && !keep[node.item_id]) { return; }
    out.push(node);
    if (node.is_folder && collapsed[node.item_id]) { return; }
    var kids = byParent[node.item_id];
    if (kids) { sortNodes(kids.slice()).forEach(walk); }
  }
  sortNodes(roots.slice()).forEach(walk);
  return out;
}
...
function cacheKey(keyword, target, maxResults, titleOnly, prefixSearch, typeScope, siteScope) {
  ...
  // 内容を変えないため、キーには含めない。
  // S04（敵対的レビューB3）: typeScope はサーバー側の絞り込みを変える
  // （EnoviaProvider._type_allowed）ため、folderOnlyとは逆にキーに含める。
  // 含めないと、スコープを切り替えても古い結果がキャッシュから再表示される。
  return keyword + "" + target + "" + maxResults + "" + titleOnly
       + "" + prefixSearch + "" + (typeScope || "")
       // S05（論点B2）: 対象サイトもサーバー側の絞り込みを変えるため、キーに含める。
       // 含めないと、サイトを切り替えても前のサイトの結果が再表示される。
       + "" + (siteScope || "");
}
...
function compareRows(a, b) {
  ...
  var c;
  if (key === "size") {
    // 文字列として比べると "9 KB" > "10 MB" のような取り違えが起きるため、
    // サイズだけは数値のまま比べる。
    var nx = Number(x) || 0, ny = Number(y) || 0;
    c = (nx < ny) ? -1 : (nx > ny) ? 1 : 0;
  }
  else if (key === "last_modified") { c = (x < y) ? -1 : (x > y) ? 1 : 0; }
...
function applyView() {
  if (isTreeView()) {
    // ツリー表示では、絞り込み・並べ替えを掛けない。親だけが消えたり
    // 順番が入れ替わったりすると、階層そのものが意味を失うため。
    // 絞り込み・並べ替えは「一覧表示」側の役割にしている。
    viewResults = buildTreeOrder(allResults);
  } else {
...
function saveState() {
  // フォルダ探索タブは保存しない。保存すると、次回起動時に target="explore"
  // で検索しようとして、前回の検索状態の復元そのものが壊れるため。
  ...
  var body = {
    ...
    enovia_type_scope: document.getElementById("enoviaScope").value,
    // S05: 名前だけを残す（次回起動時に「前回はサイト内検索だった」と知らせるため。
    // サイトの指定そのものは引き継がない）。
    site_scope_name: (currentTarget() === "sharepoint" && selectedSite)
                     ? (selectedSite.name || sitePathOf(selectedSite.url)) : "",
    explore_level: parseInt(document.getElementById("exploreLevel").value, 10) || 2,
    sort: sortState,
    filters: filters
  };
  fetch("/api/state", {
...
function renderHead() {
  ...
  COLUMNS.forEach(function (col) {
    ...
    // ツリー表示では並べ替えを使わないため、見出しは表示だけにして
    // 「押せそうに見えるのに効かない」状態を作らない。ただし種別の絞り込み
    // だけは、階層を保ったまま適用できるためツリー表示でも残す。
    var treeFilterable = isTreeView() && col.key === "doc_type";
    var plainHeader = isTreeView() && col.type !== "select";
    ...
    if (col.type === "link" || col.type === "explore"
        || (plainHeader && !treeFilterable)) {
    ...
    if (plainHeader) {
      // ツリー表示では並べ替えない（階層が壊れるため）。押せる見た目にしない。
    ...
    // サイズは連続値で、値ごとのチェックボックス一覧にしても選びようがない
    // ため、並べ替えだけを提供する（絞り込みは付けない）。
    if (col.type !== "num") {
...
function buildCell(r, col) {
  ...
  if (col.key === "title") {
    ...
    if (isTreeView()) {
      // 階層は左の余白の幅で表す。1階層あたり18px。
      var indent = document.createElement("span");
      indent.className = "tindent";
      indent.style.width = (Number(r.depth || 0) * 18) + "px";
      td.appendChild(indent);

      var kids = treeChildren[r.item_id];
      var toggle = document.createElement("span");
      if (r.is_folder && kids && kids.length) {
        toggle.className = "ttoggle";
        toggle.textContent = collapsed[r.item_id] ? TRI_CLOSED : TRI_OPEN;
        toggle.title = collapsed[r.item_id] ? "このフォルダを開く" : "このフォルダを閉じる";
        toggle.addEventListener("click", function () {
          if (collapsed[r.item_id]) { delete collapsed[r.item_id]; }
          else { collapsed[r.item_id] = true; }
          applyView();
        });
      } else if (r.is_folder && !r._loaded && Number(r.child_count || 0) > 0) {
        // 指定した深さより先のフォルダ。▶を押すと中身（次の1階層）を読み込む。
        var loadingThis = (exploreLoadingId === r.item_id);
        toggle.className = "ttoggle lazy" + (loadingThis ? " loading" : "");
        toggle.textContent = loadingThis ? TRI_LOADING : TRI_CLOSED;
        toggle.title = loadingThis ? "読み込んでいます…"
                                   : "このフォルダを開く（中身 " + r.child_count + " 件を読み込みます）";
        toggle.addEventListener("click", function () { loadExploreChildren(r); });
      } else {
        // 中身が無いフォルダ・ファイルは開閉できない。記号の分だけ幅を空けて
        // 名前の開始位置を揃える。
        toggle.className = "ttoggle empty";
        toggle.textContent = "\u00a0";
      }
      ...
      icon.textContent = r.is_folder ? ICON_FOLDER : ICON_FILE;
      td.appendChild(icon);
    } else if (isExploreView()) {
      // 一覧表示では階層が見えないため、フォルダとファイルを記号で見分ける。
      var icon2 = document.createElement("span");
      icon2.className = "ticon";
      icon2.textContent = r.is_folder ? ICON_FOLDER : ICON_FILE;
    ...
    if (isTreeView() && r.is_folder) {
      var tnote = null;
      if (!r._loaded && !treeChildren[r.item_id] && Number(r.child_count || 0) > 0) {
        tnote = "（" + r.child_count + "件・未読込）";
      } else if (r._loaded && r._empty) {
        tnote = "（空）";
      }
      if (tnote) {
        var tn = document.createElement("span");
        tn.className = "tnote";
        tn.textContent = tnote;
        td.appendChild(tn);
      }
    }
    // S04/B6: Enoviaは型によって「フォルダ」「最終更新者」が構造的に
    // 空欄になることを実機診断で確認した（取得漏れではなく型の仕様）。
    // 誤解を避けるため、該当行に注記アイコンを添える（列は作り分けない）。
    if (r.source === "Enovia" && r.enovia_type_sparse) {
      var sparseNote = document.createElement("span");
      sparseNote.className = "muted";
      sparseNote.textContent = " ⓘ";
      sparseNote.title = "この型（" + (r.enovia_type || "不明") + "）では、"
        + "フォルダ・最終更新者などの項目が元々取得できません"
        + "（Enovia型診断で確認済みの仕様であり、不具合ではありません）。";
      td.appendChild(sparseNote);
    }
    return td;
  }
  ...
  if (col.key === "size") {
    td.className = "nowrap";
    td.textContent = r.is_folder ? "" : formatSize(r.size);
    return td;
  }

  if (col.key === "__explore") {
    td.className = "selcol";
    var canExplore = !!(r.is_folder && r.url && r.source !== "Enovia");
    var ebox = document.createElement("input");
    ebox.type = "checkbox";
    ebox.checked = !!exploreSelected[r._idx];
    ebox.disabled = !canExplore;
    ebox.title = canExplore
      ? "このフォルダの配下を再帰的に探索する"
      : "フォルダの行だけが探索できます";
    ebox.addEventListener("change", function () {
      if (ebox.checked) { exploreSelected[r._idx] = true; }
      else { delete exploreSelected[r._idx]; }
      updateCounts();
    });
    td.appendChild(ebox);
    return td;
  ...
  if (col.key === "site") {
    ...
    if (r.site_url) {
      ...
      // S05（案B）: 検索結果から、その文書があるサイトを選んでサイト内検索する。
      // SharePointの行だけに付ける（Nexusは専用タブがあるため）。
      if (r.source === "SharePoint") {
        var go = document.createElement("button");
        go.type = "button";
        go.className = "mini sitego";
        go.textContent = "🔍";
        go.title = "このサイト内で検索: " + sitePathOf(r.site_url);
        go.addEventListener("click", function (e) {
          e.stopPropagation();
          selectSite({ name: r.site || "", url: r.site_url }, true);
        });
        td.appendChild(go);
      }
    } else {
...
// 対象は SharePoint / Nexus の .docx / .pptx のみ。判定はサーバー側
// （_summarizable_reason）と同じ考え方をクライアント側でも行い、
// 対象外の行はボタンを最初から無効化する（押してからエラーになる体験を避ける）。
var SUMMARIZABLE_EXTENSIONS = ["docx", "pptx", "xlsx", "xlsm", "pdf"];
function summaryUnavailableReason(r) {
  ...
  if (SUMMARIZABLE_EXTENSIONS.indexOf(ext) === -1) {
    var formats = SUMMARIZABLE_EXTENSIONS.map(function (e) { return "." + e; }).join("/");
    return "「." + (ext || "不明") + "」形式は現在、要約に対応していません（現在の対応形式: " + formats + "）。";
  }
  return "";
}
...
function openSummaryPopup(r) {
  ...
  requestSummary(r, body, overlay, false, false);
}

function requestSummary(r, body, overlay, confirmTruncated, confirmDrawing) {
  body.innerHTML = "";
  ...
  fetch("/api/summarize", {
    ...
    body: JSON.stringify({
      idx: String(r._idx),
      scope: isExploreView() ? "explore" : "search",
      confirm_truncated: confirmTruncated,
      confirm_drawing: confirmDrawing,
    }),
  })
    .then(function (resp) {
    ...
    .then(function (result) {
      ...
      if (result.data.drawing_pdf) {
        renderDrawingPdfNotice(body, result.data, function () {
          // 「それでも要約する」を選んだ場合は、切り詰めの確認も済んだものと
          // して続行する（本文の総字数はこの案内にも出しているため）。
          requestSummary(r, body, overlay, true, true);
        });
        return;
      }
      if (result.data.needs_confirmation) {
        renderTruncationConfirm(body, result.data, function () {
          requestSummary(r, body, overlay, true, confirmDrawing);
        });
        return;
      }
...
// 本文が上限を超えて切り詰められる場合、Geminiを呼ぶ前に確認する
// （越智さんのフィードバック：切り詰めに気づかず部分的な要約だけを
// 見てしまうのを防ぐため）。
// 回路図など、図面が主体のPDFの案内。要約を押し付けず、
// 「ファイルを開いた方が早い」という判断をそのまま伝える（越智さんの方針）。
function renderDrawingPdfNotice(box, data, onConfirm) {
  box.innerHTML = "";

  var head = document.createElement("p");
  head.textContent = ICON_DRAWING + " このPDFは図面が主体と判断しました。"
                   + "本文の要約は行いません。";
  ...
  note.textContent = "回路図のような図面PDFは、部品番号・ネット名・ピン名が"
                   + "すべて文字として埋め込まれているため、文字数だけは"
                   + "大きくなります（このファイルは "
                   + (data.total_chars || 0).toLocaleString() + " 字）。"
                   + "ただし文章ではないため、要約しても内容は分かりません。"
                   + "ファイルを開いてご覧いただく方が確実で、早いはずです。";
  box.appendChild(note);

  var row = document.createElement("div");
  row.className = "summary-confirm-row";
  ...
  anywayBtn.className = "mini";
  anywayBtn.textContent = "それでも要約する";
  anywayBtn.title = "図面の文字の羅列をそのままAIに渡します。"
                  + "意味のある要約にならない可能性が高い操作です。";
  anywayBtn.addEventListener("click", onConfirm);
  row.appendChild(anywayBtn);
...
function renderTruncationConfirm(box, data, onConfirm) {
  box.innerHTML = "";
  var msg = document.createElement("p");
  msg.textContent = "本文が " + data.total_chars.toLocaleString() + " 字あり、"
                   + "現在の上限（" + data.max_chars.toLocaleString() + " 字）を"
                   + "超えています。先頭 " + data.max_chars.toLocaleString() + " 字だけで"
                   + "要約を進めますか？（文書全体の要約にはならない可能性があります）";
  box.appendChild(msg);

  var row = document.createElement("div");
  row.className = "summary-confirm-row";
  var yesBtn = document.createElement("button");
  yesBtn.textContent = "続ける（先頭部分だけで要約）";
  yesBtn.addEventListener("click", onConfirm);
  row.appendChild(yesBtn);
  var noBtn = document.createElement("button");
  noBtn.className = "mini";
  noBtn.textContent = "やめる";
  noBtn.addEventListener("click", closeSummaryPopup);
...
function updateExploreUi() {
  ...
  var count = Object.keys(exploreSelected).length;
  // 「探索」列を出しているのはSharePointの結果のときだけなので、
  // ボタンもそのときだけ見せる（押せない場所に置いて迷わせない）。
  btn.hidden = (shownTarget !== "sharepoint");
  document.getElementById("exploreLevel").hidden = btn.hidden;
  btn.disabled = (count === 0);
  btn.textContent = ICON_FOLDER + " 選択フォルダを探索" + (count ? " (" + count + ")" : "");

  // 「最新の情報に更新」「さらに取得」は検索専用の操作。探索タブに置いたままだと
  // 押した瞬間に target="explore" で検索し直そうとして、探索結果が消えてしまう。
  // 押せない理由を説明するより、その場に出さない方が分かりやすい。
  document.getElementById("btnRefresh").hidden = isExploreView();
  document.getElementById("btnLoadMore").hidden = isExploreView();

  document.getElementById("viewToggle").hidden = !isExploreView();
  document.getElementById("btnTreeView").className = "mini" + (exploreTreeMode ? " on" : "");
  document.getElementById("btnFlatView").className = "mini" + (exploreTreeMode ? "" : " on");

  var note = document.getElementById("exploreNote");
  if (!isExploreView()) { note.hidden = true; note.textContent = ""; return; }
  note.hidden = false;
  note.textContent = exploreTreeMode
    ? ("ツリー表示です。フォルダ左の記号で開閉できます。種別での絞り込みは"
       + "このままお使いいただけます（該当ファイルまでの親フォルダは残ります）。"
       + "名前・日付での絞り込みや並べ替えは「一覧表示」に切り替えてください。")
    : ("一覧表示です。見出しの文字で並べ替え、" + ICON_FILTER
       + " で絞り込みができます。チェックを付けてZIP取得・Excel/CSV出力・要約が使えます。");
}

function updateCounts() {
  var text = (isExploreView() ? "探索結果: " : "検索結果: ") + allResults.length + " 件";
...
function setBusy(busy) {
  ...
  document.getElementById("btnEnoviaTypeDiag").disabled = busy;
  document.getElementById("btnExploreDiag").disabled = busy;
  document.getElementById("btnSiteSearch").disabled = busy;
  document.getElementById("btnRefresh").disabled = busy;
  if (busy) { document.getElementById("btnExplore").disabled = true; }
...
function setTarget(target) {
  ...
  // S04（敵対的レビューM1）: スコープ選択はEnoviaタブでのみ表示する。
  // 「0. All」タブでも選んだスコープは検索に反映されるが、選択操作自体は
  // Enoviaタブでのみ行う（folderOnlyRowと同じ考え方）。
  document.getElementById("enoviaScopeRow").hidden = (target !== "enovia");
  // S05: 対象サイト（サイト内検索）はSharePointタブでのみ表示・使用する
  // （越智さんの判断。Allタブは今までどおり全社検索）。
...
document.getElementById("tabs").addEventListener("click", function (e) {
  ...
  if (newTarget === "explore") {
    // 探索結果は既に手元にあるため、再検索はしない（検索とは別系統のデータ）。
    setTarget("explore");
    if (exploreData.length) {
      enterExploreView(exploreData);
    } else {
      log(ICON_INFO + " まだ探索結果がありません。SharePointタブでフォルダの"
          + "「探索」に印を付け、「選択フォルダを探索」を押してください。");
    }
    return;
  }
  setTarget(newTarget);
...
function currentSearchParams() {
  return {
    ...
    typeScope: document.getElementById("enoviaScope").value,
    // S05: 対象サイトは「1. SharePoint」タブのときだけ送る。
    siteScope: (currentTarget() === "sharepoint" && selectedSite) ? selectedSite.url : "",
    siteName: (currentTarget() === "sharepoint" && selectedSite) ? (selectedSite.name || "") : "",
  };
}
...
function applySearchResponse(data, target, fromCache) {
  ...
  selected = {};
  // 索引（_idx）は検索のたびに振り直されるため、前の検索で付けた
  // 「探索」の印を残すと、まったく別のフォルダを探索してしまう。
  exploreSelected = {};
  if (pendingState) {
...
function runSearch() {
  ...
  if (p.target === "explore") {
    // 念のための歯止め。探索結果は検索とは別系統のデータで、
    // target="explore" で検索してよい系統は存在しない。
    log(ICON_INFO + " フォルダ探索タブでは検索できません。"
        + "検索するには 0〜3 のタブを選んでください。");
    return;
  }
  // S05: サイトを選んでいれば、キーワードが空でもサイト内の一覧として検索する。
  if (!p.keyword && !p.siteScope) { log("キーワードが空です。"); return; }
  var key = cacheKey(p.keyword, p.target, p.maxResults, p.titleOnly, p.prefixSearch,
                     p.typeScope, p.siteScope);
  ...
  log(ICON_SEARCH + " 検索開始: \"" + p.keyword + "\" / 対象: " + p.target
      ...
      + (p.siteScope ? " / サイト: " + (p.siteName || sitePathOf(p.siteScope))
                       + "（" + sitePathOf(p.siteScope) + "）" : "")
      + " / 上限: " + p.maxResults + " 件");
  ...
  fetch("/api/search", {
    ...
    body: JSON.stringify({ keyword: p.keyword, target: p.target,
                           ...
                           prefix_search: p.prefixSearch, type_scope: p.typeScope,
                           site_scope: p.siteScope })
  })
...
// ── S05 本実装: サイト内検索（SharePointタブのみ） ─────────────
// 選んだサイトは selectedSite に持つ（{name, url}。url はサイトのトップ）。
// 検索のときに site_scope としてサーバーへ送り、サーバー側で必ず検査してから
// KQLの path:"..." に使う。次回起動時には引き継がない（S05 Q1=A案）。
var selectedSite = null;
var favoriteSites = [];

function sitePathOf(url) {
  var m = String(url || "").match(/^https?:\/\/[^\/]+(\/(?:sites|teams)\/[^\/?#]+)/i);
  return m ? m[1] : String(url || "");
}

function isFavoriteSite(url) {
  return favoriteSites.some(function (s) { return s.url === url; });
}

function renderSiteChip() {
  var chip = document.getElementById("siteChip");
  chip.hidden = !selectedSite;
  document.getElementById("siteNone").hidden = !!selectedSite;
  if (!selectedSite) { return; }
  // 同じ番号・似た名前のサイトが複数ある（実機で確認）ため、名前だけでなく
  // URL上の場所も必ず並べて表示し、取り違えを防ぐ。
  document.getElementById("siteChipName").textContent = selectedSite.name || sitePathOf(selectedSite.url);
  document.getElementById("siteChipPath").textContent = "（" + sitePathOf(selectedSite.url) + "）";
  chip.title = selectedSite.url;
  var fav = isFavoriteSite(selectedSite.url);
  var btn = document.getElementById("btnFavToggle");
  btn.textContent = fav ? "★" : "☆";
  btn.title = fav ? "よく使うサイトから外します" : "よく使うサイトに登録します";
}

function renderFavoriteSites() {
  var sel = document.getElementById("favoriteSites");
  sel.innerHTML = "";
  var head = document.createElement("option");
  head.value = "";
  head.textContent = favoriteSites.length ? "★ よく使うサイト（" + favoriteSites.length + "）"
                                          : "★ よく使うサイト（未登録）";
  sel.appendChild(head);
  favoriteSites.forEach(function (s, i) {
    var o = document.createElement("option");
    o.value = String(i);
    o.textContent = (s.name || sitePathOf(s.url)) + "  " + sitePathOf(s.url);
    sel.appendChild(o);
  });
  renderSiteChip();
}
...
function loadFavoriteSites() {
  return fetch("/api/favorite_sites")
    .then(function (r) { return r.json(); })
    .then(function (data) { favoriteSites = data.sites || []; renderFavoriteSites(); })
    .catch(function () { /* 読めなくても検索はできる */ });
}

// サイトを選ぶ。runNow のときは、そのまま「1. SharePoint」タブで検索する
// （キーワードが空ならサイト内の一覧になる）。
function selectSite(site, runNow) {
  selectedSite = { name: site.name || "", url: site.url };
  document.getElementById("siteCandidates").hidden = true;
  renderSiteChip();
  log(ICON_INFO + " 対象サイトを「" + (selectedSite.name || sitePathOf(site.url)) + "」（"
      + sitePathOf(site.url) + "）にしました。この中だけを検索します。");
  if (currentTarget() !== "sharepoint") { setTarget("sharepoint"); }
  if (runNow) { runSearch(); } else { saveState(); }
}

function clearSite() {
  if (!selectedSite) { return; }
  log(ICON_INFO + " 対象サイトの指定を外しました（全社検索に戻ります）。");
  selectedSite = null;
  renderSiteChip();
  saveState();
}

function renderSiteCandidates(data) {
  var box = document.getElementById("siteCandidates");
  box.innerHTML = "";
  var cands = data.candidates || [];
  if (!cands.length) { box.hidden = true; return; }
  var label = document.createElement("span");
  label.className = "muted";
  label.textContent = "候補（クリックで選ぶ）:";
  box.appendChild(label);
  cands.forEach(function (c) {
    var b = document.createElement("button");
    b.type = "button";
    b.className = "mini sitecand";
    b.textContent = "🌐 " + (c.name || sitePathOf(c.url)) + "  " + sitePathOf(c.url);
    b.title = c.url + (c.description ? "\n" + c.description : "");
    b.addEventListener("click", function () { selectSite(c, true); });
    box.appendChild(b);
  });
  box.hidden = false;
}

document.getElementById("btnSiteSearch").addEventListener("click", function () {
  var term = document.getElementById("siteSearchInput").value.trim();
  if (!term) { log(ICON_INFO + " サイト名の一部（例: P010008）を入れてから押してください。"); return; }
  setBusy(true);
  log(ICON_SEARCH + " サイトを探しています: \"" + term + "\"");
  fetch("/api/site_search", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    ...
    .then(function (data) {
      if (data.error) { log(ICON_ERROR + " " + data.error); return; }
      if (!data.ok) { log(ICON_ERROR + " " + data.message); return; }
      if (data.retried_with) {
        log(ICON_INFO + " 「" + data.term + "」では見つからなかったため、「"
            + data.retried_with + "」で探しました。");
      }
      var n = (data.candidates || []).length;
      if (!n) {
        log(ICON_EMPTY + " 該当するサイトが見つかりませんでした。番号（例: P010008）や"
            + "名前の一部で探し直してください。");
      } else {
        log(ICON_OK + " サイトの候補 " + n + " 件。下の一覧から選んでください。");
      }
      renderSiteCandidates(data);
    })
    .catch(function (e) { log(ICON_ERROR + " 通信エラー: " + e); })
    .finally(function () { setBusy(false); });
});
...
document.getElementById("siteSearchInput").addEventListener("keydown", function (e) {
  if (e.key === "Enter") { document.getElementById("btnSiteSearch").click(); }
});

document.getElementById("btnClearSite").addEventListener("click", function () { clearSite(); });

document.getElementById("favoriteSites").addEventListener("change", function () {
  var i = this.value;
  this.value = "";
  if (i === "" || !favoriteSites[+i]) { return; }
  selectSite(favoriteSites[+i], true);
});

document.getElementById("btnFavToggle").addEventListener("click", function () {
  if (!selectedSite) { return; }
  var removing = isFavoriteSite(selectedSite.url);
  fetch("/api/favorite_sites", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: removing ? "remove" : "add",
                           name: selectedSite.name, url: selectedSite.url })
  })
    .then(function (r) { return r.json(); })
    .then(function (data) {
      if (data.error) { log(ICON_ERROR + " " + data.error); return; }
      favoriteSites = data.sites || [];
      renderFavoriteSites();
      log(ICON_OK + " 「" + (selectedSite.name || sitePathOf(selectedSite.url)) + "」を"
          + (removing ? "よく使うサイトから外しました。" : "よく使うサイトに登録しました。"));
    })
    .catch(function (e) { log(ICON_ERROR + " 通信エラー: " + e); });
});
...
document.getElementById("btnEnoviaLogin").addEventListener("click", function () {
  setBusy(true);
  // 手順を具体的に出す。「検索を1回」は必須の手順で、これを省くと
  // federated 側のセッションが発行されず、検索が必ず失敗する（実機で確認）。
  log(ICON_INFO + " Enoviaのログイン画面（Edge）を開きます。次の順でお願いします…");
  log("　1) Enoviaにログインする（SSOで自動的に入る場合はそのまま）");
  log("　2) " + ICON_SEARCH + " 画面上部の検索ボックスで1回検索し、結果が出るのを確認する");
  log("　3) そのウィンドウを閉じる（ここでログイン情報が保存されます）");
...
// S04 Phase3-0: Enovia型診断。全種別（191型）で検索し、型ごとの件数・
// 返ってくる属性・「Enoviaで開く」URLを実測する（本実装の前に必ず1回実行する）。
document.getElementById("btnEnoviaTypeDiag").addEventListener("click", function () {
  var keyword = document.getElementById("keyword").value.trim();
  if (!keyword) { log("キーワードが空です。"); return; }
  setBusy(true);
  log(ICON_INFO + " Enovia型診断を実行します（全種別を対象に取得し、型ごとに集計）...");
  fetch("/api/enovia_type_diag", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ keyword: keyword })
  })
    ...
    .then(function (data) {
      ...
      if (!data.ok && !(data.rows || []).length) {
        log(ICON_ERROR + " " + (data.message || "診断に失敗しました。"));
        return;
      }
      log(ICON_INFO + " 全 " + data.nhits + " 件中、先頭 " + data.fetched_raw
          + " 件（" + data.pages_fetched + " ページ）をサンプル取得しました。"
          + " 以下の件数はこのサンプル内の内訳であり、Enovia画面の"
          + "ファセット（絞り込み欄）の全件集計とは一致しない場合があります。");
      if (data.truncated) {
        log(ICON_INFO + " ページ上限に達したため打ち切りました"
            + "（レアな型はサンプルに含まれていない可能性があります）。");
      }
      if (data.message) { log(ICON_ERROR + " " + data.message); }

      (data.rows || []).forEach(function (row) {
        log(ICON_OK + " 型: " + row.type + "  → サンプル内 " + row.count + " 件");
        if (row.example_url) {
          log("      Enoviaで開くURL（この型で実際に開くか、クリックしてご確認ください）: "
              + row.example_url);
        } else {
          log("      Enoviaで開くURL: resourceidが取れず生成できませんでした。");
        }
        log("      属性の充足率（値あり件数 / この型のサンプル件数）:");
        (row.required || []).forEach(function (r) {
          log("        " + r.label + " : " + r.present + " / " + r.total);
        });
        log("      返ってきた属性をそのまま表示（代表1件）:");
        (row.sample_attrs || []).forEach(function (a) {
          log("        " + a.name + " = " + a.value);
        });
      });
      log(ICON_INFO + " ①属性の充足率 ②URLが開けるか ③型の英語名とEnovia画面の"
          + "日本語ラベルの対応、をこの結果からご確認ください。");
    })
    .catch(function (e) { log(ICON_ERROR + " 通信エラー: " + e); })
    .finally(function () { setBusy(false); });
});

// フォルダ探索機能の実装前確認。検索結果の最初のフォルダ（または選択した
// フォルダ行）について、配下を再帰的にたどれるかを実際に試す。読み取りのみ。
document.getElementById("btnExploreDiag").addEventListener("click", function () {
  var picked = selectedIndices();
  var body = {};
  if (picked.length) { body.idx = String(picked[0]); }

  setBusy(true);
  log(ICON_INFO + " フォルダ探索診断を実行します"
      + (picked.length ? "（選択中の行）" : "（検索結果の最初のフォルダ）") + "...");

  fetch("/api/explore_diag", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body)
  })
    ...
    .then(function (data) {
      ...
      log(ICON_INFO + " 対象: " + (data.target ? data.target.title : ""));
      (data.rows || []).forEach(function (row) {
        var icon = row.ok ? ICON_OK : (row.skipped ? ICON_PENDING : ICON_ERROR);
        log(icon + " " + row.label
      ...
      var missing = (data.fields || []).filter(function (f) { return !f.present; });
      if ((data.fields || []).length) {
        if (missing.length) {
          log(ICON_EMPTY + " 応答に含まれていない項目: "
              + missing.map(function (f) { return f.label; }).join(" / ")
              + "（この列は空欄になります）");
        } else {
          log(ICON_OK + " 必要な項目はすべて応答に含まれています"
              + "（ファイル名・サイズ・最終更新日・リンク・作成者・親フォルダ）。");
        }
      }

      (data.samples || []).forEach(function (line) { log("      " + line); });

      if (data.verdict) {
        log((data.verdict.ok ? ICON_OK : ICON_ERROR) + " 判定: " + data.verdict.message);
      }
    })
    .catch(function (e) { log(ICON_ERROR + " 通信エラー: " + e); })
    .finally(function () { setBusy(false); });
...
document.getElementById("btnDownload").addEventListener("click", function () {
  ...
  log(ICON_INFO + " " + idx.length + " 件のファイルをZIPにまとめています。"
      + "件数が多いと時間がかかります。");
  window.location.href = "/api/download?scope=" + (isExploreView() ? "explore" : "search")
    + "&idx=" + encodeURIComponent(idx.join(","));
});

// ── フォルダ探索 ─────────────────────────────────────────
// ── 指定した深さより先のフォルダを、▶で1階層ずつ読み込む ─────────
// 読み込んだ行は、サーバー側の探索結果の末尾に同じ順で足される。画面は、
// サーバーが返した「足し始めた位置(start)」が自分の行数と一致するときだけ
// 足す。ずれたまま足すと、行の選択番号(_idx)がサーバーと食い違い、
// ZIP・Excel・要約で別のファイルが選ばれてしまうため（Phase2 問題1）。
// 同時に読み込むのは1つだけにする（順番が入れ替わるとずれの原因になる）。
var exploreLoadingId = "";

function loadExploreChildren(r) {
  if (exploreLoadingId) {
    log(ICON_INFO + " 別のフォルダを読み込んでいます。終わってからもう一度押してください。");
    return;
  }
  exploreLoadingId = r.item_id;
  applyView();
  fetch("/api/explore_children", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ item_id: r.item_id, drive_id: r.drive_id })
  })
    .then(function (resp) { return resp.json(); })
    .then(function (data) {
      if (data.error) { log(ICON_ERROR + " " + data.error); return; }
      if (!data.added) {
        // 読み込み済み（サーバー側に既にある）。画面にもあるはずなので開くだけ。
        r._loaded = true;
        delete collapsed[r.item_id];
        return;
      }
      if (data.start !== exploreData.length) {
        log(ICON_ERROR + " 画面とサーバーの探索結果が食い違ったため、読み込みを中止しました。"
            + "もう一度「選択フォルダを探索」からやり直してください。");
        return;
      }
      (data.rows || []).forEach(function (row) {
        row._idx = exploreData.length;
        exploreData.push(row);
      });
      r._loaded = true;
      r._empty = !(data.rows || []).length;
      delete collapsed[r.item_id];
      log(ICON_FOLDER + " 「" + r.title + "」を開きました（" + (data.rows || []).length + " 件）。");
    })
    .catch(function (e) { log(ICON_ERROR + " 読み込めませんでした: " + e); })
    .finally(function () {
      exploreLoadingId = "";
      applyView();
    });
...
function enterExploreView(rows) {
  exploreData = rows;
  document.getElementById("tabExplore").hidden = false;
  applyColumnSet("explore");   // shownTarget も "explore" になる
  setTarget("explore");
  allResults = rows;
  allResults.forEach(function (r, i) { r._idx = i; });
  filters = {};
...
var exploreLastCurrent = "";

function pollExplore(job) {
  fetch("/api/explore_status?job=" + encodeURIComponent(job))
    .then(function (resp) { return resp.json(); })
    .then(function (st) {
      if (st.error) { log(ICON_ERROR + " " + st.error); setBusy(false); updateCounts(); return; }
      if (!st.done) {
        // 同じ場所を何度も書くとログが読めなくなるため、進んだときだけ出す。
        if (st.current && st.current !== exploreLastCurrent) {
          exploreLastCurrent = st.current;
          log(ICON_SEARCH + " 探索中: " + st.current
              + "（走査済み " + st.scanned + " フォルダ / 検出 " + st.found + " 件）");
        }
        setTimeout(function () { pollExplore(job); }, 1200);
        return;
      }
      setBusy(false);
      (st.errors || []).forEach(function (m) { log(ICON_ERROR + " " + m); });
      if (st.stopped) { log(ICON_EMPTY + " " + st.stopped); }
      log(ICON_OK + " 探索完了（Lv" + (st.level || "?") + "まで）: フォルダ " + st.folders
          + " 件 / ファイル " + st.files + " 件（走査 " + st.scanned + " フォルダ）");
      if (st.unloaded) {
        log(ICON_INFO + " その先に中身のあるフォルダが " + st.unloaded + " 件あります。"
            + "ツリーの " + TRI_CLOSED + " を押すと、次の階層を読み込みます。");
      }
      if (!(st.results || []).length) {
        log(ICON_EMPTY + " 探索できた項目がありませんでした。");
        updateCounts();
        return;
      }
      enterExploreView(st.results);
    })
    .catch(function (e) {
      log(ICON_ERROR + " 進捗を取得できませんでした: " + e);
      setBusy(false);
      updateCounts();
    });
}

document.getElementById("exploreLevel").addEventListener("change", function () {
  log(ICON_INFO + " 探索の深さを Lv" + this.value + " にしました（次回起動時も使います）。");
  saveState();
});

document.getElementById("btnExplore").addEventListener("click", function () {
  var idx = viewResults.filter(function (r) { return exploreSelected[r._idx]; })
                       .map(function (r) { return r._idx; });
  if (!idx.length) { return; }
  var level = parseInt(document.getElementById("exploreLevel").value, 10) || 2;
  setBusy(true);
  log(ICON_FOLDER + " フォルダ探索を開始します（" + idx.length + " フォルダ / "
      + "Lv" + level + "まで・上限2000件）。件数が多いと時間がかかることがあります。");
  fetch("/api/explore", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ idx: idx.join(","), level: level }),
  })
    .then(function (resp) { return resp.json(); })
    .then(function (data) {
      if (data.error) { log(ICON_ERROR + " " + data.error); setBusy(false); updateCounts(); return; }
      log(ICON_INFO + " 起点フォルダ: " + (data.roots || []).join(" / "));
      exploreLastCurrent = "";
      pollExplore(data.job);
    })
    .catch(function (e) {
      log(ICON_ERROR + " 探索を開始できませんでした: " + e);
      setBusy(false);
      updateCounts();
    });
});
...
document.getElementById("btnTreeView").addEventListener("click", function () {
  if (exploreTreeMode) { return; }
  exploreTreeMode = true;
  // ツリー表示で使えるのは種別の絞り込みだけ。他の条件を残したままにすると
  // 「絞り込んだはずなのに全部出ている」と見えるため、ここで落としておく。
  var docType = filters.doc_type;
  filters = {};
...
function restoreState() {
  return fetch("/api/state")
    .then(function (r) { return r.json(); })
    .then(function (s) {
      // フォルダ探索の深さは、キーワードの有無に関係なく復元する（越智さんの選択）。
      if (s && [1, 2, 3, 4].indexOf(Number(s.explore_level)) >= 0) {
        document.getElementById("exploreLevel").value = String(s.explore_level);
      }
      ...
      // S04（Q1=B案）: 前回選んだEnoviaのスコープを引き継ぐ。
      if (s.enovia_type_scope) {
        document.getElementById("enoviaScope").value = s.enovia_type_scope;
      }
      ...
      if (s.site_scope_name) {
        log(ICON_INFO + " 前回は「" + s.site_scope_name + "」サイト内での検索でした。"
            + "サイトの指定は引き継がないため、全社検索で表示します"
            + "（必要なら「サイトを探す」か「★ よく使うサイト」で選び直してください）。");
      }
      document.getElementById("btnSearch").click();
    })
...
# フォルダ探索の結果と、実行中ジョブの進捗。検索結果とは別に保持する
# （探索中でも検索結果を見失わないようにするため）。
_explore_results: List[SearchResult] = []
...
def index() -> Response:
    ...
    # 版を上げてサーバーを入れ替えても、ブラウザが前の版のHTMLを使い回して
    # しまい「新機能が出ない」と見える事故が実際に起きたため、この画面は
    # 常に取得し直させる（画面はこのHTML1枚だけなので副作用は無い）。
...
def api_state():
    ...
    state = {
        ...
        # S04（Q1=B案）: Enoviaのスコープ（①Documentのみ/②よく使う型/③全種別）は、
        # 前回選んだものを次回起動時も引き継ぐ。未保存時はenovia_type_scope_default。
        "enovia_type_scope": str(payload.get("enovia_type_scope") or
                                 _cfg.get("enovia_type_scope_default", "document")),
        "sort": payload.get("sort") or {},
        "filters": payload.get("filters") or {},
        # S05: サイト内検索の対象サイトは**次回起動時に引き継がない**（Q1=A案。
        # 絞り込んだまま翌日に別の件を検索し、違うサイトの結果を見てしまう事故を
        # 避けるため）。ここに残すのは名前だけで、起動時に「前回はサイト内検索
        # だった」と知らせるためにのみ使う。
        "site_scope_name": " ".join(str(payload.get("site_scope_name") or "").split())[:200],
        # フォルダ探索の深さ（Lv1〜4）。越智さんの選択で次回起動時も引き継ぐ。
        "explore_level": _explore_level(payload.get("explore_level")),
        "saved_at": datetime.now(JST).strftime("%Y-%m-%d %H:%M:%S"),
    }
    try:
...
@flask_app.route("/api/site_scope_diag", methods=["POST"])
def api_site_scope_diag():
    """S05 Phase3-0: SharePointのサイト絞り込み（KQLの `path:`）を試す診断。

    メイン検索にはまだ反映しない。サイト名/URLの許可文字検証・KQLの
    組み立て・該当件数と先頭数件を、会社PCで実際に確認するためのもの。
    """
    payload = flask_req.get_json(silent=True) or {}
    keyword = str(payload.get("keyword", "")).strip()
    site_input = str(payload.get("site", "")).strip()
    title_only = bool(payload.get("title_only", _cfg.get("title_only_default", True)))
    prefix_search = bool(payload.get("prefix_search",
                                     _cfg.get("prefix_search_default", False)))

    provider = _manager.providers.get(TARGET_SHAREPOINT)
    if provider is None:
        return jsonify({"error": "SharePointプロバイダが見つかりません。"}), 500

    try:
        result = provider.diagnose_site_scope(
            keyword, site_input, title_only=title_only, prefix_search=prefix_search)
    except Exception as e:
        return jsonify({"error": f"診断に失敗しました: {e}"}), 500

    if result.get("ok"):
        print(f"🩺 サイト絞り込み診断: \"{keyword}\" / サイト={site_input or '(指定なし)'} "
              f"/ 該当 {result.get('total')} 件 / サンプル {result.get('sample_count')} 件")
    else:
        print(f"🩺 サイト絞り込み診断（失敗）: {result.get('message')}")
    return jsonify(result)


@flask_app.route("/api/site_search_diag", methods=["POST"])
def api_site_search_diag():
    """S05 Phase3-0b: サイト名の一部からサイトを探せるかを2方式で試す診断。

    本実装の「サイトを探す」ボタンに使う方式を決めるためのもの。
    メイン検索にはまだ反映しない。
    """
    payload = flask_req.get_json(silent=True) or {}
    ...
    try:
        result = provider.diagnose_site_search(term)
    except Exception as e:
        return jsonify({"error": f"診断に失敗しました: {e}"}), 500

    if result.get("ok"):
        print(f"🩺 サイト名検索診断: \"{result.get('term')}\"")
        for row in result.get("methods") or []:
            if row.get("ok"):
                print(f"   {row['label']}: 候補 {len(row.get('candidates') or [])} 件"
                      f"（total {row.get('total')}）")
            else:
                print(f"   {row['label']}: HTTP {row.get('status')} {row.get('message', '')[:120]}")
    else:
        print(f"🩺 サイト名検索診断（失敗）: {result.get('message')}")
    return jsonify(result)


@flask_app.route("/api/site_search", methods=["POST"])
def api_site_search():
    """S05本実装: 「サイトを探す」ボタン。サイト名の一部から候補を返す。"""
    payload = flask_req.get_json(silent=True) or {}
    provider = _manager.providers.get(TARGET_SHAREPOINT)
    if provider is None:
        return jsonify({"error": "SharePointプロバイダが見つかりません。"}), 500
    try:
        result = provider.search_sites(str(payload.get("term", "")))
    except Exception as e:
        return jsonify({"error": f"サイトの検索に失敗しました: {e}"}), 500

    if result.get("ok"):
        again = (f"（「{result['term']}」では0件のため「{result['retried_with']}」で再検索）"
                 if result.get("retried_with") else "")
        print(f"🌐 サイト検索: \"{result.get('term')}\" → 候補 "
              f"{len(result.get('candidates') or [])} 件 {again}".rstrip())
    else:
        print(f"🌐 サイト検索（失敗）: {result.get('message')}")
    return jsonify(result)


def _load_favorite_sites() -> List[Dict[str, str]]:
    """よく使うサイトの一覧を読む。壊れていても起動は止めない（空として扱う）。"""
    if not FAVORITE_SITES_PATH.exists():
        return []
    try:
        with open(FAVORITE_SITES_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
    except Exception as e:
        print(f"⚠️  よく使うサイトの一覧を読み込めませんでした（空として扱います）: {e}")
        return []
    sites = data.get("sites") if isinstance(data, dict) else None
    out: List[Dict[str, str]] = []
    for s in sites or []:
        if isinstance(s, dict) and s.get("url"):
            out.append({"name": str(s.get("name") or ""), "url": str(s["url"])})
    return out


def _save_favorite_sites(sites: List[Dict[str, str]]) -> None:
    """よく使うサイトの一覧を保存する（一時ファイルに書いてから置き換える）。"""
    tmp = FAVORITE_SITES_PATH.with_suffix(".json.tmp")
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump({"sites": sites}, f, ensure_ascii=False, indent=2)
    tmp.replace(FAVORITE_SITES_PATH)


@flask_app.route("/api/favorite_sites", methods=["GET", "POST"])
def api_favorite_sites():
    """S05本実装: よく使うサイト（☆）の一覧・登録・解除。

    登録するURLも、検索に使うときと同じ検査を通し、サイトのトップURLに
    揃えてから保存する（同じサイトを二重に登録しないため）。
    """
    if flask_req.method == "GET":
        return jsonify({"sites": _load_favorite_sites()})

    payload = flask_req.get_json(silent=True) or {}
    action = str(payload.get("action") or "")
    try:
        url = _normalize_site_scope_url(str(payload.get("url") or ""), _cfg)
    except ValueError as e:
        return jsonify({"error": str(e)}), 400
    if not url:
        return jsonify({"error": "サイトのURLが指定されていません。"}), 400

    sites = [s for s in _load_favorite_sites() if s["url"] != url]
    if action == "add":
        if len(sites) >= FAVORITE_SITES_MAX:
            return jsonify({"error": f"登録できるのは {FAVORITE_SITES_MAX} 件までです。"
                                     "使わないサイトの☆を外してください。"}), 400
        name = " ".join(str(payload.get("name") or "").split())[:200]
        sites.append({"name": name or _site_path_of(url), "url": url})
    elif action != "remove":
        return jsonify({"error": "action は add か remove を指定してください。"}), 400

    try:
        _save_favorite_sites(sites)
    except Exception as e:
        return jsonify({"error": f"よく使うサイトを保存できませんでした: {e}"}), 500
    print(f"☆ よく使うサイトを{'登録' if action == 'add' else '解除'}しました: "
          f"{_site_path_of(url)}（計 {len(sites)} 件）")
...
@flask_app.route("/api/enovia_type_diag", methods=["POST"])
def api_enovia_type_diag():
    """S04 Phase3-0: Document以外の型を含め、型ごとの件数・属性を実測する。"""
    payload = flask_req.get_json(silent=True) or {}
    keyword = str(payload.get("keyword", "")).strip()
    if not keyword:
        return jsonify({"error": "キーワードが指定されていません。"}), 400
    ...
    try:
        result = provider.diagnose_types(keyword)
    except Exception as e:
        return jsonify({"error": f"診断に失敗しました: {e}"}), 500
    ...
    print(f"🩺 Enovia型診断: \"{keyword}\" / 型 {len(result.get('rows') or [])} 種類 "
          f"/ サンプル {result.get('fetched_raw')} 件（全 {result.get('nhits')} 件中）")
    for row in result.get("rows") or []:
        print(f"   {row['type']}: {row['count']} 件")
    return jsonify(result)
...
def api_search():
    ...
    prefix_search = bool(payload.get("prefix_search",
                                     _cfg.get("prefix_search_default", False)))
    type_scope = str(payload.get("type_scope") or
                     _cfg.get("enovia_type_scope_default", "document"))
    if type_scope not in ("document", "major", "all"):
        type_scope = _cfg.get("enovia_type_scope_default", "document")

    # S05: サイト内検索。「1. SharePoint」タブのときだけ受け付ける
    # （他のタブで届いても使わない。Allタブは今までどおり全社検索）。
    site_scope = ""
    raw_site = str(payload.get("site_scope") or "").strip()
    if raw_site and target == TARGET_SHAREPOINT:
        try:
            site_scope = _normalize_site_scope_url(raw_site, _cfg)
        except ValueError as e:
            return jsonify({"error": f"対象サイトの指定が正しくありません: {e}"}), 400

    # キーワードが空でも、サイトを指定していればサイト内の一覧として検索する
    # （S05 Q3=A案：「このサイトに何があるか一覧したい」という使い方）。
    if not keyword and not site_scope:
        return jsonify({"error": "キーワードが指定されていません。"}), 400
    ...
    try:
        outcome = _manager.search(keyword, target, max_results,
                                  ...
                                  type_scope=type_scope, site_scope=site_scope)
    except Exception as e:
    ...
    site_path = _site_path_of(site_scope) if site_scope else ""
    with _lock:
        _last_results = results
        # Excel/CSVのファイル名に使う。サイト内の一覧（キーワード空）では
        # サイト名を使う（空だと「result」になり、何の出力か分からないため）。
        _last_keyword = keyword or site_path.rsplit("/", 1)[-1]

    scope_label = "タイトルのみ" if title_only else "全文"
    print(f"🔍 検索: \"{keyword}\" / 対象={target} / 範囲={scope_label} / "
          + (f"サイト={site_path} / " if site_path else "")
...
def _pick_results(idx_param: Optional[str],
                  scope: str = "search") -> List[SearchResult]:
    """idx（カンマ区切りの索引）で、画面に表示されている行・並び順を再現する。

    idx が無い場合は全件を対象にする。範囲外・不正な値は無視する。
    scope="explore" のときは、直近のフォルダ探索の結果から選ぶ
    （検索結果と探索結果は別々に保持しており、索引も別系統のため）。
    """
    with _lock:
        results = list(_explore_results if scope == "explore" else _last_results)
...
def api_export():
    ...
    scope = str(flask_req.args.get("scope") or "search")
    results = _pick_results(flask_req.args.get("idx"), scope)
    with _lock:
        keyword = _last_keyword
    if scope == "explore":
        # 出力ファイル名は「何を出したか」が分かる方が探しやすいため、
        # 探索結果のときは検索キーワードではなく起点フォルダ名を使う。
        keyword = results[0].root_label if results else "explore"
    ...
    if nexus_only:
        ...
        spec = [
            ...
            ("Enovia型",        lambda r: r.enovia_type,     None),
        ...
        widths = [16, 40, 16, 40, 8, 22, 20, 20, 20, 14, 14, 30, 10, 14]
    else:
        spec = [
...
- executive_summary: 300字以内。1文につき事実は1つだけとし、「、」や
  「〜し、〜しており」で複数の事実をつなげない（一文一義）。結果として
  4〜5文程度に分かれる形が目安。「〜を定めたものです」「〜を目的として
  います」のような、文書の存在を説明するだけの言い回しは使わない。
  1文目で「この文書がなぜ自分に関係するか」を書き、以降の文で最も重要な
  具体的事実（基準名・数値・適用範囲等）を書く。chaptersで述べる章立ての
  内容を先取りして繰り返さない。
...
# 要約可能な拡張子と、本文抽出に使う関数の対応。新しい形式を追加するときは
# ここに1行足すだけでよい（Phase B: pptx、Phase C: xlsx/xlsmを追加。pdfは未対応）。
# xlsmはマクロ有効ブックだが、ファイル形式（zip+XML）自体はxlsxと同じで、
# openpyxlでの値の読み取り方法も変わらないため、xlsxと同じ抽出関数を使う。
SUMMARIZABLE_EXTENSIONS = ("docx", "pptx", "xlsx", "xlsm", "pdf")


def _summarizable_reason(result: SearchResult) -> "tuple[bool, str]":
    """要約可能かどうかと、不可の場合の理由を返す（対象を明示的に絞る）。"""
    if result.is_folder:
    ...
    if ext not in SUMMARIZABLE_EXTENSIONS:
        formats = "/".join("." + e for e in SUMMARIZABLE_EXTENSIONS)
        return False, f"「.{ext or '不明'}」形式は現在、要約に対応していません（現在の対応形式: {formats}）。"
    return True, ""


def _extract_docx_text(content: bytes, max_chars: int) -> "tuple[str, bool, int]":
    """docxのバイト列から本文テキストを抽出する。(本文, 切り詰めたか, 切り詰め前の文字数) を返す。
    ...
    if total_chars > max_chars:
        return full_text[:max_chars], True, total_chars
    return full_text, False, total_chars


def _extract_pptx_text(content: bytes, max_chars: int) -> "tuple[str, bool, int]":
    """pptxのバイト列から本文テキストを抽出する。(本文, 切り詰めたか, 切り詰め前の文字数) を返す。

    スライドの区切りを"# Slide N"（タイトルがあれば併記）として付与し、
    docxの見出し（"# "）と同じ考え方で章立ての手がかりを残す。
    スライド内の表・グラフ・画像中の文字列、発表者ノートは
    Phase Bのスコープ外（docxの表と同じ理由。DESIGN_NOTES参照）。
    """
    from pptx import Presentation  # 遅延importで、未インストールでも他機能に影響しない
    ...
    for i, slide in enumerate(prs.slides, start=1):
        ...
        lines.append(f"# Slide {i}" + (f": {title_text}" if title_text else ""))
...
def _extract_xlsx_text(content: bytes, max_chars: int) -> "tuple[str, bool, int]":
    """xlsx/xlsmのバイト列から本文テキストを抽出する。(本文, 切り詰めたか, 切り詰め前の文字数) を返す。

    xlsm（マクロ有効ブック）はファイル形式（zip+XML）自体がxlsxと同じで、
    openpyxlでの値の読み取り方も変わらないため、この関数をそのまま使う
    （マクロのコード自体は要約の対象にしない）。

    シートの区切りを"# Sheet: <シート名>"として付与し、docx/pptxの見出しと
    同じ考え方で構造（章立て＝シート単位）の手がかりを残す。数式セルは
    Excelが最後に保存した計算済みの値のみを読み取る（data_only=True。
    値のキャッシュが無い場合は空欄扱いになる）。グラフ・画像・条件付き
    書式・非表示行列の区別はPhase Cのスコープ外（docx/pptxの表・画像と
    同じ理由。DESIGN_NOTES参照）。行・シート数の上限は設けていない
    （docx/pptxの段落・スライド数と同じ扱い。切り詰めはmax_charsで行う）。
    """
    from openpyxl import load_workbook  # 遅延importで、未インストールでも他機能に影響しない
    ...
    try:
        ...
        for ws in wb.worksheets:
            ...
            for row in ws.iter_rows(values_only=True):
                cells = [str(v) if not isinstance(v, float) else f"{v:g}"
...
def _extract_pdf_text(content: bytes, max_chars: int) -> "tuple[str, bool, int]":
    """PDFのバイト列から本文テキストを抽出する。(本文, 切り詰めたか, 切り詰め前の文字数) を返す。

    見出しの取り方は他形式と揃える。しおり（目次）があればそれを"# "見出しと
    して使い、無ければページ番号を見出しにする。こうすることで、要約用の
    プロンプトを形式ごとに分岐させずに済む（docx=見出し / pptx=スライド /
    xlsx=シート と同じ考え方）。

    スキャン画像だけのPDF（文字情報を持たないもの）は、抽出結果が空になる。
    その場合はここでは判断せず、呼び出し側が「本文を取り出せませんでした」
    として扱う（OCRはスコープ外）。
    """
    import fitz  # PyMuPDF。遅延importで、未インストールでも他機能に影響しない

    doc = fitz.open(stream=content, filetype="pdf")
    try:
        # しおりから「このページから始まる章」の対応表を作る
        heading_of_page: Dict[int, str] = {}
        try:
            for level, title, page_no in (doc.get_toc() or []):
                if page_no >= 1 and page_no not in heading_of_page:
                    heading_of_page[page_no] = str(title).strip()
        except Exception:
            heading_of_page = {}

        lines = []
        for number, page in enumerate(doc, start=1):
            heading = heading_of_page.get(number)
            lines.append(f"# {heading}" if heading else f"# Page {number}")
...
def _extract_text_for_summary(doc_type: str, content: bytes, max_chars: int) -> "tuple[str, bool, int]":
    """拡張子に応じた本文抽出関数へ振り分ける。"""
    ext = (doc_type or "").lower()
    if ext == "pptx":
        return _extract_pptx_text(content, max_chars)
    if ext in ("xlsx", "xlsm"):
        return _extract_xlsx_text(content, max_chars)
    if ext == "pdf":
        return _extract_pdf_text(content, max_chars)
    return _extract_docx_text(content, max_chars)


# 文末とみなす記号。英文の "." は小数点（10.5）と区別するため、
# 直後が空白か行末のときだけ数える。
_SENTENCE_END_RE = re.compile(r"[。．！？!?]|\.(?=\s|$)")


def _pdf_text_metrics(text: str) -> Dict[str, Any]:
    """抽出した本文が「文章」か「羅列」かを測る。判定はしない（測るだけ）。

    見出しとして自前で付けた "# " の行は、本文の性質とは無関係なので除く。
    """
    lines = [ln.strip() for ln in text.splitlines()]
    body_lines = [ln for ln in lines if ln and not ln.startswith("# ")]
    body_text = "\n".join(body_lines)
    chars = len(body_text)
    sentences = len(_SENTENCE_END_RE.findall(body_text))
    return {
        "chars": chars,
        "lines": len(body_lines),
        "sentences": sentences,
        # 文末記号1つあたり何文字か（文章なら概ね30〜60字に1つ）
        "sentence_ratio": (sentences / chars) if chars else 0.0,
        "avg_line_chars": (chars / len(body_lines)) if body_lines else 0.0,
    }


def _looks_like_drawing_pdf(metrics: Dict[str, Any], cfg: Dict[str, Any],
                            total_chars: int = 0) -> "tuple[bool, str]":
    """図面が主体のPDFかどうかを判定し、そう判断した理由も返す。

    回路図PDFは、線が図形・文字が文字として保存されたベクター図面で、
    部品番号（R23）・ネット名（VDD_3V3）・ピン名が全て文字として
    埋め込まれている。文字数は数十万に達するが、文章ではないため
    要約しても意味を成さない（Geminiを呼ぶだけ無駄になる）。

    そこで「文末記号がほとんど無い」かつ「1行が極端に短い」の**両方**が
    成り立つときだけ、図面が主体と判断する。片方だけでは、箇条書き中心の
    正当な文書を巻き込んでしまうため。
    """
    # 判定するかどうかは「文書全体の大きさ」で決める。上限で切り詰められて
    # いても、元が大きければ判定の対象にする（判定そのものは、実際に読んだ
    # 範囲＝metrics の値で行う）。
    min_chars = int(cfg.get("pdf_drawing_check_min_chars", 5000))
    if max(total_chars, metrics["chars"]) < min_chars:
        # 短いPDFは、仮に図面でも要約の負担が小さい。判定して誤るより見送る。
        return False, ""

    max_ratio = float(cfg.get("pdf_drawing_sentence_ratio", 0.005))
    max_line = float(cfg.get("pdf_drawing_avg_line_chars", 12))
    if metrics["sentence_ratio"] >= max_ratio:
        return False, ""
    if metrics["avg_line_chars"] >= max_line:
        return False, ""

    if metrics["sentences"] == 0:
        density = "句点や終止符が1つもなく"
    else:
        per_sentence = metrics["chars"] / metrics["sentences"]
        density = (f"句点や終止符は {metrics['sentences']:,} 個だけ"
                   f"（約 {per_sentence:,.0f} 字に1つ）で")
    return True, (f"読み取った {metrics['chars']:,} 字のうち、{density}、"
                  f"1行あたり平均 {metrics['avg_line_chars']:.1f} 字でした。")
...
def api_summarize():
    ...
    # 本文が上限を超えて切り詰められる場合、既定では要約を実行せず
    # 「続けるか」を確認する応答を返す（越智さんのフィードバック：
    # 切り詰めが起きることを知らずに、部分的な要約だけを見てしまうのを防ぐ）。
    # 確認済みで続行する場合は、画面から confirm_truncated: true を送る。
    confirm_truncated = bool(payload.get("confirm_truncated", False))
    # 図面が主体のPDFと判定した場合も、既定では要約せず確認を返す。
    # 「それでも要約する」を選んだときだけ true が送られてくる。
    confirm_drawing = bool(payload.get("confirm_drawing", False))
    scope = str(payload.get("scope", "search")).strip() or "search"

    results = _pick_results(idx_param, scope)
    if not results:
    ...
    max_chars = int(_cfg.get("summary_max_chars", 40000))
    try:
        text, truncated, total_chars = _extract_text_for_summary(
            result.doc_type, content, max_chars)
    except Exception as e:
        return jsonify({"error": f"本文の抽出に失敗しました: {e}"}), 500

    # 図面の判定は、切り詰めの確認より先に行う。回路図PDFは切り詰めにも
    # 該当するため、先に切り詰めを尋ねると「続ける」を押した後で改めて
    # 「図面です」と言うことになり、二度手間になる。
    if (result.doc_type or "").lower() == "pdf" and not confirm_drawing:
        metrics = _pdf_text_metrics(text)
        is_drawing, why = _looks_like_drawing_pdf(metrics, _cfg, total_chars)
        if is_drawing:
            print(f"📐 図面が主体のPDFと判定しました: \"{result.title}\" （{why}）")
            return jsonify({
    ...
    if truncated and not confirm_truncated:
        # ここではGeminiを呼ばず、まだキャッシュもしない（越智さんの確認待ち）。
        return jsonify({
            "needs_confirmation": True,
            "truncated": True,
            "title": result.title,
...
# フォルダ探索の事前診断
#
# 「フォルダを選んで配下を再帰的に探索する」機能（設計提案済み）の実装に
# 入る前に、**現在の権限（Sites.Read.All のみ）でフォルダの子要素を
# 取得できるか**を実機で1回確かめるための診断。
#
# 既存の /shares/{token}/driveItem（Nexusの列取得で実績あり）は動くことが
# 分かっているが、/children が同じ権限で通るかは未確認のため、
# 推測で本実装に入らず、ここで事実を確定させる。読み取りのみで、
# 何も書き換えない。
# ─────────────────────────────────────────────────────────────

# children の応答にこれらが含まれていれば、探索結果の各列を
# 検索結果と同じように埋められる（含まれない項目は空欄になる）。
EXPLORE_WANTED_FIELDS = [
    ("name", "ファイル名"),
    ("size", "サイズ"),
    ("lastModifiedDateTime", "最終更新日"),
    ("webUrl", "リンク（ファイル名クリックで開く）"),
    ("folder", "フォルダかどうかの判定"),
    ("file", "ファイルかどうかの判定"),
    ("createdBy", "作成者"),
    ("parentReference", "親フォルダ（ツリーの組み立て）"),
]


def _explore_diag_get(token: str, url: str, timeout: int) -> Dict[str, Any]:
    """診断用に Graph を1回呼ぶ。例外は投げず、成否と要点だけを返す。"""
    try:
        resp = http_req.get(url, headers={"Authorization": f"Bearer {token}"},
                            timeout=timeout)
    except Exception as e:
    ...
    try:
        ...
        return {"ok": False, "status": 200,
                "message": "応答がJSON形式ではありません。"}


def _explore_child_label(child: Dict[str, Any]) -> str:
    """診断の見本表示用に、子要素1件を1行の文字列にする。"""
    name = str(child.get("name") or "(名前なし)")
    if isinstance(child.get("folder"), dict):
        count = child["folder"].get("childCount")
        return f"📁 {name}" + (f"（中に {count} 件）" if count is not None else "")
    size = child.get("size")
    if isinstance(size, int):
        if size >= 1048576:
            size_text = f"{size / 1048576:.1f} MB"
        elif size >= 1024:
            size_text = f"{round(size / 1024)} KB"
        else:
            size_text = f"{size} B"
...
def _explore_diagnose(token: str, result: SearchResult, timeout: int) -> Dict[str, Any]:
    """1つのフォルダについて、再帰探索に必要な3つの呼び出しを実際に試す。

    ①起点フォルダの情報（id / driveId）を取れるか
    ②その直下の子要素を取れるか（1階層目が開けるか）
    ③子要素の id から、さらにその中の子要素を取れるか（＝再帰できるか）

    ③まで通れば、設計どおりの再帰探索が現在の権限で実装できる。
    """
    rows: List[Dict[str, Any]] = []
    share = _share_token(result.url)

    # ① 起点フォルダ自体（既存のNexus列取得と同じ呼び方。実績あり）
    step1 = _explore_diag_get(token, f"{GRAPH_V1}/shares/{share}/driveItem", timeout)
    item = step1.get("data") or {}
    item_id = str(item.get("id") or "")
    drive_id = str((item.get("parentReference") or {}).get("driveId") or "")
    rows.append({
        "label": "① 起点フォルダの情報を取得",
        "endpoint": "GET /shares/{token}/driveItem",
        "ok": bool(step1.get("ok") and item_id),
        "status": step1.get("status"),
        "message": step1.get("message", "") if not step1.get("ok") else "",
        "detail": (f"id={item_id or '(取得できず)'} / "
                   f"driveId={drive_id or '(取得できず)'}") if step1.get("ok") else "",
    })

    # ② 直下の子要素（1階層目を開けるか）
    children: List[Dict[str, Any]] = []
    step2 = {"ok": False, "status": None, "message": "①が失敗したため実行していません。"}
    if item_id:
        step2 = _explore_diag_get(
            token, f"{GRAPH_V1}/shares/{share}/driveItem/children?$top=10", timeout)
        children = list((step2.get("data") or {}).get("value") or [])
    has_next = bool((step2.get("data") or {}).get("@odata.nextLink"))
    rows.append({
        "label": "② 直下の子要素を取得（1階層目）",
        "endpoint": "GET /shares/{token}/driveItem/children",
        "ok": bool(step2.get("ok")),
        "status": step2.get("status"),
        "message": step2.get("message", "") if not step2.get("ok") else "",
        "detail": (f"{len(children)} 件取得"
                   + ("（続きあり＝ページングが必要）" if has_next else "")) if step2.get("ok") else "",
    })

    # ③ 子フォルダの id から、さらに中を取れるか（＝再帰できるか）
    sub_folder = next((c for c in children if isinstance(c.get("folder"), dict)), None)
    if not drive_id or not sub_folder:
        reason = ("②で子フォルダが見つからなかったため実行していません"
                  "（中にフォルダがある行で再度お試しください）。"
                  if step2.get("ok") else "②が失敗したため実行していません。")
        rows.append({
            "label": "③ さらに1階層下を取得（再帰できるか）",
            "endpoint": "GET /drives/{driveId}/items/{itemId}/children",
            "ok": False, "status": None, "message": reason, "detail": "",
            "skipped": True,
        })
        step3_ok = None       # 判定不能
    else:
        step3 = _explore_diag_get(
            token,
            f"{GRAPH_V1}/drives/{drive_id}/items/{sub_folder.get('id')}/children?$top=10",
            timeout)
        grand = list((step3.get("data") or {}).get("value") or [])
        rows.append({
            "label": "③ さらに1階層下を取得（再帰できるか）",
            "endpoint": "GET /drives/{driveId}/items/{itemId}/children",
            "ok": bool(step3.get("ok")),
            "status": step3.get("status"),
            "message": step3.get("message", "") if not step3.get("ok") else "",
            "detail": (f"「{sub_folder.get('name')}」の中から {len(grand)} 件取得"
                       if step3.get("ok") else ""),
        })
        step3_ok = bool(step3.get("ok"))

    # 応答に必要な項目が揃っているか（列を埋められるかの確認）
    sample = children[0] if children else {}
    fields = [{"key": key, "label": label, "present": key in sample}
              for key, label in EXPLORE_WANTED_FIELDS]

    # 総合判定
    if not rows[0]["ok"]:
        verdict = {"ok": False,
                   "message": "起点フォルダの情報が取得できませんでした。"
                              "フォルダ以外の行を選んでいないか、"
                              "リンクが有効かをご確認ください。"}
    elif not rows[1]["ok"]:
        verdict = {"ok": False,
                   "message": "子要素の取得が拒否されました。現在の権限では"
                              "フォルダ探索を実装できません（設計の見直しが必要です）。"}
    elif step3_ok is False:
        verdict = {"ok": False,
                   "message": "1階層目は開けましたが、その先へ潜れませんでした。"
                              "再帰探索は現在の方式では実装できません。"}
    elif step3_ok is None:
        verdict = {"ok": True,
                   "message": "1階層目は開けました。ただし中にフォルダが無かったため、"
                              "再帰（2階層目以降）は未確認です。"
                              "サブフォルダを持つフォルダで、もう一度お試しください。"}
    else:
        verdict = {"ok": True,
                   "message": "①〜③すべて成功しました。現在の権限のまま、"
                              "設計どおりの再帰探索を実装できます。"}

    return {
        "target": {"title": result.title, "url": result.url},
        "rows": rows,
        "fields": fields,
        "samples": [_explore_child_label(c) for c in children[:5]],
        "verdict": verdict,
    }


@flask_app.route("/api/explore_diag", methods=["POST"])
def api_explore_diag():
    """フォルダ探索が現在の権限で可能かを、実際のGraph呼び出しで確かめる。

    idx を省略した場合は、直近の検索結果から最初のフォルダ行を自動で選ぶ
    （越智さんが行を選ばなくてもボタン1つで試せるようにするため）。
    """
    payload = flask_req.get_json(silent=True) or {}
    idx_param = str(payload.get("idx", "")).strip()

    if idx_param:
        picked = _pick_results(idx_param)
        result = picked[0] if picked else None
    else:
        with _lock:
            candidates = list(_last_results)
        result = next((r for r in candidates if r.is_folder and r.url), None)

    if result is None:
        return jsonify({"error": "検索結果にフォルダがありません。"
                                "先にSharePointを検索して、フォルダを含む結果を"
                                "表示してから実行してください。"}), 400
    if not result.is_folder:
        return jsonify({"error": f"「{result.title}」はフォルダではありません。"
                                "フォルダの行を選んで実行してください。"}), 400
    if result.source == "Enovia":
        return jsonify({"error": "Enoviaのフォルダは探索の対象外です"
                                "（検索も認証も別系統のため）。"}), 400
    ...
    timeout = int(_cfg.get("request_timeout_sec", 30))
    try:
        report = _explore_diagnose(token, result, timeout)
    except Exception as e:
        return jsonify({"error": f"診断に失敗しました: {e}"}), 500

    print(f"🩺 フォルダ探索診断: \"{result.title}\"")
    for row in report["rows"]:
        mark = "🟢" if row["ok"] else ("⚪" if row.get("skipped") else "🔴")
        print(f"   {mark} {row['label']} … "
              f"{row.get('detail') or row.get('message') or ''}")
    print(f"   判定: {report['verdict']['message']}")
...
#   ① GET /shares/{token}/driveItem
#   ② GET /shares/{token}/driveItem/children
#   ③ GET /drives/{driveId}/items/{itemId}/children
# の3段すべてが通ることを実機で確認済み。本節はその結果に基づく実装。
#
# 探索結果は SearchResult として組み立てる。こうすることで、絞り込み・
# 並べ替え・選択・ZIP取得・Excel/CSV出力・要約という既存の仕組みを
# そのまま再利用できる（DESIGN_NOTES 3-6）。
# ─────────────────────────────────────────────────────────────

# 越智さん承認済みの上限。無制限にすると、巨大なライブラリを指定したときに
# Graphへの呼び出しが数千回に膨らみ、実質的に止められなくなるため。
# 探索の深さ（Lv）。起点フォルダ＝Lv0、そのすぐ下＝Lv1、もう一つ下＝Lv2。
# 越智さんの指定（2026-09-25）：Lv1〜Lv4から選べて、標準はLv2。
# 選んだLvまでを一括で取り、その先は画面の▶で1階層ずつ読み込む
# （/api/explore_children）。以前は常に5階層まで掘っており、数分かかっていた。
EXPLORE_LEVELS = (1, 2, 3, 4)
EXPLORE_DEFAULT_LEVEL = 2
EXPLORE_MAX_ITEMS = 2000   # 起点フォルダを除く、見つけた項目数の上限
EXPLORE_PAGE_SIZE = 200    # 1回の /children で取る件数（Graphの実用的な上限）
EXPLORE_MAX_PAGES = 50     # 1フォルダあたりのページ送り回数の上限（安全弁）

# 取得する項目を明示する。既定では file/folder の判定に必要な情報が
# 省かれることがあり、事前診断でも「ファイルかどうかの判定」が
# 応答に含まれない事例を観測したため（診断ログの🟡）。
EXPLORE_SELECT = ("id,name,size,lastModifiedDateTime,webUrl,"
                  "folder,file,createdBy,parentReference")


def _explore_get_json(token: str, url: str, timeout: int) -> Dict[str, Any]:
    """Graphを1回呼び、JSONを返す。失敗は例外にする（呼び出し側でまとめて扱う）。"""
    resp = http_req.get(url, headers={"Authorization": f"Bearer {token}"},
                        timeout=timeout)
    if resp.status_code != 200:
...
def _explore_resolve_root(token: str, result: SearchResult,
                          timeout: int) -> "tuple[str, str]":
    """起点フォルダの共有URLから (item_id, drive_id) を得る。"""
    url = (f"{GRAPH_V1}/shares/{_share_token(result.url)}/driveItem"
           f"?$select={EXPLORE_SELECT}")
    item = _explore_get_json(token, url, timeout)
    item_id = str(item.get("id") or "")
    drive_id = str((item.get("parentReference") or {}).get("driveId") or "")
    if not item_id or not drive_id:
        raise RuntimeError("フォルダの id / driveId を取得できませんでした。")
    return item_id, drive_id


def _explore_list_children(token: str, drive_id: str, item_id: str,
                           timeout: int) -> List[Dict[str, Any]]:
    """1フォルダの直下を、ページ送りを最後まで追って全件返す。"""
    url = (f"{GRAPH_V1}/drives/{drive_id}/items/{item_id}/children"
           f"?$top={EXPLORE_PAGE_SIZE}&$select={EXPLORE_SELECT}")
    items: List[Dict[str, Any]] = []
    pages = 0
    while url and pages < EXPLORE_MAX_PAGES:
        data = _explore_get_json(token, url, timeout)
        items.extend(data.get("value") or [])
        # nextLink には $top / $select が引き継がれているため、そのまま使う
        url = str(data.get("@odata.nextLink") or "")
        pages += 1
    return items


def _explore_child_to_result(item: Dict[str, Any], root: SearchResult,
                             parent_item_id: str, drive_id: str,
                             depth: int, path_text: str) -> SearchResult:
    """Graphの driveItem 1件を、画面と同じ SearchResult に変換する。"""
    is_folder = isinstance(item.get("folder"), dict)
    name = str(item.get("name") or "(名前なし)")
    size = item.get("size")
    author = str((((item.get("createdBy") or {}).get("user") or {})
                  .get("displayName")) or "")
    return SearchResult(
        source=root.source or "SharePoint",
        title=name,
        author=author,
        last_modified=_format_datetime(str(item.get("lastModifiedDateTime") or "")),
        doc_type=("フォルダ" if is_folder else _extension_of(name)),
        site=root.site,
        site_url=root.site_url,
        folder=path_text,
        folder_full=path_text,
        folder_url=root.url,
        is_folder=is_folder,
        ...
        drive_id=drive_id,
        depth=depth,
        size=int(size) if isinstance(size, int) and not is_folder else 0,
        child_count=int((item.get("folder") or {}).get("childCount") or 0)
                    if is_folder else 0,
        root_label=root.root_label or root.title,
        path_text=path_text,
    )


def _explore_root_to_result(root: SearchResult, item_id: str,
                            drive_id: str) -> SearchResult:
    """起点フォルダ自身を、ツリーの根としての1行にする。"""
    return SearchResult(
        source=root.source or "SharePoint",
        title=root.title,
        author=root.author,
        ...
        site_url=root.site_url,
        folder=root.folder,
        folder_full=root.folder_full,
        folder_url=root.folder_url,
        is_folder=True,
        url=root.url,
        item_id=item_id,
        parent_item_id="",
        drive_id=drive_id,
        depth=0,
        root_label=root.title,
        path_text="",
    )


def _explore_progress(job_id: str, **fields) -> None:
    """進捗を更新する（画面はこれを一定間隔で読みに来る）。"""
    with _explore_lock:
...
def _explore_level(value: Any) -> int:
    """画面・保存ファイルから届いた深さを検査する。1〜4以外は標準のLv2にする。"""
    try:
        level = int(value)
    except (TypeError, ValueError):
        return EXPLORE_DEFAULT_LEVEL
    return level if level in EXPLORE_LEVELS else EXPLORE_DEFAULT_LEVEL


def _explore_run(job_id: str, roots: List[SearchResult], token: str,
                 timeout: int, level: int = EXPLORE_DEFAULT_LEVEL) -> None:
    """幅優先で、指定した深さ（Lv）まで配下をたどる。

    Lv の深さにあるフォルダは行として並べるが、中身は取らない（画面の▶で
    開いたときに /api/explore_children で1階層ずつ読み込む）。
    件数の上限に当たったら、そこで打ち切って理由を残す。

    幅優先にしているのは、上限で打ち切られたときに「浅い階層は全部見えている」
    状態で終われるため。深さ優先だと、最初の枝だけ深く掘って他が空になる。
    """
    global _explore_results
    found: List[SearchResult] = []
    errors: List[str] = []
    # (drive_id, item_id, 子の深さ, 子に付けるパス表記, 起点)
    queue: "List[tuple]" = []

    for root in roots:
        try:
            item_id, drive_id = _explore_resolve_root(token, root, timeout)
        except Exception as e:
            errors.append(f"{root.title}: 起点フォルダを特定できませんでした（{e}）")
            continue
        found.append(_explore_root_to_result(root, item_id, drive_id))
        queue.append((drive_id, item_id, 1, root.title, root))

    # 起点フォルダの特定に失敗したものがあると len(roots) とはずれるため、
    # 実際に根として並べた行数を数えておく（検出件数は「起点を除いた数」）。
    root_count = len(found)
    scanned = 0
    stopped = ""
    while queue:
        if len(found) - root_count >= EXPLORE_MAX_ITEMS:
            stopped = (f"上限の {EXPLORE_MAX_ITEMS} 件に達したため、"
                       "ここで打ち切りました。")
            break

        drive_id, item_id, depth, path_text, root = queue.pop(0)
        _explore_progress(job_id, current=path_text, scanned=scanned,
                          found=len(found) - root_count)
        try:
            children = _explore_list_children(token, drive_id, item_id, timeout)
        except Exception as e:
            errors.append(f"{path_text}: 配下を取得できませんでした（{e}）")
            continue
        scanned += 1
        ...
        for child in children:
            if len(found) - root_count >= EXPLORE_MAX_ITEMS:
                ...
                break
            name = str(child.get("name") or "(名前なし)")
            child_path = f"{path_text}/{name}" if path_text else name
            row = _explore_child_to_result(child, root, item_id, drive_id,
                                           depth, path_text)
            found.append(row)
            if row.is_folder and depth < level:
                queue.append((drive_id, row.item_id, depth + 1, child_path, root))

    with _lock:
        _explore_results = found

    folders = sum(1 for r in found if r.is_folder)
    # 指定Lvの深さにあり、中身がまだ読み込まれていないフォルダの数
    # （画面で「▶で次の階層を読み込めます」と案内するため）。
    unloaded = sum(1 for r in found
                   if r.is_folder and r.depth >= level and r.child_count > 0)
    _explore_progress(
        job_id,
        done=True,
        level=level,
        ...
        found=len(found) - root_count,
        folders=folders,
        files=len(found) - folders,
        stopped=stopped,
        errors=errors,
        results=[asdict(r) for r in found],
    )
    print(f"🌲 フォルダ探索: 起点 {len(roots)} 件 / Lv{level}まで / 走査 {scanned} フォルダ / "
          f"検出 {len(found) - root_count} 件"
          + (f" / 打ち切り: {stopped}" if stopped else ""))


@flask_app.route("/api/explore", methods=["POST"])
def api_explore():
    """選択されたフォルダの配下を、別スレッドで再帰的にたどり始める。

    時間のかかる処理を要求の中で完結させると、画面が固まったのか
    動いているのか分からなくなるため、ジョブ化して進捗を返す方式にする。
    """
    payload = flask_req.get_json(silent=True) or {}
    idx_param = str(payload.get("idx", "")).strip()
    if not idx_param:
        return jsonify({"error": "探索するフォルダが選択されていません。"}), 400

    picked = _pick_results(idx_param)
    roots = [r for r in picked if r.is_folder and r.url and r.source != "Enovia"]
    if not roots:
        return jsonify({"error": ("探索できるフォルダが選択されていません"
                                  "（SharePoint / Nexus のフォルダのみ対象です）。")}), 400
    ...
    timeout = int(_cfg.get("request_timeout_sec", 60))
    level = _explore_level(payload.get("level"))
    job_id = uuid.uuid4().hex
    with _explore_lock:
        _explore_jobs.clear()   # 同時に走らせない（進捗の取り違えを防ぐ）
        _explore_jobs[job_id] = {
            "done": False, "current": "", "scanned": 0, "found": 0,
            "folders": 0, "files": 0, "stopped": "", "errors": [],
            "roots": [r.title for r in roots], "level": level,
        }

    thread = threading.Thread(target=_explore_run,
                              args=(job_id, roots, token, timeout, level), daemon=True)
    thread.start()
    return jsonify({"job": job_id, "roots": [r.title for r in roots], "level": level})


@flask_app.route("/api/explore_children", methods=["POST"])
def api_explore_children():
    """探索結果のフォルダ1つについて、中身（次の1階層）を読み込んで足す。

    画面のツリーで、指定Lvより先のフォルダの▶を押したときに呼ばれる。
    読み込んだ行はサーバー側の探索結果（_explore_results）の**末尾に同じ順で**
    足し、足し始めた位置（start）を返す。画面はこの位置が自分の行数と一致する
    ときだけ足す。一致しないまま足すと、行の選択番号がずれて、ZIP・Excel・
    要約で別のファイルが選ばれてしまうため（Phase2 問題1）。
      - 読み込み済みのフォルダなら、追加せず既存の行の位置を返す（問題2）
      - 今の探索結果に無いフォルダは読み込まない（問題3：古い画面からの要求）
    """
    payload = flask_req.get_json(silent=True) or {}
    item_id = str(payload.get("item_id") or "").strip()
    drive_id = str(payload.get("drive_id") or "").strip()
    if not item_id or not drive_id:
        return jsonify({"error": "開くフォルダが指定されていません。"}), 400

    with _lock:
        parent = next((r for r in _explore_results
                       if r.item_id == item_id and r.drive_id == drive_id and r.is_folder),
                      None)
        existing = [i for i, r in enumerate(_explore_results)
                    if r.parent_item_id == item_id and r.drive_id == drive_id]
    if parent is None:
        return jsonify({"error": "このフォルダは今の探索結果にありません。"
                                 "もう一度「選択フォルダを探索」からやり直してください。"}), 409
    if existing:
        return jsonify({"added": False, "indices": existing})
    ...
    try:
        ...
        return jsonify({"error": f"「{parent.title}」の中身を取得できませんでした: {e}"}), 502

    # 子の「場所」は、親フォルダの場所＋親フォルダ名（_explore_run と同じ付け方）
    child_path = f"{parent.path_text}/{parent.title}" if parent.path_text else parent.title
    base = SearchResult(source=parent.source, title=parent.root_label, site=parent.site,
                        site_url=parent.site_url, url=parent.url,
                        root_label=parent.root_label)
    rows = [_explore_child_to_result(child, base, item_id, drive_id,
                                     parent.depth + 1, child_path)
            for child in children]

    with _lock:
        # 読み込みの間に別の要求が同じフォルダを足していないか、もう一度確かめる
        again = [i for i, r in enumerate(_explore_results)
                 if r.parent_item_id == item_id and r.drive_id == drive_id]
        if again:
            return jsonify({"added": False, "indices": again})
        start = len(_explore_results)
        _explore_results.extend(rows)
    print(f"🌲 フォルダを開きました: {child_path}（{len(rows)} 件）")
    return jsonify({"added": True, "start": start, "rows": [asdict(r) for r in rows]})


@flask_app.route("/api/explore_status")
def api_explore_status():
    """探索ジョブの進捗を返す。完了していれば結果もあわせて返す。"""
    job_id = str(flask_req.args.get("job") or "")
    with _explore_lock:
        job = _explore_jobs.get(job_id)
        snapshot = dict(job) if job else None
    if snapshot is None:
        return jsonify({"error": "その探索は見つかりません（やり直してください）。"}), 404
...
def api_download():
    ...
    results = [r for r in _pick_results(idx_param, scope)
               if r.url and not r.is_folder]
    if not results:
        return jsonify({"error": "ダウンロード対象が選択されていません。"}), 400
...
def _port_in_use(port: int) -> bool:
    """127.0.0.1 の指定ポートで、既に何かが待ち受けているかを調べる。

    Flask(Werkzeug)は SO_REUSEADDR を有効にするため、Windowsでは同じポートに
    2つ目のプロセスが「成功」として結び付いてしまう。その場合、通信は先に
    居座っている古いプロセスが処理し、画面だけが古い版のまま残る。
    起動前にこの関数で検出し、はっきり止める。
    """
...
# ─────────────────────────────────────────────────────────────
# ポート使用中の自動解消（越智さんのご要望、2026-09-24）
#
# 「毎回起動時にポート使用中になる」の実態は、ほぼ常に**自分自身の
# 旧起動（閉じ忘れたコマンドプロンプト）**である。そこで、ポートを
# 使っているプロセスが「自分自身のDocument Search Managerである」と
# 確認できた場合だけ自動終了し、確認できない場合（無関係なプロセスかも
# しれない）は、従来どおり起動を中止して案内する（誤って無関係な
# プロセスを終了させる事故を避けるため。安全側に倒す設計）。
# 新規の依存パッケージは追加せず、Windows標準コマンド
# （netstat / wmic またはPowerShell / taskkill）のみで実現する。
# ─────────────────────────────────────────────────────────────
def _is_windows() -> bool:
    """Windows上で動作しているかどうか（テストで差し替えられるよう関数化）。"""
    return os.name == "nt"


def _find_port_owner_pid(port: int) -> Optional[int]:
    """netstatの出力から、127.0.0.1:<port>でLISTENINGしているPIDを1つ探す。

    見つからない・複数見つかった・netstat自体が使えない場合は、いずれも
    Noneを返す（呼び出し側で「特定できない＝自動終了しない」につなげる）。
    """
    if not _is_windows():
        return None
    try:
    ...
    # netstatの「状態」列は日本語Windowsでも LISTENING のまま変わらない
    # （実機の複数バージョンで確認済み）。念のため大文字小文字は無視する。
    # ★2026-09-25 実機バグ対応★ netstatの見出し行（「ローカル アドレス」等）は
    # 日本語Windowsでは CP932(Shift-JIS) で出力される。errors="replace" が
    # 無いと、その環境依存の文字でデコードに失敗し、result.stdout が
    # None のまま返ってきて次の .splitlines() で丸ごとクラッシュしていた
    # （実機で確認済み）。見出し行の内容自体は使わないため、文字化けしても
    # 実害は無い（下の判定はTCP/LISTENING/ポート番号という数字・英字のみで
    # 行うため）。
    ...
    for line in (result.stdout or "").splitlines():
        ...
        if len(parts) < 5:
            continue
        proto, local_addr, state, pid = parts[0], parts[1], parts[-2], parts[-1]
        if proto.upper() != "TCP" or state.upper() != "LISTENING":
            continue
        if local_addr != target:
            continue
        if pid.isdigit():
            pids.add(int(pid))

    if len(pids) != 1:
        return None  # 0件・複数件はどちらも「特定できない」扱い（安全側）
    return pids.pop()


def _parse_wmic_list_output(text: str) -> Dict[str, str]:
    """wmicの /FORMAT:LIST 出力（Key=Value形式）からName/CommandLineを取り出す。"""
    out: Dict[str, str] = {}
    for line in text.splitlines():
        ...
        if key == "name":
            ...
            out["command_line"] = value
    return out


def _process_command_line(pid: int) -> Optional[Dict[str, str]]:
    """指定PIDのイメージ名とコマンドラインを調べる。

    wmicを先に試し（多くのWindows 10/11でまだ使える）、失敗したら
    PowerShellのGet-CimInstanceにフォールバックする（wmicが廃止・
    無効化された環境向け）。どちらも失敗したらNone（＝確認できない）。
    """
    if not _is_windows():
        return None
    no_window = getattr(subprocess, "CREATE_NO_WINDOW", 0)

    # ★2026-09-25 実機バグ対応★ 以下2箇所とも errors="replace" が必須。
    # コマンドライン自体（Name/CommandLine）は英数字のみの想定だが、
    # 環境（エラーメッセージ等）によっては日本語が混ざることがあり、
    # デコード失敗でstdoutがNoneになり得るため .strip()/.partition() の
    # 前に (result.stdout or "") でNoneを吸収する（netstat側と同じ原因）。
    try:
        result = subprocess.run(
        ...
        if result.returncode == 0 and stdout.strip():
            info = _parse_wmic_list_output(stdout)
            if info.get("name"):
                return info
    except Exception:
        pass
    ...
    try:
        ps_cmd = (
            ...
            "if ($p) { Write-Output ($p.Name + '|' + $p.CommandLine) }"
        )
        result = subprocess.run(
        ...
        if result.returncode == 0 and "|" in stdout:
            name, _, cmdline = stdout.strip().partition("|")
            if name:
                return {"name": name, "command_line": cmdline}
    except Exception:
        pass
...
def _confirm_self_process(pid: int) -> bool:
    """指定PIDが「自分自身（Document Search Manager）」だと確認できるか判定する。

    python.exe / pythonw.exe のいずれかで、コマンドラインに
    document_search_manager を含む場合だけTrue。それ以外（別プロセス・
    情報が取れない）はFalse（自動終了の対象にしない。安全側に倒す）。
    """
    info = _process_command_line(pid)
    if not info:
        return False
    name = str(info.get("name") or "").strip().lower()
    cmdline = str(info.get("command_line") or "").lower()
    if name not in ("python.exe", "pythonw.exe"):
        return False
    return "document_search_manager" in cmdline


def _try_auto_kill_port_owner(port: int, cfg: Dict[str, Any]) -> bool:
    """ポートを使用しているプロセスが自分自身の旧起動だと確認できた場合だけ、
    自動的に終了させてポートを空ける。実際に空いたことまで確認できたときに
    Trueを返す。確認できない・終了に失敗した・終了後も空かない場合は、
    すべてFalseを返し、呼び出し側の従来どおりの中止案内に委ねる。
    """
    if not cfg.get("auto_kill_port_conflict", True):
        return False
    if not _is_windows():
        return False

    pid = _find_port_owner_pid(port)
    if pid is None:
        print(f"⚠️  ポート {port} を使用しているプロセスを特定できませんでした。")
        return False

    if not _confirm_self_process(pid):
        print(f"⚠️  ポート {port} を使用しているプロセス(PID {pid})が、"
              "Document Search Manager自身であると確認できませんでした。"
              "無関係なプロセスを誤って終了させないため、自動終了は行いません。")
        return False
    ...
    try:
        ...
        print(f"⚠️  旧プロセス(PID {pid})の終了に失敗しました: {e}")
        return False

    if result.returncode != 0:
        print(f"⚠️  旧プロセス(PID {pid})の終了に失敗しました: "
              f"{(result.stderr or '').strip()}")
        return False

    print(f"🔧 ポート {port} を使用していた旧プロセス(PID {pid})を終了しました"
          "（自分自身のDocument Search Managerと確認済み）。")

    # taskkill直後は、OSがポートを解放するまで一瞬かかることがあるため、
    # 短い間隔で数回だけ再確認してから結果を返す。
    for _ in range(10):
        time.sleep(0.3)
        if not _port_in_use(port):
            return True

    print(f"⚠️  旧プロセスは終了しましたが、ポート {port} がまだ解放されていません。")
    return False
...
def main() -> None:
    ...
    print("=" * 70)
    print("📚 Document Search Manager  v20260925_04  (探索の深さ指定)")
    print("=" * 70)
    ...
    if _port_in_use(port) and not _try_auto_kill_port_owner(port, _cfg):
        print("=" * 70)
        print(f"⛔ ポート {port} は既に使用中です。古いウィンドウが残っている"
              "可能性があります。")
        print("   Windowsでは、この状態でも起動に成功したように見えてしまい、")
        print("   ブラウザには古い版の画面が表示され続けます（実際に発生した事故）。")
        print("   対処: 開いているコマンドプロンプトをすべて閉じるか、")
        print("         taskkill /F /IM python.exe を実行してから、もう一度起動してください。")
        print("=" * 70)
        sys.exit(1)
