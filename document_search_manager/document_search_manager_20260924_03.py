...
# -*- coding: utf-8 -*-
"""
Document Search Manager  v20260924_03
=====================================
...
<header>
  ...
  <div class="sub">社内ドキュメント横断検索 &nbsp;/&nbsp; v20260924_03 (ポート使用中の自動解消)</div>
</header>
...
def _process_command_line(pid: int) -> Optional[Dict[str, str]]:
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            info = _parse_wmic_list_output(result.stdout)
            if info.get("name"):
                return info
    except Exception:
        pass
    ...
    try:
        ...
        if result.returncode == 0 and "|" in result.stdout:
            name, _, cmdline = result.stdout.strip().partition("|")
            if name:
...
def _try_auto_kill_port_owner(port: int, cfg: Dict[str, Any]) -> bool:
    ...
    if result.returncode != 0:
        print(f"⚠️  旧プロセス(PID {pid})の終了に失敗しました: {result.stderr.strip()}")
        return False
...
def main() -> None:
    ...
    print("=" * 70)
    print("📚 Document Search Manager  v20260924_03  (ポート使用中の自動解消)")
    print("=" * 70)
