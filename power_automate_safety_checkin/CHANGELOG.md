...
`EQ05_Status_Summary_DEV`をオンにした際、S02時点の実機事故で作られた古いイベント
(`EQ-20260901-014730-TOKYO`。`AlertStatus`が`Open`のまま放置されていた)を
拾ってしまい、TOKYOの実チャネルへ集計カードが15分ごとに2回投稿された。
個人宛の安否確認カードが新たに実在の社員へ送られたわけではないが、実チャネルへの
意図しない投稿であるため記録する。`docs/NEXT_TASK.md`のKnown Risksに事前に
記載されていた懸念がそのまま発生したもの。

`EQ05`をオフにし、該当イベントの`AlertStatus`を手動で`Closed`に修正して解消した。
`eventClose`機能により今後作成されるイベントは自動的にClosedになるため再発しないが、
**`eventClose`実装前に作られた古いイベントは`Open`のまま残っている**ため、
`EQ05`を長時間オンにする前の棚卸しをP1完成条件のチェック項目に追加した
(`docs/GATE_STATUS.md`参照)。

## 一括クローズ - 2026-09-03

上記インシデントの再発防止として、`AlertStatus=Open`のまま残っていた9/1付けの
`NARA`テストイベント10件を、SharePoint REST APIをブラウザのコンソールから
直接呼び出して一括で`Closed`に更新した(まず`$filter=field_9 eq 'Open'`で対象を
確認してから、`X-HTTP-Method: MERGE`で1件ずつPATCHする2段階)。全10件`204`で成功。
これで`EQ_Events`に`Open`の行は残っていない状態になった。

## [20260901_05] - 2026-09-02 〜 2026-09-03(実機確認完了)

**クローズ処理の実機確認が完了した(2026-09-03)。** NARA拠点で訓練を1回実行し、
全員回答後に`EQ_Events`の該当行(`EQ-20260903-062327-NARA`)の`AlertStatus`が
`Closed`になることを確認した。それ以前(9/1以前、機能有効化前)に実行した4回は
`Open`のまま残っているが、クローズ処理は実行後の回にしか効かないため想定どおり。

これでP1完成条件のうち「エラー処理」「イベントのクローズ処理」が両方とも
実機で動作確認済みになった。

`EQ05_Status_Summary_DEV`を実機でオンにできることを確認した(2026-09-02)。

`runtimeSource`を`tenant`に変えたことで、`invoker`のときは不要だった
「フロー内の各SharePoint/Teamsアクションが、どの接続を使うか」という紐付けを
GUIで個別に選び直す作業が追加で必要になった(`docs/MANUAL_STEPS.md`が以前から
Gate Eとして挙げていた「接続参照の初回マッピング」)。`EQ05`には接続が必要な
アクションが5つあり、1つ直すと保存エラーが次の未解決アクションへ移る形で
順に表面化した。手順は`docs/GATE_STATUS.md`に記録した。

これでP1完成条件のうち「エラー処理」「イベントのクローズ処理」がどちらも
実機で動作確認済みになった。
