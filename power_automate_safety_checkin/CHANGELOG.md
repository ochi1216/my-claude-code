...
- **エラー処理・クローズ処理を含むSolutionのインポートが成功した(2026-09-01)。**
  ...
  実行時の動作は引き続き未検証。
- **エラー処理の動作確認を、誰にもカードを送らずに行えるようにした。**
  `--write-error-test-copy`で、EQ_EventsのリストGUIDだけを存在しない値へ
  差し替えた設定の写しを作れる(元のファイルには触らない)。`SP_Create_Event`は
  フローで最初のコネクタ呼び出しのため、ここで落とせばTeamsへの投稿は一度も
  起きない。`features.errorLogging`が有効でなければ実行を断る。
