...
**`IsTest`(訓練)で実行したときだけ、名簿のアドレスではなくその宛先へ**個人カードを送る。

検証段階で名簿に実在の同僚が入っていると、テスト実行のたびに本人へ安否確認カードが
届いてしまうため(S02で実際に発生した)、その事故を防ぐための安全弁である。
本番運用に移る際は、この値を空文字にすること。

`IsTest`をオフにした実行では、この設定に関係なく名簿どおりの宛先へ送信される。
