<#
.SYNOPSIS
    地震安否確認システム用のSharePointリスト(4個)と列を自動作成し、
    メンバー一覧(config/members.example.json)を一括投入する。
...
.DESCRIPTION
    ...
    拠点(大分・大阪・東京)の設定は、震度閾値・通知先が固定運用のため
    SharePointリストではなくPower Automateのフロー内(Switchアクション)に
    直接持たせる方針に変更した。このスクリプトは`EQ_Config_Sites`リストを
    作成しない(docs/FLOW_LOGIC_SPEC.md参照)。
...
.EXAMPLE
    ...
    ./provision_sharepoint_20260730_03.ps1 -SiteUrl "https://contoso.sharepoint.com/sites/EQSafetyCheckin" `
...
# モジュールの自動読み込みが働かない環境があるため明示的にインポートする
# (実機で "Connect-PnPOnline is not recognized" が発生したことへの対処)。
...
Write-Host "`n=== Done. 4 lists ready, members loaded. ==="
Write-Host "Next: create SharePoint columns' internal names snapshot for Gate D evidence:"
Write-Host "  Get-PnPField -List EQ_Config_Members | Select DisplayName, InternalName | Format-Table"
