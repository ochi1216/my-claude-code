...
使い方:
    streamlit run project_cost_analyzer_20260724_01.py
...
def render_period_bar(
    ...
    clickable=Trueの場合、バーをクリックしてperiod_labelを選択できるようにし、
    ...
    # クリック時のx値が"YYYY-MM-01"のような日付文字列になりperiod_labelと一致しなくなる
    fig.update_layout(
        xaxis=dict(title="会計年度-期", type="category"),
        yaxis_title=f"Cost ({CURRENCY})",
        legend_title=stack_col or "",
    )
    chart_kwargs = dict(use_container_width=True, key=f"{key_prefix}_period_chart")
    if clickable:
        chart_kwargs.update(on_select="rerun", selection_mode="points")
    event = st.plotly_chart(fig, **chart_kwargs)
    ...
    valid_periods = set(data["period_label"])
    if clicked not in valid_periods:
        try:
            parsed = pd.to_datetime(clicked)
            clicked = f"{parsed.year}-{parsed.month:02d}"
        except (ValueError, TypeError):
            return None
    return clicked if clicked in valid_periods else None
...
def render_breakdown(
    ...
    clickable=Trueの場合、グラフの棒／円をクリックしてgroup_colの値を選択できるようにし、
    クリックされた値を返す（未クリック・非クリック指定時はNone）。選択状態はStreamlit側の
    ウィジェット単位で維持されるため、再クリックするまで前回選択した値が返り続ける。
    """
    ...
    chart_kwargs = dict(use_container_width=True, key=f"{key}_chart")
    if clickable:
        chart_kwargs.update(on_select="rerun", selection_mode="points")
    event = st.plotly_chart(fig, **chart_kwargs)
    ...
    if not points:
        return None
    # 円グラフのクリックはlabel、横棒グラフのクリックはカテゴリ軸(y)にgroup_colの値が入る
    return points[0].get("label") if as_pie else points[0].get("y")
...
def render_axis_drilldown(df: pd.DataFrame, key_prefix: str = "dd") -> None:
    ...
    clicked_value = render_breakdown(
        scoped, axis_col, f"{axis_label} 内訳", key=f"{key_prefix}_breakdown", clickable=True
    )

    values = sorted(scoped[axis_col].dropna().unique())
    widget_key = f"{value_key}_widget"
    # グラフの棒／円をクリックした場合、下のセレクトボックスにもその選択を反映する。
    # selectbox自体のkey（widget_key）はStreamlit側が独自に状態を保持するため、
    # 永続化用のvalue_keyだけを更新してもindex指定は2回目以降のrerunでは無視される
    # （ウィジェットが初めて生成された回以降はウィジェット自身の状態が優先されるため）。
    # クリックで表示を変えるには、ウィジェット自身のkeyにも直接書き込む必要がある。
    if clicked_value in values:
        st.session_state[value_key] = clicked_value
        st.session_state[widget_key] = clicked_value
    ...
    value_index = values.index(st.session_state[widget_key])
    picked = st.selectbox(
        f"{axis_label}の項目を選んで明細を表示（グラフをクリックしても選択できます）",
        values, index=value_index, key=widget_key
    )
...
def tab_portfolio(df: pd.DataFrame, default_stacked: bool) -> None:
    ...
    clicked_pf_period = render_period_bar(df, key_prefix="pf", default_stacked=default_stacked, clickable=True)
    render_period_click_drilldown(df, clicked_pf_period, key_prefix="pf")
...
def render_period_click_drilldown(df: pd.DataFrame, clicked_period: str | None, key_prefix: str) -> None:
    """クリックされた期（period_label）のコスト種別・外部購買内訳・明細テーブルを、
    枠線付きコンテナ内に表示する（プロジェクト単位・事業部俯瞰など、期間別バーチャートを
    クリック可能にした呼び出し元から共通で使う）。

    `df`はクリックされた期でフィルタする対象の全体（プロジェクト単位なら`sub`、事業部俯瞰
    なら対象スコープ全体の`df`）。`clicked_period`がNoneの場合はヒントのみ表示する。
    """
    if not clicked_period:
        st.caption("💡 バーをクリックすると、その期のコスト種別・外部購買内訳・明細を表示できます。")
        return

    period_sub = df[df["period_label"] == clicked_period]
    with st.container(border=True, key=f"{key_prefix}_click_drilldown"):
        ...
        c1, c2 = st.columns(2)
        with c1:
            selected_costcat = render_breakdown(
                period_sub, "Cost Category", f"コスト種別（内部労務 vs 外部購買）― {clicked_period}のみ",
                key=f"{key_prefix}_click_costcat", clickable=True,
            )
        with c2:
            ...
            if ext.empty:
                ...
                selected_fsi = render_breakdown(
                    ext, "FSI Description", "外部購買内訳（FSI Description別）",
                    key=f"{key_prefix}_click_fsi", clickable=True,
                )

        table_source = period_sub
        narrow_notes = []
        if selected_costcat:
            table_source = table_source[table_source["Cost Category"] == selected_costcat]
            narrow_notes.append(f"Cost Category「{selected_costcat}」")
        if selected_fsi:
            table_source = table_source[table_source["FSI Description"] == selected_fsi]
            narrow_notes.append(f"FSI Description「{selected_fsi}」")

        st.markdown(f"###### {clicked_period} の明細（{len(table_source):,}行）")
        caption = f"🔒 この表はあらかじめ **{clicked_period}** の明細のみに絞り込まれています"
        if narrow_notes:
            caption += "。さらに上の棒グラフのクリックで " + " かつ ".join(narrow_notes) + " に絞り込み中"
        caption += "（下の「🔍 フィルタ」は、この範囲内でのさらなる絞り込み用です）。"
        st.caption(caption)
        render_filterable_table(table_source, key=f"{key_prefix}_click_table")
...
def render_project_burn_chart(
    ...
    # 月次コストのバーをクリックすると、その期のコスト種別内訳・外部購買内訳・明細を表示する
    # （バーは常にcurve_number=0の1本目のトレース。累計線・マイルストーンの点はcurve_number>0）
    points = (event.get("selection") or {}).get("points", []) if event else []
    ...
    if bar_points:
        ...
        if clicked_period not in visible_periods:
            ...
            try:
                ...
                pass
    render_period_click_drilldown(sub, clicked_period, key_prefix)
...
def tab_project(
    ...
    # 内部労務 vs 外部購買（クリックしたCost Categoryでプロジェクト全期間の明細を絞り込み表示）
    selected_costcat_all = render_breakdown(
        sub, "Cost Category", "コスト種別（内部労務 vs 外部購買）― プロジェクト全体", key="proj_costcat",
        clickable=True,
    )
    if selected_costcat_all:
        st.caption(f"🎯 Cost Category「{selected_costcat_all}」の明細（グラフを再クリックで変更）")
        render_filterable_table(
            sub[sub["Cost Category"] == selected_costcat_all], key="proj_costcat_click_table"
        )
    else:
        st.caption("💡 上のグラフの棒／円をクリックすると、そのCost Categoryの明細を表示できます。")
...
def main() -> None:
    ...
    # クリックドリルダウンの枠線コンテナ（key末尾が"_click_drilldown"のst.container）を、
    # 太枠線＋背景色で視覚的に目立たせる（ライト/ダーク両テーマで視認できる半透明の色）
    st.markdown(
        """
        <style>
        div[class*="_click_drilldown"] {
