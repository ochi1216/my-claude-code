...
使い方:
    streamlit run project_cost_analyzer_20260722_04.py
...
def tab_project(df: pd.DataFrame) -> None:
    ...
    proj = st.selectbox("プロジェクトを選択", projects, key="s_project")
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("##### Function別")
        f = agg_by(sub, "Function")
        if f.empty:
            st.caption("Function列が空です。")
        else:
            st.plotly_chart(px.bar(f, x="Cost", y="Function", orientation="h"), use_container_width=True)
    with c2:
        st.markdown("##### 組織(Organization)別")
        o = agg_by(sub, "Organization")
        st.plotly_chart(px.bar(o, x="Cost", y="Organization", orientation="h"), use_container_width=True)

    # 内部労務 vs 外部購買
    st.markdown("##### コスト種別（内部労務 vs 外部購買）")
    cat = agg_by(sub, "Cost Category")
    st.plotly_chart(px.bar(cat, x="Cost Category", y="Cost"), use_container_width=True)
    ...
    if mm.empty or mm["Man month"].sum() == 0:
        ...
        st.plotly_chart(px.bar(mmf, x="Man month", y="Function", orientation="h"), use_container_width=True)
...
def tab_function(df: pd.DataFrame) -> None:
    ...
    func = st.selectbox("Function（職種）を選択", functions, key="s_function")
    ...
    top = byproj.head(20)
    st.plotly_chart(px.bar(top, x="Cost", y=PROJECT_COL, orientation="h"), use_container_width=True)
...
def main() -> None:
    ...
    save_settings(
        {
            "s_path": st.session_state.get("s_path"),
            "s_profit_centers": st.session_state.get("s_profit_centers"),
            "s_fiscal_years": st.session_state.get("s_fiscal_years"),
            "s_statuses": st.session_state.get("s_statuses"),
            "pf_stacked": st.session_state.get("pf_stacked", False),
        }
    )
