...
使い方:
    streamlit run project_cost_analyzer_20260722_10.py
...
def render_filterable_table(df: pd.DataFrame, key: str) -> None:
    ...
    if not work.empty:
        ...
        if viz_options:
            ...
            viz_top_n = 20 if n_unique > 20 else None
            viz_title = f"{viz_axis}別" + ("（上位20）" if viz_top_n else "")
            render_breakdown(work, viz_axis, viz_title, key=f"{key}_viz_breakdown", top_n=viz_top_n)
