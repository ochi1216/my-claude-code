...
使い方:
    streamlit run project_cost_analyzer_20260722_07.py
...
DETAIL_FILTER_COLUMNS = ["Project Name", "PM", "Cost element name", "Profit Center", "Fiscal Year"]
...
def render_filterable_table(df: pd.DataFrame, key: str) -> None:
    ...
    with st.expander(f"🔍 フィルタ（{len(df):,}行が対象）", expanded=False):
        cols = st.columns(len(DETAIL_FILTER_COLUMNS))
        picks: dict[str, list] = {}
        for c, col_name in zip(cols, DETAIL_FILTER_COLUMNS):
    ...
    sort_cols = [c for c in DETAIL_TABLE_COLUMNS if c in work.columns]
    ...
    display_cols = [c for c in DETAIL_TABLE_COLUMNS if c in work.columns]
    st.dataframe(work[display_cols], use_container_width=True, hide_index=True, height=400)

    st.download_button(
        "この明細をExcelでダウンロード",
        data=_to_excel_bytes({"明細": work[display_cols]}),
...
def render_axis_drilldown(df: pd.DataFrame) -> None:
    ...
    st.session_state.setdefault("dd_axis", axis_labels[0])
    if st.session_state["dd_axis"] not in axis_labels:
        st.session_state["dd_axis"] = axis_labels[0]
    axis_index = axis_labels.index(st.session_state["dd_axis"])
    axis_label = st.selectbox("分類軸を選択", axis_labels, index=axis_index, key="dd_axis_widget")
    st.session_state["dd_axis"] = axis_label
    ...
    render_breakdown(scoped, axis_col, f"{axis_label} 内訳", key="dd_breakdown")

    values = sorted(scoped[axis_col].dropna().unique())
    if st.session_state.get("dd_value") not in values:
        st.session_state["dd_value"] = values[0]
    value_index = values.index(st.session_state["dd_value"])
    picked = st.selectbox(
        f"{axis_label}の項目を選んで明細を表示", values, index=value_index, key="dd_value_widget"
    )
    st.session_state["dd_value"] = picked

    detail = scoped[scoped[axis_col] == picked]
    st.markdown(f"##### 明細: {axis_label} = 「{picked}」")
    render_filterable_table(detail, key="dd_table")
