@echo off
rem PowerPoint Gemini Translator - launcher script.
...
rem NOTE: the folder is named ppt_translator but the script files are named
rem ppt_translation_yyyymmdd_NN.py (the prefix was kept from the original
rem tool). Do not "fix" the wildcard below to ppt_translator_*.py or nothing
...
set "TARGET_SCRIPT="
for /f "delims=" %%f in ('dir /b /o-n "ppt_translation_????????_??.py" 2^>nul') do (
...
:run_tool
echo Starting PowerPoint Gemini Translator...
...
:no_script
echo [ERROR] No file matching ppt_translation_yyyymmdd_NN.py was found.
pause
exit /b 1
