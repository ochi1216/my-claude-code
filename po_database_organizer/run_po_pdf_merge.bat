...
rem ── 同じフォルダ内の po_pdf_merge_*.py のうち、
rem    ファイル名の並び順で最新（日付_連番が最大）のものを自動選択する ──
set "LATEST="
for /f "delims=" %%F in ('dir /b /a-d /o-n "po_pdf_merge_*.py" 2^>nul') do (
    if not defined LATEST set "LATEST=%%F"
)

if not defined LATEST (
    echo [エラー] po_pdf_merge_*.py が見つかりません。
...
echo ============================================================
echo  PO PDF Merge Integrator を起動します
echo  実行ファイル: %LATEST%
echo ============================================================
echo.
echo 起動後にファイル選択ダイアログが開きます。
echo PO一覧シートがあるExcelファイルを選択してください。
echo.
...
%PYCMD% -m pip show pdfplumber >nul 2>nul
if errorlevel 1 (
