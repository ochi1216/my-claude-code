...
| 2 | **B: 自社データの権威化＋A: 自社を競合比較に必ず含める** | 未着手 |
| 3 | **C: ステージ4の主語転換＋レポート文言修正** | 未着手 |
| 4 | **D: ロードマップビュー** | 未着手 |
...
### 14.5 優先度2以降の実装メモ（次回着手時の参考）

- **自社データの権威化**: `config/own_products.json`のような、越智さんが直接メンテナンスする「Nexperia自社製品の正確なスペック」データベースを新設し、`fact構造`の`source_type: "user_input"`で扱う。ステージ3の比較で、LLM検索結果より優先的にこちらを参照する
- **自社を競合比較に必ず含める**: `ic_schema.pick_regional_representatives()`と`load_competitors()`を呼ぶ`ic_engine.py`のステージ3処理で、選定企業リストに`own_company_name()`が含まれていなければ強制的に追加する処理を入れる（`ic_engine.IcPipeline`側の変更）
- **ステージ4の主語転換**: `ic_prompts.STAGE4_NEXT_GEN`のプロンプト文面を「TIが次に開発すべき」から「Nexperiaが対抗するために開発すべき」に書き換え、出力スキーマに「TIとの差別化ポイント」「対抗の緊急度」等のフィールドを追加。`ic_report.py`のレポートタイトル・文言も同様に修正
- **ロードマップビュー**: 単一製品ではなくカテゴリ単位で、複数世代にわたる打ち手の時系列（TIの推定リフレッシュ周期 vs Nexperiaの対抗タイミング）を可視化する新タブまたは新セクションが必要。データモデル面でも「1製品=1回の分析」から「1カテゴリ=時系列の複数打ち手」への拡張が必要になるため、設計の練り直しが必要
