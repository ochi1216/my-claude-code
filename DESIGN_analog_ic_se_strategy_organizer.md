...
### 14.6 優先度3（ロードマップビュー）の実装メモ（次回着手時の参考）

単一製品ではなくカテゴリ単位で、複数世代にわたる打ち手の時系列（TIの推定リフレッシュ周期 vs
自社の対抗タイミング）を可視化する新タブまたは新セクションが必要。データモデル面でも
「1製品=1回の分析」から「1カテゴリ=時系列の複数打ち手」への拡張が必要になるため、設計の
練り直しが必要。
