...
* 主要フォルダ
  ...
  * `outlook_total_organizer/`: Outlook オーガナイザー本体（本セッションで既存コードをリポジトリへ取り込み）
    ...
    * `outlook_total_organizer_20260716_01_01.py`: 本セッション（S02）で追加。「R19Proj以外」フィルタボタンを追加したバージョン
...
* 「🚫 R19Proj以外」ボタン（**本セッションS02で新規追加**）: R19Proj以外の案件のみに絞り込み。上記R19Projボタンとは排他的に動作（片方をONにするともう片方は自動OFF）
...
* 暫定対応: なし
* 技術的リスク:
  * 本ツールはWindows専用（`win32com`, `pythoncom`使用）かつOutlookデスクトップクライアント・Gemini APIキーに依存するため、本セッションの実行環境（Linuxコンテナ）では実機起動テストができなかった。今回の変更箇所（HTML/JS）は、生成されるHTML断片を切り出しPlaywrightで独立検証済み（後述）。

## 7. Test and Execution

* 起動方法: 未確認（GUIアプリのため、Outlookインストール済みWindows環境での起動が前提と推測されるが、`config.json`等のセットアップ手順は未確認）
* テスト方法:
  * 本セッションでは、`generate_action_dashboard_report`が出力するHTML/CSS/JS部分（コントロールバーのフィルタボタンとJSロジック）のみを抽出し、Playwrightのヘッドレスブラウザで以下を検証した:
    * 初期状態は全カード表示
    * 「🧩 R19Proj」クリックでR19案件のみ表示、ボタンがactive化
    * 「🚫 R19Proj以外」クリックで非R19案件のみ表示に切り替わり、R19Projボタンは自動的に非active化
    * 同じボタンを再クリックすると全件表示に戻る
  * `ast.parse`によるPython構文チェック（`outlook_total_organizer_20260716_01_01.py`）
...
* 変更禁止事項:
  ...
  * `outlook_total_organizer`内の既存バージョンファイル（`_20260713_03_01`）は削除・上書きしない
* セキュリティ上の注意:
