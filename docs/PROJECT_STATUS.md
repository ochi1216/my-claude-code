...
- 本プロジェクト固有の仕様・設計方針（確定分）:
  ...
  - UI形式: GUIを採用（ラジオボタンでの方式選択が要件のため、CLIのみの構成では不十分）。フレームワークは未確定だが、既存の`youtube_summary_list`と同様の軽量デスクトップUI（Tkinter）を軸に検討中（`shareflex_dashboard`のStreamlit採用例もあり、要ユーザー確認）。
- 検討中の論点（未決定）:
  - **pyannote.audio等の専用ダイアライゼーション（将来の拡張候補）**: 現時点では簡易分離（Gemini任せ）を採用するが、長時間・多人数会議でチャンクをまたいで話者ラベルがずれる等、精度不足が判明した場合の拡張候補として保留。pyannoteは音声の声紋（embedding）に基づきクラスタリングするため同一人物への一貫性は高いが、GPU推奨・HuggingFaceトークン管理・追加ライブラリが必要になり、かつ話者名との紐付けは別途必要。導入する場合は「pyannoteで話者境界を検出→Geminiで文字起こし」のハイブリッド構成を想定。
  - **GUIフレームワークの最終決定**: Tkinter（youtube_summary_list踏襲、軽量・追加サーバ不要）かStreamlit（shareflex_dashboard踏襲、Webブラウザ表示・将来の拡張がしやすい）かは未確定。

## 5. Current Status

- **完了済み**: セッション管理用ファイル（`CLAUDE.md`, `docs/PROJECT_STATUS.md`, `docs/SESSION_HISTORY.md`, `docs/NEXT_TASK.md`）の初期セットアップ。リポジトリの既存慣習（Python製・フォルダ単位・Gemini API利用・バージョン管理規約）の調査。文字起こしエンジン・話者分離・出力形式の3点についてユーザー確認済み（上記4.参照）。
- **作業中**: 会議音声をGeminiへ送信したくない場合の代替経路（ローカル処理オプション）の設計方針をユーザーと検討中。
- **未着手**: 上記が確定した後の実装一式（フォルダ作成、音声抽出、文字起こし、要約、CLI化）。
