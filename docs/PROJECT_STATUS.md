...
* 主要フォルダ
  ...
  * `outlook_total_organizer/`: Outlook オーガナイザー本体（本セッションで既存コードをリポジトリへ取り込み）
    ...
    * `outlook_total_organizer_20260716_02.py`: 本セッション（S03）で追加。アクションタブの対象期間に「3週間」「1ヶ月」を追加したバージョン（本バージョンから命名規則を`_yyyymmdd_NN.py`に変更、後述）
...
* 確定済みの仕様:
  ...
  * **S03（本セッション、`outlook_total_organizer_20260716_02.py`）以降はユーザー指示により命名規則を`outlook_total_organizer_yyyymmdd_NN.py`（末尾の`_01`を廃止）に変更し、今後この新規則に統一する。** これはリポジトリ直下README.mdの命名規則（`ツール名_yyyymmdd_連番.py`）に近い形式になった。既存の旧命名規則ファイル（`_20260713_03_01.py`, `_20260716_01_01.py`）は遡ってリネームしていない。
* 維持すべき設計方針:
