                    ...
                    for category, content in details.items():
                        ...
                        if isinstance(content, dict):
                            for key, val in content.items():
                                html_content += f"・<strong>{key}</strong>: {val}<br>"
                            html_content += "<br>"
                        else:
...
def generate():
    ...
    threading.Thread(
        target=_generate_worker, args=(page_ids, section_name, notebook_name, site_id), daemon=True
    ).start()
    return jsonify({"status": "started"})

def _generate_worker(page_ids, section_name, notebook_name, site_id):
    try:
        ...
        ReportGenerator.generate_html(results, out_path, section_name, cost_info=cost_info)
...
if __name__ == "__main__":
    print("OneNote Report Generator 20260706_01 を起動します...")
