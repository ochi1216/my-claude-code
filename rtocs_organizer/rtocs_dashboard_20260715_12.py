# -*- coding: utf-8 -*-
"""RTOCS 統合マネジメントダッシュボード v20260715_12
...
起動: streamlit run rtocs_dashboard_20260715_12.py

v12での変更点:
  - 戦略分析タブに「分析前の確認・修正チェックポイント」（オプション・デフォルトOFF）を追加。
    ONにすると、会社分析〜他業種事例分析までの前半ステージだけを先に実行して中間結果を表示し、
    経営者が内容を確認した上で追加コメント・修正指示を入力してから、課題分析・戦略策定の
    後半ステージを実行できる（strategy_engine.run_pipeline(stop_before_issues=True) /
    continue_pipeline()）。デフォルト（OFF）では従来通り全ステージが自動で一気通貫実行される
  - （改善構想「5軸MECE整理」の軸3-②に対応）
"""
