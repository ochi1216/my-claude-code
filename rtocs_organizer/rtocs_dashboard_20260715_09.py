# -*- coding: utf-8 -*-
"""RTOCS 統合マネジメントダッシュボード v20260715_09
...
起動: streamlit run rtocs_dashboard_20260715_09.py

v09での変更点（strategy_prompts.py / strategy_report.pyの更新に対応）:
  - 戦略策定ステージが、ブルーオーシャン戦略・ジョブ理論・アンゾフの成長マトリクス・
    ポートフォリオ経営(BCG的発想)の4フレームワークを横断的に検討し、3つの戦略が
    単一の切り口に偏らないようにした。各戦略にどのフレームワークが発想の起点になったか
    (framework_lens)を明示させ、レポート上にバッジ表示する
