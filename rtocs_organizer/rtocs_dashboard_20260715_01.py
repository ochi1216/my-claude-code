# -*- coding: utf-8 -*-
"""RTOCS 統合マネジメントダッシュボード v20260715_01

タブ構成:
  📋 一覧・検索   … 従来機能（検索・絞り込み・HTML/PDFを開く）
  📈 傾向分析     … 全RTOCS俯瞰（業界×年、キーワード変遷、AI俯瞰総評 ほか）
  🎯 戦略分析     … 企業名を入力→7ステージ一気通貫の戦略レポートHTML生成

起動: streamlit run rtocs_dashboard_20260715_01.py
"""
...
with tab_list:
    ...
    if not filtered_df.empty:
        ...
        with col_btn1:
            if st.button("🌐 HTMLレポートを開く", use_container_width=True,
        ...
        with col_btn2:
            if st.button("📑 RTOCS PDFを開く", use_container_width=True,
...
with tab_trend:
    ...
    with col_b:
        ...
        if not revisit.empty:
            st.dataframe(revisit, use_container_width=True, hide_index=True)
        else:
            st.info("複数回登場した企業はありません。")

    # --- 5. キーワードトレンド線 ---
    st.markdown("### 📉 キーワードの年次トレンド比較")
    all_kw = [kw for kws, _ in Counter(
        kw for kws in df["_keywords"] for kw in kws).most_common(50) for kws in [kws]]
...
with tab_strategy:
    ...
    if "last_result" in st.session_state:
        ...
        with col_r1:
            if st.button("🌐 レポートを開く", use_container_width=True, key="btn_open_report"):
