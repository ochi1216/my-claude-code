# -*- coding: utf-8 -*-
"""RTOCS 統合マネジメントダッシュボード v20260719_05

タブ構成:
  📋 一覧・検索   … 従来機能（検索・絞り込み・HTML/PDFを開く）
  📈 傾向分析     … 全RTOCS俯瞰（業界×年、キーワード変遷、AI俯瞰総評 ほか）
  🎯 戦略分析     … 企業名を入力→11〜12ステージ一気通貫の戦略レポートHTML生成＋深掘りチャット（最大5社の一括分析対応）
  🗂 分析履歴     … 過去に生成した戦略分析レポートの一覧・検索・再オープン

起動: streamlit run rtocs_dashboard_20260719_05.py

v20260719_05での変更点（strategy_prompts.py / strategy_engine.py / strategy_report.pyの更新に対応）:
  - 【新機能】軸2-⑤「複数ペルソナ討議」を実装。「🎭 複数ペルソナ討議を実行する」チェックボックス
    （オプション、既定はOFF）をONにすると、戦略策定の直後にCFO/CTO/アクティビスト投資家/破壊者
    （新規参入スタートアップの視点）の4つの立場から3つの戦略提言を討議させる追加ステージを実行する
    （追加API呼び出し1回）。各ペルソナが3戦略それぞれにコメント・懸念度をつけ、どの戦略を支持するか
    を明言し、最後に4つの立場の一致点・対立点を総括する
  - このオプションは単独企業・一括分析のどちらでも利用可能。確認チェックポイント機能とも併用できる
    （チェックポイントで一時停止した場合も、設定は保持されて後半ステージ実行時に反映される）
  - レポートに「🎭 複数ペルソナ討議」カードを新設（戦略提言カードの直後）。OFFの場合はカード自体が
    表示されない（軸5-①の「前回分析との比較」カードと同じ、存在する場合のみ表示するパターン）
"""
...
with tab_strategy:
    ...
    enable_persona_debate = st.checkbox(
        "🎭 複数ペルソナ討議を実行する（オプション、既定はOFF・追加コスト）",
        value=False,
        help="ONにすると、戦略策定の直後にCFO/CTO/アクティビスト投資家/破壊者（新規参入スタートアップの"
             "視点）の4つの立場から戦略提言を討議させる追加ステージを実行します（1回分の追加API呼び出し）。"
             "複数社を一括分析する場合は全社に適用されます。")
