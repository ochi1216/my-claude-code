...
# Task
対象企業「{company}」について、以下の実データ（yfinance取得）と、これまでの分析結果（直近ニュース・
アナリスト洞察・マクロ/技術トレンド）を統合し、株式市場の視点から分析してください。
数値はデータをそのまま使い、あなたの推測で数値を作らないでください。

**重要**: 現状の数値を説明するだけで終わらせないこと。バリュエーション・トレンド・アナリスト見解・
マクロトレンドを統合して「今後株価がどちらに向かいやすいか」という見立てと、その見立てを崩しうる
具体的なリスク要因、株価を動かしうる今後のイベント（カタリスト）まで踏み込んで分析してください。
...
# これまでの分析結果（直近ニュース・アナリスト洞察・マクロ/技術トレンドの要点）
{context_summary}
...
{{
  ...
  "outlook": {{
    "direction": "強気(上昇期待)/中立/弱気(下落リスク)/不透明、のいずれか",
    "time_horizon": "この見立てが想定する期間(例: 6-12ヶ月)",
    "rationale": "バリュエーション・トレンド・アナリスト見解・マクロトレンドを統合した根拠(200文字程度。単一の要因だけで結論づけないこと)"
  }},
  "key_risks": [
    {{"risk": "想定リスクの見出し(具体的に。例:「主力事業の価格競争激化による利益率低下」)",
      "trigger": "何が起きるとこのリスクが顕在化するか(80文字程度)",
      "potential_impact": "株価・事業への影響度と方向性(高/中/低、上振れ/下振れ)"}}
  ],
  "catalysts": [
    {{"event": "今後株価を動かしうる具体的なイベント(決算発表・新製品発表・規制決定・M&A発表等)",
      "expected_timing": "想定時期(例: 次回決算/YYYY-MM頃。不明なら「時期未定」)",
      "potential_impact": "実現した場合に株価が上振れ/下振れどちらに動きやすいか、その理由"}}
  ],
""" + _SOURCE_TAG_FIELDS.format(extra_hint="（数値自体はyfinanceの実データのためhigh寄りだが、outlookやkey_risksの推論部分はAIの知識に基づく点を踏まえて判断）") + """
}}
key_risksは2〜4個、catalystsは1〜3個挙げてください。無理に数を揃えず、根拠のあるものだけ挙げてください
（情報が乏しい場合は少数、または空配列でも構いません）。
"""
...
# 複数ペルソナ討議（軸2-⑤）。選択制(enable_persona_debate=True)の場合のみ実行する。
# 戦略策定ステージの最終結果（ディープモードの批判・改訂パスがあれば改訂後）を、4つの異なる立場の
# 人物になりきってレビューさせる。事実主張ではなくロールプレイ的推論のため、source_confidence/note は付与しない。
STAGE_PERSONA_DEBATE = SYSTEM_PREFIX + """
# Task
以下は対象企業「{company}」に対して策定された3つの戦略提言です。
この提言を、以下の4つの異なる立場の人物になりきってレビューしてください。それぞれの立場ならではの
関心事・評価基準に忠実に、遠慮なくコメントしてください（全員が同じ結論に達する必要はありません。
意見が対立すること自体が価値のある情報です）。

# レビューする4つの立場
- **CFO**: 資金効率・財務リスク・ROI・キャッシュフローへの影響を最優先する。実行可能な資金計画が
  無い提言には厳しく指摘する
- **CTO**: 技術的実現性、既存システム・人材で実行できるか、スピードと技術的負債のリスクを最優先する
- **アクティビスト投資家**: 株主価値の最大化、資本効率、不要事業の売却・資源再配分を要求する。
  実行の速さと株価への短期インパクトを重視する
- **破壊者（新規参入スタートアップの視点）**: 既存の前提・しがらみを一切気にせず、「外部の新規参入者
  としてこの会社が属する市場を攻めるならどうするか」という視点で評価する。大企業ゆえの遅さ・
  保守性・社内政治を厳しく指摘する
...
{{
  "persona_reactions": [
    {{
      "persona": "CFO/CTO/アクティビスト投資家/破壊者（新規参入スタートアップの視点）のいずれか",
      "reactions": [
        {{"strategy_title": "対象の戦略見出し（戦略提言のtitleと一致させること）",
          "comment": "この立場から見たコメント(100文字程度)",
          "concern_level": "高/中/低（この立場にとっての懸念の大きさ）"}}
      ],
      "overall_recommendation": "3つの戦略のうちどれを最も支持するか、またはどれも支持しないかとその理由(100文字程度)"
    }}
  ],
  "convergence_note": "4つの立場の意見がどこで一致し、どこで対立するかの総括(150文字程度)"
}}
persona_reactionsは必ず4件（CFO/CTO/アクティビスト投資家/破壊者を1件ずつ）、各personaのreactionsは
必ず3件（3つの戦略それぞれに1件ずつ）出力してください。
"""
