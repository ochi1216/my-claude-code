# -*- coding: utf-8 -*-
"""RTOCS 統合マネジメントダッシュボード v20260715_10

タブ構成:
  📋 一覧・検索   … 従来機能（検索・絞り込み・HTML/PDFを開く）
  📈 傾向分析     … 全RTOCS俯瞰（業界×年、キーワード変遷、AI俯瞰総評 ほか）
  🎯 戦略分析     … 企業名を入力→10ステージ一気通貫の戦略レポートHTML生成

起動: streamlit run rtocs_dashboard_20260715_10.py

v10での変更点（strategy_prompts.py / strategy_report.pyの更新に対応。ダッシュボード本体のコード変更なし）:
  - 戦略策定ステージの3つの戦略それぞれに「大胆さのモード」(保守的/野心的/破壊的)を必須指定させ、
    破壊的モードには制約を外す思考実験（今日創業するなら／最大の強みを捨てたら／リソース無限なら）
    を明示的に組み込んだ。レポート上に色分けされたモードバッジを表示する
  - 類似RTOCS事例選定を「構造課題の近さ」優先に強化し、異業種の跳躍的アナロジーを最低2件は
    選ぶよう努めさせた（is_cross_industryフラグ、レポート上に🌉バッジ表示）
  - 戦略提言全体に対する「悪魔の代弁者」所見(devils_advocate_note)を追加。追加のAPI呼び出しは
    発生せず、既存の戦略策定ステージの1回の呼び出し内でモード指定・異業種選定強化・反証を行う
  - （改善構想「5軸MECE整理」の軸2-②③④に対応）
"""
