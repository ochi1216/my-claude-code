# -*- coding: utf-8 -*-
"""RTOCS 統合マネジメントダッシュボード v20260715_14
...
起動: streamlit run rtocs_dashboard_20260715_14.py

v14での変更点（strategy_prompts.py / strategy_engine.py / strategy_report.pyの更新に対応。
ダッシュボード本体のコード変更なし。STAGE_LABELSからステージ進捗表示が自動更新される）:
  - 戦略提言の各戦略に「1年後のマイルストーン」「3年後のビジョン」を追加し、最初の90日と合わせた
    3つの時間軸でロードマップを示すようにした（軸5-②）
  - 同一企業を過去に分析した際のレポート（JSONサイドカーとして自動保存）を検出し、今回の分析完了後に
    「⑪ 前回分析との比較（進捗トラッキング）」ステージを追加実行。前回指摘した課題の改善状況・新たな
    課題・戦略の継続性を比較する。過去レポートが無い場合（初回分析）は自動でスキップされる（軸5-①）
  - レポートに「📈 前回分析との比較」カードを新設（戦略提言カードの直後）
  - （改善構想「5軸MECE整理」の軸5-①②に対応。これでMECE5軸すべての優先実装項目に着手完了）
"""
