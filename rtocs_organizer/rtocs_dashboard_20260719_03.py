# -*- coding: utf-8 -*-
"""RTOCS 統合マネジメントダッシュボード v20260719_03
...
起動: streamlit run rtocs_dashboard_20260719_03.py

v20260719_03での変更点:
  - 【新機能】「以前の分析結果をもう一度見たいときにアクセスできるか」というユーザーからの
    問い合わせを受けて追加。新タブ「🗂 分析履歴」で、`data/strategy_reports/` に保存された
    過去の戦略分析レポートを一覧表示し、企業名で検索した上でレポートを開く・ダウンロードできる
  - `strategy_report.py`に`list_saved_reports()`を新規追加。JSONサイドカー（軸5-①で導入）が
    あれば会社名・生成日時・モード・コスト・エグゼクティブサマリーを読み取り、無い場合は
    ファイル名と更新日時から推定するため、JSONサイドカー導入前に生成した古いレポートも一覧に含まれる
"""
