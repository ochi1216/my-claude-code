# -*- coding: utf-8 -*-
"""RTOCS 統合マネジメントダッシュボード v20260715_04

タブ構成:
  📋 一覧・検索   … 従来機能（検索・絞り込み・HTML/PDFを開く）
  📈 傾向分析     … 全RTOCS俯瞰（業界×年、キーワード変遷、AI俯瞰総評 ほか）
  🎯 戦略分析     … 企業名を入力→8ステージ一気通貫の戦略レポートHTML生成

起動: streamlit run rtocs_dashboard_20260715_04.py

v04での変更点（strategy_engine.pyの更新に対応）:
  - 直近ニュース収集ステージが「400 google_search_retrieval is not supported」エラーで
    失敗する不具合を修正。グラウンディング呼び出しをレガシーSDK(google-generativeai)から
    後継の統合SDK(google-genai)に切り替え、Gemini 2.x系が要求するgoogle_searchツールに対応
"""
