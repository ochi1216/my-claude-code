...
- `strategy_prompts.py` / `strategy_engine.py` — 企業名を1つ入力すると「会社分析→直近ニュース収集(英/日/中)→アナリスト洞察収集→株式市場分析→業界・競合分析→類似RTOCS事例選定→他業種事例分析→課題分析→戦略策定」を自動で実行するパイプライン
- `strategy_report.py` — 分析結果を1枚の自己完結HTMLレポートに整形
- `rtocs_dashboard_20260715_05.py` — 上記をまとめたStreamlitダッシュボード本体（最新版。変更履歴は [`CHANGELOG.md`](CHANGELOG.md) を参照）

### 起動方法

```
pip install -r requirements.txt
streamlit run rtocs_dashboard_20260715_05.py
```
...
3. **🎯 戦略分析** — 任意の企業名を入力すると、蓄積RTOCSをケースライブラリとして活用しながら一気通貫で戦略分析レポート（HTML）を生成する。会社分析の直後に、Google Search Groundingで英語・日本語・中国語の企業名表記から直近12ヶ月のニュースを検索するステージ、続けてセルサイドアナリストの見解・決算説明会でのマネジメント発言・市場評価と自己認識のギャップを検索するステージが入る（株価の生数値だけでなく市場参加者の解釈を戦略提言に反映する）。「通常」（gemini-2.5-flash、数十円/回）と「ディープ」（gemini-2.5-pro＋戦略の批判・改訂パス、数百円/回）の2モード
