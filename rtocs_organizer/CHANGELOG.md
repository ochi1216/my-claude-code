...
**追加ファイル:** `rtocs_dashboard_20260719_05.py`（`20260719_04`からのコピー＋機能追加。旧版はそのまま残置）

MECE改善構想（5軸）の**軸2「PROCESS（思考法・創造性エンジン）」の項目⑤**に対応。実行するかどうかを選択制にしたいというユーザー指示により、既存の「分析前の確認チェックポイント」（軸3-②）と同じ「オプション・デフォルトOFF」の扱いにした:

- `strategy_prompts.py`に`STAGE_PERSONA_DEBATE`を新規追加。CFO（財務リスク重視）・CTO（技術実現性重視）・アクティビスト投資家（株主価値重視）・破壊者（新規参入スタートアップの視点、既存の戦略策定内の「破壊的モード」＝自社の中の人がどれだけ大胆になれるか、とは異なる外部攻撃者視点として明確に区別）の4つの立場から、戦略策定ステージの最終結果（ディープモードの改訂後があればそちら）をレビューさせる。事実主張ではなくロールプレイ的推論のため出典確度タグは付与しない
- `strategy_engine.py`: `StrategyEngine.run_pipeline(..., enable_persona_debate=False)`を追加。ONの場合のみ戦略策定の直後に新ステージ「⑪ 複数ペルソナ討議」を実行し、`stages`に`"personas"`キーが追加される（OFFの場合はキー自体が存在せず、軸5-①の`progress`ステージと同じ「存在する場合のみ」パターン）。`stop_before_issues`によるチェックポイント一時停止との併用にも対応し、`enable_persona_debate`の設定値は`_pending_continuation`に保存され`continue_pipeline()`実行時に引き継がれる。既存の`progress`ステージのラベル番号を⑪→⑫に振り直し
- `strategy_report.py`に`_sec_personas()`を新規追加。ペルソナごとに3戦略への反応（コメント・懸念度）の表と支持戦略のハイライト、最後に4つの立場の一致点・対立点の総括を表示。「🎭 複数ペルソナ討議」カードを「🎯 戦略提言」カードの直後に追加（`"personas"`キーが存在する場合のみ）
- ダッシュボードに「🎭 複数ペルソナ討議を実行する」チェックボックス（既定OFF）を追加。単独企業・一括分析（最大5社）のどちらでも利用可能で、全社に共通の設定として適用される

## [20260719_04] - 2026-07-19

**追加ファイル:** `rtocs_dashboard_20260719_04.py`（`20260719_03`からのコピー＋機能追加。旧版はそのまま残置）
**変更ファイル:** `README.md`

ユーザーより「現在は1つの企業分析しか開始できない。最大5件を1つの実行ボタンで開始できるようにしたい」との要望があり対応（`strategy_prompts.py` / `strategy_engine.py` / `strategy_report.py`の変更は無し。ダッシュボード本体のUI・実行ロジックのみの変更）:

- 「🎯 戦略分析」タブの企業名入力を`st.text_input`（単一）から`st.text_area`（複数行）に変更。1行1社として最大5社まで受け付け、重複した企業名は自動的に除去、5社を超える入力は先頭5社のみを対象にして警告を表示する
- 複数社入力時（`is_batch`）は「分析前に中間結果を確認する」チェックポイントを自動的に無効化（複数社の一時停止・個別レビューはUIが複雑になるため対象外とし、常に最後まで自動実行する）。1社のみの入力時は既存のチェックポイント・深掘りチャット機能をそのまま利用できる
- 実行ボタンを押すと、入力された企業を1社ずつ順番に処理する（`StrategyEngine`のインスタンスは企業ごとに新規生成し、コスト集計を分離）。各社の`run_pipeline()`実行を個別にtry/exceptで保護し、1社が例外で失敗しても残りの企業の分析は継続する（既存の「ステージ単位の失敗耐性」を「企業単位」にも拡張）
- 結果表示: 1社のみ成功した場合は既存の単独結果表示（エグゼクティブサマリー・レポートを開く/ダウンロード・深掘りチャット）をそのまま使う。2社以上、または失敗を含む場合は新設の「📊 一括分析結果」セクションに、成功/失敗件数・バッチ全体のコスト・各社の詳細（展開式）を表示する
- 生成された全レポートは既存仕様通り`data/strategy_reports/`に保存されるため、「🗂 分析履歴」タブからも後で見つけて開ける

## [20260719_03] - 2026-07-19

**変更ファイル:** `strategy_report.py`, `README.md`
**追加ファイル:** `rtocs_dashboard_20260719_03.py`（`20260719_02`からのコピー＋新タブ追加。旧版はそのまま残置）

ユーザーより「戦略を策定した企業情報はどこかに登録されているのか、過去の分析結果に後からアクセスできるのか」との問い合わせがあり対応:

- `strategy_report.py`に`list_saved_reports()`を新規追加。`data/strategy_reports/`に保存された過去の戦略分析レポート一覧を生成日時の新しい順で返す
  - JSONサイドカー（軸5-①、2026-07-18導入）があれば会社名・生成日時・モード・コスト・エグゼクティブサマリーを読み取る
  - JSONサイドカー導入前に生成されたHTMLのみの古いレポートも、ファイル名（`Strategy_{企業名}_{yyyymmdd_HHMMSS}.html`）とファイルの更新日時から会社名・生成日時を推定して一覧に含める（後方互換）
  - JSONが壊れている場合もファイル名フォールバックで一覧から漏れないようにする
- ダッシュボードに新タブ「🗂 分析履歴」を追加（3タブ→4タブ）。過去のレポートを企業名で検索し、一覧表（企業名・生成日時・モード・コスト）から選択してレポートを開く・ダウンロードできる
- これにより、分析結果は`data/strategy_reports/`に自動保存・上書きされない（既存仕様）ことに加え、ダッシュボード上から探して再アクセスできるようになった
