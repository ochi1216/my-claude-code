# -*- coding: utf-8 -*-
"""RTOCS 統合マネジメントダッシュボード v20260715_06
...
起動: streamlit run rtocs_dashboard_20260715_06.py

v06での変更点（strategy_engine.py / strategy_prompts.py / strategy_report.pyの更新に対応）:
  - 「マクロ・技術トレンド分析」ステージを追加（アナリスト洞察の直後、全体で9→10ステージに）。
    個社分析ではなく業界全体・時代全体に影響する構造変化（AI化、地政学、規制、サプライチェーン）
    をGoogle Search Groundingで検索・要約する。従来「業界・競合分析」ステージに埋没していた
    トレンド列挙をこちらに分離し、業界分析は対象企業固有の競合構造に集中させた
    （改善構想「5軸MECE整理」の軸1-②に対応）
"""
