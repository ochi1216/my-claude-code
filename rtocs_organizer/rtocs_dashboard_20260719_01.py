# -*- coding: utf-8 -*-
"""RTOCS 統合マネジメントダッシュボード v20260719_01
...
起動: streamlit run rtocs_dashboard_20260719_01.py

v20260719_01での変更点（緊急バグ修正。strategy_prompts.py / strategy_engine.py /
strategy_report.pyの更新に対応。ダッシュボード本体のコード変更なし）:
  - 【重要】実利用でティッカー誤認による誤った株式データ表示が報告された問題を修正。
    AIが推測したticker_candidatesが実際に対象企業のものかを検証せずに採用していたことが原因
  - 会社分析ステージが`official_name_en`（英語表記の正式社名）を新たに出力し、yfinanceが返す
    企業名(longName)と突合。一致しない候補（別会社の可能性が高い）は自動的にスキップし、
    一致する候補があればそちらを優先する。全候補が不一致の場合はレポート上に⚠️警告を表示する
  - dividendYield（配当利回り）の表示不具合を修正。Yahoo Financeが2024年頃に単位を「小数」から
    「%そのもの」に変更したため、旧仕様のまま無条件に100倍すると実際の利回りの100倍の値が
    表示されていた。値の大きさから形式を自動判定するよう修正
  - 株式市場データにYahoo Financeの該当ページへの直接リンクを追加し、ユーザー自身がワンクリックで
    データを裏取りできるようにした
  - ニュース・アナリスト洞察・マクロトレンドの各ステージ（Google Search Grounding使用）に、
    APIが実際に参照した検索結果のURL（grounding_sources、LLM生成ではなくAPIメタデータ由来のため
    ハルシネーションしない）を出典リンクとして追加表示
"""
