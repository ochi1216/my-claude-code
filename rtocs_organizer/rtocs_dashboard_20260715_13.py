# -*- coding: utf-8 -*-
"""RTOCS 統合マネジメントダッシュボード v20260715_13

タブ構成:
  📋 一覧・検索   … 従来機能（検索・絞り込み・HTML/PDFを開く）
  📈 傾向分析     … 全RTOCS俯瞰（業界×年、キーワード変遷、AI俯瞰総評 ほか）
  🎯 戦略分析     … 企業名を入力→10ステージ一気通貫の戦略レポートHTML生成＋深掘りチャット

起動: streamlit run rtocs_dashboard_20260715_13.py

v13での変更点（strategy_prompts.py / strategy_engine.py / strategy_report.pyの更新に対応。
ダッシュボード本体のコード変更なし）:
  - 会社分析・ニュース・アナリスト洞察・マクロトレンド・株式市場・業界競合・他業種事例・課題分析・
    戦略策定の各ステージ出力に`source_confidence`(high/medium/low)と`source_note`を追加し、
    レポート上に「出典確度」バッジとして表示（軸4-①）
  - 戦略提言のコスト規模を示す語彙（M&A・大規模投資等）と、既存の財務実行可能性データ
    （手元資金・FCF・負債資本比率）をPythonロジックで粗く比較し、無理がありそうな戦略に
    ⚠️注記(feasibility_flag)を自動付与（追加のAPI呼び出しなし。軸4-②）
  - （改善構想「5軸MECE整理」の軸4-①②に対応）
"""
